# Contributing to LenOS

Welcome, and thank you for your interest in contributing! LenOS is an
open-source project and we're glad you're here. This guide will help you
get from zero to a merged pull request.

If you have questions that aren't answered here, [open an issue](https://github.com/BuildGrowthNow/LenOS/issues/new).
...
unacceptable behavior to **conduct@lenos-relay.org**.

---

## Before You Open a PR

Before starting, search [open PRs](https://github.com/BuildGrowthNow/LenOS/pulls) and [open issues](https://github.com/BuildGrowthNow/LenOS/issues) for duplicates — someone may already be working on the same thing. When you open your PR, link the closest existing one in the description (or say "none found").
...
LenOS is an agent platform, so AI-assisted PRs are welcome. No need to disclose the tools you used, but you own and must have reviewed the final code. Submissions that are clearly unreviewed may be closed with a pointer here.
...
This repo uses [Hermit](https://rustup.rs/) for toolchain
...
just reset   # Wipe all dev state and recreate it; installed LenOS is preserved
```

Development desktop state uses separate bundle identifiers
(`xyz.block.lenos.app.dev` and per-worktree variants), a separate keyring service
(`lenos-desktop-dev`), and `~/.lenos-dev`. `just reset` removes those dev-only
locations and the local Docker volumes. It does not touch the installed app's
`xyz.block.lenos.app` data, `lenos-desktop` keyring service, or `~/.lenos` nest.
...
End-to-end tests live in `crates/lenos-test-client/tests/`:
...
of `lenos-core` types, which are shared everywhere).
...
LenOS is developed across multiple repositories. This repo (`BuildGrowthNow/LenOS`)
...
**External contributors:** Fork `BuildGrowthNow/LenOS`, open a PR, and CI runs
automatically. No special access is required.

**Block team members:** See the internal
[sprout-releases CONTRIBUTING.md](https://github.com/BuildGrowthNow/LenOS/blob/main/CONTRIBUTING.md)
...
2. **Define the payload type** in the appropriate module in `lenos-core/src/`
...
3. **Register the kind's required scope** in
   `crates/lenos-relay/src/handlers/ingest.rs` inside
...
4. **Handle post-storage side effects** by adding a match arm in
   `crates/lenos-relay/src/handlers/side_effects.rs` inside
   `handle_side_effects()`:
   ...
   `crates/lenos-relay/src/api/` and register it in
   `crates/lenos-relay/src/router.rs`.

5. **Persist to the database** — if the event needs to be queryable, add a
   handler in `lenos-db/src/` (e.g., `lenos-db/src/my_feature.rs`) with
...
8. **Write tests** — add a unit test for payload serialization in
   `lenos-core` and an integration test in `lenos-test-client` that sends
...
1. **Define the handler** in the appropriate module under
   `crates/lenos-relay/src/api/`. Resolve the request tenant before any auth or
   data lookup, use NIP-98 when the endpoint accepts user credentials, and keep
   community scoping explicit.

2. **Register the route** in `crates/lenos-relay/src/router.rs` using the
   narrowest path possible. Do not add new `/api/*` compatibility routes unless
   the product decision explicitly calls for one.

3. **Add database queries** in `lenos-db/src/` only when the endpoint cannot be
   expressed through the existing event query paths.

4. **Handle errors** using the `api_error()`, `internal_error()`, and
   `not_found()` helpers in `lenos-relay/src/api/mod.rs`. Return
   `(StatusCode, Json<Value>)` tuples.

5. **Write tests** with the `lenos-test-client` harness in
   `crates/lenos-test-client/tests/`, covering auth, community scoping, and the
...
LenOS is licensed under the **Apache License, Version 2.0**. See
...
*Thank you for contributing to LenOS. Every bug report, documentation fix,
