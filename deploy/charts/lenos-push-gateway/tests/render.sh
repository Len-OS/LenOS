...
helm lint deploy/charts/lenos-push-gateway >/dev/null
helm template push deploy/charts/lenos-push-gateway >"$out"
# Production values must attach push.lenos.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/lenos-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'appAttestAppId=REALTEAM.xyz.lenos'
...
helm lint deploy/charts/lenos-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/lenos-push-gateway "${production_args[@]}" >"$production_out"
...
runtime={'app.kubernetes.io/name':'lenos-push-gateway','app.kubernetes.io/instance':'push','app.kubernetes.io/component':'runtime'}
...
assert jenv['LENOS_PUSH_RUNTIME_DATABASE_ROLE']['value']=='lenos_push_gateway_runtime'
...
required={'DATABASE_URL','LENOS_PUSH_APNS_KEY_ID','LENOS_PUSH_APNS_TEAM_ID','LENOS_PUSH_APNS_TOPIC','LENOS_PUSH_GRANT_KEYS','LENOS_PUSH_TOKEN_KEYS','LENOS_PUSH_MAX_GRANT_LIFETIME_SECONDS'}
assert required <= env
...
np=next(x for x in nps if x['metadata']['name']=='push-lenos-push-gateway')
migration_np=next(x for x in nps if x['metadata']['name']=='push-lenos-push-gateway-migration')
...
assert 'push.lenos.xyz' in route['spec']['hostnames']
PY

# Enabling a route without a Gateway attachment must fail schema validation.
if helm template push deploy/charts/lenos-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/lenos-push-gateway -f deploy/charts/lenos-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/lenos-push-gateway \
...
np=next(x for x in xs if x and x.get('kind')=='NetworkPolicy' and x['metadata']['name']=='push-lenos-push-gateway')
...
if helm template push deploy/charts/lenos-push-gateway \
  --set podMonitor.enabled=true \
