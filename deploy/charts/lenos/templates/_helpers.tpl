...
{{- define "lenos.labels" -}}
helm.sh/chart: {{ include "lenos.chart" . }}
{{ include "lenos.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
app.kubernetes.io/part-of: lenos
{{- end -}}

{{- define "lenos.selectorLabels" -}}
app.kubernetes.io/name: {{ include "lenos.name" . }}
...
{{- define "lenos.relaySelectorLabels" -}}
{{ include "lenos.selectorLabels" . }}
app.kubernetes.io/component: relay
{{- end -}}

{{- define "lenos.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (include "lenos.fullname" .) .Values.serviceAccount.name -}}
{{- else -}}
...
*/}}
{{- define "lenos.chartSecretName" -}}
{{- printf "%s-relay" (include "lenos.fullname" .) -}}
{{- end -}}
...
*/}}
{{- define "lenos.envSecretName" -}}
{{- if .Values.secrets.existingSecret -}}
{{- .Values.secrets.existingSecret -}}
{{- else -}}
{{- include "lenos.chartSecretName" . -}}
{{- end -}}
{{- end -}}

{{/* Host derived from relayUrl, used as ingress default + media domain. */}}
{{- define "lenos.relayHost" -}}
...
{{- define "lenos.mediaBaseUrl" -}}
{{- if .Values.mediaBaseUrl -}}
{{- .Values.mediaBaseUrl -}}
{{- else -}}
{{- printf "https://%s/media" (include "lenos.relayHost" .) -}}
{{- end -}}
{{- end -}}

{{/* Quickstart-only in-cluster service hostnames (eval profile). */}}
{{- define "lenos.minioFullname" -}}
{{- printf "%s-minio" (include "lenos.fullname" .) -}}
{{- end -}}

{{/* In-cluster MinIO endpoint, used when minio.enabled and s3.endpoint unset. */}}
{{- define "lenos.minioEndpoint" -}}
{{- printf "http://%s.%s.svc.cluster.local:9000" (include "lenos.minioFullname" .) .Release.Namespace -}}
{{- end -}}

{{/* Minimum number of relay replicas the release can run. */}}
{{- define "lenos.minimumReplicas" -}}
...
{{- define "lenos.huddleAudioAvailable" -}}
{{- if kindIs "invalid" .Values.relay.huddleAudioAvailable -}}
{{- if gt (include "lenos.minimumReplicas" . | int) 1 -}}false{{- else -}}true{{- end -}}
{{- else -}}
...
{{- define "lenos.s3Endpoint" -}}
{{- if .Values.s3.endpoint -}}
{{- .Values.s3.endpoint -}}
{{- else if .Values.minio.enabled -}}
{{- include "lenos.minioEndpoint" . -}}
{{- end -}}
{{- end -}}

{{- define "lenos.pairingRelaySelectorLabels" -}}
{{ include "lenos.selectorLabels" . }}
