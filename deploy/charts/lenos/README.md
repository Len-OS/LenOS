...
[LenOS](https://github.com/BuildGrowthNow/LenOS) is a Nostr-based messaging platform for human–agent collaboration: a single relay binary serving WebSocket + REST + web UI, backed by PostgreSQL, Redis, and S3-compatible object storage.
...
```sh
helm install lenos oci://ghcr.io/buildgrowthnow/lenos/charts/lenos --version 0.1.7 \
  --create-namespace --namespace lenos \
  --set quickstart=true \
  --set postgresql.enabled=true \
  --set redis.enabled=true \
  --set minio.enabled=true \
  --set relayUrl=wss://lenos.example.com \
...
`LENOS_S3_ENDPOINT` plus autogenerated credentials automatically. No external services required. The `quickstart=true` flag is an
...
LenOS uses one URL style for both media and Git/CAS object-store requests:
...
`LENOS_S3_ADDRESSING_STYLE`. It renders `s3.region` as `LENOS_S3_REGION` only
...
Store `LENOS_S3_ACCESS_KEY=${{Object Storage.ACCESS_KEY_ID}}` and
`LENOS_S3_SECRET_KEY=${{Object Storage.SECRET_ACCESS_KEY}}` in the Secret named by
...
`LENOS_GIT_CONFORMANCE_PROBE` is enabled (the relay default). A probe failure is
...
relay:
  ...
  args: [/usr/local/bin/lenos-relay]
  extraVolumeMounts:
    - name: wrapper
      mountPath: /opt/wrapper
```
...
The chart can run LenOS's stateless pairing WebSocket relay as an independent
...
`pairingRelay.url` is advertised in the main relay's NIP-11 document so LenOS
...
`<release>-lenos-pairing:5000` with your platform's ingress configuration.

## HA (production)

`replicaCount > 1` hard-requires Redis:

- Redis (`redis.enabled=true`, `externalRedis.url`, or `REDIS_URL` in `existingSecret`) — for `lenos-pubsub` fan-out
...
autoscaling:
  ...
  websocketMetricName: lenos_ws_connections_active
...
`lenos_ws_connections_active` gauge as a pod metric with the name in
...
1. `LENOS_RELAY_PRIVATE_KEY` — relay identity. Rotating it = new identity (federation peers will not recognize the relay).
2. PostgreSQL database — the canonical event store.
3. S3 bucket — media blobs (chart default bucket: `lenos-media`).
...
- **Bundled MinIO is eval-only.** The quickstart profile runs an in-cluster
  ...
  `minio.enabled` off and points `s3.endpoint` (or `LENOS_S3_*` in
...
- **Minimal-mode is not yet supported.** The relay's `LENOS_PUBSUB=local` /
...
`oci://ghcr.io/buildgrowthnow/lenos/charts/lenos` by the `helm chart` workflow
...
for f in ci/*-values.yaml tests/fixtures/*-values.yaml; do
  helm template lenos . -f "$f" >/dev/null && echo "ok: $f"
done
