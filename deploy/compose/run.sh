...
if [[ "${LENOS_COMPOSE_TLS:-false}" == "true" ]]; then
  COMPOSE_FILES+=(-f compose.caddy.yml)
fi
if [[ "${LENOS_COMPOSE_DEV:-false}" == "true" ]]; then
...
- deploy/compose/.env, especially LENOS_RELAY_PRIVATE_KEY, DB/Redis/S3 secrets, and LENOS_GIT_HOOK_HMAC_SECRET
...
- lenos-git-data volume (LENOS_GIT_REPO_PATH=/data/git)
...
case "${1:-help}" in
  ...
  backup-hint)
    ...
    ;;
  add-member)
    docker compose exec relay /usr/local/bin/lenos-admin add-member --pubkey "${2:?Usage: ./run.sh add-member <npub-or-hex> [--role member|admin]}" "${@:3}"
    ;;
  remove-member)
    docker compose exec relay /usr/local/bin/lenos-admin remove-member --pubkey "${2:?Usage: ./run.sh remove-member <npub-or-hex> [--role member|admin]}" "${@:3}"
    ;;
  list-members)
    docker compose exec relay /usr/local/bin/lenos-admin list-members
    ;;
  help|-h|--help)
    cat <<'MSG'
...
Commands:
  start         Start LenOS with docker compose up -d --wait
...
Environment switches:
  LENOS_COMPOSE_TLS=true   Include compose.caddy.yml for automatic HTTPS
  LENOS_COMPOSE_DEV=true   Include compose.dev.yml for local admin ports/tools
MSG
    ;;
  *)
