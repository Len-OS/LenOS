#!/usr/bin/env bash
# Local docker-desktop k8s testbed for the LenOS relay mesh.
#
...
NS="${NS:-lenos-mesh}"
RELEASE="${RELEASE:-lenos}"
IMAGE_REPO="${IMAGE_REPO:-lenos-relay}"
IMAGE_TAG="${IMAGE_TAG:-mesh-local}"
CHART="${REPO_ROOT}/deploy/charts/lenos"
...
if [ "$helm_rc" != 0 ]; then
  ...
  kubectl -n "$NS" describe pods -l app.kubernetes.io/name=lenos | tee "$EVID/describe-onfail.txt"
  kubectl -n "$NS" logs -l app.kubernetes.io/name=lenos --tail=100 --all-containers | tee "$EVID/logs-onfail.txt"
  die "helm install returned rc=$helm_rc"
fi

# ── 4. verify 3/3 Ready ───────────────────────────────────────────────────────
# Find the relay Deployment: everything under this release named "lenos" except
...
# chart name, so the name isn't always "<release>-lenos".)
DEPLOY=""
...
# The bundled MinIO + init pods share app.kubernetes.io/name=lenos, so select by
...
RELAY_PODS=$(kubectl -n "$NS" get pods \
  -l "app.kubernetes.io/name=lenos,app.kubernetes.io/instance=$RELEASE" \
