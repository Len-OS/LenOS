# LenOS relay bus scaling harness
...
- **new/community-scoped bus:** each pod retains only the server-resolved community topics for which it has local subscribers (`lenos:{community_id}:global` or `lenos:{community_id}:channel:{channel_id}`).
...
The scoped Redis channel format corresponds to `lenos_pubsub::EventTopicKey::redis_channel()` in `crates/lenos-pubsub/src/topic.rs`:

- global: `lenos:{community_id}:global`
- channel: `lenos:{community_id}:channel:{channel_id}`
