#!/usr/bin/env python3
"""Reproducible LenOS relay Redis bus scaling harness.
...
LENOS_PREFIX = "lenos"
OLD_GLOBAL_CHANNEL = f"{LENOS_PREFIX}:global"
...
def scoped_global_channel(community: str) -> str:
    # Mirrors crates/lenos-pubsub/src/topic.rs EventTopic::Global:
    # format!("lenos:{}:global", self.community_id)
    return f"{LENOS_PREFIX}:{community}:global"
...
def print_rows(args: argparse.Namespace, rows: list[Measurement]) -> None:
    print("LenOS relay Redis bus scaling harness")
    print("====================================")
    print(
        "scenario: "
