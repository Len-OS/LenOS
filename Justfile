...
test-unit:
    ...
    if command -v cargo-nextest &>/dev/null; then
        cargo nextest run -p lenos-core -p lenos-auth --lib
        cargo nextest run -p lenos-voice --lib
        cargo nextest run -p lenos-cli
        # lenos-db migrator/lint tests: pure SQL-parsing unit tests (no infra).
        ...
        # and the tenant-scoping lints. The Postgres-backed lenos-db tests are
        # #[ignore]d, so --lib runs only the infra-free set. Without this gate a
        # stray file in migrations/ or a broken lint ships green.
        cargo nextest run -p lenos-db --lib
        # Multi-tenant conformance gate (lenos-conformance): the independent
        ...
        cargo nextest run -p lenos-conformance
        ...
        cargo nextest run -p lenos-push-gateway
        ...
        cargo nextest run -p lenos-backend-kubernetes
    else
