...
import "./lenos-logo-animation.css";
...
export type LenOSLogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function LenOSLogoAnimation({
  ariaLabel = "LenOS logo animation",
  className = "",
  fullScreen = true,
  loop = false,
  loopRestSeconds = 0,
  ...
  const maskId = `lenos-logo-cutouts-${idSuffix}`;
  const textureId = `lenos-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "lenos-logo",
    fullScreen && "lenos-logo--screen",
    !fullScreen && "lenos-logo--compact",
    showBackground && "lenos-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
