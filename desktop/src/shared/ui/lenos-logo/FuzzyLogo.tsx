...
import LenOSLogoAnimation, {
  type LenOSLogoAnimationProps,
} from "./LenOSLogoAnimation";
...
export type FuzzyLogoProps = {
  ...
  pulse?: boolean;
  reverse?: boolean;
  variant?: LenOSLogoAnimationProps["variant"];
};

/**
 * The fuzzy LenOS mark. v8 ships a built-in animated texture (looping fractal-noise
...
export function FuzzyLogo({
  ...
  return (
    <LenOSLogoAnimation
      ariaLabel={ariaLabel}
      className={cn(
        pulse && !fuzz && !hasRestWindow && "lenos-logo--pulse",
        className,
      )}
      fullScreen={false}
      loop={loop}
