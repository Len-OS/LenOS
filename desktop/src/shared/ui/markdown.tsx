...
function ImageBlock({ alt, dim, resolvedSrc, src, thumbSrc }: ImageBlockProps) {
  ...
  React.useLayoutEffect(() => {
    ...
    const spoiler = trigger.closest(".lenos-spoiler[data-spoiler]");
    if (!spoiler) return;
 ...
 *
 * LenOS renders inside a native webview whose default context menu has no
...
function createMarkdownComponents(
  ...
  function MarkdownAnchor({
    ...
    // Intercept `lenos://message?channel=…&id=…` links so a click navigates
  ...
  return {
    ...
    "message-link": function MarkdownMessageLink({
      ...
      if (!parsed.ok) {
        // Malformed `lenos://message?…` — render the raw URL as plain text
