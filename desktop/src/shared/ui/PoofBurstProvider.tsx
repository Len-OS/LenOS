...
export const POOF_TRIGGER_CLASS = "lenos-poof-trigger";
export const POOF_ORIGIN_CLASS = "lenos-poof-origin";
export const POOF_POINTER_ORIGIN_CLASS = "lenos-poof-pointer-origin";
...
export function PoofBurstProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      <div aria-hidden="true" className="lenos-poof-layer">
        {bursts.map((burst) => (
          <div
            className="lenos-poof-burst"
            key={burst.id}
            style={
              {
                "--lenos-poof-size": `${burst.size}px`,
                "--lenos-poof-x": `${burst.x}px`,
                "--lenos-poof-y": `${burst.y}px`,
              } as PoofStyle
            }
          >
            {POOF_FRAMES.map((frame, index) => (
              <img
                alt=""
                className={`lenos-poof-frame lenos-poof-frame-${index + 1}`}
                decoding="async"
                draggable={false}
