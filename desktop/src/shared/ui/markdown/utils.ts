...
export function imageReserveStyle(args: {
  ...
  if (hiddenSpoilerMediaSize) {
    ...
    return {
      "--lenos-spoiler-media-aspect-ratio": ratio,
      "--lenos-spoiler-media-width": `${hiddenSpoilerMediaSize.width}px`,
      aspectRatio: ratio,
      height: "auto",
...
export function isInsideHiddenSpoiler(element: Element): boolean {
  return (
    element.closest('.lenos-spoiler[data-spoiler][data-revealed="false"]') !==
    null
  );
}

/**
 * `urlTransform` for `<ReactMarkdown>` that preserves `lenos://message?…`
