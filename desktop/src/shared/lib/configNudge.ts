/**
 * Utilities for extracting and parsing the `lenos:config-nudge` sentinel that
 * `lenos-acp`'s setup-listener appends to its kind:9 nudge body.
 *
...
/**
 * The structured payload embedded in the `lenos:config-nudge` sentinel block.
...
const FENCE_OPEN = "```lenos:config-nudge";
const FENCE_CLOSE = "```";
...
/**
 * Strip the `lenos:config-nudge` sentinel block (and any preceding blank line)
