...
test("detects known custom emoji-only shortcode messages", () => {
  assert.equal(isEmojiOnlyMessage(":lenos:", CUSTOM_EMOJI), true);
  assert.equal(
    isEmojiOnlyMessage(":lenos: :party_parrot:", CUSTOM_EMOJI),
    true,
  );
  assert.equal(isEmojiOnlyMessage(":LenOS:", CUSTOM_EMOJI), true);
});
...
test("allows mixed unicode and custom emoji", () => {
  assert.equal(isEmojiOnlyMessage("😀 :lenos: ❤️", CUSTOM_EMOJI), true);
});
