...
const CHANNELS = ["lenos-bugs", "lenos dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#lenos", CHANNELS), {
    query: "lenos",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #lenos", CHANNELS), {
    query: "lenos",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#lenos", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#lenos de", CHANNELS), {
    query: "lenos de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#lenos de` — `#` preceded by `x`, no boundary → no query even though
  // "lenos de" is a prefix of a known channel.
  assert.equal(at("#", "x#lenos de", CHANNELS), null);
});
