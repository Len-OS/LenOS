...
function createLenOSCodeBlockHtml(code: string) {
  ...
  return `<pre ${LENOS_CODE_BLOCK_ATTRIBUTE}="true"><code>${escapeHtml(code)}</code></pre>`;
}
  ...
  if (
    ...
    try {
      await clipboard.write([
        new ClipboardItem({
          "text/html": new Blob([createLenOSCodeBlockHtml(code)], {
            type: "text/html",
          }),
...
export function getLenOSCodeBlockClipboardText(
  ...
  const code = document.querySelector(`[${LENOS_CODE_BLOCK_ATTRIBUTE}] code`);
  const fallback = document.querySelector(`[${LENOS_CODE_BLOCK_ATTRIBUTE}]`);
