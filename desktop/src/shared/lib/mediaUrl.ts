 ...
 *
 * Only URLs hosted on the LenOS relay are rewritten. External Blossom URLs
 ...
 * (the LenOS relay), resulting in 404s.
 */
...
/**
 * Cached relay origin (e.g. "https://lenos-oss.stage.blox.sqprod.co"),
 ...
 * (`normalize_host` in lenos-core), but the saved community relay URL keeps
...
/**
 * The relay origin (e.g. `https://lenos-oss.stage.blox.sqprod.co`) if it has
...
/**
 * If `url` is a Blossom media URL hosted on the LenOS relay, rewrite it
 * to go through the local streaming proxy. External Blossom URLs and
 * non-Blossom URLs are returned unchanged.
 *
 * Falls back to lenos-media:// if the proxy port isn't available yet.
 */
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (e.g. wss://PENDING-SEED.communities.lenos.xyz).
  if (cachedRelayOrigin) {
  ...
  return `lenos-media://localhost/media/${m[1]}`;
}
