...
test("applyLegacyCommunityStorage seeds missing communities and active community", () => {
  ...
  assert.equal(storage.getItem("lenos-communities"), legacyCommunities);
  assert.equal(
    storage.getItem("lenos-active-community-id"),
    "legacy-community",
  );
});

test("applyLegacyCommunityStorage preserves existing non-local LenOS communities", () => {
  const storage = createMemoryStorage({
    "lenos-communities": currentCommunities,
    "lenos-active-community-id": "current-community",
  });
  ...
  assert.equal(storage.getItem("lenos-communities"), currentCommunities);
  assert.equal(
    storage.getItem("lenos-active-community-id"),
    "current-community",
  );
...
test("applyLegacyCommunityStorage replaces broken localhost first-run community", () => {
  const storage = createMemoryStorage({
    "lenos-communities": localhostCommunities,
    "lenos-active-community-id": "local-community",
  });
...
test("applyLegacyCommunityStorage treats trailing-slash localhost as broken", () => {
  const storage = createMemoryStorage({
    "lenos-communities": JSON.stringify([
      {
        id: "local-community",
        name: "Local Dev",
        relayUrl: "ws://localhost:3000/",
...
test("applyLegacyCommunityStorage migrates onboarding completion keys", () => {
  ...
  assert.equal(storage.getItem("lenos-onboarding-complete.v1:abc123"), "true");
});
