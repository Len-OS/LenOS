/**
 * Tier-1 lenos-agent model-tuning UI fields.
 *
...
} from "./lenosAgentConfig";

/**
 * Shared effort-select dropdown for the `LENOS_AGENT_THINKING_EFFORT` env var.
 *
 * Used by both `LenOSAgentModelTuningFields` (per-agent/persona dialogs) and
...
export function EffortSelectField({
   ...
   * Defaults to `"Inherit"`.
   *
   * Per-agent callers (LenOSAgentModelTuningFields) pass `"Inherit (agent default)"`
  ...
  const effortOptions: AgentDropdownOption[] = [
    ...
    ...LENOS_AGENT_THINKING_EFFORT_VALUES.flatMap((v) => {
...
/**
 * Auto-clear hook: resets `LENOS_AGENT_THINKING_EFFORT` to "" (Inherit) when
...
export function LenOSAgentModelTuningFields({
  envVars,
  inheritedEnvVars,
  model,
  onEnvVarChange,
  ...
  const currentEffort = envVars[LENOS_AGENT_THINKING_EFFORT] ?? "";
  useEffortAutoClear({
    currentEffort,
    effortValid,
    onClear: () => onEnvVarChange(LENOS_AGENT_THINKING_EFFORT, ""),
  });

  return (
    <div className="space-y-4">
      <p className="text-2xs font-semibold uppercase tracking-wide text-muted-foreground">
        lenos-agent model tuning
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <EffortSelectField
            ...
            inheritedEffort={inheritedEnvVars[LENOS_AGENT_THINKING_EFFORT]}
            inheritFallbackLabel="Inherit (agent default)"
            label="Thinking / Effort"
            onChange={(value) =>
              onEnvVarChange(LENOS_AGENT_THINKING_EFFORT, value)
            }
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="0"
            onChange={(event) =>
              onEnvVarChange(LENOS_AGENT_MAX_ROUNDS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[LENOS_AGENT_MAX_ROUNDS]
                ? `Inherit (${inheritedEnvVars[LENOS_AGENT_MAX_ROUNDS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[LENOS_AGENT_MAX_ROUNDS] ?? ""}
          />
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="1"
            onChange={(event) =>
              onEnvVarChange(LENOS_AGENT_MAX_OUTPUT_TOKENS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[LENOS_AGENT_MAX_OUTPUT_TOKENS]
                ? `Inherit (${inheritedEnvVars[LENOS_AGENT_MAX_OUTPUT_TOKENS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[LENOS_AGENT_MAX_OUTPUT_TOKENS] ?? ""}
          />
          <p
            className="text-xs text-muted-foreground"
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="1"
            onChange={(event) =>
              onEnvVarChange(LENOS_AGENT_MAX_CONTEXT_TOKENS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[LENOS_AGENT_MAX_CONTEXT_TOKENS]
                ? `Inherit (${inheritedEnvVars[LENOS_AGENT_MAX_CONTEXT_TOKENS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[LENOS_AGENT_MAX_CONTEXT_TOKENS] ?? ""}
          />
          <p
            className="text-xs text-muted-foreground"
            id="help-ba-max-context-tokens"
          >
            Maximum context window tokens lenos-agent tracks before a handoff.
