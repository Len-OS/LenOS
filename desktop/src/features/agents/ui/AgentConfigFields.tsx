...
} from "@/features/agents/ui/lenosAgentConfig";
import {
  EffortSelectField,
  useEffortAutoClear,
} from "@/features/agents/ui/lenosAgentModelTuningFields";
...
export function AgentConfigFields({
  ...
  const bakedProvider = React.useMemo(
    () => bakedEnv.find((e) => e.key === "LENOS_AGENT_PROVIDER")?.value ?? null,
    [bakedEnv],
  );
                  ...
                  <EnvVarsEditor
                    ...
                    value={Object.fromEntries(
                      Object.entries(config.env_vars).filter(
                        ([k]) => k !== LENOS_AGENT_THINKING_EFFORT,
                      ),
                    )}
                  />
                </motion.div>
