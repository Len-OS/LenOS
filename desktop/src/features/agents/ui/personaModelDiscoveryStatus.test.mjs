...
test("LenOS shared compute names the empty state and next action", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("no LenOS shared compute serving members are available"),
    "relay-mesh",
  );
...
test("LenOS shared compute distinguishes relay lookup failures", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("LenOS shared compute model discovery failed: relay offline"),
    "relay-mesh",
  );
...
test("LenOS shared compute names a missing relay member roster", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("LenOS shared compute is waiting for the current member roster"),
    "relay-mesh",
  );
...
test("auth-required errors name the agent and ask for sign-in", () => {
  // Real shape from run_agent_models_command wrapping lenos-acp stderr when
  // cursor-agent is signed out (spec ErrorCode::AuthRequired text).
  const status = formatModelDiscoveryErrorStatus(
    new Error(
      "lenos-acp models failed (exit 1): agent communication failed: Agent reported error (code -32000): Authentication required",
    ),
    "",
    "Cursor",
  );
...
test("non-auth -32000 errors do NOT get the sign-in copy", () => {
  ...
  const status = formatModelDiscoveryErrorStatus(
    new Error(
      "lenos-acp models failed (exit 1): Agent reported error (code -32000): model catalog fetch timed out",
    ),
    "anthropic",
    "Cursor",
  );
