...
function isEmptySharedComputeError(message: string): boolean {
  ...
  return (
    ...
    normalized.includes("no lenos shared compute serving members") ||
    normalized.includes("no live lenos shared compute models") ||
...
export function formatModelDiscoveryErrorStatus(
  ...
  if (provider.trim() === "relay-mesh") {
    if (message.includes("waiting for the current member roster")) {
      return {
        message:
          "LenOS is waiting for the relay's member roster. Try again shortly; if this persists, check the relay's membership configuration.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute is not available in this build")) {
      return {
        message:
          "This version of LenOS cannot use shared compute. Update LenOS or choose another provider.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute status is malformed")) {
      return {
        message:
          "LenOS received an invalid shared compute status. Check the member machine, then try again.",
        tone: "warning",
      };
    }
    ...
    return {
      message:
        "LenOS couldn't check shared compute through the relay. Check your relay connection and try again.",
      tone: "warning",
    };
  }

  // Spec-reserved auth error text (agent-client-protocol ErrorCode::AuthRequired),
  // surfaced verbatim through lenos-acp's stderr — generic across conformant
  ...
  if (
    ...
    message.includes("LENOS_AGENT_PROVIDER is required")
  ) {
    return null;
  }
