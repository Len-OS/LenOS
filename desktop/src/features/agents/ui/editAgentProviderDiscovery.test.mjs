...
test("editAgent_providerFieldVisible_forLenOSAgent", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("lenos-agent"),
    true,
    "lenos-agent runtime must expose the provider picker",
  );
});
...
test("editAgent_providerOptions_includesDatabricksV2AndV1OnOSS", () => {
  ...
  const options = getPersonaProviderOptions("", "lenos-agent");
...
test("editAgent_providerOptions_includesCurrentIfCustom", () => {
  const options = getPersonaProviderOptions("my-custom-llm", "lenos-agent");
  const ids = options.map((o) => o.id);
  assert.ok(
...
//
// Switching to lenos-agent runtime (which supports LLM provider selection)
// must make the provider field visible, enabling live discovery.

test("editAgent_runtimeSwitch_toLenOSAgentEnablesProvider", () => {
  // Simulate: user switches from "claude" to "lenos-agent"
  const previousRuntime = "claude";
  const nextRuntime = "lenos-agent";
...
test("editAgent_modelClearedOnRuntimeChange", () => {
  const previousRuntime = "lenos-agent";
...
test("editAgent_catalogArrival_rederivesRuntimeIdWhenNotTouched", () => {
  ...
  const agentCommand = "/usr/local/bin/lenos-agent";
  const catalog = [
    { id: "lenos-agent", command: agentCommand, defaultArgs: [] },
...
test("editAgent_noOpSavePreservesProvider_whenCatalogLate", () => {
  ...
  // If the catalog-arrival effect correctly sets selectedRuntimeId = "lenos-agent",
  // then llmProviderFieldVisible = true and the provider is preserved on save.
  const selectedRuntimeId = "lenos-agent"; // correctly derived after catalog arrival
  const savedProvider = "databricks_v2";
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible once catalog derives lenos-agent runtime",
  );
  assert.equal(
    providerUpdate,
    undefined,
...
test("editAgent_runtimeDropdown_pinsHarnessWhenConcreteCatalogRuntimeSelected", () => {
  ...
  const _nextRuntimeId = "lenos-agent";
  const catalogRuntime = {
    id: "lenos-agent",
    command: "/usr/local/bin/lenos-agent",
    defaultArgs: [],
  };
...
test("editAgent_runtimeDropdown_keepsInheritWhenCatalogEntryHasNoCommand", () => {
  ...
  const NO_RUNTIME_DROPDOWN_VALUE = "__none__";
  const nextValue = "lenos-agent"; // catalog entry, but adapter missing
  const nextRuntimeId =
    nextValue === NO_RUNTIME_DROPDOWN_VALUE ? "" : nextValue;
  const resolvedRuntimeId = nextRuntimeId || "custom";
  const nextRuntime = { id: "lenos-agent", command: null, defaultArgs: [] };
...
test("editAgent_missingRequiredEnvKey_blocksSaveViaValidity", () => {
  ...
  const base = {
    ...
    selectedRuntimeId: "lenos-agent",
    inheritHarness: false,
    agentCommand: "lenos-agent",
...
test("editAgent_inheritedAgentRuntimeSwitch_producesConsistentCommandProviderPair", () => {
  ...
  //
  // After fix: selecting lenos-agent sets inheritHarness=false, so agentCommandUpdate
  // resolves to the lenos-agent command, and provider persists consistently.

  // Initial state: inherited Claude agent
  const inheritHarness = false; // after fix: pinned by runtime switch
  const selectedRuntimeCommand = "/usr/local/bin/lenos-agent";
  ...
  const selectedRuntimeId = "lenos-agent";
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
...
test("editAgent_inheritCheckboxRoundTrip_doesNotPersistProviderOnInheritedRuntime", () => {
  ...
  // → user picks lenos-agent (inheritHarness→false, selectedRuntimeId='lenos-agent')
  // → user picks databricks_v2
  // → user RE-CHECKS inherit (inheritHarness→true, selectedRuntimeId STAYS 'lenos-agent')
  ...
  const selectedRuntimeId = "lenos-agent"; // dropdown state (stale after re-check)
  ...
  const chosenProvider = "databricks_v2"; // chosen while dropdown was lenos-agent

  // llmProviderFieldVisible is driven by the live dropdown (lenos-agent → true).
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field is visible (dropdown shows lenos-agent) — this is the UX state",
  );
...
test("editAgent_inheritCheckboxRoundTrip_clearsStaleSavedProviderWhenRevertingToInherit", () => {
  // Variant: agent previously had a provider saved (e.g. was pinned to lenos-agent
  ...
  const selectedRuntimeId = "lenos-agent"; // dropdown state
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
// persona's runtime is lenos-agent/Goose legitimately carries a provider
...
test("editAgent_inheritedLenOSAgentProvider_preservedOnNameOnlySave", () => {
  // Inherited lenos-agent persona with databricks_v2 snapshot.
  // User makes a name-only edit (never touches runtime or provider).
  // The catalog-arrival effect correctly derived selectedRuntimeId="lenos-agent".

  const inheritHarness = true; // agentCommandOverride == null → inheriting
  const agentCommand = "/usr/local/bin/lenos-agent"; // inherited lenos-agent command
  const runtimes = [
    {
      id: "lenos-agent",
      command: "/usr/local/bin/lenos-agent",
  ...
  const selectedRuntimeId = "lenos-agent"; // correctly derived by catalog-arrival effect
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
  const savedProvider = "databricks_v2"; // valid provider snapshot
  const currentProvider = "databricks_v2"; // unchanged by user

  // llmProviderFieldVisible (UX) is true since dropdown shows lenos-agent.
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible for inherited lenos-agent",
  );

  // The effective runtime for submit: inherited → match agentCommand in catalog → lenos-agent.
  const llmProviderCanPersistAtSubmit = computeCanPersistAtSubmit({
    inheritHarness,
    agentCommand,
    runtimes,
  ...
  assert.equal(
    llmProviderCanPersistAtSubmit,
    true,
    "inherited lenos-agent runtime IS provider-capable — provider must be persistable",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited lenos-agent must omit provider (not send null to clear it)",
  );
});

test("editAgent_inheritedLenOSAgentProvider_clearsWhenUserSwitchesToInheritedClaude", () => {
  // An agent inheriting lenos-agent with databricks_v2, but the persona was
...
test("editAgent_findingE_emptyCatalog_providerOmittedNotCleared", () => {
  ...
  // Inherited lenos-agent agent with saved databricks_v2.
  // A name-only save must omit provider (not send null).

  const inheritHarness = true;
  const agentCommand = "lenos-agent"; // inherited command (short form)
...
test("editAgent_findingE_commandNullCatalogEntry_providerPreservedByIdMatch", () => {
  // Form 2: catalog loaded, but lenos-agent's entry has command:null (adapter
  ...
  const inheritHarness = true;
  const agentCommand = "lenos-agent"; // inherited command (short form = runtime id)
  const runtimes = [
    { id: "lenos-agent", command: null, defaultArgs: [] }, // adapter missing
  ...
  assert.equal(
    capability,
    "capable",
    "command:null lenos-agent entry must resolve to 'capable' via id-based fallback",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited lenos-agent (command:null) must omit provider, not clear it",
  );
});
...
test("editAgent_findingE_lockedRuntimeStillClears", () => {
  ...
  const runtimes = [
    { id: "lenos-agent", command: "lenos-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  const selectedRuntimeId = "lenos-agent"; // stale dropdown state (irrelevant)
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
test("editAgent_findingE_capableLenOSAgentLoadedCatalog_preservedOnNoOpSave", () => {
  // Confirm loaded-catalog inherited lenos-agent still preserves provider.
  // This is Finding D's good path — must not regress with the tri-state change.

  const inheritHarness = true;
  const agentCommand = "lenos-agent";
  const runtimes = [
    { id: "lenos-agent", command: "lenos-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  ...
  assert.equal(capability, "capable", "loaded lenos-agent must be 'capable'");
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "no-op save on inherited lenos-agent (loaded catalog) must omit provider",
  );
});
...
//
// Problem: lenos-agent stores agentCommand="lenos-agent" (short name) while the
// catalog entry has command="/Applications/LenOS.app/.../lenos-agent" (resolved path).
...
test("editAgent_bugA_inheritedShortName_resolvesViaIdFallback", () => {
  // The core regression: agentCommand is the short name "lenos-agent" but the
  // catalog's command is the resolved binary path. Command match fails; id
  // fallback must rescue it.

  const agentCommand = "lenos-agent"; // short name stored by effective_agent_command
  const runtimes = [
    {
      id: "lenos-agent",
      command: "/Applications/LenOS.app/Contents/MacOS/lenos-agent", // resolved path
  ...
  assert.equal(
    selectedRuntimeId,
    "lenos-agent",
    "short-name agentCommand must resolve to lenos-agent id via id-fallback when command path differs",
  );
});
...
test("editAgent_bugA_inheritedShortName_commandMatchStillWinsWhenPresent", () => {
  ...
  const agentCommand = "/usr/local/bin/lenos-agent"; // full path (explicit pin)
  const runtimes = [
    {
      id: "lenos-agent",
      command: "/usr/local/bin/lenos-agent",
...
test("editAgent_bugA_inheritedShortName_discoveryGatePasses", () => {
  // Once selectedRuntimeId is correctly resolved to "lenos-agent" via id-fallback,
  ...
  const agentCommand = "lenos-agent"; // short name
  const runtimes = [
    {
      id: "lenos-agent",
      command: "/Applications/LenOS.app/Contents/MacOS/lenos-agent",
      availability: "available",
      defaultArgs: [],
    },
  ];
  ...
  assert.equal(
    selectedRuntimeId,
    "lenos-agent",
    "id-fallback must resolve selectedRuntimeId to lenos-agent",
  );
  assert.ok(
  ...
  assert.equal(
    ...
    "/Applications/LenOS.app/Contents/MacOS/lenos-agent",
    "discoveryAgentCommand must be the resolved path from the catalog entry",
  );
  assert.equal(
    canDiscoverModelOptions,
    true,
    "discovery gate must pass for inherited lenos-agent with databricks_v2 provider once id-fallback resolves the runtime",
  );
});
...
test("requiredCredentialEnvKeys: lenos-agent + anthropic → ANTHROPIC_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("lenos-agent", "anthropic");
  assert.deepEqual(keys, ["ANTHROPIC_API_KEY"]);
});

test("requiredCredentialEnvKeys: lenos-agent + openai → OPENAI_COMPAT_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("lenos-agent", "openai");
  assert.deepEqual(keys, ["OPENAI_COMPAT_API_KEY"]);
});

test("requiredCredentialEnvKeys: lenos-agent + databricks → DATABRICKS_HOST only (no token)", () => {
  const keys = requiredCredentialEnvKeys("lenos-agent", "databricks");
...
test("requiredCredentialEnvKeys: lenos-agent + databricks_v2 → DATABRICKS_HOST only", () => {
  const keys = requiredCredentialEnvKeys("lenos-agent", "databricks_v2");
  assert.deepEqual(keys, ["DATABRICKS_HOST"]);
});
...
test("requiredCredentialEnvKeys: lenos-agent + no provider → empty (provider not yet selected)", () => {
  const keys = requiredCredentialEnvKeys("lenos-agent", "");
...
test("blockSave_lenosAgentAnthropicMissingKey_blocked", () => {
  // Will's exact case: lenos-agent / anthropic / opus / no ANTHROPIC_API_KEY
  const requiredKeys = requiredCredentialEnvKeys("lenos-agent", "anthropic");
...
test("blockSave_lenosAgentAnthropicKeyProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("lenos-agent", "anthropic");
...
test("blockSave_lenosAgentAnthropicEmptyStringKey_blocked", () => {
...
test("blockSave_lenosAgentDatabricksMissingHost_blocked", () => {
  const requiredKeys = requiredCredentialEnvKeys("lenos-agent", "databricks");
  const envVars = {};
  assert.equal(
    hasRequiredEnvKeyMissing(requiredKeys, envVars),
    true,
...
test("blockSave_lenosAgentDatabricksHostProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("lenos-agent", "databricks");
...
//   FALSE-ALLOW: claude pin → inherit lenos-agent persona → missing ANTHROPIC_API_KEY
//     → must be BLOCKED (prospective runtime is lenos-agent/anthropic, key absent)
//   FALSE-BLOCK: lenos-agent pin → inherit claude persona
//     → must NOT be blocked (claude has no dialog-fixable credential requirement)

test("blockSave_inheritTransition_claudePin_toLenOSAgentPersona_missingKey_blocked", () => {
  ...
  // from persona" where the persona uses lenos-agent/anthropic.
  // prospectiveRuntimeId resolves to "lenos-agent"; providerForRequiredKeys must
  // use the PROSPECTIVE runtime's provider-field visibility (lenos-agent supports
  // provider selection) rather than the current locked runtime's suppression.
  const prospectiveRuntimeId = "lenos-agent"; // resolved from persona's agentCommand
  ...
  // providerForRequiredKeys must be "anthropic" (lenos-agent supports selection)
  ...
  assert.equal(
    missing,
    true,
    "inheriting lenos-agent/anthropic persona with no key must BLOCK save (false-allow prevented)",
  );
});

test("blockSave_inheritTransition_lenosAgentPin_toClaudePersona_notBlocked", () => {
  // Scenario: agent is pinned to lenos-agent/anthropic. The user checks
