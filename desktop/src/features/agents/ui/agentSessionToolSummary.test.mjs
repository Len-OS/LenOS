...
test("buildCompactToolSummary formats LenOS send_message preview", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "send_message",
      lenosToolName: "send_message",
      title: "Send Message",
...
test("buildCompactToolSummary treats lenos messages send commands as messages", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "lenos-dev-mcp__shell",
      args: {
        command:
          'lenos --format compact messages send --channel channel-1 --content "@Ned are you working"',
      },
    }),
  );
...
test("buildCompactToolSummary returns null preview for piped stdin sends", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      ...
      args: {
        command:
          'echo "hello from stdin" | ./target/release/lenos messages send --channel channel-1 --content -',
      },
    }),
  );
...
test("buildCompactToolSummary formats view_image thumbnail source", () => {
  ...
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "lenos-dev-mcp__view_image",
      args: { source },
    }),
  );
...
test("buildCompactToolSummary promotes non-send lenos CLI commands to relay ops", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: {
        command: "lenos channels get --channel channel-1",
      },
    }),
  );
  ...
  assert.equal(summary.presentation, "inline");
  assert.equal(summary.shellContent, "lenos channels get --channel channel-1");
});

test("buildCompactToolSummary exposes shellContent for shell-sourced lenos CLI reads", () => {
  const command =
    "sleep 45; lenos messages thread --channel channel-uuid --event abc | tail -n 20";
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: { command },
...
test("buildCompactToolSummary derives structured actions for native LenOS MCP tools", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "get_channel",
      lenosToolName: "get_channel",
      args: {
