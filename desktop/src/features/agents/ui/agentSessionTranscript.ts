...
function upsertTool(
  ...
  const existing = d.itemsById.get(id);
  const canonicalLenOSToolName =
    lenosToolName ?? findLenOSToolName(toolName, true);
  if (existing?.type === "tool") {
    const updatedTitle = !isGenericToolTitle(title) ? title : existing.title;
    let updatedToolName = existing.toolName;
    let updatedLenOSToolName = existing.lenosToolName;
    if (canonicalLenOSToolName) {
      updatedLenOSToolName = canonicalLenOSToolName;
      updatedToolName = canonicalLenOSToolName;
    } else if (!existing.lenosToolName && !isGenericToolTitle(toolName)) {
    ...
    const descriptor = classifyTool({
      ...
      lenosToolName: updatedLenOSToolName,
      args: updatedArgs,
  ...
  const resolvedToolName = canonicalLenOSToolName ?? toolName;
  const descriptor = classifyTool({
    title,
