...
test("localMode_lenosAgent_supportsProviderSelection", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("lenos-agent"),
    true,
    "lenos-agent must support LLM provider selection",
  );
});
...
test("localMode_lenosAgent_emptyProvider_notSatisfied", () => {
  // Scenario: user selects lenos-agent but leaves provider empty.
  // Rust readiness requires LENOS_AGENT_PROVIDER — empty = NotReady.
...
test("localMode_lenosAgent_emptyModel_notSatisfied", () => {
  // Scenario: lenos-agent + anthropic + API key present, but model left empty.
  // Rust readiness requires LENOS_AGENT_MODEL — empty = NotReady.
...
// ── Gate: lenos-agent / anthropic with missing key → markers shown ─────────

test("localMode_lenosAgent_anthropic_missingKey_notSatisfied", () => {
  // Scenario: user selects lenos-agent/anthropic + fills model, but hasn't
...
test("localMode_lenosAgent_anthropic_allRequired_present_allowed", () => {
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("lenos-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for lenos-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("lenos-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in LenOS env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("lenos-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("lenos-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("lenos-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("lenos-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("lenos-agent", "relay-mesh"),
    "Use agent defaults (LenOS shared compute)",
  );
});
...
test("bakedDefaults_emptyGlobal_usesBuildValuesForCreateAndEditLabels", () => {
  const bakedEnv = [
    { key: "LENOS_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    {
  ...
  const model = resolveInheritedDefault(null, bakedEnv, "LENOS_AGENT_MODEL");
  const effort = resolveInheritedDefault(
    null,
    bakedEnv,
    "LENOS_AGENT_THINKING_EFFORT",
  );
  const providerOptions = getPersonaProviderOptions("", "lenos-agent");
...
test("bakedDefaults_explicitGlobalsOverrideBuildValues", () => {
  ...
  assert.deepEqual(
    resolveInheritedDefault("anthropic", bakedEnv, "LENOS_AGENT_PROVIDER"),
    { source: "global", value: "anthropic" },
  );
  assert.deepEqual(
    resolveInheritedDefault("global-model", bakedEnv, "LENOS_AGENT_MODEL"),
    { source: "global", value: "global-model" },
  );
  assert.deepEqual(
    resolveInheritedDefault("low", bakedEnv, "LENOS_AGENT_THINKING_EFFORT"),
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("lenos-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("lenos-agent", "");
...
// include ANTHROPIC_API_KEY for explicit lenos-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("lenos-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "lenos-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("lenos-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("lenos-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  const databricksKeys = requiredCredentialEnvKeys("lenos-agent", "databricks");
  assert.ok(
    databricksKeys.includes("DATABRICKS_HOST"),
    "lenos-agent + databricks must list DATABRICKS_HOST as required",
  );
