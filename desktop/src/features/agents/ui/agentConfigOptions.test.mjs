...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "lenos-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "lenos-agent");
  const tail = options.at(-1);
...
test("getDefaultPersonaRuntime ignores an unavailable global preference", () => {
  ...
  assert.equal(getDefaultPersonaRuntime(runtimes, "claude")?.id, "lenos-agent");
});

test("getDefaultPersonaRuntime returns lenos-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("lenos-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "lenos-agent");
});

test("getDefaultPersonaRuntime falls back to goose when lenos-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("lenos-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither lenos-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("lenos-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for lenos-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("lenos-agent"), true);
...
test("resetConfigForHarnessChange clears harness-specific values", () => {
  const config = {
    env_vars: { LENOS_AGENT_THINKING_EFFORT: "high", KEEP_ME: "yes" },
    model: "claude-opus",
...
test("getPersonaModelOptions for lenos-agent with anthropic returns the default model", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("lenos-agent", "anthropic");
  const zeroValue = options.find((o) => o.id === "");
  assert.equal(
    zeroValue,
    options[0],
    "LenOS Agent keeps the default model option for provider discovery",
  );
});

test("getPersonaModelOptions for lenos-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("lenos-agent", "");
