 ...
 * persona runtime string (e.g. `lenos-agent`). A catalog lookup then returns
 ...
 *
 * - lenos-agent / goose → "capable" (`isProviderCapable`, id-based).
 ...
 * lenos-agent/Anthropic). Persisting the local (empty) provider + credential-
 ...
 *
 * MODEL follows the same transition rule. lenos-agent/goose readiness requires a
