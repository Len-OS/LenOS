...
export function AgentDefinitionDialog({
  ...
  const hideProviderIds = React.useMemo(
    () =>
      (bakedEnvKeys ?? []).includes("LENOS_AGENT_PROVIDER")
        ? BLOCK_BUILD_HIDDEN_PROVIDER_IDS
        : new Set<string>(),
    [bakedEnvKeys],
  );
  ...
  function handleProviderDropdownChange(nextValue: string) {
    ...
    if (nextProvider === "relay-mesh" && runtime !== "lenos-agent") {
      handleRuntimeDropdownChange("lenos-agent");
    }
    const nextSelection = selectionOnProviderDropdownChange(selection, {
      runtime: nextProvider === "relay-mesh" ? "lenos-agent" : runtime,
      nextValue,
