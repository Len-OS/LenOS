...
import {
  ...
  LENOS_AGENT_MAX_ROUNDS,
  LENOS_AGENT_THINKING_EFFORT,
  LENOS_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isLenOSAgentRuntime,
} from "./lenosAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("LENOS_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...LENOS_AGENT_THINKING_EFFORT_VALUES],
...
test("LENOS_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(LENOS_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, LENOS_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected LENOS_AGENT_* names", () => {
  assert.equal(LENOS_AGENT_THINKING_EFFORT, "LENOS_AGENT_THINKING_EFFORT");
  assert.equal(LENOS_AGENT_MAX_OUTPUT_TOKENS, "LENOS_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(
    LENOS_AGENT_MAX_CONTEXT_TOKENS,
    "LENOS_AGENT_MAX_CONTEXT_TOKENS",
  );
  assert.equal(LENOS_AGENT_MAX_ROUNDS, "LENOS_AGENT_MAX_ROUNDS");
});
...
test("isLenOSAgentRuntime returns true only for lenos-agent id", () => {
  assert.equal(isLenOSAgentRuntime("lenos-agent"), true);
});

test("isLenOSAgentRuntime returns false for other runtimes", () => {
  assert.equal(isLenOSAgentRuntime("goose"), false);
  assert.equal(isLenOSAgentRuntime("custom"), false);
  assert.equal(isLenOSAgentRuntime(""), false);
  assert.equal(isLenOSAgentRuntime("lenos-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  ...
  assert.equal(result[LENOS_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [LENOS_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, LENOS_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, LENOS_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact LENOS_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    LENOS_AGENT_MAX_OUTPUT_TOKENS,
  ...
  assert.equal(result[LENOS_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [LENOS_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, LENOS_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, LENOS_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact LENOS_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    LENOS_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[LENOS_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [LENOS_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, LENOS_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, LENOS_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact LENOS_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, LENOS_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[LENOS_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [LENOS_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, LENOS_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, LENOS_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [LENOS_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, LENOS_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[LENOS_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[LENOS_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      LENOS_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(LENOS_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[LENOS_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isLenOSAgent` derivation in CreateAgentRuntimeFields.
...
test("isLenOSAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves lenos-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is lenos-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "lenos-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "lenos-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isLenOSAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isLenOSAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'lenos-agent' MUST trigger model-tuning fields",
  );
});

test("isLenOSAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned lenos-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "lenos-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "lenos-agent";

  assert.equal(isLenOSAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isLenOSAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isLenOSAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-lenos-agent", () => {
  // E.g. user switches from lenos-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isLenOSAgentRuntime(prospectiveRuntimeId), false);
});

test("isLenOSAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
...
// useEffect in LenOSAgentModelTuningFields and AgentDefaultsSettingsCard
