/**
 * Source-of-truth constants for lenos-agent model-tuning configuration knobs.
 *
 * Values must stay in sync with `crates/lenos-agent/src/config.rs`
 * `parse_thinking_effort` — that function is the authoritative list.
 */

/** Env var key for the thinking/effort level sent to the LLM. */
export const LENOS_AGENT_THINKING_EFFORT = "LENOS_AGENT_THINKING_EFFORT";

/** Env var key for the maximum output token count per turn. */
export const LENOS_AGENT_MAX_OUTPUT_TOKENS = "LENOS_AGENT_MAX_OUTPUT_TOKENS";

/** Env var key for the context window token limit. */
export const LENOS_AGENT_MAX_CONTEXT_TOKENS = "LENOS_AGENT_MAX_CONTEXT_TOKENS";

/** Env var key for the maximum number of LLM/tool rounds per turn. */
export const LENOS_AGENT_MAX_ROUNDS = "LENOS_AGENT_MAX_ROUNDS";

/**
 * Ordered set of valid thinking-effort values accepted by lenos-agent.
 * Mirrors `parse_thinking_effort` in `crates/lenos-agent/src/config.rs`.
 */
export const LENOS_AGENT_THINKING_EFFORT_VALUES = [
  "none",
  "minimal",
  "low",
  "medium",
...
export type ThinkingEffortValue =
  (typeof LENOS_AGENT_THINKING_EFFORT_VALUES)[number];
 ...
 *
 * Mirrors the model-family tables in `crates/lenos-agent/src/config.rs`
...
const ALL_VALUES = LENOS_AGENT_THINKING_EFFORT_VALUES;
...
/**
 * Returns true when the given runtime id is lenos-agent, which is the only
 * runtime that supports the tier-1 model-tuning knobs above.
 */
export function isLenOSAgentRuntime(runtimeId: string): boolean {
  return runtimeId === "lenos-agent";
}
