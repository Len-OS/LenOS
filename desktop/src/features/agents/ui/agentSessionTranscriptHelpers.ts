...
export function parsePromptText(text: string): {
  ...
  const eventSection = sections.find((section) => {
    ...
    return title.startsWith("lenos event");
  });
  ...
  return {
    ...
    userTitle: eventKind ? titleCase(eventKind) : "LenOS event",
 ...
 *    `with_team()` in `lenos-acp/src/pool.rs`. Same two cases (start-of-string
 ...
 *    (produced by the now-removed `compose_prompt()` in lenos-persona), the body
...
export function parseSystemPromptSections(
  ...
  // with_team() in lenos-acp/src/pool.rs appends "\n\n[Team Instructions]\n{instructions}"
...
export function extractToolIdentity(update: Record<string, unknown>): {
  ...
  const knownName = candidates
    .map((candidate) => findLenOSToolName(candidate, true))
