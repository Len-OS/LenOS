 ...
 *   lenos-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/lenos-agent/src/types.rs`.
 *   lenos-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped lenos-agent prefix or the lenos-acp v0 wrap.
  if (
