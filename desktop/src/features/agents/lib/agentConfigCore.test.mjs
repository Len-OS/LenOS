...
test("LenOS Agent exposes provider, model, and LenOS-owned effort", () => {
  const model = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("lenos-agent", {
      modelEnvVar: "LENOS_AGENT_MODEL",
      providerEnvVar: "LENOS_AGENT_PROVIDER",
      thinkingEnvVar: "LENOS_AGENT_THINKING_EFFORT",
    }),
    scope: "global",
  });
  ...
  assert.equal(field(model, "effort").optionSource, "lenosAgentCatalog");
...
test("catalog mismatch cleanup is named and restricted to onboarding", () => {
  const selectedRuntime = runtime("lenos-agent", {
    modelEnvVar: "LENOS_AGENT_MODEL",
    providerEnvVar: "LENOS_AGENT_PROVIDER",
    thinkingEnvVar: "LENOS_AGENT_THINKING_EFFORT",
  });
