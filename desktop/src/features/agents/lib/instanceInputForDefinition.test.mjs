...
const lenosAgentRuntime = {
  ...gooseRuntime,
  id: "lenos-agent",
  label: "LenOS Agent",
  command: "lenos-agent-cmd",
  mcpCommand: null,
};
...
test("mapping carries the runtime and definition fields", async () => {
  ...
  assert.equal(input.acpCommand, "lenos-acp");
...
test("LenOS shared compute definition carries native provider and auto model", async () => {
  const input = await buildInstanceInputForDefinition(
    persona({
      runtime: "lenos-agent",
      provider: "relay-mesh",
    ...
    { ...gooseRuntime, id: "lenos-agent", command: "lenos-agent" },
  );
  assert.equal(input.agentCommand, "lenos-agent");
...
// ── item-13 regression: lenos-agent-first default runtime ────────────────────
//
// Before this fix, resolveStartRuntimeForDefinition used runtimes[0] (catalog
// order: goose, claude, codex, lenos-agent), so an installed goose would beat
// the bundled lenos-agent sidecar as the default for runtime-less personas.
// The fix applies the preference order: lenos-agent → goose → first available.

test("item-13: goose+lenos-agent both available — persona with no runtime resolves lenos-agent", () => {
  const { runtime, warnings } = resolveStartRuntimeForDefinition(
    persona({ runtime: undefined }),
    [gooseRuntime, claudeRuntime, lenosAgentRuntime],
  );
  assert.equal(
    runtime.id,
    "lenos-agent",
    "lenos-agent must win over catalog-first goose for runtime-less personas",
  );
  assert.deepEqual(warnings, []);
});
