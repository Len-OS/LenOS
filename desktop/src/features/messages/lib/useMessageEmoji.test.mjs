...
const EMOJI_TAGS = [["emoji", "lenos", "https://relay/lenos.png"]];
...
test("derives custom emoji and emoji-only flag from event tags", () => {
  ...
  assert.deepEqual(customEmoji, [
    { shortcode: "lenos", url: "https://relay/lenos.png" },
  ]);
  assert.equal(isEmojiOnlyMessage(":lenos:", customEmoji), true);
  assert.equal(isEmojiOnlyMessage("hi :lenos:", customEmoji), false);
});

test("messages without tags get no custom emoji and are never emoji-only", () => {
  const customEmoji = undefined;
  assert.equal(isEmojiOnlyMessage(":lenos:", customEmoji), false);
});
