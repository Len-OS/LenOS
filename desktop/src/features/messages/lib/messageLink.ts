/**
 * `lenos://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `lenos://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "lenos:";
...
/**
 * Build a `lenos://message` URL for a given channel + message.
 *
...
/**
 * Parse a `lenos://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("lenos://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("lenos://message?") || href === "lenos://message";
}
 ...
 * CommonMark autolinks (`<lenos://message?...>`) and explicitly labeled links
