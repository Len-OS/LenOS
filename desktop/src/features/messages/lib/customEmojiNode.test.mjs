...
test("rule fires for a boundary :shortcode: (start of string)", () => {
  const rule = captureRule(["lenos"]);
  const { matched, advanced } = runRule(rule, ":lenos:", 0);
  assert.equal(matched, true);
  assert.equal(advanced, ":lenos:".length);
});

test("rule fires for a :shortcode: preceded by whitespace", () => {
  const rule = captureRule(["lenos"]);
  // pos points at the `:` after the space.
  const { matched } = runRule(rule, "hi :lenos:", 3);
  assert.equal(matched, true);
});

test("rule does NOT fire when the colon is glued to a word char (not:lenos:)", () => {
  const rule = captureRule(["lenos"]);
  // pos points at the `:` immediately after `not`.
  const { matched } = runRule(rule, "not:lenos:", 3);
  assert.equal(matched, false);
});

test("rule does NOT fire inside a URL-like sequence (http://x:y:lenos:)", () => {
  const rule = captureRule(["lenos"]);
  const src = "http://x:y:lenos:";
  // pos points at the `:` immediately after `y` (a word char).
  const { matched } = runRule(rule, src, src.indexOf(":lenos:"));
  assert.equal(matched, false);
});
...
test("rule fires after punctuation boundary (e.g. parenthesis)", () => {
  ...
  const { matched } = runRule(rule, "(:lenos:)", 1);
  assert.equal(matched, true);
});
