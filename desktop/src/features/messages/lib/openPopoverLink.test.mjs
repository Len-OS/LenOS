...
test("lenos://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(`lenos://message?channel=${CHANNEL}&id=${MESSAGE}`, handlers);
...
test("non-message lenos:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("lenos://channel?foo=bar", handlers);
  assert.deepEqual(external, ["lenos://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed lenos://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with lenos://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("lenos://message?nope=1", handlers);
  assert.deepEqual(external, ["lenos://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
