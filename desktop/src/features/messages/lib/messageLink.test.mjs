...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `lenos://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `lenos://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `lenos://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects lenos:// with wrong host", () => {
  const r = parseMessageLink(`lenos://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`lenos://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`lenos://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy lenos://message links", () => {
  const r = parseMessageLink(
    `lenos://message?channel=${CHANNEL}&id=${MESSAGE}`,
  );
...
test("isMessageLink matches lenos://message and legacy lenos://message", () => {
  assert.equal(
    isMessageLink(`lenos://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`lenos://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("lenos://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("lenos://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `lenos://message?channel=${CHANNEL}&id=${MESSAGE}`;
