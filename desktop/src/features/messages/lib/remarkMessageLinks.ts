/**
 * Remark plugin that detects bare `lenos://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `lenos://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `lenos://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](lenos://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /(?:lenos|lenos):\/\/message\?[^\s<>"')\]]+/g;
