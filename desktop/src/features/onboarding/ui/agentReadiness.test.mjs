...
test("resolveAgentReadiness_cli_ignores_lenos_agent_runtime", () => {
  // lenos-agent with availability=available and logged_in must NOT trigger the CLI path.
  const runtimes = [
    makeRuntime({
      id: "lenos-agent",
      label: "lenos-agent",
...
test("resolveAgentReadiness_lenos_agent_ready_when_provider_model_and_key_set", () => {
  // anthropic requires ANTHROPIC_API_KEY
  const result = resolveAgentReadiness(
    [makeRuntime({ id: "lenos-agent", label: "LenOS Agent" })],
    makeConfig({
  ...
  assert.deepEqual(result, { ready: true, reason: "lenos-agent" });
});

test("resolveAgentReadiness_lenos_agent_not_ready_when_missing_required_credential_key", () => {
...
test("resolveAgentReadiness_lenos_agent_not_ready_when_provider_missing", () => {
  const config = makeConfig({
    provider: null,
    model: "claude-3-5-sonnet-latest",
    env_vars: { ANTHROPIC_API_KEY: "sk-ant-test" },
...
test("resolveAgentReadiness_lenos_agent_not_ready_when_model_missing", () => {
  const config = makeConfig({
    provider: "anthropic",
    model: null,
    env_vars: { ANTHROPIC_API_KEY: "sk-ant-test" },
...
test("resolveAgentReadiness_legacy_config_without_preference_uses_lenos_agent_fields", () => {
  const runtimes = [makeRuntime({ id: "lenos-agent", label: "LenOS Agent" })];
  const result = resolveAgentReadiness(
    runtimes,
    makeConfig({
...
test("resolveAgentReadiness_preferred_goose_does_not_borrow_ready_lenos_agent_config", () => {
  const runtimes = [
    makeRuntime({ id: "goose", label: "Goose" }),
    makeRuntime({ id: "lenos-agent", label: "LenOS Agent" }),
  ];
