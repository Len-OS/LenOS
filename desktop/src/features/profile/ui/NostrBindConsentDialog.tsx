...
const COPY_SUCCESS_MESSAGE =
  "Signed response copied. Paste it into the LenOS admin console.";
const PREVIEW_COPY_SUCCESS_MESSAGE = "Preview response copied.";
const COPY_FAILURE_MESSAGE = "LenOS couldn't access the clipboard. Try again.";
...
function appendCallbackStatus(callbackUrl: string): string {
  ...
  url.searchParams.set("lenos_bind", "signed");
  return url.toString();
}
    ...
    const unlistenPromise = listenForNostrBindDeepLinks((nextPayload) => {
      ...
      getIdentity()
        ...
        .catch((error) => {
          ...
          setIdentity(null);
          setError("Could not load the current LenOS identity.");
        });
    });
      ...
      <DialogPrimitive.Portal>
        {payload ? (
          <DialogPrimitive.Content
            ...
            className="lenos-onboarding-neutral-theme lenos-startup-shell fixed inset-0 z-50 flex overflow-y-auto bg-background px-4 py-12 text-foreground outline-hidden"
            ...
            <div className="m-auto flex w-full max-w-[500px] flex-col items-center text-center">
              ...
              {signedResponse ? (
                ...
                >
                  ...
                  >
                    {payload.returnMode === "browser_fragment_v1"
                      ? "LenOS opened your browser to finish verification."
                      : "Copy the response below, then paste it into the LenOS website to finish verification."}
                  </DialogPrimitive.Description>
