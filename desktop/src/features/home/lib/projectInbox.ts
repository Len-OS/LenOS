...
/** Returns the canonical LenOS Git repository and root event for an Inbox row. */
...
/** Whether a feed event belongs to a LenOS Git pull request or issue thread. */
export function isProjectInboxItem(item: FeedItem) {
  return getProjectInboxReference(item) !== null;
}

/** Resolves an Inbox event to the current canonical LenOS Git work item. */
