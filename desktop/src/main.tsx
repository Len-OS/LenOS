...
function resetDevWebviewStateFromUrl() {
  ...
  // WebKit groups every LenOS binary under one disk directory, but storage is
...
function configureDevE2eBridgeFromUrl() {
  ...
  e2eWindow.__LENOS_E2E__ ??= { mode: "mock" };
  ...
  window.localStorage.setItem("lenos-communities", JSON.stringify([community]));
  window.localStorage.setItem("lenos-active-community-id", E2E_COMMUNITY_ID);
    ...
    <React.StrictMode>
      <CommunitiesProvider>
        <CommunityOnboardingProvider>
          <ThemeProvider defaultTheme="lenos">
...
async function installE2eBridgeIfConfigured() {
  ...
  if (
    ...
    !(window as E2eWindow).__LENOS_E2E__
  ) {
    return;
  }
