...
async function seedAudience(page: Page, pubkeys: string[], theme = "lenos") {
  await page.addInitScript(
    ({ audience, scope, selectedTheme }) => {
      window.localStorage.setItem("lenos:keep-addressed-agents-active", "1");
      window.localStorage.setItem(
        "lenos:persistent-agent-audiences:v2",
      ...
      page.evaluate(
        ({ owner, channelId, rootId }) => {
          const stored = JSON.parse(
            localStorage.getItem("lenos:persistent-agent-audiences:v2") ?? "{}",
          );
...
for (const theme of ["lenos", "lenos-dark"]) {
