...
const LENOS_REPO_ADDRESS = `30617:${DEFAULT_MOCK_PUBKEY}:lenos`;

test("LenOS Git pull request renders and stays actionable in Inbox", async ({
  page,
}) => {
  ...
  await page.evaluate(
    ({ author, id, repoAddress, viewer }) => {
      window.__LENOS_E2E_PUSH_MOCK_FEED_ITEM__?.({
        id,
        kind: 1618,
        pubkey: author,
    ...
    },
    {
      ...
      repoAddress: LENOS_REPO_ADDRESS,
