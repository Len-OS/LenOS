...
function runtime(
  id: "lenos-agent" | "claude" | "codex" | "goose",
  availability: string,
...
test("setup shows all bundled harnesses as detected", async ({ page }) => {
  await installMockBridge(
    ...
    {
      acpRuntimesCatalog: [
        runtime("lenos-agent", "available", { status: "not_applicable" }),
...
test("defaults renders only fields supported by the selected harness", async ({
  ...
  await installMockBridge(
    page,
    {
      ...
      globalAgentConfig: {
        env_vars: { LENOS_AGENT_THINKING_EFFORT: "high" },
        provider: null,
        model: "stale-model",
...
test("defaults auto-selects the only ready visible harness", async ({
  ...
  await installMockBridge(
    ...
    {
      acpRuntimesCatalog: [
        runtime("lenos-agent", "not_installed", { status: "not_applicable" }),
...
test("defaults requires a choice when multiple visible harnesses are ready", async ({
  ...
  await expect(
    page.getByTestId("global-agent-default-harness-option-lenos-agent"),
  ).toBeVisible();
  await page.getByTestId("global-agent-default-harness-option-codex").click();
...
test("Finish stays disabled until a provider-required harness is fully configured", async ({
  ...
  // lenos-agent auto-selects as the only ready harness, but with no provider
  ...
  await finish.click();
  await expect(page.getByText("Join or create a community")).toBeVisible();
  expect(await readSavedRuntime(page)).toBe("lenos-agent");
});
...
test("baked build config keeps Finish enabled without manual provider setup", async ({
  ...
  await installMockBridge(
    page,
    {
      ...
      bakedBuildEnv: [
        { key: "LENOS_AGENT_PROVIDER", masked: false, value: "databricks_v2" },
        {
          key: "DATABRICKS_HOST",
          masked: false,
