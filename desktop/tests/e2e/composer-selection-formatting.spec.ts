    ...
    .poll(() =>
      page.evaluate(
        () =>
          (
            window as Window & {
              __LENOS_E2E_SIGNED_EVENTS__?: Array<{ content: string }>;
            }
          ).__LENOS_E2E_SIGNED_EVENTS__?.at(-1)?.content,
      ),
    )
    .toBe("```\none\ntwo\nthree\n```");
});
...
test("LenOS theme uses the primary color for the selection formatter", async ({
  page,
}) => {
  await openGeneral(page);
