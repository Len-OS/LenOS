...
const enabled = process.env.LENOS_E2E_AGENTS_EVERYWHERE === "1";
const relayBin = process.env.LENOS_E2E_RELAY_BIN;
const cliBin = process.env.LENOS_E2E_CLI_BIN;
const adminBin = process.env.LENOS_E2E_ADMIN_BIN;
...
async function run(
  ...
  const { stdout } = await exec(binary, args, {
    cwd: "..",
    env: { ...process.env, LENOS_AUTH_TAG: "", ...env },
  });
  return stdout;
}
...
test.describe("agents everywhere live two-relay gate", () => {
  test.skip(!enabled, "set LENOS_E2E_AGENTS_EVERYWHERE=1 to run live gate");
  ...
  test("same agent listens and wakes independently in two communities", async () => {
    ...
    const specs: [RelaySpec, RelaySpec] = [
      {
        ...
        redisUrl: process.env.LENOS_E2E_REDIS_A ?? "redis://127.0.0.1:6379/11",
      },
      {
        name: "relay-b",
        ports: {
        ...
        redisUrl: process.env.LENOS_E2E_REDIS_B ?? "redis://127.0.0.1:6379/12",
      },
    ];
    ...
    try {
      ...
      for (const relay of specs) {
        ...
        const senderEnv = {
          LENOS_RELAY_URL: relayHttp,
          LENOS_PRIVATE_KEY: senderKey,
        };
        const created = JSON.parse(
          await run(
            required("LENOS_E2E_CLI_BIN", cliBin),
            [
              "channels",
              "create",
              "--name",
        ...
        await run(
          ...
          { LENOS_RELAY_URL: relayHttp, LENOS_PRIVATE_KEY: agentKey },
        );
      ...
      for (const { relay } of channels) {
        acpChildren.push(
          await harness.startAcp(
            ...
            agentKey,
            { LENOS_ACP_RESPOND_TO: "anyone", LENOS_ACP_NO_MEMORY: "true" },
          ),
        );
      }
      ...
      for (const { relay, id } of channels) {
        ...
        await run(
          ...
          { LENOS_RELAY_URL: relayHttp, LENOS_PRIVATE_KEY: senderKey },
        );
