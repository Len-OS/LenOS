...
test("mobile pairing starts on demand and reveals the QR code", async ({
  ...
  expect(await qrCode.locator(".lenos-qr-cell-reveal").count()).toBeGreaterThan(
    100,
  );
  await expect(qrCode.locator(".lenos-qr-cell-reveal").first()).toHaveCSS(
    "animation-name",
    "lenos-qr-cell-reveal",
  );
  await expect(qrCode.locator(".lenos-qr-cell-reveal").first()).toHaveCSS(
    "animation-duration",
    "0.058s",
  ...
  await expect(
    qrCode.locator('[data-qr-cell-row="56"].lenos-qr-cell-reveal').first(),
