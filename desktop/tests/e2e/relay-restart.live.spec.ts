...
// Live gate: boots a REAL lenos-relay process, points the app at it, SIGTERMs
...
//
// Requires: LENOS_E2E_RELAY_RESTART=1, LENOS_E2E_RELAY_BIN, and
// LENOS_E2E_DATABASE_URL (plus reachable Redis and media object store, same
// infra as the agents-everywhere live gate).
const enabled = process.env.LENOS_E2E_RELAY_RESTART === "1";
...
async function runCli(args: string[], relayUrl: string, privateKey: string) {
  const binary = required("LENOS_E2E_CLI_BIN", process.env.LENOS_E2E_CLI_BIN);
  const { stdout } = await exec(binary, args, {
    cwd: "..",
    env: {
      ...process.env,
      LENOS_AUTH_TAG: "",
      LENOS_PRIVATE_KEY: privateKey,
      LENOS_RELAY_URL: relayUrl,
    },
  });
  return stdout;
}
...
async function connectionState(page: Page): Promise<string> {
  return page.evaluate(() => {
    const win = window as Window & {
      __LENOS_E2E_GET_RELAY_CONNECTION_STATE__?: () => string;
    };
    return win.__LENOS_E2E_GET_RELAY_CONNECTION_STATE__?.() ?? "uninstalled";
  });
}
...
async function exerciseBackgroundTraffic(page: Page, durationMs: number) {
  await page.evaluate(async (duration) => {
    ...
    while (Date.now() < deadline) {
      void window.__LENOS_E2E_QUERY_CLIENT__?.invalidateQueries({
        queryKey: ["channels"],
      });
...
test.describe("relay restart live gate", () => {
  test.skip(!enabled, "set LENOS_E2E_RELAY_RESTART=1 to run live gate");
  ...
  test("client reconnects after the relay is SIGTERMed and restarted", async ({
    ...
    const spec: RelaySpec = {
      ...
      databaseUrl: required(
        "LENOS_E2E_DATABASE_URL",
        process.env.LENOS_E2E_DATABASE_URL,
      ),
      redisUrl:
        process.env.LENOS_E2E_REDIS_RESTART ?? "redis://127.0.0.1:6379/13",
    };
