...
// The relay serves it as a row (`lenos-db/src/thread.rs`: "top-level rows =
...
async function waitForMockLiveSubscription(
  ...
  await expect
    .poll(() =>
      page.evaluate(
        (ch) =>
          window.__LENOS_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({
            channelName: ch,
          }) ?? false,
        channelName,
      ),
...
async function liveOverlayContents(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    const qc = window.__LENOS_E2E_QUERY_CLIENT__ as unknown as {
