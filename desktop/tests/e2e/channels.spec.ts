...
type MockFeedWindow = Window & {
  ...
  __LENOS_E2E_PUSH_MOCK_FEED_ITEM__?: (item: {
...
async function hasOutgoingEventWithContent(
  ...
  return page.evaluate((expectedContent) => {
    return (
      ...
      ).__LENOS_E2E_COMMAND_LOG__?.some((entry) => {
      ...
      return page.evaluate(
        ({ currentChannelName, kind }) => {
          return (
            ...
            ).__LENOS_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({
...
async function addGenericAgent(
  ...
  return page.evaluate(
    async ({ agentName, channelId }) => {
      ...
      ).__LENOS_E2E_QUERY_CLIENT__?.invalidateQueries();
...
test("start a new direct message from the sidebar", async ({ page }) => {
  ...
  await expect(page.locator(".lenos-poof-burst")).toHaveCount(1);
  await page.mouse.up();
...
test("closes direct message results while opening", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __LENOS_E2E__?: { mock?: { openDmDelayMs?: number } };
    };
    testWindow.__LENOS_E2E__ ??= {};
    testWindow.__LENOS_E2E__.mock ??= {};
    testWindow.__LENOS_E2E__.mock.openDmDelayMs = 1_000;
  });
...
test("does not reopen a sent direct message after leaving during cache reseed", async ({
  ...
  await page.evaluate(() => {
    ...
    testWindow.__LENOS_E2E__ ??= {};
    testWindow.__LENOS_E2E__.mock ??= {};
    testWindow.__LENOS_E2E__.mock.channelsReadDelayMs = 1_000;
  });
...
test("profile renders live activity for a viewer-owned relay agent", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__LENOS_E2E_SEED_ACTIVE_TURNS__ ===
      "function",
  );
  await page.evaluate(
    ({ agentPubkey, channelId }) => {
      const seedActiveTurns = (window as MockFeedWindow)
        .__LENOS_E2E_SEED_ACTIVE_TURNS__;
      if (!seedActiveTurns) {
...
async function seedHomeInboxMention(
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__LENOS_E2E_PUSH_MOCK_FEED_ITEM__ ===
      "function",
  );
  ...
  await page.evaluate(
    ({
      ...
      const pushFeedItem = (window as MockFeedWindow)
        .__LENOS_E2E_PUSH_MOCK_FEED_ITEM__;
      if (!pushFeedItem) {
...
test("Inbox All excludes generic channel traffic", async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return typeof win.__LENOS_E2E_PUSH_MOCK_FEED_ITEM__ === "function";
  });
...
test("Inbox All never lists drafts and unread-only hides reminders", async ({
  ...
  await page.addInitScript(
    ...
    },
    {
      ...
      draftStoreKey: `lenos-drafts.v2:ws://localhost:3000:${MOCK_IDENTITY_PUBKEY}`,
    },
  );
  await page.goto("/");
  await page.waitForFunction(() => {
  ...
  await page.evaluate(
    async ({
      ...
      window.__LENOS_E2E_SEED_MOCK_REMINDERS__?.([
        {
          id: reminderId,
...
test("Inbox keeps the unread boundary for replies from multiple agents", async ({
  ...
  await page.waitForFunction(() => {
    const win = window as MockFeedWindow;
    return (
      typeof win.__LENOS_E2E_EMIT_MOCK_MESSAGE__ === "function" &&
      typeof win.__LENOS_E2E_PUSH_MOCK_FEED_ITEM__ === "function"
    );
  });
  ...
  await page.evaluate(
    ({ agentPubkeys, channelId, currentPubkey, ids }) => {
      const win = window as MockFeedWindow;
      const emitMessage = win.__LENOS_E2E_EMIT_MOCK_MESSAGE__;
      const pushFeedItem = win.__LENOS_E2E_PUSH_MOCK_FEED_ITEM__;
...
test("stopping a managed bot in one community leaves its other communities running", async ({
  ...
  // (`seedDefaultCommunity` follows LENOS_E2E_RELAY_URL); the agent also has a
  // live runtime in a second community on another relay.
  const activeRelayUrl = (
    process.env.LENOS_E2E_RELAY_URL ?? "http://localhost:3000"
  ).replace(/^http/, "ws");
