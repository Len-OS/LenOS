...
test.describe("config bridge screenshots", () => {
  ...
  test("01 — folded config panel", async ({ page }) => {
    ...
    await expect(panel.getByText("Set in LenOS").first()).toBeVisible();
  ...
  test("06 — lenos-agent empty MCP servers", async ({ page }) => {
    await installMockBridge(page, {
      managedAgents: [
        {
          pubkey: LENOS_AGENT_PUBKEY,
          name: "LenOS Agent",
          status: "running" as const,
          channelNames: ["agents"],
        },
    ...
    const panel = await openAgentProfileFromChannel(page, "LenOS Agent");
