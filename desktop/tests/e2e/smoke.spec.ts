...
test("LenOS shared compute explains automatic model selection", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => {
...
test("create agent persists LenOS shared compute with auto model", async ({
  page,
}) => {
...
test("create agent supports parallelism and system prompt overrides", async ({
  ...
  // The lenos-agent runtime auto-selects once the ACP runtime catalog loads;
