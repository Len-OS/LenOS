...
const SHOTS = "test-results/lenos-theme";
const THEME_STORAGE_KEY = "lenos-theme";
  ...
  await page.addInitScript(
    ({ channelId, pubkey }) => {
      window.localStorage.setItem(
        `lenos-channel-sections.v1:${pubkey}`,
        JSON.stringify({
          version: 1,
          sections: [
            {
...
async function expectLenOSSidebarPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
  ...
  const sidebarScroller = page.locator(".lenos-sidebar-scrollbar");
...
async function expectLenOSContentShadow(page: Page, mode: "light" | "dark") {
  const effects = await page.evaluate(() => {
    const shell = document.querySelector(".lenos-huddle-shell");
    const content = document.querySelector("[data-lenos-content-surface]");
...
async function expectLenOSGradientPaint(
  page: Page,
  mode: "light" | "dark",
): Promise<string> {
  const paint = await page.evaluate(() => {
    const root = document.documentElement;
    const appSurface = document.querySelector(".lenos-huddle-app-surface");
    const lightLayer = document.querySelector('[data-lenos-gradient="light"]');
    const darkLayer = document.querySelector('[data-lenos-gradient="dark"]');
    ...
    return {
      ...
      theme: root.getAttribute("data-lenos-theme"),
  ...
  expect(paint.theme).toBe(mode === "light" ? "lenos" : "lenos-dark");
...
async function expectLenOSSettingsPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
    mode === "light" ? "rgba(0, 0, 0, 0.4)" : "rgba(255, 255, 255, 0.4)";
  ...
  await expectLenOSGradientPaint(page, mode);
...
async function expectAppliedLenOSTheme(
  page: Page,
  themeName: "lenos" | "lenos-dark",
  storedTheme: "lenos" | "lenos-dark" = themeName,
) {
  const isDark = themeName === "lenos-dark";
  await expect
    .poll(() =>
      page.evaluate((storageKey) => {
        ...
        return {
          ...
          isDark: root.classList.contains("dark"),
          lenosTheme: root.getAttribute("data-lenos-theme"),
          gradientTop: styles.getPropertyValue("--lenos-gradient-top").trim(),
          gradientBottom: styles
            .getPropertyValue("--lenos-gradient-bottom")
            .trim(),
        };
      }, THEME_STORAGE_KEY),
    )
...
test("lenos light sidebar gradient", async ({ page }) => {
  await seedTheme(page, "lenos");
  await installMockBridge(page);
  await openChannel(page);
  await expectLenOSGradientPaint(page, "light");
  await expectLenOSSidebarPalette(page, "light");
  await expectLenOSContentShadow(page, "light");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/01-lenos-light-sidebar.png` });
});

test("lenos dark sidebar gradient", async ({ page }) => {
  await seedTheme(page, "lenos-dark");
  await installMockBridge(page);
  await openChannel(page);
  await expectLenOSGradientPaint(page, "dark");
  await expectLenOSSidebarPalette(page, "dark");
  await expectLenOSContentShadow(page, "dark");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await expect(page.locator("[data-lenos-content-surface]")).toHaveCSS(
    "background-color",
    "rgb(26, 26, 26)",
  );
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/02-lenos-dark-sidebar.png` });
});
...
test("appearance picker — system tab (LenOS follows OS)", async ({ page }) => {
  await seedTheme(page, "lenos");
  await installMockBridge(page);
...
test("appearance picker — light tab (LenOS)", async ({ page }) => {
  await seedTheme(page, "lenos");
  await installMockBridge(page);
...
test("appearance picker — dark tab (LenOS Dark)", async ({ page }) => {
  await seedTheme(page, "lenos-dark");
  await installMockBridge(page);
...
test("settings nav uses LenOS active pill + hover (light)", async ({
  ...
  // Appearance is the active section here; its nav row should carry the LenOS
  ...
  await expectLenOSSettingsPalette(page, "light");
...
test("settings nav uses LenOS active pill + hover (dark)", async ({ page }) => {
  await seedTheme(page, "lenos-dark");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  await expect(sidebar).toBeVisible({ timeout: 10_000 });
  await page.getByTestId("settings-nav-appearance").click();
  await expectLenOSSettingsPalette(page, "dark");
...
test("appearance hides accent picker under LenOS", async ({ page }) => {
  await seedTheme(page, "lenos");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a LenOS theme is active. Its neutral
...
test("accent picker reveals/hides when toggling LenOS", async ({ page }) => {
  // Start on a non-LenOS theme so the accent picker is present, then select the
  // LenOS tile — the picker should animate out and unmount. Reselecting a
  // non-LenOS tile brings it back. Asserts the presence toggle (the motion
  ...
  // Switch to LenOS — picker should leave (allow the exit animation to settle).
  await page.getByTestId("theme-option-lenos").click();
  await expect(page.getByTestId("accent-color-neutral")).toHaveCount(0);

  // Back to a non-LenOS theme — picker returns.
  await page.getByTestId("theme-option-github-light").click();
  await expect(page.getByTestId("accent-color-neutral")).toBeVisible();
});

test("LenOS light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "lenos");
  await installMockBridge(page);
  await openAppearance(page, "light");
  await expectAppliedLenOSTheme(page, "lenos");
  const lightGradient = await expectLenOSGradientPaint(page, "light");

  await page.getByTestId("appearance-mode-dark").click();
  await expectAppliedLenOSTheme(page, "lenos-dark");
  const darkGradient = await expectLenOSGradientPaint(page, "dark");
...
test("LenOS follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "lenos");
  await page.addInitScript(() => {
  ...
  await expectAppliedLenOSTheme(page, "lenos-dark", "lenos");
  await expectLenOSGradientPaint(page, "dark");

  await emitNativeThemeChange(page, "light");
  await expectAppliedLenOSTheme(page, "lenos", "lenos");
  await expectLenOSGradientPaint(page, "light");
});
