...
export class TwoRelayHarness {
  ...
  static async create(relays: readonly RelaySpec[]) {
    return new TwoRelayHarness(
      await mkdtemp(join(tmpdir(), "lenos-ae-e2e-")),
      relays,
    );
  }

  async startRelays(binary = process.env.LENOS_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("LENOS_E2E_RELAY_BIN is required for the live gate");
    await Promise.all(
  ...
  async restartRelay(name: string, binary = process.env.LENOS_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("LENOS_E2E_RELAY_BIN is required for the live gate");
  ...
  async startAcp(
    ...
    const binary = process.env.LENOS_E2E_ACP_BIN;
    if (!binary)
      throw new Error("LENOS_E2E_ACP_BIN is required for the live gate");
    return this.spawnOwned(name, binary, [], {
      LENOS_RELAY_URL: relayWsUrl,
      LENOS_PRIVATE_KEY: privateKey,
      LENOS_AUTH_TAG: "",
      LENOS_ACP_LAZY_POOL: "true",
      LENOS_ACP_AGENT_COMMAND: process.execPath,
      LENOS_ACP_AGENT_ARGS: resolve("tests/e2e/fixtures/fake-acp-agent.mjs"),
      LENOS_E2E_CLI_BIN: process.env.LENOS_E2E_CLI_BIN,
      ...extraEnv,
    });
  }
  ...
  private async startRelay(binary: string, relay: RelaySpec) {
    const child = this.spawnOwned(relay.name, binary, [], {
      ...
      LENOS_BIND_ADDR: `127.0.0.1:${relay.ports.main}`,
      LENOS_HEALTH_PORT: String(relay.ports.health),
      LENOS_METRICS_PORT: String(relay.ports.metrics),
