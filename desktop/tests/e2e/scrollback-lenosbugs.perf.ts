...
/**
 * SCROLL-BACK latency profile for one channel (#lenos-bugs) against a LIVE
 ...
 *   LENOS_E2E_RELAY_URL=http://127.0.0.1:13000 \
 *   LENOS_COMMUNITY_HOST=sprout-oss.stage.blox.sqprod.co \
 *   LENOS_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-lenosbugs.perf.ts
 */

const RELAY_HTTP =
  process.env.LENOS_E2E_RELAY_URL ?? "https://sprout-oss.stage.blox.sqprod.co";
const NSEC = process.env.LENOS_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.LENOS_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.LENOS_PERF_CHANNEL ?? "lenos-bugs";
const PAGES = Number(process.env.LENOS_PERF_PAGES ?? 10);

const IDENTITY_OVERRIDE_KEY = "lenos:e2e-identity-override.v1";
const ONBOARDING_PREFIX = "lenos-onboarding-complete.v1:";
const WELCOME_PREFIX = "lenos-welcome-channel-ensured.v2:";
...
test("MEASURE: scroll-back pagination latency in target channel", async ({
  ...
  if (!NSEC) throw new Error("Set LENOS_PERF_NSEC to a real member nsec");
