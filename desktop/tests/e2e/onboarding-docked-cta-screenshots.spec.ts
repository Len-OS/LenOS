...
test("machine onboarding: landing, backup, setup docked CTAs", async ({
  ...
  await expect(
    page.getByTestId("backup-options").locator(".lenos-card-textured"),
  ).toHaveCount(0);
  ...
  await expect(passwordPanel).not.toHaveClass(/lenos-card-textured/);
  ...
  await expect(generatorPopover).not.toHaveClass(/lenos-card-textured/);
  await waitForAnimations(page);
