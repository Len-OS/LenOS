...
const SIDEBAR_WIDTH_STORAGE_KEY = "lenos-sidebar-width";
...
async function loadTheme(page: Page, theme: string) {
  await page.addInitScript((selectedTheme) => {
    window.localStorage.setItem("lenos-theme", selectedTheme);
  }, theme);
  await installMockBridge(page);
  await page.goto("/");
}
...
test("supports API tokens without cluttering the default join form", async ({
  ...
  await page.getByLabel("API token").fill("lenos_secret");
...
for (const theme of ["lenos", "github-light", "catppuccin-mocha"]) {
...
test("aligns the sidebar search with the channel title outside the LenOS theme", async ({
  page,
}) => {
  ...
  await expect(root).not.toHaveAttribute("data-lenos-sidebar", "");
...
test("shows a sidebar update card when an update is ready", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __LENOS_E2E__?: { mock?: { updateAvailable?: boolean } };
    };
