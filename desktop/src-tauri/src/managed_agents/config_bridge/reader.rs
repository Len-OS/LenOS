...
pub(crate) fn read_config_surface(
    ...
    let (file_config, file_was_read) = runtime_meta
        ...
        .and_then(|id| match id {
            ...
            "lenos-agent" => super::lenos_agent::read_config_file().map(|c| (c, true)),
            _ => None,
        })
        .unwrap_or_else(|| (RuntimeFileConfig::default(), false));

    // Tier 2a: record-level values (LenOS-explicit).
    ...
    let normalized = NormalizedConfig {
        ...
        system_prompt: build_system_prompt_field(
            &record
                .system_prompt
                .clone()
                .or_else(|| record.env_vars.get("LENOS_ACP_SYSTEM_PROMPT").cloned()),
    ...
    for (k, v) in &record.env_vars {
        ...
        advanced.push(ConfigField {
            ...
            label: k.clone(),
            value: Some(v.clone()),
            origin: ConfigOrigin::LenOSExplicit,
            schema_type: ConfigFieldType::String,
...
fn build_model_field(
    ...
    // Precedence: LenOS-explicit > ACP current > config file
    let (value, origin) = if let Some(ref m) = record_model {
        (Some(m.clone()), ConfigOrigin::LenOSExplicit)
    ...
    // LenOS-explicit model shadowing a config-file model. The live-session
...
/// `(record_model, LenOSExplicit)` for a genuine-explicit agent that live-
...
fn build_provider_field(
    ...
    let tiers: &[(Option<&str>, ConfigOrigin)] = &[
        (record_provider.as_deref(), ConfigOrigin::LenOSExplicit),
...
fn build_thinking_field(
    ...
    let tiers: &[(Option<&str>, ConfigOrigin)] = &[
        (record_effort.as_deref(), ConfigOrigin::LenOSExplicit),
...
/// env_vars map the field is LenOSExplicit + RespawnWithEnvVar; otherwise if the
...
/// Record/env prompt wins (LenOSExplicit, respawnable); a config-file prompt it
...
fn build_system_prompt_field(
    ...
    if let Some(v) = record_prompt {
        return Some(NormalizedField {
            ...
            origin: ConfigOrigin::LenOSExplicit,
            write_via: ConfigWriteMechanism::RespawnWithEnvVar {
                env_key: "LENOS_ACP_SYSTEM_PROMPT".to_string(),
            },
