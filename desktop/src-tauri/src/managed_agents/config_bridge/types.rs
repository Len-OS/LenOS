...
pub enum ConfigOrigin {
    /// Explicitly set in LenOS UI / ManagedAgentRecord (highest precedence).
    LenOSExplicit,
    ...
    /// re-tag injected fields from `LenOSExplicit` to `PersonaDefault`.
    PersonaDefault,
    ...
    /// the linked persona specifies a value. Re-tagged from `LenOSExplicit` by the
