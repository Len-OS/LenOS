...
//!
//! A locked card carries the same `lenos_agent_snapshot` tEXt chunk as a plain
...
//!
//! Wire contract (agreed with Wren, lenos-agent-trading-cards thread):
...
use lenos_core_pkg::engram::NIP44_PLAINTEXT_MAX;
...
/// `lenos-agent-snapshot` so detection never guesses.
pub const LOCKED_FORMAT: &str = "lenos-agent-snapshot-encrypted";
...
/// Typed outer envelope stored (base64 JSON) in the `lenos_agent_snapshot`
...
///
/// - `lenos-agent-snapshot` → full plain-manifest decode + validation.
/// - `lenos-agent-snapshot-encrypted` → size caps, typed envelope parse,
...
mod tests {
    ...
    fn unknown_format_version_scheme_rejected() {
        ...
        let unknown = serde_json::json!({"format": "lenos-agent-snapshot-v9", "version": 1});
