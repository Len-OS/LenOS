...
pub struct AgentDefinition {
    ...
    /// LenOS spawns. When deploying from this persona, this runtime is pre-selected in the UI.
    ...
    /// direct). LenOS stores and passes through without interpretation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Opaque to LenOS — keys and values are runtime-specific.
    ///
...
pub struct ManagedAgentRecord {
    ...
    /// Deprecated: `LENOS_ACP_TURN_TIMEOUT` is ignored by the harness and the
    ...
    /// Idle timeout in seconds (`LENOS_ACP_IDLE_TIMEOUT`): how long the agent
    ...
    /// Inbound author gate mode. Translates to `LENOS_ACP_RESPOND_TO`.
    #[serde(default)]
    ...
    /// inference through LenOS's relay-mesh local endpoint; the `model_ref` is
...
pub struct ManagedAgentProcess {
    ...
    /// `LENOS_ACP_SETUP_PAYLOAD` was set at launch because the agent was
...
pub enum AuthStatus {
    ...
    /// This runtime does not have a login step (e.g. goose, lenos-agent).
    NotApplicable,
...
pub struct InstallStepResult {
    ...
    /// recognized condition (e.g. EACCES writing LenOS's private npm prefix).
...
pub const DEFAULT_ACP_COMMAND: &str = "lenos-acp";
/// ~5 min (320s) — matches the CLI harness default (LENOS_ACP_IDLE_TIMEOUT).
...
// ── Inbound author gate ──────────────────────────────────────────────────────
//
// Mirrors `lenos-acp`'s `--respond-to` CLI flag and the related
// `--respond-to-allowlist` option. Persisted per agent so the desktop can
// translate the user's choice into `LENOS_ACP_RESPOND_TO` /
// `LENOS_ACP_RESPOND_TO_ALLOWLIST` env vars at spawn time.
//
...
impl RespondTo {
    /// CLI/env wire string (matches `lenos-acp`'s `--respond-to`).
...
///
/// Rules mirror `lenos-acp/src/config.rs::validate_allowlist`:
