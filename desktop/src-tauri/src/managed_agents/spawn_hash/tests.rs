...
fn allowlist_ignored_when_mode_is_not_allowlist() {
    // Spawn only sets LENOS_ACP_RESPOND_TO_ALLOWLIST in allowlist mode, so
...
fn display_name_edit_changes_hash() {
    // The spawn writes LENOS_ACP_SESSION_TITLE from display_name-or-name, so a
...
fn display_name_edit_does_not_change_hash_under_an_explicit_title_override() {
    // User env is written AFTER the LenOS-set title (last-wins), so an explicit
    // LENOS_ACP_SESSION_TITLE is what the child actually runs with. Renaming the
    ...
    let mut rec = record();
    rec.env_vars
        .insert("LENOS_ACP_SESSION_TITLE".into(), "Pinned Title".into());
...
fn title_override_edit_changes_hash() {
    ...
    edited
        .env_vars
        .insert("LENOS_ACP_SESSION_TITLE".into(), "Other Title".into());
    assert_ne!(
        spawn_config_hash(&rec, &[], &[], "wss://ws.example", &Default::default()),
        spawn_config_hash(&edited, &[], &[], "wss://ws.example", &Default::default()),
