...
/// Binary names for the LenOS desktop/Tauri process. Used by dead-instance
...
const DESKTOP_BINARY_NAMES: &[&str] = &[
    ...
    // AppImage shim's real executable name, `lenos-desktop.bin`.
    "lenos-desktop.bi",
];

/// Check if a process name matches a known LenOS desktop binary.
...
/// Extract the `LENOS_MANAGED_AGENT` value from a process's environment.
/// Returns `None` if the process doesn't have the marker or can't be read.
#[cfg(target_os = "macos")]
fn extract_lenos_marker_value(pid: u32) -> Option<String> {
    let prefix = b"LENOS_MANAGED_AGENT=";
...
#[cfg(not(unix))]
fn extract_lenos_marker_value(_pid: u32) -> Option<String> {
    None
}

/// Check if a LenOS desktop process is still alive for the given instance ID.
/// Scans all user-owned processes named "LenOS" or "lenos-desktop" and checks
...
/// Reap agent processes belonging to dead LenOS desktop instances.
///
/// Scans all user processes for `LENOS_MANAGED_AGENT=*`, groups them by
/// instance ID, and for each foreign instance (≠ `our_instance_id`) checks
/// whether a LenOS desktop binary is still alive for that instance. If not,
...
pub(crate) fn reap_dead_instance_agents(our_instance_id: &str, skip_pids: &[u32]) {
    ...
    for &pid in &pids {
        ...
        // arbitrary binary names and LENOS_MANAGED_AGENT is the authoritative
        // ownership proof.
        let Some(agent_instance_id) = extract_lenos_marker_value(upid) else {
            continue;
        };
    ...
    for (instance_id, agent_pids) in &foreign_agents {
        ...
        eprintln!(
            "lenos-desktop: reaping {} orphaned agent(s) from dead instance '{instance_id}'",
            agent_pids.len()
        );
