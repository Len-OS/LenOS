...
/// with only LenOS's managed Node dirs — no `curl`, `sh`, or `tar`, which
...
///   1. `managed` — LenOS-controlled dirs (highest precedence, e.g. managed Node/npm bins)
...
///   2. LenOS-managed npm prefix bin dir — app-private ACP adapter shims
///   3. LenOS-managed Node.js bin dir — app-private Node/npm runtime
...
/// guards against, which left managed agents unable to find `lenos`. Returns
...
pub(in crate::managed_agents) fn build_augmented_path(
    ...
    if has_local_context {
        if let Some(managed_npm_bin) = crate::managed_agents::lenos_managed_npm_bin_dir() {
            managed.push(managed_npm_bin);
        }
        if let Some(managed_node_bin) = crate::managed_agents::lenos_managed_node_bin_dir() {
...
mod tests {
    ...
    fn splits_colon_delimited_shell_path() {
        ...
        let result = build_augmented_path(
            ...
            Some(PathBuf::from("/Applications/LenOS.app/Contents/MacOS")),
        ...
        assert!(
            result.contains(":/Applications/LenOS.app/Contents/MacOS:"),
            "{result}"
        );
        assert!(
    ...
    fn nvm_bin_inserted_after_local_bin_before_exe_parent() {
        ...
        let nvm = result
            ...
            .unwrap();
        let exe = result
            .find("/Applications/LenOS.app/Contents/MacOS")
            .unwrap();
    ...
    /// the inherited process PATH — otherwise the child gets only LenOS-managed
...
mod compose_tests {
    ...
    fn managed_entries_appear_first() {
        let managed = vec![p("/lenos/node/bin"), p("/lenos/npm/bin")];
        let login = vec![p("/usr/local/bin"), p("/usr/bin")];
        let result = compose_path_entries(managed, login, vec![], false);
        assert_eq!(result[0], p("/lenos/node/bin"), "managed[0] must be first");
        assert_eq!(result[1], p("/lenos/npm/bin"), "managed[1] must be second");
        assert_eq!(
            result[2],
            p("/usr/local/bin"),
