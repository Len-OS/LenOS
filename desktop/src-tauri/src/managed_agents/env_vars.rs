...
//!
//! A small set of *reserved* keys — LenOS's identity and secrets — are
//! rejected at save time and stripped at runtime so a typo or malicious
//! value can't swap the agent's nsec. Behavior knobs (GOOSE_MODE, LENOS_ACP_MODEL, LENOS_ACP_SYSTEM_PROMPT, …) remain
...
/// Env var keys that LenOS sets itself and users must not override from
...
/// implications. Behavior knobs (GOOSE_MODE, LENOS_ACP_MODEL, LENOS_ACP_SYSTEM_PROMPT, …) remain freely
...
pub(crate) const RESERVED_ENV_KEYS: &[&str] = &[
    ...
    "LENOS_MANAGED_AGENT",
    "LENOS_MANAGED_AGENT_START_NONCE",
];
...
/// `=`. That means a key like `LENOS_AUTH_TAG=x` with value `forged`
/// lands as `LENOS_AUTH_TAG=x=forged` in the child env and
/// `getenv("LENOS_AUTH_TAG")` returns `"x=forged"` — a full reserved-
...
pub fn validate_user_env_keys(env_vars: &BTreeMap<String, String>) -> Result<(), String> {
    ...
    if !reserved.is_empty() {
        return Err(format!(
            "the following env vars are reserved by LenOS and cannot be overridden: {}",
            reserved.join(", ")
        ));
    }
...
pub(crate) fn merged_user_env(
    ...
    merged.retain(|k, v| {
        if is_reserved_env_key(k) {
            eprintln!(
                "lenos-desktop: ignoring reserved env var `{k}` from persona/agent overrides"
            );
            return false;
        }
        if !is_well_formed_env_key(k) {
            ...
            eprintln!(
                "lenos-desktop: ignoring malformed env var key `{}` from persona/agent overrides",
        ...
        if v.contains('\0') {
            ...
            eprintln!(
                "lenos-desktop: ignoring env var `{k}` with NUL byte in value"
            );
            return false;
        }
        if v.len() > MAX_ENV_VALUE_BYTES {
            eprintln!(
                "lenos-desktop: ignoring env var `{k}` with oversize value ({} bytes > {MAX_ENV_VALUE_BYTES})",
                v.len()
            );
            return false;
        }
