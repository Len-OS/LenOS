...
pub(crate) struct KnownAcpRuntime {
    ...
    /// `Some(dir)` → LenOS creates a symlink at `<nest>/<dir>/lenos-cli`
    /// pointing to the canonical `.agents/skills/lenos-cli`. `None` → this
    ...
    /// authenticated. `None` for runtimes that have no login step (goose, lenos-agent).
