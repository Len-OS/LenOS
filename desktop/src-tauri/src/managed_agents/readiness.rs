...
/// `effective_command` is the resolved harness binary name (e.g. `"lenos-agent"`,
...
pub(crate) struct EffectiveAgentEnv {
    ...
    /// The resolved harness binary name (e.g. `"lenos-agent"`, `"goose"`).
...
pub(crate) struct EffectiveHarnessDescriptor {
    /// The raw effective command string (e.g. `"lenos-agent"`, `"my-acp-agent"`).
...
fn resolve_effective_agent_env_with_def(
    ...
    // LenOS shared compute is a native LenOS provider. Translate it to lenos-agent's
...
pub enum Requirement {
    ...
    /// Git for Windows is missing, so lenos-agent cannot launch lenos-dev-mcp's
...
/// resolved runtime:
///
/// * **lenos-agent / goose**: provider + model are required (both must be
...
/// `lenos-agent/src/config.rs:143` — it is an escape hatch for static tokens
...
fn collect_missing_requirements(
    ...
    match rt.id {
        "lenos-agent" => lenos_agent_requirements(effective),
        "goose" => {
...
/// Requirements for lenos-agent (provider + model + provider-specific creds).
fn lenos_agent_requirements(effective: &EffectiveAgentEnv) -> Vec<Requirement> {
    ...
    // Provider is required — maps to LENOS_AGENT_PROVIDER in the effective env.
    ...
    // Model is required — maps to LENOS_AGENT_MODEL in the effective env.
    // Same empty-string treatment as provider.
    // Also accept provider-specific model fallback keys, matching lenos-agent's
    ...
    // baked lenos-releases env sets DATABRICKS_MODEL but not LENOS_AGENT_MODEL,
    ...
    match provider {
        ...
        Some("databricks") | Some("databricks_v2") | Some("databricks-v2")
            ...
            // (OAuth PKCE is the normal path — see lenos-agent/src/config.rs:143).
...
///
/// Mirrors lenos-agent requirements but uses GOOSE_PROVIDER / GOOSE_MODEL.
///
...
/// require them from LenOS's env layer.  The file layer only *silences*
...
fn goose_requirements(
    ...
    // Empty string treated as absent — same as lenos_agent_requirements.
    let provider = effective
        .env
        .get("GOOSE_PROVIDER")
        .filter(|v| !v.is_empty())
    ...
    // Provider-specific credentials — same empty-string semantics as lenos-agent.
    let env_key_missing = |key: &str| effective.env.get(key).is_none_or(|v| v.is_empty());
...
mod tests {
    ...
    #[test]
    fn lenos_agent_missing_provider_returns_not_ready_with_normalized_field() {
        let env = make_env(
            "lenos-agent",
            env_with(&[("LENOS_AGENT_MODEL", "claude-opus-4-5")]),
        );
        let result = agent_readiness(&env);
        assert!(
            !result.is_ready(),
            "missing LENOS_AGENT_PROVIDER should be NotReady"
        );
    ...
    #[test]
    fn lenos_agent_missing_model_returns_not_ready_with_normalized_field() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn lenos_agent_missing_anthropic_key_returns_not_ready_with_env_key() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "anthropic"),
                ("LENOS_AGENT_MODEL", "claude-opus-4-5"),
            ]),
        );
        let result = agent_readiness(&env);
        assert!(!result.is_ready());
    ...
    #[test]
    fn lenos_agent_missing_openai_key_returns_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "openai"),
                ("LENOS_AGENT_MODEL", "gpt-4o"),
            ]),
        );
        let result = agent_readiness(&env);
        assert!(!result.is_ready());
    ...
    #[test]
    fn lenos_agent_anthropic_with_all_fields_is_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn lenos_agent_databricks_with_host_and_model_is_ready_without_token() {
        ...
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "databricks"),
                ("LENOS_AGENT_MODEL", "dbrx-instruct"),
    ...
    #[test]
    fn lenos_agent_databricks_missing_host_returns_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "databricks"),
    ...
    #[test]
    fn lenos_agent_databricks_v2_missing_host_returns_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "databricks_v2"),
                (
                    "LENOS_AGENT_MODEL",
    ...
    #[test]
    fn lenos_agent_empty_string_provider_is_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
        ...
        assert!(
            !result.is_ready(),
            "empty-string LENOS_AGENT_PROVIDER must be treated as missing"
        );
        assert!(result
            .requirements()
            .contains(&Requirement::NormalizedField {
    ...
    #[test]
    fn lenos_agent_empty_string_model_is_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "anthropic"),
        ...
        assert!(
            !result.is_ready(),
            "empty-string LENOS_AGENT_MODEL must be treated as missing"
        );
        assert!(result
            .requirements()
            .contains(&Requirement::NormalizedField {
    ...
    #[test]
    fn lenos_agent_empty_string_anthropic_key_is_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn lenos_agent_empty_string_databricks_host_is_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "databricks"),
    ...
    fn codex_not_ready_copy_does_not_mention_openai_api_key() {
        ...
        let rt = make_cli_runtime(&["__lenos_nonexistent_adapter_xyz789__"], None);
    ...
    fn cli_login_requirements_missing_binary_is_not_ready() {
        ...
        let rt = make_cli_runtime(
            &["__lenos_nonexistent_adapter_abc123__"],
            Some("__lenos_nonexistent_cli_abc123__"),
        );
        let reqs = cli_login::requirements(
            &["__lenos_nonexistent_binary_abc123__", "status"],
    ...
    fn cli_login_requirements_adapter_missing_emits_adapter_missing() {
        ...
        let rt = make_cli_runtime(&["__lenos_nonexistent_adapter_xyz789__"], Some(exe));
    ...
    fn cli_login_requirements_logged_out_emits_available() {
        ...
        let rt = make_cli_runtime(static_commands(vec![exe]), Some(exe));
        let reqs =
            cli_login::requirements(&[exe, "--lenos-probe-fail-xyz"], "run `tool login`", &rt);
        assert!(
            !reqs.is_empty(),
    ...
    fn cli_login_requirements_codex_outdated_adapter_emits_adapter_outdated() {
        ...
        // intentionally prefers LenOS's managed npm shim when it exists, which
        ...
        let reqs = cli_login::requirements(
            &[exe, "--lenos-probe-must-not-run-xyz"],
            "run `codex login`",
            &rt,
        );
    ...
    fn resolve_effective_agent_env_user_env_wins_over_structured_fields() {
        ...
        env_vars.insert("LENOS_AGENT_PROVIDER".to_string(), "anthropic".to_string());
        env_vars.insert(
            "LENOS_AGENT_MODEL".to_string(),
        ...
        let runtime = known_acp_runtime_exact("lenos-agent");
        ...
        assert_eq!(
            effective.env.get("LENOS_AGENT_MODEL").map(String::as_str),
    ...
    #[test]
    fn lenos_agent_databricks_v2_with_databricks_model_but_no_lenos_agent_model_is_ready() {
        // The baked lenos-releases env sets DATABRICKS_MODEL but not LENOS_AGENT_MODEL.
    ...
    #[test]
    fn lenos_agent_databricks_v2_hyphen_alias_with_databricks_model_is_ready() {
        // lenos-agent accepts both "databricks_v2" and "databricks-v2". The
        // readiness gate must recognize the hyphen alias and accept DATABRICKS_MODEL.
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "databricks-v2"),
                ("DATABRICKS_MODEL", "goose-claude-4-6-sonnet"),
                ("DATABRICKS_HOST", "https://dbc.example.com"),
            ]),
        );
    ...
    #[test]
    fn lenos_agent_databricks_hyphen_alias_missing_host_returns_not_ready() {
    ...
    #[test]
    fn lenos_agent_databricks_v1_with_databricks_model_but_no_lenos_agent_model_is_ready() {
    ...
    #[test]
    fn lenos_agent_anthropic_with_anthropic_model_but_no_lenos_agent_model_is_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn lenos_agent_openai_with_openai_compat_model_but_no_lenos_agent_model_is_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "openai"),
    ...
    #[test]
    fn lenos_agent_empty_provider_model_fallback_key_is_not_ready() {
        // An empty DATABRICKS_MODEL with no LENOS_AGENT_MODEL must still be NotReady.
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "databricks_v2"),
        ...
        assert!(
            !result.is_ready(),
            "empty DATABRICKS_MODEL with no LENOS_AGENT_MODEL must be NotReady"
        );
        assert!(result
            .requirements()
            .contains(&Requirement::NormalizedField {
    ...
    #[test]
    fn lenos_agent_openrouter_with_all_fields_is_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "openrouter"),
                ("LENOS_AGENT_MODEL", "anthropic/claude-sonnet-4"),
    ...
    #[test]
    fn lenos_agent_openrouter_missing_key_returns_not_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "openrouter"),
    ...
    #[test]
    fn lenos_agent_openrouter_with_provider_model_fallback_is_ready() {
        let env = make_env(
            "lenos-agent",
            env_with(&[
                ("LENOS_AGENT_PROVIDER", "openrouter"),
