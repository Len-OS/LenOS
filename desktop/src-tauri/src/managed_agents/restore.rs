...
pub fn backfill_persona_snapshots(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    for record in records.iter_mut() {
        ...
        let Some(persona) = personas.iter().find(|p| p.id == persona_id) else {
            eprintln!(
                "lenos-desktop: persona-snapshot backfill: agent {} links persona {persona_id} which no longer exists; leaving it orphaned — spawn will refuse it",
                record.pubkey
            );
            continue;
        };
...
pub async fn restore_managed_agents_on_launch(
    ...
    let mut agents_to_start: Vec<super::ManagedAgentRecord>;
    {
        ...
        // Exact-path sweep: kill any lenos-acp process whose executable path
        ...
        // LENOS_MANAGED_AGENT injection or lost their PID-file receipt.
        //
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Err(e) =
                crate::commands::reconcile_agent_profile(&state, &reconcile_app, &pubkey, &data)
                    .await
            {
                eprintln!("lenos-desktop: profile reconciliation failed for agent {pubkey}: {e}");
            }
        });
    }
