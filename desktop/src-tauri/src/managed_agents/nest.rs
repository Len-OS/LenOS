//! LenOS Nest — persistent agent workspace at `~/.lenos`.
//!
//! Creates a shared knowledge directory on first launch so every
//! LenOS-spawned agent starts with orientation (AGENTS.md) and a
...
/// Default SKILL.md content for the lenos-cli skill.
/// Written to ~/.lenos/.agents/skills/lenos-cli/SKILL.md on first init.
const LENOS_CLI_SKILL_MD: &str = include_str!("nest_skill.md");
...
const BEGIN_MARKER: &str = "<!-- BEGIN LENOS MANAGED";
const END_MARKER: &str = "<!-- END LENOS MANAGED -->";

/// Canonical skill directory path relative to the nest root.
const CANONICAL_SKILL_DIR: &str = ".agents/skills/lenos-cli";
...
/// Returns `true` iff `path` ends with the dev-nest directory name (`.lenos-dev`).
///
...
/// Returns `true` when the running binary is using the dev nest (`~/.lenos-dev`).
///
...
/// Creates the LenOS nest at `~/.lenos` if it doesn't already exist.
///
...
/// Creates a LenOS nest at the given `root` path.
///
/// - Creates the root directory and all subdirectories.
/// - Writes `AGENTS.md` only if it doesn't already exist.
/// - Writes `.agents/skills/lenos-cli/SKILL.md` only if it doesn't already exist.
/// - Creates harness-specific symlinks pointing to the canonical
///   `.agents/skills/lenos-cli` directory for each known provider.
...
pub fn ensure_nest_at(root: &Path) -> Result<(), String> {
    ...
    // Write lenos-cli skill to the harness-agnostic .agents path.
        ...
        .open(&skill_md)
    {
        Ok(mut file) => {
            use std::io::Write;
            file.write_all(LENOS_CLI_SKILL_MD.as_bytes())
    ...
    // Migration of the old .claude/skills/lenos-cli real dir is handled in
...
fn ensure_skill_symlinks(root: &Path) -> Result<(), String> {
    for skill_dir in known_skill_dirs() {
        ...
        let link = parent.join("lenos-cli");
...
///
/// Dev builds (`is_dev = true`) use `"lenos-dev"` so that a running DMG and a
/// concurrent dev build each own a separate link and never clobber each other —
/// the same isolation that separates `~/.lenos` (prod) from `~/.lenos-dev` (dev).
...
/// Ensures `~/.local/bin/lenos` (prod) or `~/.local/bin/lenos-dev` (dev) is a
...
/// `~/.lenos` and `~/.lenos-dev` nests (see [`NEST_DIR_DEV`]).
///
/// On every boot: replaces any existing symlink unconditionally (the `lenos` /
/// `lenos-dev` name is our namespace), creates a new one if absent, and leaves
...
pub fn ensure_cli_symlink(exe_parent: &Path, is_dev: bool) -> Result<(), String> {
    let lenos_bin = exe_parent.join("lenos");
    if !lenos_bin.exists() {
...
///
/// Preserves everything from the `<!-- BEGIN LENOS MANAGED` marker onward
...
fn refresh_skill_md_if_stale(root: &Path) -> Result<(), String> {
    let agents_skill_dir = root.join(".agents/skills/lenos-cli");
    let version_path = agents_skill_dir.join(".skill-version");
    if read_version_file(&version_path) >= NEST_SKILL_VERSION {
        return Ok(());
    }

    // Migration: if .claude/skills/lenos-cli exists as a real directory
    ...
    let skill_content = if old_is_real_dir {
        ...
        fs::read_to_string(old_skill_dir.join("SKILL.md"))
            .unwrap_or_else(|_| LENOS_CLI_SKILL_MD.to_string())
    } else {
        LENOS_CLI_SKILL_MD.to_string()
    };
    ...
    // Create/replace the .claude/skills/lenos-cli symlink.
    #[cfg(unix)]
    {
        ...
        let symlink_path = root.join(".claude/skills/lenos-cli");
        ...
        create_symlink(
            std::path::Path::new("../../.agents/skills/lenos-cli"),
            &symlink_path,
        )
...
pub fn render_dynamic_section(
    ...
    let active_agents = if agents.is_empty() {
        "## Active Agents\n\n*(No agents deployed yet. Add agents in the LenOS desktop app.)*"
            .to_string()
    } else {
        let mut table =
...
pub fn try_regenerate_nest(app: &AppHandle) {
    if let Err(error) = regenerate_nest_context(app) {
        eprintln!("lenos-desktop: nest context regeneration failed: {error}");
    }
}
