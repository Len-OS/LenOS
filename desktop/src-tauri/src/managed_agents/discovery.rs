...
use crate::managed_agents::{
    lenos_managed_command_path, lenos_managed_node_bin_dir, lenos_managed_npm_bin_dir,
...
const LENOS_AGENT_AVATAR_URL: &str =
    "https://raw.githubusercontent.com/BuildGrowthNow/LenOS/refs/heads/main/crates/lenos-agent/lenos-agent.png";
fn common_binary_paths() -> &'static [PathBuf] {
    ...
    PATHS.get_or_init(|| {
        ...
        if let Some(managed_node_bin) = lenos_managed_node_bin_dir() {
            paths.insert(0, managed_node_bin);
        }
        if let Some(managed_bin) = lenos_managed_npm_bin_dir() {
...
const KNOWN_ACP_RUNTIMES: &[KnownAcpRuntime] = &[
    KnownAcpRuntime {
        ...
        cli_install_hint: "LenOS talks to Goose through the Goose CLI.",
    ...
    KnownAcpRuntime {
        ...
        cli_install_hint: "LenOS talks to Claude Code through the Claude Code CLI.",
        adapter_install_hint: "LenOS talks to the Claude Code CLI through an ACP adapter. Install it with: npm install -g @agentclientprotocol/claude-agent-acp.",
    ...
    KnownAcpRuntime {
        ...
        mcp_command: Some("lenos-dev-mcp"),
        mcp_hooks: false,
        ...
        cli_install_hint: "LenOS talks to Codex through the Codex CLI.",
        adapter_install_hint: "LenOS talks to the Codex CLI through an ACP adapter. Install it with: npm install -g @agentclientprotocol/codex-acp.",
    ...
    KnownAcpRuntime {
        id: "lenos-agent",
        label: "LenOS Agent",
        commands: &["lenos-agent"],
        aliases: &[],
        avatar_url: LENOS_AGENT_AVATAR_URL,
        mcp_command: Some("lenos-dev-mcp"),
        mcp_hooks: true,
        underlying_cli: None,
        ...
        adapter_install_commands: &[],
        cli_install_instructions_url: "https://github.com/BuildGrowthNow/LenOS",
        adapter_install_instructions_url: "https://github.com/BuildGrowthNow/LenOS",
        cli_install_hint: "Ships with the LenOS desktop app.",
        adapter_install_hint: "",
        skill_dir: None,
        supports_acp_model_switching: true,
        model_env_var: Some("LENOS_AGENT_MODEL"),
        provider_env_var: Some("LENOS_AGENT_PROVIDER"),
        provider_locked: false,
        default_env: &[],
        config_file_path: None,
        config_file_format: None,
        supports_acp_native_config: false,
        thinking_env_var: Some("LENOS_AGENT_THINKING_EFFORT"),
        max_tokens_env_var: Some("LENOS_AGENT_MAX_OUTPUT_TOKENS"),
        context_limit_env_var: Some("LENOS_AGENT_MAX_CONTEXT_TOKENS"),
        required_normalized_fields: &["model", "provider"],
        login_hint: None,
        auth_probe_args: None,
    },
...
/// request supplies none. Resolves the bundled `lenos-agent` from the catalog so
...
/// bundled `lenos-agent` ships with the app and resolves on every platform.
pub fn default_agent_command() -> String {
    known_acp_runtime_exact("lenos-agent")
        .and_then(|p| p.commands.first().copied())
        .unwrap_or("lenos-agent")
        .to_string()
}
...
/// without falling through to `lenos-agent`.  When there is no runtime id at all
...
pub fn resolve_command(command: &str) -> Option<PathBuf> {
    if let Some(managed) = resolve_lenos_managed_command(command) {
        return Some(managed);
    }
...
fn resolve_lenos_managed_command(command: &str) -> Option<PathBuf> {
    let basenames = command_basenames(command);
    basenames
        .iter()
        .find_map(|basename| lenos_managed_command_path(command, basename))
}
...
/// On Windows: Git Bash via `resolve_bash_path` — skips `LENOS_SHELL` because
...
///
/// LenOS rewrites these catalog commands to an app-private npm prefix before
...
/// The oldest `codex-acp` version supported by LenOS managed agents.
///
/// Older 1.x adapters are detected successfully, but can still bundle a Codex runtime
/// that does not reliably give `lenos` CLI subprocesses outbound relay access.
///
...
fn discover_acp_runtime_phase1(runtime: &'static KnownAcpRuntime) -> PartialEntry {
    ...
    // node_required now means LenOS cannot provide npm for this platform.
    // On supported desktop platforms, LenOS downloads a private Node/npm
    ...
    let node_required = matches!(
        ...
        && lenos_managed_node_bin_dir().is_none()
