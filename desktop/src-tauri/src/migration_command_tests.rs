...
fn reconcile_legacy_command_names_rewrites_renamed_sidecars() {
    ...
    assert_eq!(records[0]["acp_command"], "lenos-acp");
    assert_eq!(records[0]["agent_command"], "lenos-agent");
    assert_eq!(records[0]["mcp_command"], "lenos-dev-mcp");
}

#[test]
fn reconcile_legacy_command_names_updates_removed_mcp_server_for_lenos_agent() {
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
fn reconcile_legacy_command_names_rewrites_persona_runtime() {
    ...
    assert_eq!(records[0]["runtime"], "lenos-agent");
}
...
fn rewrite_legacy_persona_md_runtime_rewrites_frontmatter_only() {
    ...
    assert!(updated.contains("runtime: lenos-agent\n"));
