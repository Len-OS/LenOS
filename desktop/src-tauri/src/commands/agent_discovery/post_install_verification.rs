...
pub(super) fn run(
    ...
    // Observe PATH changes and binaries added after LenOS launched.
...
fn failure(
    ...
    Some(InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "The installer finished, but LenOS still could not use {runtime_id} (observed: {observed})."
        ),
        exit_code: None,
        hint: Some(
            "LenOS requires the vendor CLI executable, not only its desktop app. If the CLI was installed while LenOS was open, restart LenOS and check again."
                .to_string(),
        ),
    })
}
