...
pub(crate) fn validate_clone_url(clone_url: &str) -> Result<(), String> {
    ...
    // LenOS git remotes are served at `…/git/<owner-pubkey>/<repo-id>` — a
    ...
    let segments = parsed
        ...
        .unwrap_or_default();
    let is_lenos_repo_path = segments
        .iter()
        ...
        .unwrap_or(false);
    if !is_lenos_repo_path {
        return Err("clone URL must point at a LenOS git repository".into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn credential_helper_config_value_uses_forward_slashes() {
        let path =
            std::path::PathBuf::from(r"C:\Users\x\AppData\Local\LenOS\git-credential-nostr.exe");
        assert_eq!(
            credential_helper_config_value(&path),
            "C:/Users/x/AppData/Local/LenOS/git-credential-nostr.exe",
        );
    }
    ...
    #[test]
    fn validate_clone_url_requires_lenos_repo_shape() {
