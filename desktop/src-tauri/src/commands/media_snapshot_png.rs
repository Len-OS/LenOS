//! Preservation of LenOS snapshot tEXt chunks through the upload sanitizer.
//!
...
//! (`lenos_agent_snapshot` / `lenos_team_snapshot`); the sanitizer's re-encode
...
//! relay allowlists exactly these keywords in `lenos-media::validation` — the
//! two lists must stay in sync.

/// tEXt keywords that carry LenOS snapshot manifests (`.agent.png` /
/// `.team.png`). These chunks are the product payload of agent/team sharing —
/// they must survive the metadata strip.
const PNG_SNAPSHOT_KEYWORDS: [&[u8]; 2] = [b"lenos_agent_snapshot", b"lenos_team_snapshot"];

/// Extract the raw bytes of the first LenOS snapshot tEXt chunk (length + type
...
mod tests {
    ...
    fn test_sanitizer_preserves_agent_snapshot_text_chunk() {
        ...
        for keyword in ["lenos_agent_snapshot", "lenos_team_snapshot"] {
    ...
    fn test_agent_snapshot_survives_full_share_pipeline() {
        ...
        let snapshot = AgentSnapshot {
            format: "lenos-agent-snapshot".to_string(),
            version: 1,
        ...
        // Relay ingest path.
        let relay_config = lenos_media_pkg::MediaConfig {
            s3_endpoint: String::new(),
            s3_access_key: String::new(),
            s3_secret_key: String::new(),
            s3_bucket: String::new(),
            s3_region: "us-east-1".to_string(),
            s3_addressing_style: lenos_media_pkg::S3AddressingStyle::Path,
        ...
        assert_eq!(
            lenos_media_pkg::validation::validate_content(&sanitized, &relay_config)
