...
impl From<String> for ProjectPullRequestMergeError {
    fn from(message: String) -> Self {
        // Relay push-policy denial for a repo with no `lenos-channel` binding.
        // The stable token is declared in `lenos-core::git_perms`
        ...
        if message.contains(lenos_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN) {
            return Self::new(
                lenos_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN,
                "This repository is not bound to a channel, so the relay cannot \
                 authorize pushes. Bind it with: lenos repos bind --id <repo> \
...
mod tests {
    ...
    fn no_channel_binding_denial_converts_to_structured_code() {
        ...
        let remote_stderr = format!(
            ...
            lenos_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_BODY
        );
        ...
        assert!(error.message.contains("lenos repos bind"));
