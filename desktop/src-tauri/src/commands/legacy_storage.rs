...
fn legacy_identifier(current_identifier: &str) -> Option<String> {
    if current_identifier.starts_with(LENOS_DEV_IDENTIFIER_PREFIX) {
        Some(current_identifier.replacen(
            LENOS_DEV_IDENTIFIER_PREFIX,
            SPROUT_DEV_IDENTIFIER_PREFIX,
            1,
        ))
    } else if current_identifier.starts_with(LENOS_RELEASE_IDENTIFIER_PREFIX) {
        Some(current_identifier.replacen(
            LENOS_RELEASE_IDENTIFIER_PREFIX,
...
/// data directory so the frontend can seed LenOS localStorage before first
    ...
    tokio::task::spawn_blocking(move || {
        ...
        for database in databases {
            match read_legacy_workspace_storage_db(&database) {
                ...
                Err(error) => eprintln!(
                    "lenos-desktop: legacy-local-storage-migration: {}: {error}",
                    database.display()
                ),
            }
        }
