...
/// with the key in goose's own keyring rather than LenOS's env) would otherwise
...
impl DiscoveryProvider {
    ...
    /// the caller must fall through to the `lenos-acp models` subprocess rather
...
/// — even on an internal build that bakes `LENOS_AGENT_PROVIDER=databricks_v2`
/// and a `DATABRICKS_HOST`. Discovery then degrades to the `lenos-acp models`
