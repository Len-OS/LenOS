...
/// Read the workspace owner pubkey without holding the lock. Used to populate `LENOS_ACP_AGENT_OWNER`
...
pub(super) fn retain_managed_agent_pending(
    ...
    if let Err(e) = result {
        eprintln!("lenos-desktop: agent-retain: {e}");
    }
}
...
pub(super) fn tombstone_managed_agent_pending(
    ...
    use lenos_core_pkg::kind::KIND_MANAGED_AGENT;
    use nostr::JsonUtil;
    ...
    if let Err(e) = result {
        eprintln!("lenos-desktop: agent-tombstone: {e}");
    }
}
...
pub(super) fn build_agent_archive_request(
        ...
        .eq_ignore_ascii_case(agent_pubkey)
    {
        ...
        let tag_json = lenos_sdk_pkg::nip_oa::compute_auth_tag(keys, &agent, "")
...
pub(super) fn archive_managed_agent_pending(app: &AppHandle, state: &AppState, agent_pubkey: &str) {
    ...
    use lenos_core_pkg::kind::KIND_IA_ARCHIVE_REQUEST;
    use nostr::JsonUtil;
    ...
    if let Err(e) = result {
        eprintln!("lenos-desktop: agent-archive: {e}");
    }
}
...
fn normalize_relay_mesh(
    ...
    if model_ref.is_empty() {
        return Err("LenOS shared compute model is required".to_string());
    }
    if backend != &BackendKind::Local {
        return Err("LenOS shared compute agents must use the local backend".to_string());
    }
...
pub async fn create_managed_agent(
    ...
    // The harness has its own validator (lenos-acp/src/config.rs) but we want
    ...
    let auth_tag = {
        ...
        // Bridge nostr 0.37 → 0.36 (lenos-sdk) via hex round-trip.
        ...
        let tag = lenos_sdk_pkg::nip_oa::compute_auth_tag(&compat_owner, &compat_agent, "")
...
pub async fn start_managed_agent(
            ...
            .load(std::sync::atomic::Ordering::Acquire)
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            {
                eprintln!(
                    "lenos-desktop: profile reconciliation failed for agent {reconcile_pubkey}: {e}"
                );
            }
        });
    }
