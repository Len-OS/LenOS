...
//! GPU/driver/compositor combinations, so LenOS comes up with no window at all
...
/// process is still single threaded, which it is nowhere else in LenOS.
///
...
pub fn apply() -> Result<(), String> {
    match plan(
        ...
        Plan::Apply { vars, why } => {
            ...
            eprintln!("lenos-desktop: {} — {why}", applied.join(" "));
            Ok(())
        }
        Plan::Leave { why } => {
            eprintln!("lenos-desktop: WebKit rendering left as-is — {why}");
            Ok(())
        }
