...
//! LenOS/Nostr identity). The node presents a signed ownership attestation
...
//! has one owner identity whether mesh runs embedded in LenOS or standalone.
...
impl OwnerIdentity {
    /// Sign a LenOS-to-MeshLLM ownership binding. The member's Nostr signature
...
pub fn member_endpoint_binding_bytes(member_pubkey: &str, endpoint_tokens: &[String]) -> Vec<u8> {
    ...
    format!(
        "lenos-mesh-owner-endpoint-binding-v1:{}:{}",
        member_pubkey.trim().to_ascii_lowercase(),
