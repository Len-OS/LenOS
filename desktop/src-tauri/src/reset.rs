...
/// (e.g. `xyz.block.lenos.app` or `xyz.block.lenos.app.dev`).
...
mod tests {
    ...
    fn test_crash_retry_cleans_prior_deterministic_trash() {
        ...
        let app_data = app_support.join("xyz.block.lenos.app");
        std::fs::create_dir_all(&app_data).unwrap();
        write_sentinel(&app_data).unwrap();
        ...
        let trash_app_dir = app_support.join("xyz.block.lenos.app.reset-trash");
    ...
    fn test_keychain_fail_restores_all_then_retry_cleans() {
        ...
        let trash_app = app_support.join("xyz.block.lenos.app.reset-trash");
