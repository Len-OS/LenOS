...
//!
//! Mirrors the lenos-sdk builder patterns but uses nostr 0.37 API
...
use lenos_core_pkg::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_IA_UNARCHIVE_REQUEST};
use nostr::{EventBuilder, EventId, Kind, Tag};
use uuid::Uuid;

// ── Constants ────────────────────────────────────────────────────────────────

/// Maximum content size — matches lenos-sdk (64 KiB).
const MAX_CONTENT_BYTES: usize = 64 * 1024;

/// Maximum mention count — matches lenos-sdk.
const MAX_MENTIONS: usize = 50;

/// Maximum emoji length in characters — matches lenos-sdk.
...
pub fn build_create_channel(
    ...
    let name = lenos_sdk_pkg::canonical_channel_name(name);
...
pub fn build_update_channel(
    ...
    let name = name.map(lenos_sdk_pkg::canonical_channel_name);
