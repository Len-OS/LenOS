...
pub fn spawn_event_sync(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        .await
        {
            eprintln!("lenos-desktop: event-sync: spawn_blocking failed: {e}");
        }
    });
}
...
pub fn migrate_personas_to_events(app: &tauri::AppHandle, keys: &nostr::Keys, db_path: &Path) {
    ...
    match migrate_personas_in_dir_at(&base_dir, keys, db_path) {
        ...
        Ok(migrated) => {
            eprintln!(
                "lenos-desktop: persona-event-migration: {migrated} personas migrated to retention"
            );
        }
        Err(e) => {
            eprintln!("lenos-desktop: persona-event-migration: {e}");
        }
    }
}
...
fn migrate_personas_in_dir_at(
    ...
    use lenos_core_pkg::kind::KIND_PERSONA;
    use nostr::JsonUtil;
    ...
    for record in &records {
        ...
        scoped_record.shared = existing
            ...
            .is_some_and(|event| lenos_core_pkg::kind::event_is_shared(&event));
...
fn migrate_teams_in_dir_at(
    ...
    use lenos_core_pkg::kind::KIND_TEAM;
    use nostr::JsonUtil;
