//! Worktree data sync and on-launch reconciliation for the LenOS desktop app.
//!
...
//! fix-up of persisted built-in command names from the Sprout→LenOS rename.
//!
...
/// receive their identity via the `LENOS_PRIVATE_KEY` env var.
...
fn run_boot_migrations_inner(app: &tauri::AppHandle, reset_completed: bool) {
    ...
    // On dev builds, copy `.repos-dir` from ~/.lenos → ~/.lenos-dev BEFORE
    ...
    // keyring ("lenos-desktop") into the dev service ("lenos-desktop-dev")
...
/// the current LenOS identifier directory. The Tauri identifier controls the app
...
pub fn migrate_legacy_app_data_dir(app: &tauri::AppHandle) {
    let current_dir = match app.path().app_data_dir() {
        Ok(dir) => dir,
        Err(e) => {
            eprintln!("lenos-desktop: app-data-migration: cannot resolve app data dir: {e}");
            return;
        }
    };
    ...
    match copy_dir_all(&legacy_dir, &current_dir) {
        Ok(()) => eprintln!(
            "lenos-desktop: app-data-migration: copied legacy data from {} to {}",
            legacy_dir.display(),
            current_dir.display()
        ),
        Err(error) => eprintln!(
            "lenos-desktop: app-data-migration: failed to copy {} to {}: {error}",
            legacy_dir.display(),
            current_dir.display()
        ),
    }
...
/// agent's accumulated knowledge in `~/.sprout` while `~/.lenos` booted empty —
...
pub fn migrate_legacy_nest() -> bool {
    let Some(home) = dirs::home_dir() else {
        eprintln!("lenos-desktop: nest-migration: cannot resolve home directory");
        return false;
    };
    // Destination is the current build's nest dir (`.lenos` or `.lenos-dev`).
    let Some(current_nest) = crate::managed_agents::nest_dir() else {
        eprintln!("lenos-desktop: nest-migration: cannot resolve nest directory");
        return false;
    };
...
fn migrate_legacy_nest_at(legacy: &Path, current: &Path) -> bool {
    ...
    if current.file_name().is_some_and(|name| name == ".lenos-dev")
    ...
    for name in LEGACY_NEST_KNOWLEDGE {
        ...
        let result = if src.is_dir() {
            ...
            // `ensure_nest` writes a default `~/.lenos/AGENTS.md` before this
        ...
        match result {
            Ok(()) => eprintln!(
                "lenos-desktop: nest-migration: migrated {} to {}",
                src.display(),
                dst.display()
            ),
            Err(error) => eprintln!(
                "lenos-desktop: nest-migration: failed to migrate {} to {}: {error}",
                src.display(),
                dst.display()
            ),
        }
...
/// knowledge migration. Presence of this file means `~/.lenos` content has
/// already been copied into `~/.lenos-dev` and subsequent boots can skip the
...
/// also copies into `~/.lenos-dev` and could otherwise set the sentinel early.
...
/// Injectable core: copy `.repos-dir` from `<home>/.lenos/` into `dev_nest`,
...
pub(crate) fn migrate_dev_repos_dir_at(home: &Path, dev_nest: &Path) {
    let src = home.join(".lenos").join(".repos-dir");
    if !src.exists() {
        return;
    }
    ...
    if let Err(e) = std::fs::create_dir_all(dev_nest) {
        eprintln!(
            "lenos-desktop: dev-nest-migration: failed to create dev nest {}: {e}",
            dev_nest.display()
        );
        return;
    }
    match std::fs::copy(&src, &dst) {
        Ok(_) => eprintln!(
            "lenos-desktop: dev-nest-migration: migrated .repos-dir to {}",
            dst.display()
        ),
        Err(e) => eprintln!("lenos-desktop: dev-nest-migration: failed to migrate .repos-dir: {e}"),
    }
}
...
/// One-time migration of dev-build nest contents from `~/.lenos` → `~/.lenos-dev`.
///
/// When a dev build first boots after this change ships, it switches from the
/// shared `~/.lenos` nest to a dedicated `~/.lenos-dev` nest. Without migration,
...
/// `~/.lenos` is never deleted — prod builds continue to use it normally.
///
/// Completion is tracked by a [`DEV_NEST_MIGRATED_SENTINEL`] file written into
/// `~/.lenos-dev`. Using an explicit sentinel (rather than RESEARCH/PLANS file
/// presence) decouples this migration from the `.sprout` → `~/.lenos-dev`
/// migration that runs earlier in the same boot, which might otherwise populate
/// RESEARCH/PLANS and incorrectly suppress the `~/.lenos` copy.
///
...
pub fn migrate_dev_nest() -> bool {
    let Some(home) = dirs::home_dir() else {
        eprintln!("lenos-desktop: dev-nest-migration: cannot resolve home directory");
        return false;
    };
    let legacy = home.join(".lenos");
    let current = home.join(".lenos-dev");
    ...
    if copied {
        ...
        if let Err(e) = std::fs::write(&sentinel, "") {
            eprintln!(
                "lenos-desktop: dev-nest-migration: failed to write sentinel {}: {e}",
                sentinel.display()
            );
        }
    }
...
fn patch_json_records(
    ...
    let Ok(mut records) = serde_json::from_str::<Vec<serde_json::Value>>(&content) else {
        eprintln!(
            "lenos-desktop: patch-json-records: failed to parse {}",
            path.display()
        );
        return;
    };
    ...
    if changed {
        if let Ok(bytes) = serde_json::to_vec_pretty(&records) {
            if let Err(e) = crate::managed_agents::atomic_write_json_restricted(path, &bytes) {
                eprintln!("lenos-desktop: patch-json-records: {e}");
            }
        }
    }
}
...
fn refresh_builtin_agent_avatars_in_file(
    ...
    let Ok(mut records) = serde_json::from_str::<Vec<serde_json::Value>>(&contents) else {
        eprintln!(
            "lenos-desktop: refresh-builtin-agent-avatars: invalid JSON in {}",
            path.display()
        );
        return;
    };
    ...
    if changed {
        if let Ok(bytes) = serde_json::to_vec_pretty(&records) {
            if let Err(e) = crate::managed_agents::atomic_write_json_restricted(path, &bytes) {
                eprintln!("lenos-desktop: refresh-builtin-agent-avatars: {e}");
            }
        }
    }
}
...
///
/// Guards:
/// - `LENOS_SHARE_IDENTITY` must be `"1"`
/// - `LENOS_PRIVATE_KEY` must parse as valid `nostr::Keys`
...
pub fn sync_shared_agent_data(app: &tauri::AppHandle) {
    ...
    let is_shared = std::env::var("LENOS_SHARE_IDENTITY")
        .map(|v| v == "1")
        .unwrap_or(false);
    if !is_shared {
        return;
    ...
    // Guard: LENOS_PRIVATE_KEY must be a valid nostr key.
    let has_valid_key = std::env::var("LENOS_PRIVATE_KEY")
        .ok()
    ...
    let current_dir = match app.path().app_data_dir() {
        Ok(dir) => dir,
        Err(e) => {
            eprintln!("lenos-desktop: shared-agent-sync: cannot resolve app data dir: {e}");
            return;
        }
    };
    ...
    if !is_dev {
        eprintln!(
            "lenos-desktop: shared-agent-sync: skipping — data dir is not a dev dir ({})",
            current_dir.display()
        );
        return;
    }
    ...
    let canonical_dir = match canonical_dev_data_dir(&current_dir) {
        Some(dir) => dir,
        None => {
            eprintln!("lenos-desktop: shared-agent-sync: cannot compute canonical dir (no parent)");
            return;
        }
    };
    ...
    if !canonical_dir.exists() {
        eprintln!(
            "lenos-desktop: shared-agent-sync: canonical dir does not exist: {}",
            ...
            if sibling_file.is_file() && !sibling_file.is_symlink() {
                if let Some(file_parent) = canonical_file.parent() {
                    if let Err(e) = std::fs::create_dir_all(file_parent) {
                        eprintln!(
                            "lenos-desktop: shared-agent-sync: failed to create {}: {e}",
                            file_parent.display()
                        );
                        break;
                    }
                ...
                eprintln!(
                    "lenos-desktop: shared-agent-sync: seeded {rel} from {}",
                    sibling.display()
                );
                break;
            }
                ...
                if let Ok(entries) = std::fs::read_dir(parent) {
                    for entry in entries.flatten() {
                        ...
                        if sibling_dir.is_dir() && !sibling_dir.is_symlink() {
                            ...
                            eprintln!(
                                "lenos-desktop: shared-agent-sync: migrated {rel} from {}",
                                sibling.display()
                            );
                            break;
                        }
    ...
    if synced > 0 {
        eprintln!(
            "lenos-desktop: shared-agent-sync: {synced} item(s) linked to {}",
            canonical_dir.display()
        );
    }
}
...
fn reconcile_mcp_commands_in_file(path: &Path) {
    ...
    patch_json_records(path, |obj| {
        ...
        if !current.is_empty() && current != "lenos-mcp-server" {
            return false;
        }
        eprintln!(
            "lenos-desktop: runtime-reconcile: {:?} ({:?}): mcp_command {:?} → {:?}",
...
fn replace_command_field(
    ...
    eprintln!(
        "lenos-desktop: command-rename-reconcile: {:?}: {field} {:?} → {:?}",
        obj.get("name").and_then(|v| v.as_str()).unwrap_or("?"),
        current,
        replacement,
    );
    ...
    patch_json_records(path, |obj| {
            ...
            .map(str::to_string)
        {
            if acp_command == "sprout-acp" {
                changed |= replace_command_field(obj, "acp_command", "lenos-acp".to_string());
            }
        }
        ...
        if agent_command == "sprout-agent" {
            agent_command = "lenos-agent".to_string();
            ...
            .map(str::to_string)
        {
            match mcp_command.as_str() {
                "sprout-dev-mcp" => {
                    changed |=
                        replace_command_field(obj, "mcp_command", "lenos-dev-mcp".to_string());
                }
                "sprout-mcp" | "sprout-mcp-server" | "lenos-mcp-server" => {
                    let replacement = if agent_command == "lenos-agent" {
                        "lenos-dev-mcp"
                    } else {
                        ""
                    };
...
fn reconcile_legacy_persona_runtimes_in_file(path: &Path) {
    patch_json_records(path, |obj| {
        ...
        eprintln!(
            "lenos-desktop: command-rename-reconcile: persona {:?}: runtime {:?} → {:?}",
        ...
        obj.insert(
            "runtime".to_string(),
            serde_json::Value::String("lenos-agent".to_string()),
        );
        true
    });
}
...
fn rewrite_legacy_persona_md_runtime(content: &str) -> Option<String> {
    let (frontmatter, body) = lenos_persona_pkg::persona::split_frontmatter(content).ok()?;
    ...
    *runtime = serde_yaml::Value::String("lenos-agent".to_string());
...
fn reconcile_legacy_team_persona_runtime_files(dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        match std::fs::write(&path, updated) {
            Ok(()) => {
                eprintln!(
                    "lenos-desktop: command-rename-reconcile: updated {}",
                    path.display()
                );
            }
            Err(error) => {
                eprintln!(
                    "lenos-desktop: command-rename-reconcile: failed to update {}: {error}",
                    path.display()
                );
            }
        }
...
/// Reconcile exact built-in command values persisted before the Sprout→LenOS
...
fn reconcile_databricks_v1_to_v2_in_file(path: &Path, rewrite_v1_provider: bool) {
    ...
    patch_json_records(path, |obj| {
        ...
        // marks this as a Block build (LENOS_AGENT_PROVIDER == "databricks_v2").
        ...
        if rewrite_v1_provider && obj.get("provider").and_then(|v| v.as_str()) == Some("databricks")
        {
            let name = obj
                ...
                .to_string();
            eprintln!(
                "lenos-desktop: databricks-v1-to-v2: {name:?}: provider \"databricks\" → \"databricks_v2\"",
            );
            obj.insert(
                "provider".to_string(),
            ...
            // (LENOS_AGENT_MODEL from runtime_metadata_env_vars takes priority in
            // lenos-agent config.rs). Clearing it lets the baked V2 default win.
            if obj.remove("model").is_some() {
                eprintln!(
                    "lenos-desktop: databricks-v1-to-v2: {name:?}: cleared stale V1 model field",
                );
            }
            changed = true;
        }
        ...
        if let Some(serde_json::Value::Object(env_vars)) = obj.get_mut("env_vars") {
            ...
            for key in stale_keys {
                ...
                eprintln!("lenos-desktop: databricks-v1-to-v2: removed stale env_vars[\"{key}\"]",);
                changed = true;
            }
        }
...
/// `LENOS_AGENT_PROVIDER=databricks_v2`): the structured `provider` field is
...
///
/// In both cases, stale `LENOS_AGENT_PROVIDER` / `LENOS_AGENT_MODEL` /
...
pub fn reconcile_databricks_v1_to_v2(app: &tauri::AppHandle) {
    ...
    // On Block builds, the baked env contains LENOS_AGENT_PROVIDER=databricks_v2.
