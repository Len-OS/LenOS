...
pub(crate) fn profile_valid_oa_owner_pubkey(event: &Event) -> Option<String> {
    ...
    for tag in event.tags.iter() {
        ...
        if let Ok(owner_pubkey) = lenos_sdk_pkg::nip_oa::verify_auth_tag(&json, &target_pubkey) {
...
mod tests {
    ...
    fn oa_profile_event(content: &str) -> (Event, String) {
        ...
        let tag_json = lenos_sdk_pkg::nip_oa::compute_auth_tag(&owner_keys, &agent_pubkey, "")
