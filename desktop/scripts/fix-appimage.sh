...
APP_BIN="$WORKDIR/squashfs-root/usr/bin/lenos-desktop"
if [[ ! -f "$APP_BIN" ]]; then
  echo "Error: app binary usr/bin/lenos-desktop not found — bundler layout changed; update fix-appimage.sh" >&2
  exit 1
fi
if [[ -e "$APP_BIN.bin" ]]; then
  echo "Error: usr/bin/lenos-desktop.bin already exists — shim already installed?" >&2
  exit 1
fi

# The real binary moves aside; lenos-desktop becomes a shim AppRun.wrapped execs.
...
exec -a "lenos-desktop" "$here/lenos-desktop.bin" "$@"
SHIM
chmod +x "$APP_BIN"
