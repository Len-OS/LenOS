...
A tiny non-AI LenOS bot example.

The bot is deliberately boring and algorithmic: it listens to one LenOS channel
...
It demonstrates that LenOS participants do not have to be LLM agents. Any
...
members list and in LenOS's mention autocomplete.
...
LENOS_CHANNEL_ID=<channel-uuid> \
LENOS_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
LENOS_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that LenOS agents
...
LENOS_CHANNEL_ID=<channel-uuid> \
LENOS_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
LENOS_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
LENOS_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
LENOS_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
LENOS_BOT_AUTH_MODE=owner-attested \
# plus LENOS_RELAY_URL, LENOS_CHANNEL_ID, LENOS_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `LENOS_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `LENOS_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The LenOS UI adds that tag when the bot is selected from
