...
A LenOS **community** is the tenant-visible workspace selected by the request host.
...
│                           CLIENTS                                    │
│                                                                      │
│  Human (Nostr app, web, mobile)    Agent (CLI tools via lenos-cli)    │
│           │                                    │                     │
│           └──────────── WebSocket ─────────────┘                    │
└─────────────────────────────────────────────────────────────────────┘
                               │
...
└──────────┬──────────────┬──────────────────────────────────────────┘
     ...
     ┌──────────────┐
     │  Postgres    │  ← lenos-search (FTS over the search_tsv
...
```
lenos-core    (zero I/O — types, verification, filter matching, kind registry)
    │
    ├── lenos-db          (Postgres: events, channels, tokens, workflows, audit)
    ├── lenos-auth        (NIP-42, NIP-98, API tokens, scopes, rate limiting)
    ├── lenos-pubsub      (Redis pub/sub, presence, typing indicators)
    ├── lenos-search      (Postgres FTS: query, delete)
    ├── lenos-audit       (hash-chain tamper-evident log)
    └── lenos-workflow    (YAML-as-code automation engine)
         │
         └── lenos-relay       (ties everything together — the server)

lenos-acp            (agent harness — bridges relay @mentions → AI agents via ACP/JSON-RPC)
lenos-sdk            (typed Nostr event builders — used by lenos-acp and lenos-cli)
lenos-media          (Blossom/S3 media storage)
lenos-cli            (agent-first CLI)
lenos-admin          (operator CLI: relay membership + key generation)
lenos-test-client    (integration test harness + manual CLI)
```

**Key architectural principle:** The relay is the single source of truth. `lenos-relay` orchestrates all subsystems by calling them directly — it imports `lenos-db`, `lenos-auth`, `lenos-pubsub`, `lenos-search`, `lenos-audit`, and `lenos-workflow`. However, those subsystems are isolated from each other: `lenos-workflow` never calls `lenos-pubsub`, `lenos-search` never calls `lenos-db`, etc. Cross-subsystem coordination happens only through the relay. In multi-community mode, the relay also owns propagation of `TenantContext`; service crates should receive community-scoped inputs rather than independently deriving tenancy from client-controlled event tags.

---

## 2. The Protocol

LenOS uses Nostr NIP-01 on the wire. Every action is a JSON event with six fields:
...
| 40000–49999 | LenOS custom kinds |

### LenOS Custom Kinds (selected)
...
`lenos-core` defines each event kind as a `pub const u32` and exports the full registry as `ALL_KINDS: &[u32]` (127 kinds at the time of writing); `crates/lenos-core/src/kind.rs` is the source of truth for the current list. Kinds are `u32` (NIP-01 specifies unsigned integer; `u32` covers the full range). LenOS uses both standard Nostr kinds (e.g., kind 7 for reactions) and custom ranges (40000+).

Note: `KIND_AUTH` (22242) is `pub const KIND_AUTH: u32` in `lenos-core/src/kind.rs` and imported by `lenos-relay/src/handlers/event.rs`. `KIND_CANVAS` (40100) is likewise `pub const KIND_CANVAS: u32` in `lenos-core/src/kind.rs`.
...
Workflow loop prevention: workflow execution kinds (46001–46012), relay-signed messages with `lenos:workflow` tag, and `KIND_GIFT_WRAP` are excluded from triggering workflows. All other stored events (including kind 9 stream messages) trigger workflow evaluation.
...
### lenos-core — Shared Types and Verification
...
### lenos-pubsub — Redis Pub/Sub, Presence, Typing

Manages Redis pub/sub fan-out, presence tracking, and typing indicators. In multi-community mode all tenant-visible keys are prefixed or otherwise partitioned by community (`lenos:{community}:...`) so channel fan-out, presence, typing, and cache invalidation cannot cross hosts.

**Architecture:**

```
Publisher  → pool connection   → PUBLISH lenos:channel:{uuid}
Subscriber → dedicated PubSub  → PSUBSCRIBE lenos:channel:*
...
**Presence:** `SET lenos:presence:{pubkey_hex} {status} EX 180` — 180-second TTL (3× the 60-second heartbeat interval). Single missed heartbeat does not cause presence flap.
...
### lenos-search — Postgres FTS Integration
...
- Permission filtering is **caller's responsibility** — `lenos-search` returns
...
Real-time voice lives inside `lenos-relay` (`src/audio/`), not a separate crate. A WebSocket endpoint (`wss://.../huddle/{channel_id}/audio`) authenticates each participant with a NIP-42 challenge, checks channel membership, admits them to an in-memory room, and forwards opaque Opus frames between peers. No external SFU.
...
### lenos-acp — Agent Communication Protocol Harness

Standalone binary that bridges LenOS relay events to AI agents via the [Agent Communication Protocol](https://agentclientprotocol.com/) (ACP).

**Architecture:**

```
LenOS Relay ──WS──→ lenos-acp ──stdio (ACP/JSON-RPC)──→ Agent (goose/codex/claude)
```

`lenos-acp` spawns AI agent subprocesses (1–32, default 1), connects to the relay via WebSocket with NIP-42 auth, discovers channels via REST API, and queues `@mention` events per channel. At most one prompt is in-flight per channel. Queued events are batched into a single prompt sent via `session/prompt` over ACP.
...
- Depends on `lenos-core` (kind constants) and `lenos-sdk` (relay/REST utilities).

**Does NOT:** persist state.

---

### lenos-admin — Operator CLI
...
The `lenos-admin` binary is shipped in the relay Docker image (`/usr/local/bin/lenos-admin`) and is the recommended way to manage relay membership in production. Use `./run.sh add-member`, `./run.sh remove-member`, and `./run.sh list-members` in Docker Compose deployments.

---

### lenos-test-client — Integration Test Harness

**`LenOSTestClient`** wraps a WebSocket connection with a `VecDeque<RelayMessage>` buffer for message interleaving. Methods: `connect`, `connect_unauthenticated`, `authenticate`, `send_event`, `send_text_message`, `subscribe`, `close_subscription`, `recv_event`, `collect_until_eose`, `disconnect`.
...
`src/main.rs` is a manual testing CLI (`lenos-test-cli`) with `--send`, `--subscribe`, `--channel`, `--url`, `--kind` flags.
...
| Concern | Mechanism |
|---------|-----------|
| Schnorr signatures | `verify_event()` in `lenos-core` — every event verified before storage |
...
`is_private_ip()` in `lenos-core` covers:
...
Applied in: `lenos-workflow` (CallWebhook action), `lenos-core` (shared utility).
