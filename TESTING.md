...
`lenos-test-client` — those are marked `#[ignore]` and require a running relay:

```bash
# Start a relay first (see below), then:
cargo test -p lenos-test-client -- --ignored
```
...
binaries once, run `lenos-relay`, and drive it with the `lenos` CLI. The
...
> **Already running LenOS Desktop?** Desktop uses the same Docker container
> names (`lenos-postgres`, `lenos-redis`) and the same
...
> (`COMPOSE_PROJECT_NAME=lenos-dev docker compose …`).

`just reset` wipes all local data and starts over — **including LenOS
...
> `LENOS_AUTH_TAG`, `LENOS_RELAY_URL`, or `LENOS_PRIVATE_KEY` from a
> prior session (or a staging config), `unset` them before continuing.
> A stale `LENOS_AUTH_TAG` fails the **local dev relay** with
> `auth_error: signature verification failed` on the first CLI write —
> it is *not* tolerated.
> ```bash
> unset LENOS_AUTH_TAG LENOS_RELAY_URL LENOS_PRIVATE_KEY
> ```

### 2. Build the binaries

```bash
cargo build --release -p lenos-relay -p lenos-cli -p lenos-admin
...
```bash
lenos-relay                     # release binary from step 2, serves ws://localhost:3000
# alternatives:
# cargo run --release -p lenos-relay     # rebuild + run in release
...
> `LENOS_HEALTH_PORT`) so K8s probes bypass auth middleware. The main app
> port also exposes `/health` for convenience.

The relay starts in dev mode (`LENOS_REQUIRE_AUTH_TOKEN=false`). The startup
...
> **Already running LenOS Desktop (or another relay) on `:3000` / `:8080` /
> `:9102`?** LenOS binds three ports — main, health, metrics — and any of
> them can collide. Use a separate terminal per role and export the right
> vars in each:
>
> **In the relay terminal** (before launching `lenos-relay`):
> ```bash
> export LENOS_BIND_ADDR=0.0.0.0:3030
> export LENOS_HEALTH_PORT=8088
> export LENOS_METRICS_PORT=9202
...
> ```bash
> export LENOS_RELAY_URL=http://localhost:3030    # CLI target
...
> overrides — or the CLI will end up talking to LenOS Desktop's relay.

> **Ignore `just setup`'s "Next steps" banner.** It still prints
> `just relay` (a debug build). Use `lenos-relay` from step 2 here —
> step 2 already built the release binary.

When you're done, stop the relay (Ctrl-C in its terminal). If it's
backgrounded or you lost the terminal: `pkill -f lenos-relay`. Leaving
...
```bash
# Generate a keypair
GEN=$(lenos-admin generate-key)
export LENOS_PRIVATE_KEY=$(echo "$GEN" | awk '/Secret key:/ {print $3}')
PUBKEY=$(echo "$GEN"           | awk '/Public key:/ {print $3}')
echo "pubkey: $PUBKEY"

# Create a channel — the UUID is returned in the response
CHANNEL=$(lenos channels create --name "smoke-$$" --type stream --visibility open | jq -r '.channel_id')
echo "channel: $CHANNEL"

# Send a message and read it back
SEND=$(lenos messages send --channel "$CHANNEL" --content "hello from smoke test")
EVENT_ID=$(echo "$SEND" | jq -r '.event_id')
lenos messages get --channel "$CHANNEL" --limit 5 | jq .

# Fetch the reply chain for a specific message (empty array on a leaf — that's fine)
lenos messages thread --channel "$CHANNEL" --event "$EVENT_ID" | jq .
```
...
follow [`crates/lenos-cli/TESTING.md`](crates/lenos-cli/TESTING.md).

The relay's HTTP bridge accepts three endpoints — useful if you're testing
a client other than `lenos-cli`:
...
`POST /query` with an `#e` filter, or `lenos messages thread`.

---

## ACP Harness (optional, end-to-end with a real agent)

`lenos-acp` connects an ACP-speaking agent (goose, codex, claude code,
lenos-agent) to the relay. The harness listens for events, drives the
...
from the sender identity (`LENOS_ACP_RESPOND_TO=anyone` still skips events
the agent signed itself).

```bash
cargo build --release -p lenos-acp
export PATH="$PWD/target/release:$PATH"

# 1. Save your sender identity from step 4 — you'll need it to @mention the agent
SENDER_SK="$LENOS_PRIVATE_KEY"

# 2. Mint a fresh agent identity and capture its pubkey
AGENT_GEN=$(lenos-admin generate-key)
...
lenos channels add-member --channel "$CHANNEL" --pubkey "$AGENT_PUBKEY" --role member

# 4. Switch to the agent identity and start it.
#    lenos-acp wants ws:// (not http://). If you set LENOS_RELAY_URL to an
#    http:// URL in step 3, set the ws:// equivalent here — same host/port.
export LENOS_PRIVATE_KEY="$AGENT_SK"
export LENOS_RELAY_URL=ws://localhost:3000   # match step 3 (e.g. ws://localhost:3030 if overridden)
export LENOS_ACP_RESPOND_TO=anyone           # default is owner-only; opens the gate for testing
# NIP-AE core-memory prompt injection is on by default; set LENOS_ACP_NO_MEMORY=true to opt out.
export GOOSE_MODE=auto                        # must be 'auto' or goose hangs on prompts

lenos-acp                                    # foreground; logs to stdout (run in a separate terminal)

# Optional: turn on per-turn tracing if the default log is too quiet.
# RUST_LOG=lenos_acp=debug lenos-acp
```
...
> code / lenos-agent, set `LENOS_ACP_AGENT_COMMAND` and `LENOS_ACP_AGENT_ARGS`
> accordingly — see `crates/lenos-acp/README.md`. Without these, lenos-acp
...
same env. See `crates/lenos-acp/README.md` for parallel agents, heartbeats,
respond-to gates, and forum subscriptions.

To exercise deferred ACP startup, add `LENOS_ACP_LAZY_POOL=true` before launching
`lenos-acp`. The harness should connect, authenticate, subscribe, and publish
...
```bash
export LENOS_PRIVATE_KEY=$SENDER_SK          # the key from step 4
lenos messages send --channel "$CHANNEL" \
...
# `lenos messages get` is how you confirm it ran.
lenos messages get --channel "$CHANNEL" --limit 5 | jq '.[] | {pubkey, content}'
```

Replies are kind:9 in the same channel; `lenos messages thread --channel <id>
...
|-----------------------------------|-----------------------------|-------|
| `LENOS_BIND_ADDR`                | `0.0.0.0:3000`              | Main app port |
| `LENOS_HEALTH_PORT`              | `8080`                      | `/_liveness`, `/_readiness` |
| `LENOS_METRICS_PORT`             | `9102`                      | Prometheus `/metrics` |
| `RELAY_URL`                       | `ws://localhost:3000`       | Advertised in NIP-11 / NIP-42 challenges. **Note: no `LENOS_` prefix.** |
| `DATABASE_URL`                    | `postgres://lenos:lenos_dev@localhost:5432/lenos` | |
| `REDIS_URL`                       | `redis://localhost:6379`    | |
| `LENOS_REQUIRE_AUTH_TOKEN`       | `false`                     | When true, REST requires NIP-98 (no `X-Pubkey` fallback) |
| `LENOS_REQUIRE_RELAY_MEMBERSHIP` | `false`                     | When true, only pubkeys in `relay_members` can connect |
| `LENOS_REQUIRE_MEDIA_GET_AUTH`   | `false`                     | When true, `GET`/`HEAD /media/*` require Blossom kind 24242 `t=get` auth plus relay membership. |
| `LENOS_AUDIT_ENABLED`            | `true`                      | Tamper-evident event/media audit log. Set `false`/`0`/`off` to skip its DB pool and writes. Does not disable the separate moderation audit trail. |
| `LENOS_AUTO_MIGRATE`             | `false`                     | Opt in with `true`/`1`/`yes`/`on` to run embedded SQLx migrations on relay startup |
| `RELAY_OWNER_PUBKEY`              | unset                       | Bootstrapped as `owner` in `relay_members` at first start |
| `LENOS_ALLOW_NIP_OA_AUTH`        | `false`                     | Enable NIP-OA owner attestation for membership |
| `LENOS_WEB_DIR`                  | unset (source), `/srv/lenos/web` (container) | Directory containing the invite landing bundle; the production container enables it so `/invite/{code}` always works |
| `LENOS_SERVE_GIT_WEB_GUI`        | `false`                     | Set to `true` or `1` to expose the bundled Git repository browser at `/` and `/repos/...`; invite routes do not depend on this flag |

CLI-side, only two matter for testing:

| Variable                | Default                  | Notes |
|-------------------------|--------------------------|-------|
| `LENOS_RELAY_URL`      | `http://localhost:3000`  | CLI relay base; accepts `ws(s)://` and normalises |
| `LENOS_PRIVATE_KEY`    | — (**required**)         | `nsec1…` or 64-char hex |
| `LENOS_AUTH_TAG`       | unset                    | Optional NIP-OA owner attestation JSON |
...
| `Address already in use` on relay start (os error 48 on macOS, 98 on Linux) | Another relay (or stale process) holding `:3000` / `:8080` / `:9102` (or your override ports) | The panic line names the failing port — read it first. Then `lsof -iTCP:3000,8080,9102 -sTCP:LISTEN` (or your override equivalents). Kill the offender (`pkill -f lenos-relay`) or use the port-override block in step 3. If you already overrode and *still* collide, a prior reviewer left a relay running on the same alt ports — kill it or pick fresh ports |
| `auth_error: LENOS_PRIVATE_KEY is required` | Env not exported into the CLI's shell | `export LENOS_PRIVATE_KEY=...` (or pass `--private-key`) |
| `auth_error: LENOS_AUTH_TAG verification failed … signature verification failed` | A stale `LENOS_AUTH_TAG` inherited from a parent shell. The local dev relay rejects it. | `unset LENOS_AUTH_TAG` (see the scrub block in step 1) |
| `auth-required: verification failed` on a closed relay | NIP-OA attestation needed | Set `LENOS_AUTH_TAG` to the owner-issued JSON, or relax `LENOS_REQUIRE_RELAY_MEMBERSHIP` |
...
| ACP agent ignores all events | `LENOS_ACP_RESPOND_TO=owner-only` (default) with no owner configured | Set `LENOS_ACP_RESPOND_TO=anyone` for testing |
| ACP logs `discovered 0 channel(s)` / `no channel subscriptions resolved` | Agent identity isn't a member of any channel | `lenos channels add-member --channel "$CHANNEL" --pubkey "$AGENT_PUBKEY" --role member` from another identity |
