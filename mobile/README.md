...
Flutter mobile client for LenOS.
...
to the **worktree directory name** (`com.lenos.lenosMobile.<slug>` on iOS,
`xyz.block.lenos.mobile.<slug>` on Android) plus a display-only branch label
in the app name (`LenOS (my-branch)`, or a short SHA when the worktree is
...
- `LENOS_ANDROID_UPLOAD_KEYSTORE_PATH`: path to a CI-vended keystore file
- `LENOS_ANDROID_UPLOAD_KEYSTORE_PASSWORD`
- `LENOS_ANDROID_UPLOAD_KEY_ALIAS`
- `LENOS_ANDROID_UPLOAD_KEY_PASSWORD`
...
a local upload keystore must set `LENOS_ANDROID_RELEASE_SIGNING=external`. That
mode produces an unsigned release bundle and refuses to run if any
`LENOS_ANDROID_UPLOAD_*` value is also set.
