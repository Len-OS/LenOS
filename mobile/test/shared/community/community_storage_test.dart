...
import 'package:lenos/shared/community/community.dart';
import 'package:lenos/shared/community/community_storage.dart';
  ...
  group('CommunityStorage', () {
    ...
    group('migration', () {
      test('migrates saved workspaces to communities', () async {
        ...
        fakeSecure['lenos_workspaces'] = jsonEncode([legacy.toJson()]);
        fakeSecure['lenos_active_workspace_id'] = legacy.id;

        final loaded = await storage.loadAll();

        expect(loaded.single.id, legacy.id);
        expect(await storage.loadActiveId(), legacy.id);
        expect(fakeSecure['lenos_communities'], isNotNull);
        expect(fakeSecure['lenos_workspaces'], isNull);
        expect(fakeSecure['lenos_active_workspace_id'], isNull);
      });

      test('migrates legacy keys to community on first load', () async {
        fakeSecure['lenos_relay_url'] = 'https://legacy.example.com';
        fakeSecure['lenos_token'] = 'legacy_token';
        fakeSecure['lenos_pubkey'] = 'legacy_pub';
        fakeSecure['lenos_nsec'] = 'legacy_nsec';
        ...
        expect(fakeSecure['lenos_relay_url'], isNull);
        expect(fakeSecure['lenos_token'], isNull);
        expect(fakeSecure['lenos_pubkey'], isNull);
        expect(fakeSecure['lenos_nsec'], isNull);
      ...
      test('migration generates name from localhost URL', () async {
        fakeSecure['lenos_relay_url'] = 'http://localhost:3000';
        fakeSecure['lenos_token'] = 'tok';
