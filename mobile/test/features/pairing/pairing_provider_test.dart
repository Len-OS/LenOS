...
import 'package:lenos/features/pairing/pairing_provider.dart';
import 'package:lenos/features/pairing/pairing_socket.dart';
import 'package:lenos/shared/auth/auth.dart';

/// Tests for [PairingNotifier]'s legacy `lenos://` payload parsing and
...
///   - Initial state.
///   - Parsing every documented payload format (raw base64, `lenos://`
///     prefix, whitespace).
...
void main() {
  group('PairingNotifier', () {
    ...
    test(
      ...
      () async {
        ...
        const code =
            ...
            '&relay=wss%3A%2F%2Fpairing.lenos.xyz&v=1';
    ...
    test('accepts lenos scheme prefix', () async {
      container = createContainer();

      final code = 'lenos://${_encodePairingCode()}';
      await container.read(pairingProvider.notifier).pair(code);
