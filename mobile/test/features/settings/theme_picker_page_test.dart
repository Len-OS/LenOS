...
import 'package:lenos/features/settings/accent_picker_page.dart';
import 'package:lenos/features/settings/theme_picker_page.dart';
...
void main() {
  group('ThemePickerPage', () {
    ...
    testWidgets('system mode normalizes a stored unpaired theme', (
      ...
      final instance = await _prefs({'lenos_color_scheme': 'snazzy-light'});
      await tester.pumpWidget(
        WidgetHelpers.testable(
      ...
      expect(
        instance.getString('lenos_color_scheme'),
    ...
    testWidgets('checks the row matching the stored selection', (tester) async {
      await _pumpPicker(
        tester,
        const ThemePickerPage(),
        prefs: {'lenos_theme_mode': 'light', 'lenos_color_scheme': 'nord'},
      );
    ...
    testWidgets('either half of a pair checks the same system row', (
      ...
      await _pumpPicker(
        tester,
        const ThemePickerPage(),
        prefs: {'lenos_color_scheme': 'github-dark'},
      );
    ...
    testWidgets('tapping a theme persists the selection', (tester) async {
      final instance = await _prefs({'lenos_theme_mode': 'dark'});
      await tester.pumpWidget(
        WidgetHelpers.testable(
          child: const ThemePickerPage(),
          overrides: [savedPrefsProvider.overrideWithValue(instance)],
      ...
      expect(instance.getString('lenos_color_scheme'), 'nord');
    });
  });
  ...
  group('AccentPickerPage', () {
    ...
    testWidgets('tapping an accent persists the index', (tester) async {
      ...
      expect(
        instance.getInt('lenos_accent_color'),
