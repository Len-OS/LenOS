...
final _lenosToolNames = <String>{..._lenosReadTools, ..._lenosWriteTools};

final _lenosToolNamesByLength = _lenosToolNames.toList()
  ..sort((a, b) => b.length.compareTo(a.length));

final _lenosToolTitleAliases = <(RegExp, String)>[
...
String? _findLenOSToolName(String value, bool includeShortNames) {
  final alias = _findLenOSToolAlias(value);
  if (alias != null) return alias;

  final normalized = _normalizeToolNameText(value);
  for (final name in _lenosToolNamesByLength) {
...
String? _findLenOSToolAlias(String value) {
  final normalizedPhrase = value
      .trim()
      .toLowerCase()
      .replaceAll(RegExp(r'[_-]+'), ' ')
      .replaceAll(RegExp(r'\s+'), ' ');
  for (final (pattern, name) in _lenosToolTitleAliases) {
...
_parsePromptText(String text) {
  ...
  for (final section in sections) {
    if (section.title.toLowerCase().startsWith('lenos event')) {
...
({String title, String toolName, String? lenosToolName}) _extractToolIdentity(
  Map<String, dynamic> update,
) {
  final candidates = _collectToolNameCandidates(update);
  String? knownName;
  for (final c in candidates) {
    knownName = _findLenOSToolName(c, true);
    if (knownName != null) break;
  }
  knownName ??= _findLenOSToolName(_safeJsonEncode(update), false);
...
List<TranscriptItem> buildTranscript(List<ObserverFrame> events) {
  ...
  void upsertTool(
    ...
    final existing = itemsById[id];
    final canonicalLenOSToolName =
        lenosToolName ?? _findLenOSToolName(toolName, true);
    if (existing is ToolItem) {
      if (!_isGenericToolTitle(title)) {
        existing.title = title;
      }
      if (canonicalLenOSToolName != null) {
        existing.lenosToolName = canonicalLenOSToolName;
        existing.toolName = canonicalLenOSToolName;
      } else if (existing.lenosToolName == null &&
    ...
    final item = ToolItem(
      id: id,
      title: title,
      toolName: canonicalLenOSToolName ?? toolName,
      lenosToolName: canonicalLenOSToolName,
      status: status,
      args: args,
      result: result,
      isError: isError,
