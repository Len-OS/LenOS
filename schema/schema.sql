-- LenOS initial Postgres schema — multi-tenant.
--
...
-- `resolve_host()` (lenos-core) applies the identical normalization before
...
-- (crates/lenos-db/src/push.rs) to close the lost-wake race.
...
BEGIN
    ...
    IF NEW.kind IN (7, 9, 1059, 40007, 46010) THEN
        PERFORM pg_advisory_xact_lock_shared(
            hashtextextended('lenos_push_gate:' || NEW.community_id::text, 0));
        IF EXISTS (
...
-- crates/lenos-db/src/channel.rs), which forces the same total order the
    ...
    IF NEW.channel_id IS NOT NULL AND NEW.kind <> 9007 THEN
        BEGIN
            PERFORM pg_advisory_xact_lock_shared(hashtextextended(
                'lenos_channel_ttl:' || NEW.community_id::text || ':' || NEW.channel_id::text, 0));
...
-- event rows are no older than `lenos.created_at_floor` seconds before commit
...
LANGUAGE plpgsql AS $$
DECLARE
    floor_secs numeric := nullif(current_setting('lenos.created_at_floor', true), '')::numeric;
BEGIN
...
CREATE TABLE push_gateway_installations (
    ...
    app_profile TEXT NOT NULL CHECK (app_profile IN ('lenos-ios-production','lenos-ios-sandbox')),
...
-- crates/lenos-db/src/replica_fence.rs and migrations/0026). Exactly one row;
