...
from harbor_lenos_orchestra import (
    AgentCredential,
    LenOSOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = LenOSOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = LenOSOrchestraAgent(
        ...
        lenos_acp_binary="/pinned/lenos-acp",
        lenos_agent_binary="/pinned/lenos-agent",
        lenos_dev_mcp_binary="/pinned/lenos-dev-mcp",
        lenos_cli_binary="/pinned/lenos",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.lenos_acp_binary == "/pinned/lenos-acp"
    assert agent.runtime.lenos_agent_binary == "/pinned/lenos-agent"
    assert agent.runtime.lenos_dev_mcp_binary == "/pinned/lenos-dev-mcp"
    assert agent.runtime.lenos_cli_binary == "/pinned/lenos"
