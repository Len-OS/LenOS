"""Channel-per-trial LenOS provisioning against a local benchmark stack."""
...
from .lenos_cli import LenOSCli
...
def provisioner_from_dict(config: dict[str, object]) -> LenOSTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return LenOSTrialProvisioner(TestbedConfig(**config))


class LenOSTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live LenOS relay.

    Guarantees (contract PLANS/HARBOR_LENOS_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production LenOS, where a human owns the channel their
        # agents work in.
    ...
    def _cli_for(self, credential: AgentCredential) -> LenOSCli:
        return LenOSCli(
