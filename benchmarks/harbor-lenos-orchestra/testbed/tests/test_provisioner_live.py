...
Gated by LENOS_TESTBED_LIVE=1 with stack coordinates in the environment:
  LENOS_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  LENOS_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  LENOS_TESTBED_OWNER_KEY    relay owner secret key (hex)
  LENOS_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
from harbor_lenos_testbed.lenos_cli import LenOSCli, LenOSCliError
from harbor_lenos_testbed.provisioner import (
...
pytestmark = pytest.mark.skipif(
    os.environ.get("LENOS_TESTBED_LIVE") != "1",
    reason="live testbed suite; set LENOS_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> LenOSTrialProvisioner:
    owner_key = os.environ.get("LENOS_TESTBED_OWNER_KEY")
    dsn = os.environ.get("LENOS_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("LENOS_TESTBED_OWNER_KEY and LENOS_TESTBED_PG_DSN are required")
    return LenOSTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "LENOS_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "LENOS_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: LenOSTrialProvisioner, credential) -> LenOSCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(LenOSCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["LENOS_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
