#!/usr/bin/env python3
"""One-command benchmark: bring up the LenOS stack in Docker and run it.
...
- A dedicated ``lenos-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the LenOS
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-lenos-orchestra/testbed \
        benchmarks/harbor-lenos-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "lenos-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "xyz.block.lenos.app.benchmark"
...
AGENT_BINARIES = ("lenos-acp", "lenos-agent", "lenos-dev-mcp")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open the LenOS desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
...
def load_state() -> dict[str, str]:
    ...
    from harbor_lenos_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_lenos_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "LENOS_IMAGE": os.environ.get("LENOS_IMAGE", "ghcr.io/BuildGrowthNow/LenOS:main"),
        "LENOS_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "LENOS_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "LENOS_MEDIA_SERVER_DOMAIN": "localhost",
        "LENOS_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "LENOS_REQUIRE_AUTH_TOKEN": "true",
        "LENOS_REQUIRE_RELAY_MEMBERSHIP": "true",
        "LENOS_ALLOW_NIP_OA_AUTH": "true",
        "LENOS_AUTO_MIGRATE": "true",
        "LENOS_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "lenos_relay=info,lenos_db=info,lenos_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "LENOS_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "LENOS_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "lenos",
        "POSTGRES_USER": "lenos",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "LENOS_S3_ACCESS_KEY": state["s3_access_key"],
        "LENOS_S3_SECRET_KEY": state["s3_secret_key"],
        "LENOS_S3_BUCKET": "lenos-media",
        "LENOS_HTTP_PORT": str(RELAY_HTTP_PORT),
        "LENOS_PG_HOST_PORT": str(PG_HOST_PORT),
        "LENOS_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
        f"postgresql://lenos:{state['postgres_password']}@127.0.0.1:{PG_HOST_PORT}/lenos"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host lenos CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host lenos CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "lenos-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    lenos-acp → lenos-agent → lenos-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the LenOS desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via lenos-admin inside the container —
    ...
    for name in (
        ...
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"lenos-{triple}"
    real_cli.write_bytes(binaries["lenos"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening LenOS GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides LENOS_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "LenOS Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "LENOS_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "LENOS_PRIVATE_KEY": state["user_secret_key"],
        },
    )
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # LENOS_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        (
            f"{os.environ.get('LENOS_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
            f":{RELAY_HTTP_PORT}"
        ),
        "--n-concurrent",
