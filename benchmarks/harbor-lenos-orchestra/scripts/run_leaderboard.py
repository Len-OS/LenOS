...
importable:
    ...
    uv run --project benchmarks/harbor-lenos-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux lenos-acp/lenos-agent/lenos-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_lenos_orchestra:LenOSOrchestraAgent"
PROVISIONER_FACTORY = "harbor_lenos_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the LenOS relay/Postgres provisioner",
    )
    parser.add_argument(
        "--lenos-bin-dir",
        type=Path,
        default=None,
        help="Directory with the host lenos CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir",
        type=Path,
        required=True,
        help="Directory with Linux builds of lenos-acp/lenos-agent/lenos-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"lenos binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --lenos-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "lenos_acp_binary": agent_binaries["lenos-acp"],
        "lenos_agent_binary": agent_binaries["lenos-agent"],
        "lenos_dev_mcp_binary": agent_binaries["lenos-dev-mcp"],
        "lenos_cli_binary": binaries["lenos"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/BuildGrowthNow/LenOS",
        "agent_display_name": f"LenOS Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.lenos_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.lenos_bin_dir)
