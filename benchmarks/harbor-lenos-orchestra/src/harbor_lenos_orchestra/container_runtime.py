"""Run the production LenOS agent stack inside the Harbor task container.

Each provisioned identity is a full ``lenos-acp`` → ``lenos-agent`` →
``lenos-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``lenos`` CLI on
PATH) that the desktop app gives a LenOS agent. The harness stays outside:
...
# Container-side layout for the uploaded LenOS stack.
REMOTE_ROOT = "/opt/lenos"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a LenOS agent process cannot be launched or exits early."""
...
class LenOSContainerRuntime:
    """Launch one production LenOS agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        lenos_acp_binary: str = "lenos-acp",
        lenos_agent_binary: str = "lenos-agent",
        lenos_dev_mcp_binary: str = "lenos-dev-mcp",
        lenos_cli_binary: str = "lenos",
        relay_gateway: str = "",
        ...
        self.lenos_acp_binary = lenos_acp_binary
        self.lenos_agent_binary = lenos_agent_binary
        self.lenos_dev_mcp_binary = lenos_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.lenos_cli_binary = lenos_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        if not workers:
            raise RuntimeLaunchError("LenOS orchestration requires at least one worker")
        trial_dir = self.logs_dir / "lenos"
        ...
        try:
            ...
            # The task arrives exactly as it would in production LenOS: a
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/lenos-acp": self.lenos_acp_binary,
            f"{REMOTE_BIN}/lenos-agent": self.lenos_agent_binary,
            f"{REMOTE_BIN}/lenos-dev-mcp": self.lenos_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/lenos-acp')} </dev/null "
    ...
    def _agent_env(
        ...
        *,
        ...
        return {
            **endpoint.env,
            "LENOS_RELAY_URL": trial.relay_ws_url,
            "LENOS_PRIVATE_KEY": credential.nostr_secret_key,
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on lenos-acp
            # so lenos-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "LENOS_AUTH_TAG": credential.nostr_auth_tag,
            "LENOS_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/lenos-agent",
            "LENOS_ACP_AGENT_ARGS": "",
            "LENOS_ACP_MCP_COMMAND": f"{REMOTE_BIN}/lenos-dev-mcp",
            "LENOS_ACP_CHANNELS": trial.channel_id,
            "LENOS_ACP_SUBSCRIBE": "mentions",
            "LENOS_ACP_RESPOND_TO": "anyone",
            "LENOS_ACP_NO_MEMORY": "true",
            "LENOS_ACP_SYSTEM_PROMPT_FILE": remote_prompt,
            "LENOS_AGENT_PROVIDER": endpoint.provider,
            "LENOS_AGENT_MODEL": credential.llm_endpoint,
            "LENOS_AGENT_MAX_OUTPUT_TOKENS": str(
                agent_class.generation.max_output_tokens
            ),
            "LENOS_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "LENOS_AGENT_MAX_ROUNDS": str(
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._lenos_json(
                trial.user,
                trial,
                "messages",
                "get",
    ...
    # -- LenOS CLI as the trial user / provisioning identities -------------------
    ...
    async def _lenos_json(
        self, credential: AgentCredential, trial: TrialHandle, *args: str
    ) -> Any:
        process = await asyncio.create_subprocess_exec(
            self.lenos_cli_binary,
            *args,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env={
                **os.environ,
                "LENOS_RELAY_URL": self._user_relay_url(trial),
                "LENOS_PRIVATE_KEY": credential.nostr_secret_key,
                "LENOS_AUTH_TAG": credential.nostr_auth_tag,
            },
        )
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"lenos {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("lenos returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return LenOSContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production LenOS workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in LenOS channel `{trial.channel_id}`.",
            (
    ...
    def _reject_identity_overrides(endpoint: EndpointLaunchConfig) -> None:
        forbidden = {
            ...
            "LENOS_AUTH_TAG",
            "LENOS_ACP_CHANNELS",
            "LENOS_ACP_MCP_COMMAND",
            "LENOS_ACP_AGENT_COMMAND",
        }
