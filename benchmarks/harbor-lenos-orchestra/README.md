# Harbor LenOS Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
LenOS stack. Harbor sees one `LenOSOrchestraAgent`; behind that adapter, one
...
`lenos-acp` → `lenos-agent` → `lenos-dev-mcp` process tree the desktop app
launches: the production MCP toolset (shell, file tools, todo) with the
`lenos` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/harbor-lenos-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_lenos_orchestra:LenOSOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_lenos_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-lenos-orchestra --agent-kwarg lenos_acp_binary=<LINUX_BIN>/lenos-acp --agent-kwarg lenos_agent_binary=<LINUX_BIN>/lenos-agent --agent-kwarg lenos_dev_mcp_binary=<LINUX_BIN>/lenos-dev-mcp --agent-kwarg lenos_cli_binary=target/debug/lenos --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`lenos_acp_binary`/`lenos_agent_binary`/`lenos_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `lenos_cli_binary` is the **host** CLI the harness
...
Each trial gets fresh keys and a private LenOS channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `lenos/`
artifacts) available for analysis.

## Leaderboard runs

`just benchmark` is the one-command path: it stands up a dedicated Docker
stack (`lenos-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
relay membership list and opens the LenOS desktop app logged in as them, so
...
uv run --project benchmarks/harbor-lenos-orchestra/testbed \
    benchmarks/harbor-lenos-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/harbor-lenos-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/harbor-lenos-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/harbor-lenos-orchestra
