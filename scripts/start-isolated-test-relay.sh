...
# default `lenos-*` dev stack. Backing services run under the dedicated
# `lenos-harness` Compose project (docker-compose.harness.yml); the relay runs
# in the foreground on override ports.
#
#   Topology (reuse this exact tuple for desktop parity runs):
#     compose project : lenos-harness
#     postgres        : localhost:5471  (db=lenos, user=lenos, pass=lenos_dev)
#     redis           : localhost:6471
#     minio           : localhost:9471 (console 9472)
#     relay main      : localhost:3030   ← LENOS_E2E_RELAY_URL=http://localhost:3030
...
#   docker compose -p lenos-harness -f docker-compose.harness.yml down -v
# =============================================================================
set -euo pipefail
...
# ── Backing services (scoped to lenos-harness only) ───────────────────────────
...
wait_pg() {
  for _ in $(seq 1 60); do
    if docker compose -p "${PROJECT}" -f "${COMPOSE_FILE}" exec -T postgres \
         pg_isready -U lenos >/dev/null 2>&1; then
...
# ── Schema + partitions ──────────────────────────────────────────────────────
export PGPASSWORD=lenos_dev
psql_h() { docker compose -p "${PROJECT}" -f "${COMPOSE_FILE}" exec -T postgres \
  psql -U lenos -d lenos -v ON_ERROR_STOP=1 "$@"; }

log "Resetting isolated database and applying schema..."
# This database belongs only to the lenos-harness Compose project. Reset it on
...
export PGSCHEMA_PLAN_DB=lenos PGSCHEMA_PLAN_USER=lenos PGSCHEMA_PLAN_PASSWORD=lenos_dev
export PGHOST=localhost PGPORT=${PG_PORT} PGUSER=lenos PGDATABASE=lenos
...
# LENOS_DB_HOST/PORT rather than the shared `lenos-postgres` container.)
log "Seeding community (host=${COMMUNITY_HOST}), channels, and members..."
LENOS_COMMUNITY_HOST="${COMMUNITY_HOST}" \
  LENOS_DB_HOST=localhost LENOS_DB_PORT=${PG_PORT} LENOS_DB_USER=lenos \
  LENOS_DB_PASS=lenos_dev LENOS_DB_NAME=lenos \
  LENOS_DB_DOCKER_CONTAINER="${PROJECT}-postgres-1" \
...
cargo build --profile "${CARGO_BUILD_PROFILE}" -p lenos-relay
ok "Relay built"
...
tmux new-session -d -s "${TMUX_SESSION}" "cd '${REPO_ROOT}' && env \
  DATABASE_URL=postgres://lenos:lenos_dev@localhost:${PG_PORT}/lenos \
  REDIS_URL=redis://localhost:${REDIS_PORT} \
  RELAY_URL=ws://localhost:${RELAY_MAIN} \
  LENOS_BIND_ADDR=0.0.0.0:${RELAY_MAIN} \
  LENOS_HEALTH_PORT=${RELAY_HEALTH} \
  LENOS_METRICS_PORT=${RELAY_METRICS} \
  LENOS_S3_ENDPOINT=http://localhost:${MINIO_PORT} \
  LENOS_S3_ACCESS_KEY=lenos_dev \
  LENOS_S3_SECRET_KEY=lenos_dev_secret \
  LENOS_S3_BUCKET=lenos-media \
  LENOS_REQUIRE_AUTH_TOKEN=false \
  LENOS_RECONCILE_CHANNELS=true \
  './target/${CARGO_TARGET_PROFILE}/lenos-relay' > '${RELAY_LOG}' 2>&1"

# Wait for the main port to accept connections.
for _ in $(seq 1 30); do
  if curl -s -o /dev/null "http://localhost:${RELAY_MAIN}/"; then
    ok "Relay live — LENOS_E2E_RELAY_URL=http://localhost:${RELAY_MAIN}"
