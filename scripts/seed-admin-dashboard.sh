...
export PGUSER="${PGUSER:-lenos}"
export PGPASSWORD="${PGPASSWORD:-lenos_dev}"
export PGDATABASE="${PGDATABASE:-lenos}"
...
elif docker exec lenos-postgres psql --version >/dev/null 2>&1; then
  run_psql() {
    docker exec -i -e PGPASSWORD="${PGPASSWORD}" lenos-postgres \
      psql -U "${PGUSER}" -d "${PGDATABASE}" "$@"
  }
else
  echo "error: neither psql nor lenos-postgres docker psql is available" >&2
  exit 1
fi
...
upload_fixture() {
  ...
  docker exec -i lenos-minio mc pipe --quiet --attr "Content-Type=${mime}" \
    "local/${LENOS_S3_BUCKET:-lenos-media}/${hash}.${extension}" < "${path}"
  printf '%s' "${sidecar}" | docker exec -i lenos-minio mc pipe --quiet \
    --attr "Content-Type=application/json" \
    "local/${LENOS_S3_BUCKET:-lenos-media}/_meta/${community_id}/${hash}.json"
}

fixture_dir="$(mktemp -d "${TMPDIR:-/tmp}/lenos-admin-feedback.XXXXXX")"
...
printf '%s\n' "lenos feedback diagnostics" "area: composer" \
  "event: resumed_from_sleep" "result: composer_unresponsive" > "${composer_diagnostics}"
printf '%s\n' "lenos feedback diagnostics" "area: workspace-switching" \
...
if ! docker exec lenos-minio mc alias set local http://localhost:9000 \
  "${LENOS_S3_ACCESS_KEY:-lenos_dev}" "${LENOS_S3_SECRET_KEY:-lenos_dev_secret}" >/dev/null; then
