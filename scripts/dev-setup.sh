...
load_env() {
  ...
  # Smooth the local rename path for developers with a pre-LenOS .env copied
  ...
  if [[ "${DATABASE_URL:-}" == "postgres://sprout:sprout_dev@localhost:5432/sprout" ]]; then
    warn "Migrating legacy default DATABASE_URL from sprout to lenos for this setup run"
    DATABASE_URL="postgres://lenos:lenos_dev@localhost:5432/lenos"
  fi
  if [[ "${PGUSER:-}" == "sprout" ]]; then PGUSER="lenos"; fi
  if [[ "${PGPASSWORD:-}" == "sprout_dev" ]]; then PGPASSWORD="lenos_dev"; fi
  if [[ "${PGDATABASE:-}" == "sprout" ]]; then PGDATABASE="lenos"; fi

  export DATABASE_URL="${DATABASE_URL:-postgres://lenos:lenos_dev@localhost:5432/lenos}"
...
cleanup_legacy_sprout_containers() {
  ...
  warn "Stopping/removing legacy sprout-* dev containers so lenos-* containers can bind the standard ports"
...
fail_if_local_redis_blocks_compose() {
  ...
  if docker ps --format '{{.Names}}' | grep -qx 'lenos-redis'; then
    return
  fi
  local redis_pids
...
"${REPO_ROOT}/bin/cargo" run -p lenos-admin -- migrate
...
echo ""
echo -e "${GREEN}=======================================================${NC}"
echo -e "${GREEN}  LenOS dev environment is ready!${NC}"
echo -e "${GREEN}=======================================================${NC}"
echo ""
