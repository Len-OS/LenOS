...
grep -q '^BUNDLE_IDENTIFIER = com\.lenos\.lenosMobile\.feature-work-1$' "$ios" \
...
grep -q '^APP_DISPLAY_NAME = LenOS (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = LenOS \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = com\.lenos\.lenosMobile$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = com.lenos.lenosMobile"
grep -q '^APP_DISPLAY_NAME = LenOS$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = LenOS"
...
grep -q 'resValue("string", "app_name", "LenOS")' "$gradle" \
  && pass "Gradle default app_name stays LenOS" \
...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:xyz.block.lenos.mobile\n'
  printf 'package:xyz.block.lenos.mobile.feature_work_1\n'
  printf 'package:xyz.block.lenos.mobile.w_2fast\n'
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*xyz\.block\.lenos\.mobile$'; then
