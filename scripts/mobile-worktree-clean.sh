#!/usr/bin/env bash
# Uninstalls stale worktree-suffixed LenOS debug builds from booted iOS
# simulators and connected Android devices/emulators. Production installs
# (com.lenos.lenosMobile / xyz.block.lenos.mobile, no suffix) are never touched:
...
ios_prefix="com.lenos.lenosMobile."
android_prefix="xyz.block.lenos.mobile."
...
if [[ "$removed" == "0" ]]; then
    echo "no worktree-suffixed LenOS installs found (production apps untouched)"
elif [[ "$dry_run" == "1" ]]; then
