...
# Production state (`com.lengrowth.lenos`, `~/.lenos`, and `lenos-desktop`) is
...
remove_bundle_state() {
  ...
  [[ -d "$base" ]] || return 0
  shopt -s nullglob
  for prefix in com.lengrowth.lenos.dev xyz.block.sprout.app.dev; do
...
case "$(uname -s)" in
  Darwin)
    ...
    if command -v security >/dev/null 2>&1; then
      while security delete-generic-password -s lenos-desktop-dev >/dev/null 2>&1; do :; done
...
remove_path "$HOME/.lenos-dev"
remove_path "$HOME/.sprout-dev"

# A fresh dev nest must not re-import the installed app's ~/.lenos contents on
# its next boot. The sentinel is the same one used by migrate_dev_nest().
mkdir -p "$HOME/.lenos-dev"
: > "$HOME/.lenos-dev/.dev-nest-migrated"

log "Development desktop state removed; production LenOS state was not touched"
