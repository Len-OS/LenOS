#!/usr/bin/env bash
# =============================================================================
# start-relay-for-tests.sh — Start the LenOS relay and its backing services
# =============================================================================
...
wait_healthy "Postgres" "lenos-postgres"
wait_healthy "Redis" "lenos-redis"
wait_healthy "MinIO" "lenos-minio"
...
export PGSCHEMA_PLAN_DB=lenos
export PGSCHEMA_PLAN_USER=lenos
export PGSCHEMA_PLAN_PASSWORD=lenos_dev

./bin/pgschema apply --file schema/schema.sql --auto-approve
docker exec -i -e PGPASSWORD="${PGPASSWORD}" lenos-postgres \
...
# psql is not on PATH in the hermit env; postgres runs as the lenos-postgres
...
if command -v psql >/dev/null 2>&1; then
  ...
  seed_psql() { docker exec -e PGPASSWORD="${PGPASSWORD}" lenos-postgres psql -U "${PGUSER}" -d "${PGDATABASE}" -qtA "$@"; }
fi
seed_psql -c "
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  for bin in lenos-relay git-credential-nostr; do
  ...
  cargo build --profile "${CARGO_PROFILE}" -p lenos-relay -p git-credential-nostr
  ok "Relay built"
fi
...
nohup env \
  ...
  RELAY_URL=ws://localhost:3000 \
  LENOS_BIND_ADDR=0.0.0.0:3000 \
  LENOS_REQUIRE_AUTH_TOKEN=false \
  LENOS_RECONCILE_CHANNELS=true \
  LENOS_GIT_PROBE_WRITERS=8 \
  "./target/${CARGO_PROFILE}/lenos-relay" > /tmp/lenos-relay.log 2>&1 &
echo $! > /tmp/lenos-relay.pid

# ── Poll readiness ───────────────────────────────────────────────────────────

log "Waiting for relay readiness..."
for attempt in $(seq 1 60); do
  if ! kill -0 "$(cat /tmp/lenos-relay.pid)" 2>/dev/null; then
    err "Relay process died"
    cat /tmp/lenos-relay.log
    exit 1
  fi
