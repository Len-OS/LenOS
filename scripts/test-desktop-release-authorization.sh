...
elif [[ "$#" -eq 4 && "$2" == --paginate && "$3" == --slurp && "$4" == "repos/BuildGrowthNow/LenOS/pulls/123/reviews?per_page=100&page=1" ]]; then
...
run_authorization() {
  (cd "$repo_root" && PATH="$tmp/bin:$PATH" GH_CALLS="$tmp/calls" GH_TOKEN=test \
    GITHUB_REPOSITORY=BuildGrowthNow/LenOS PR_NUMBER=123 PR_HEAD_SHA=head \
