...
case "${1:-}:${2:-}" in
  api:repos/BuildGrowthNow/LenOS/rulesets/14378754)
    case "$*" in
    ...
    ;;
  api:repos/BuildGrowthNow/LenOS/git/ref/heads/main) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:repos/BuildGrowthNow/LenOS/commits/*) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:--paginate)
    [[ "$3" == "repos/BuildGrowthNow/LenOS/git/matching-refs/tags/mobile-v1.2.3-rc." ]]
    printf '%s' "${GH_EXISTING_REFS:-}"
    ;;
  api:--method)
    endpoint="$4"
    case "$endpoint" in
      repos/BuildGrowthNow/LenOS/git/tags)
        record "$*"
        printf '%s\n' "$GH_TAG_OBJECT_SHA"
        ;;
      repos/BuildGrowthNow/LenOS/git/refs)
        record "$*"
        ;;
      *) exit 2 ;;
    esac
    ;;
  api:repos/BuildGrowthNow/LenOS/git/ref/tags/mobile-v1.2.3-rc.*)
    if [[ "$*" == *'.object.type'* ]]; then
    ...
    ;;
  api:repos/BuildGrowthNow/LenOS/git/tags/*)
    if [[ "$*" == *'.object.type'* ]]; then
...
export GITHUB_REPOSITORY=BuildGrowthNow/LenOS
...
grep -Fq -- '-f message=LenOS Mobile 1.2.3 release candidate 1' "$GH_CALLS"
...
if GITHUB_REPOSITORY=attacker/lenos "$publisher" 1.2.3 1 "$GH_TARGET_SHA" >/dev/null 2>&1; then
