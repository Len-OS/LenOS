#!/usr/bin/env bash
# Build Sprig — one deploy-anywhere multicall binary for the LenOS ACP
# harness, agent, and developer MCP. The archive exposes these command names:
#
#   sprig            implementation binary
#   lenos-acp       link to sprig (ACP harness)
#   lenos-agent     link to sprig (ACP-compliant agent)
#   lenos-dev-mcp   link to sprig (developer MCP server; also dispatches
#                    rg/tree/lenos/git-credential-nostr/git-sign-nostr)
#
# Usage:
...
BUNDLE_BIN="sprig"
COMMANDS=(lenos-acp lenos-agent lenos-dev-mcp)
...
Sprig is the all-in-one LenOS agent binary for deploy-anywhere environments.
...
- `lenos-acp` — ACP harness that bridges LenOS channel events to an
  ACP-compliant agent over stdio.
- `lenos-agent` — ACP-compliant agent (spawns MCP servers, calls LLMs).
- `lenos-dev-mcp` — Developer MCP server (shell, str_replace, todo) and
  multicall entrypoint for `rg`, `tree`, `lenos`, `git-credential-nostr`,
  `git-sign-nostr`.
...
```bash
# Agent provider
export LENOS_AGENT_PROVIDER=anthropic            # or openai
export ANTHROPIC_API_KEY=sk-...
export ANTHROPIC_MODEL=claude-sonnet-4-20250514

# Nostr identity (shared by lenos-acp, git auth, signing, and lenos CLI)
export NOSTR_PRIVATE_KEY=nsec1...
export LENOS_PRIVATE_KEY="$NOSTR_PRIVATE_KEY"
export LENOS_RELAY_URL=https://your-relay.example.com
```
README
