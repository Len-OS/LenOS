...
IMAGE="${1:-lenos-sprig:contract-test}"
...
assert_run '
  ...
  test "$(readlink /usr/local/bin/lenos-acp)" = sprig
  for name in lenos-agent lenos-dev-mcp rg tree lenos git-credential-nostr git-sign-nostr; do
...
assert_run '
  grep -Eq "^[[:space:]]*exec lenos-acp" /usr/local/bin/sprig-entrypoint
  ! grep -Eq "^[[:space:]]*(lenos-acp|bash -c .*lenos-acp)" /usr/local/bin/sprig-entrypoint
'

docker run --rm --entrypoint /bin/bash \
  -e LENOS_RELAY_URL=wss://relay.example.test/ "$IMAGE" -ceu '
