#!/usr/bin/env bash
# grab-emoji.sh — Register custom Slack emoji in LenOS
#
# Looks up each emoji name in your Slack workspace and registers it in LenOS
# via `lenos emoji set`, making it available as :name: in the LenOS emoji picker.
#
# Usage:
#   SLACK_TOKEN=xoxp-... ./scripts/grab-emoji.sh [--name <lenos-name>] <emoji-name> [emoji-name ...]
#
# Options:
#   --name <lenos-name>  Override the shortcode used in LenOS (only valid with a single emoji)
#
# Env:
#   SLACK_TOKEN  — Slack user token (xoxp-...) with emoji:read scope
#
# Output:
#   name → registered as :name: in LenOS   on success
...
if [[ $# -eq 0 ]]; then
  echo "Usage: SLACK_TOKEN=xoxp-... $0 [--name <lenos-name>] <emoji-name> [emoji-name ...]" >&2
  exit 1
fi

if [[ -n "$LENOS_NAME" && $# -ne 1 ]]; then
...
if ! command -v lenos &>/dev/null; then
  echo "ERROR: 'lenos' not found in PATH. Install the LenOS CLI and retry." >&2
  exit 1
fi
...
for emoji_name in "$@"; do
  ...
  lenos_shortcode="${LENOS_NAME:-$emoji_name}"
  ...
  # Register in LenOS
  set_output=$(lenos emoji set --shortcode "$lenos_shortcode" --url "$emoji_url" 2>&1) || {
    echo "${emoji_name} → ERROR: lenos emoji set failed — ${set_output}"
    continue
  }

  echo "${emoji_name} → registered as :${lenos_shortcode}: in LenOS"
done
