...
# can finish, so agent workers (goose, lenos-agent, ...) it spawned in their
...
# `LENOS_MANAGED_AGENT` env var from the shell: on macOS `pkill -f` matches only
...
#   <instance-id> is the desktop bundle identifier, e.g. `com.lengrowth.lenos.dev`
#   (main checkout) or `com.lengrowth.lenos.dev.my-branch` (a worktree).
