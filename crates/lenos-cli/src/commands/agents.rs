use lenos_core::kind::KIND_IA_ARCHIVED_LIST;
use lenos_sdk::builders::{build_archive_identity_request, build_unarchive_identity_request};
use nostr::PublicKey;
use serde_json::json;
...
pub async fn dispatch(command: AgentsCmd, client: &LenOSClient) -> Result<(), CliError> {
    match command {
        AgentsCmd::DraftCreate {
            ...
            if let Some(obj) = output.as_object_mut() {
                ...
                obj.insert(
                    "message".into(),
                    "Draft sent to LenOS Desktop for owner review. Nothing changes until the owner saves it."
                        .into(),
                );
            }
            println!("{output}");
...
/// Require `LENOS_AUTH_TAG` and parse the owner pubkey from it. Used only by
/// the `draft-create` and `draft-update` paths.
fn require_owner(client: &LenOSClient) -> Result<PublicKey, CliError> {
    let hex = client
        .auth_tag_owner_hex()
        .ok_or_else(|| CliError::Auth("agent draft requests require LENOS_AUTH_TAG".into()))?;
...
pub(crate) async fn fetch_archived_snapshot(client: &LenOSClient) -> Result<Vec<String>, CliError> {
...
/// `lenos agents archived`: read path over [`fetch_archived_snapshot`] for
/// direct invocation — a trust failure (state 3) is fatal here so a
/// verification command can never look like success.
async fn cmd_archived(client: &LenOSClient) -> Result<(), CliError> {
