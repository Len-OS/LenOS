...
use lenos_sdk::{GitAppliedPatchRef, GitPatchMeta, GitRepoCoord, GitStatus, GitStatusMeta};
...
pub async fn cmd_send_patch(
    ...
    let builder = lenos_sdk::build_git_patch(&repo, &content, &meta).map_err(sdk_err)?;
...
pub async fn cmd_get_patch(client: &LenOSClient, event: &str) -> Result<(), CliError> {
...
pub async fn cmd_patch_status(
    ...
    let builder = lenos_sdk::build_git_status(status, &body, &meta).map_err(sdk_err)?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
    Ok(())
...
/// status kind either way. Shared by `lenos issues status`.
...
pub async fn dispatch(cmd: crate::PatchesCmd, client: &LenOSClient) -> Result<(), CliError> {
