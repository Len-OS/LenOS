...
use crate::client::{normalize_write_response, LenOSClient};
use crate::error::CliError;
use lenos_sdk::CustomEmoji;
...
const CUSTOM_EMOJI_SET_D_TAG: &str = lenos_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
/// own kind:30030 set (d=`lenos:custom-emoji`).
async fn cmd_list(client: &LenOSClient) -> Result<(), CliError> {
    let filter = serde_json::json!({
        "kinds": [lenos_sdk::kind::KIND_EMOJI_SET],
...
async fn fetch_own_emoji(client: &LenOSClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &LenOSClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = lenos_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &LenOSClient, shortcode: &str, url: &str) -> Result<(), CliError> {
    let normalized = lenos_sdk::normalize_custom_emoji_shortcode(shortcode)
...
async fn cmd_rm(client: &LenOSClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = lenos_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &LenOSClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
