...
use lenos_sdk::{GitIssueMeta, GitRepoCoord, GitStatusMeta};
...
pub async fn cmd_create_issue(
    ...
    let builder = lenos_sdk::build_git_issue(&repo, subject, &body, &meta).map_err(sdk_err)?;
...
pub async fn cmd_get_issue(client: &LenOSClient, event: &str) -> Result<(), CliError> {
...
pub async fn cmd_issue_status(
    ...
    // Mirrors `lenos patches status`: default a `p` tag to the repo owner
...
pub async fn dispatch(cmd: crate::IssuesCmd, client: &LenOSClient) -> Result<(), CliError> {
    use crate::IssuesCmd;
    match cmd {
        IssuesCmd::Create {
            repo_owner,
