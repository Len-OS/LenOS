use lenos_sdk::{DeleteMessageOptions, DiffMeta, ThreadRef, VoteDirection};
use nostr::PublicKey;
use uuid::Uuid;

use crate::client::{normalize_events, normalize_write_response, LenOSClient};
use crate::error::CliError;
use crate::validate::{
    infer_language, parse_event_id, parse_uuid, read_or_stdin, truncate_diff,
    validate_content_size, validate_hex64, validate_uuid, MAX_DIFF_BYTES,
};
use lenos_sdk::mentions::{
...
async fn resolve_channel_id(client: &LenOSClient, event_id: &str) -> Result<Uuid, CliError> {
...
async fn resolve_author(client: &LenOSClient, author: &str) -> Result<String, CliError> {
...
pub async fn cmd_send_message(
    ...
    if !missing.is_empty() {
        return Err(CliError::Usage(
            serde_json::json!({
                ...
                "add_member_command": format!("lenos channels add-member --channel {} --pubkey <pubkey> --role <member|bot>", p.channel_id),
            })
            .to_string(),
        ));
    }
    ...
    let builder = match p.kind {
        Some(45001) => {
            lenos_sdk::build_forum_post(channel_uuid, &final_content, &mention_refs, &media_tags)
        ...
        None | Some(9) => lenos_sdk::build_message(
            channel_uuid,
            &final_content,
            thread_ref.as_ref(),
            &mention_refs,
...
pub async fn cmd_send_diff_message(
    ...
    let builder =
        lenos_sdk::build_diff_message(channel_uuid, &diff, &diff_meta, thread_ref.as_ref())
...
pub async fn cmd_delete_message(
    ...
    let builder = lenos_sdk::build_delete_message_with_options(
        channel_uuid,
        target_eid,
...
pub async fn cmd_edit_message(
    ...
    let builder = lenos_sdk::build_edit(channel_uuid, target_eid, content)
...
pub async fn cmd_vote_on_post(
    ...
    let builder = lenos_sdk::build_vote(channel_uuid, target_eid, vote_dir)
