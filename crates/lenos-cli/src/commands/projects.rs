//! `lenos projects` commands — NIP-MP kind:30621 write path.
//!
...
use lenos_core::kind::KIND_PROJECT;
use lenos_sdk::{
...
// ── LenOS repo-ID grammar (bare --repo shorthand) ─────────────────────────────

/// Pattern for a LenOS-hosted repo identifier (bare `--repo` shorthand).
...
async fn fetch_own_project(client: &LenOSClient, slug: &str) -> Result<Option<Event>, CliError> {
...
async fn submit_project(client: &LenOSClient, builder: EventBuilder) -> Result<(), CliError> {
...
pub async fn cmd_create(
    ...
    if fetch_own_project(client, slug).await?.is_some() {
        return Err(CliError::Conflict(format!(
            "project {slug:?} already exists; use 'lenos projects update' to modify it"
        )));
    }
...
pub async fn cmd_update(
    ...
    if !has_mutation {
        return Err(CliError::Usage(
            "lenos projects update requires at least one of: \
    ...
    let singleton_fields = ["name", "description", "lenos-channel", "lenos-visibility"];
            ...
            if let Some(field) = tag_name(t) {
                if singleton_fields.contains(&field) {
                    let clear = match field {
                        ...
                        "lenos-channel" => clear_channel || channel.is_some(),
                        "lenos-visibility" => clear_visibility || visibility.is_some(),
                        _ => false,
                    };
                    return !clear;
                }
    ...
    if let Some(ch) = channel {
        tags.push(make_tag(&["lenos-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(make_tag(&["lenos-visibility", vis])?);
    }
...
pub async fn cmd_delete(client: &LenOSClient, slug: &str) -> Result<(), CliError> {
    validate_project_slug(slug)?;
...
/// Does NOT impose the LenOS repo-ID grammar — project slugs are more permissive.
...
/// Validate a `lenos-visibility` value at the writer level.
...
pub async fn dispatch(cmd: crate::ProjectsCmd, client: &LenOSClient) -> Result<(), CliError> {
...
#[cfg(test)]
mod tests {
    use lenos_sdk::{validate_project_envelope, PROJECT_MEMBER_CAP};
    use nostr::Tag;
    ...
    fn expand_repo_coord_rejects_uppercase_owner() {
        let upper = "30617:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA:lenos";
    ...
    fn rebuild_project_preserves_hinted_member_tags() {
        ...
        let coord = format!("30617:{OWNER_HEX}:lenos");
    ...
    fn apply_update_tags(
        ...
        if let Some(ch) = channel {
            tags.push(make_test_tag(&["lenos-channel", ch]));
        }
        if let Some(vis) = visibility {
            tags.push(make_test_tag(&["lenos-visibility", vis]));
        }
        tags
    }
    ...
    fn update_clear_visibility_drops_tag() {
        let head = make_head_tags(&[make_test_tag(&["lenos-visibility", "unlisted"])]);
        let result = apply_update_tags(head, None, false, None, false, None, false, None, true);
        assert!(!result
            .iter()
            .any(|t| tag_name(t) == Some("lenos-visibility")));
    }

    #[test]
    fn update_exactly_one_singleton_after_replace() {
        // Start with a lenos-channel; replace with a new one; must have exactly one.
        let uuid1 = "3580ca9b-47b4-4af9-b22a-1068778f26c6";
        let uuid2 = "00000000-0000-0000-0000-000000000000";
        let head = make_head_tags(&[make_test_tag(&["lenos-channel", uuid1])]);
        let result = apply_update_tags(
            head,
            None,
            false,
        ...
        let channels: Vec<_> = result
            .iter()
            .filter(|t| tag_name(t) == Some("lenos-channel"))
            .collect();
    ...
    fn next_timestamp_returns_head_plus_one_when_head_is_ahead_of_wall_clock() {
        ...
        let tags = vec![
            make_test_tag(&["d", "platform"]),
            make_test_tag(&["a", &format!("30617:{OWNER_HEX}:lenos")]),
        ];
    ...
    async fn empty_update_returns_usage_error_before_any_network_call() {
        ...
        let client = crate::client::LenOSClient::new("http://127.0.0.1:9".into(), keys, None, None)
    ...
    fn discard_client() -> crate::client::LenOSClient {
        let keys = nostr::Keys::generate();
        crate::client::LenOSClient::new("http://127.0.0.1:9".into(), keys, None, None)
            .expect("client construction")
    }
    ...
    // (step 7: lenos already present → exit=5). No relay mock is available
