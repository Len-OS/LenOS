...
use client::LenOSClient;
use error::CliError;
use nostr::Keys;
use uuid::Uuid;

/// Run the LenOS CLI from raw arguments (including `argv[0]`).
///
...
///
/// ```ignore
/// let code = lenos_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from lenos-acp/lenos-dev-mcp)
    ...
    // lenos-dev-mcp delegates to run_from_args, it has already installed ring; the
...
#[command(
    name = "lenos",
    about = "LenOS CLI — interact with a LenOS relay",
    long_about = "\
LenOS CLI — interact with a LenOS relay

Configuration (flags override env vars):
  LENOS_RELAY_URL     Relay base URL        [default: http://localhost:3000]
  LENOS_PRIVATE_KEY   Nostr private key (hex or nsec)  [required]
  LENOS_AUTH_TAG      NIP-OA auth tag JSON  [optional]
...
struct Cli {
    /// Relay URL (http:// or https://). Overrides LENOS_RELAY_URL env var.
    #[arg(long, env = "LENOS_RELAY_URL", default_value = "http://localhost:3000")]
    relay: String,
    ...
    #[arg(long, env = "LENOS_AUTH_TAG", hide_env_values = true)]
...
#[derive(Subcommand)]
pub enum AgentsCmd {
    /// Open a prefilled create-agent form in the owner's LenOS Desktop
    DraftCreate {
    ...
    /// Open a prefilled edit-agent form in the owner's LenOS Desktop
    DraftUpdate {
...
running under LENOS_AUTH_TAG signs as itself, so it can only ever satisfy \
the self path (target == signer) — not the owner-of-agent path for another \
identity.\n\n\
Examples:\n  \
lenos agents archive <PUBKEY> --reason retired\n  \
lenos agents archive <PUBKEY> --reason bot-rebuilt --replaced-by <NEW_PUBKEY>"
    )]
    Archive {
...
lenos agents unarchive <PUBKEY> --reason returned")]
    Unarchive {
        /// Target identity pubkey (hex)
        target_pubkey: String,
        /// Machine-readable reason code, max 64 UTF-8 bytes
...
pub enum MessagesCmd {
    ...
    #[command(
        after_help = "Examples:\n  lenos messages send --channel <UUID> --content \"hello\"\n  lenos messages send --channel <UUID> --content \"@alice check this\"\n  echo \"hello from stdin\" | lenos messages send --channel <UUID> --content -"
    )]
    Send {
        /// Channel UUID (from 'lenos channels list')
        #[arg(long)]
        channel: String,
    ...
    #[command(
        after_help = "Examples:\n  lenos messages get --channel <UUID>\n  lenos messages get --channel <UUID> --limit 50 --kinds 1,1984"
    )]
    Get {
        /// Channel UUID
        #[arg(long)]
    ...
    #[command(
        after_help = "Examples:\n  lenos messages search --query checkout\n  lenos messages search --author npub1... --since 1783497600\n  lenos messages search --author Aaron --query checkout --limit 20"
    )]
    Search {
...
pub enum ChannelsCmd {
    ...
    #[command(
        after_help = "Examples:\n  lenos channels list\n  lenos channels list --visibility open"
    )]
    List {
    ...
    #[command(
        after_help = "Examples:\n  lenos channels search --query composer\n  lenos channels search --query lenos-chat-composer --exact\n  lenos channels search --query design --include-archived"
    )]
    Search {
    ...
    #[command(
        after_help = "Examples:\n  lenos channels create --name general --type stream --visibility open\n  lenos channels create --name design --type forum --visibility open --description \"Design discussions\"\n  lenos channels create --name standup --type stream --visibility open --ttl 3600  # ephemeral, archived after 1h idle\n  lenos channels create --name project-x --template \"LenOS Team\"  # type/visibility/canvas/roster from the template; explicit flags override"
    )]
    Create {
        /// Channel name
        #[arg(long)]
...
pub enum WorkflowsCmd {
    ...
    #[command(
        after_help = "Examples:\n  lenos workflows trigger --workflow <UUID>\n  lenos workflows trigger --workflow <UUID> --inputs '{\"key\":\"value\"}'"
    )]
    Trigger {
        /// Workflow UUID
        #[arg(long)]
...
pub enum NotesCmd {
    ...
    #[command(
        after_help = "Examples:\n  echo '# Hello' | lenos notes set --name hello --title 'Hello' --content -\n  lenos notes set --name hello --tag onboarding --content - < draft.md"
    )]
    Set {
...
pub enum ReposCmd {
    ...
    Create {
        ...
        relays: Vec<String>,
        /// Channel UUID to bind the repo to. The `lenos-channel` tag is the
        /// git ACL: without it the relay 404s every clone/fetch/push until
        /// the author runs `lenos repos bind` (issue #3527).
        #[arg(long)]
    ...
    ///
    /// The `lenos-channel` tag on the announcement is the git ACL: the relay
...
pub enum ProjectsCmd {
    ...
    Create {
        ...
        slug: String,
        /// Member repository coordinate: bare LenOS repo id (e.g. `lenos`) or full
    ...
    Update {
        ...
        /// Set the associated LenOS channel UUID
        #[arg(long, group = "mutation")]
        channel: Option<String>,
...
pub enum PatchesCmd {
    ...
    #[command(
        after_help = "Examples:\n  git format-patch -1 HEAD --stdout | lenos patches send --repo-owner <hex> --repo-id myrepo --patch-file - --root\n  lenos patches send --repo-owner <hex> --repo-id myrepo --patch-file 0001-fix.patch --reply-to <prev-patch-id>"
    )]
    Send {
...
pub enum PrCmd {
    ...
    #[command(
        after_help = "Examples:\n  lenos pr open --repo-owner <hex> --repo-id myrepo --subject 'Fix bug' --body-file - --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo --branch-name fix-bug\n  lenos pr update --repo-owner <hex> --repo-id myrepo --pr <event> --pr-author <hex> --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo"
    )]
    Open {
        /// Repo owner pubkey (64-char hex)
        #[arg(long)]
...
/// Subcommands for `lenos mem`.
#[derive(Subcommand)]
pub enum MemCmd {
    /// List non-tombstoned memory entries
    Ls {
        /// Owner pubkey (hex). Overrides LENOS_AUTH_TAG.
        #[arg(long)]
        owner: Option<String>,
    ...
    Patch {
        ...
        /// against. Hashes the exact UTF-8 bytes returned by `lenos mem get`,
        /// not normalized lines. Run `lenos mem hash <slug>` to capture this
        /// before editing.
        #[arg(long)]
...
/// `LENOS_RELAY_URL` — moderation commands are community-global and carry no
...
pub enum ModerationCmd {
    ...
    #[command(
        after_help = "Examples:\n  lenos moderation reports\n  lenos moderation reports --status open --limit 20"
    )]
    Reports {
    ...
    #[command(
        after_help = "Examples:\n  lenos moderation resolve --report <REPORT_EVENT_ID> --status dismissed --action dismiss\n  lenos moderation resolve --report <REPORT_EVENT_ID> --status resolved --action ban --reason \"rule 3\""
    )]
    Resolve {
    ...
    #[command(
        after_help = "Examples:\n  lenos moderation ban --pubkey <HEX>\n  lenos moderation ban --pubkey <HEX> --expires-in 604800 --reason \"repeated spam\""
    )]
    Ban {
    ...
    #[command(
        after_help = "Examples:\n  lenos moderation timeout --pubkey <HEX> --expires-in 3600\n  lenos moderation timeout --pubkey <HEX> --expires-at 1783500000 --reason \"cool off\""
    )]
    Timeout {
        /// Target member pubkey (hex)
        #[arg(long)]
...
/// Normalize hand-authored `LENOS_AUTH_TAG` input to strict JSON.
///
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let private_key_str = cli.private_key.ok_or_else(|| {
        CliError::Auth("LENOS_PRIVATE_KEY is required (use --private-key or set env var)".into())
    })?;
    let keys = Keys::parse(&private_key_str)
        .map_err(|e| CliError::Key(format!("invalid LENOS_PRIVATE_KEY: {e}")))?;

    // NIP-OA: parse and verify the auth tag if provided.
    //
    // `LENOS_AUTH_TAG` is hand-authored configuration, so the unquoted raw
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref input) if !input.is_empty() => {
            ...
            let tag = lenos_sdk::nip_oa::parse_auth_tag(&json)
                .map_err(|e| CliError::Auth(format!("LENOS_AUTH_TAG is malformed: {e}")))?;
            lenos_sdk::nip_oa::verify_auth_tag(&json, &keys.public_key()).map_err(|e| {
                CliError::Auth(format!(
                    "LENOS_AUTH_TAG verification failed for pubkey {}: {e}",
                    keys.public_key().to_hex()
                ))
            })?;
            // Canonical wire form derives from the parsed-and-verified tag
            // (same shape as lenos-acp's RestClient), never from raw input.
            let canonical = serde_json::to_string(tag.as_slice())
                .map_err(|e| CliError::Auth(format!("LENOS_AUTH_TAG serialization failed: {e}")))?;
            (Some(tag), Some(canonical))
        }
        _ => (None, None),
    };

    let client = LenOSClient::new(relay_url, keys, auth_tag, auth_tag_json)?;
...
mod tests {
    ...
    fn set_status_clear_rejects_text_and_emoji() {
        for extra in [["--text", "busy"], ["--emoji", "🎶"]] {
            let args = ["lenos", "users", "set-status", "--clear"]
                .into_iter()
                .chain(extra);
            assert!(
    ...
    fn set_status_requires_text_or_clear() {
        assert!(Cli::try_parse_from(["lenos", "users", "set-status"]).is_err());
        assert!(
            Cli::try_parse_from(["lenos", "users", "set-status", "--emoji", "🎶"]).is_err(),
            "--emoji alone must not imply a status"
        );
        assert!(Cli::try_parse_from(["lenos", "users", "set-status", "--clear"]).is_ok());
    }
    ...
    fn projects_update_no_mutation_is_rejected_by_clap() {
        ...
        assert!(
            Cli::try_parse_from(["lenos", "projects", "update", "my-slug"]).is_err(),
