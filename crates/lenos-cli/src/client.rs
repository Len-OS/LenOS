...
fn relay_server_tag(relay_url: &str) -> Option<String> {
    let authority = lenos_core::tenant::relay_url_authority(relay_url);
...
pub struct LenOSClient {
    http: reqwest::Client,
    relay_url: String, // base URL, no trailing slash, e.g. "https://relay.lenos.place"
    keys: Keys,
...
impl LenOSClient {
    ...
    ///
    /// - `LENOS_CONNECT_TIMEOUT_SECS` — TCP connect timeout (default 15 s)
    /// - `LENOS_TIMEOUT_SECS` — per-request total timeout (default 30 s)
    ///
    ...
    pub fn new(
        ...
        let http = reqwest::Client::builder()
            .timeout(env_duration_secs("LENOS_TIMEOUT_SECS", 30))
            .connect_timeout(env_duration_secs("LENOS_CONNECT_TIMEOUT_SECS", 15))
            .build()
                ...
                Ok(resp) => {
                    ...
                    if !resp_was_success(status) {
                        let message = serde_json::from_str::<serde_json::Value>(&body_text)
                            ...
                            .unwrap_or(body_text);
                        let message = if status == 403 && std::env::var("LENOS_AUTH_TAG").is_ok() {
                            format!(
                                "{message} (LENOS_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                            )
                        } else {
                            message
                        };
    ...
    /// `lenos_ws_client::publish_event` which handles connect, NIP-42 auth,
    ...
    pub async fn publish_ephemeral_event(&self, event: nostr::Event) -> Result<String, CliError> {
        ...
        // See lenos_ws_client::{AUTH_CHALLENGE_TIMEOUT_SECS, AUTH_OK_TIMEOUT_SECS,
        // PUBLISH_OK_TIMEOUT_SECS} for the inner ceilings.
        let ok =
            lenos_ws_client::publish_event(&ws_url, event, &self.keys, self.auth_tag.as_ref(), 75)
                .await
                .map_err(|e| CliError::Other(e.to_string()))?;
    ...
    async fn handle_response(&self, resp: reqwest::Response) -> Result<String, CliError> {
        if !resp.status().is_success() {
            ...
            let message = serde_json::from_str::<serde_json::Value>(&body)
                ...
                .unwrap_or(body);
            if status == 403 && std::env::var("LENOS_AUTH_TAG").is_ok() {
                let message = format!(
                    "{message} (LENOS_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                );
                return Err(CliError::Relay {
...
/// LENOS_RELAY_URL may be ws/wss (copied from MCP config).
...
mod retry_tests {
    ...
    fn env_duration_secs_parsing() {
        ...
        const KEY: &str = "LENOS_CLI_TEST_DURATION_SECS";
...
/// through `LenOSClient` to verify behavioural properties — not implementation details.
#[cfg(test)]
mod retry_policy_tests {
    ...
    fn test_client(base_url: &str) -> LenOSClient {
        let keys = Keys::generate();
        LenOSClient::new(base_url.to_string(), keys, None, None).unwrap()
    }
...
mod tests {
    use super::{
        advance_query_cursor, create_response_with_id, extract_relay_response_field, LenOSClient,
    };
    ...
    fn with_auth_tag_omits_header_when_not_configured() {
        let keys = Keys::generate();
        let client = LenOSClient::new("https://test.relay".into(), keys, None, None).unwrap();
