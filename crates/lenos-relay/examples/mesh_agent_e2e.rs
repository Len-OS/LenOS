...
//!   P4 agentic tool use     — agent + lenos-dev-mcp writes a file on disk.
//!
...
//! `lenos-agent` binary with the exact env vars the relay-mesh preset ships.
//!
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when lenos-agent's default 32768
    // budget met a 32k-context model). 150k output: passes lenos-agent's own
    ...
    // P4: agentic tool use via lenos-dev-mcp — write a real file inside the
    ...
    let prompt = format!(
        "Use your developer tools to create {marker_name} in the current working directory containing exactly the text LENOS_OK (no quotes, no newline commentary). Then confirm."
    );
    let mcp = vec![("dev".to_string(), repo_bin("lenos-dev-mcp")?)];
    let (r, marker) =
        agent_chat_with_marker(&base, &served_id, None, &prompt, &mcp, &marker_name).await;
    let file_ok = std::fs::read_to_string(&marker)
        .map(|c| c.contains("LENOS_OK"))
        .unwrap_or(false);
    match r {
        Ok(text) => record(
            "P4 agentic tool use",
...
/// Spawn the real lenos-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("lenos-agent") {
        Ok(agent) => agent,
    ...
    command
        ...
        .env("LENOS_AGENT_MODEL", model)
        .env("OPENAI_COMPAT_BASE_URL", base)
        .env("OPENAI_COMPAT_MODEL", model)
        .env("OPENAI_COMPAT_API_KEY", "lenos-mesh-local")
        .env("OPENAI_COMPAT_API", "chat")
        .env("LENOS_AGENT_MAX_OUTPUT_TOKENS", "4096")
        // No LENOS_AGENT_THINKING_EFFORT: apply_relay_mesh_env() deliberately
    ...
    if let Some(value) = max_output_tokens {
        command.env("LENOS_AGENT_MAX_OUTPUT_TOKENS", value);
    }
