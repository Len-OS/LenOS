...
//!
//! Proves the security claim of LenOS's membership-derived mesh admission:
...
//!      same builder calls LenOS desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how LenOS runs it in production anyway (one
//! desktop = one node).
//!
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch = std::env::temp_dir().join(format!("lenos-mesh-admission-{}", std::process::id()));
