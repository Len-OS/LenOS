...
use lenos_db::Db;
use lenos_media::MediaStorage;
use lenos_pubsub::cache_invalidation::CacheInvalidation;
use lenos_pubsub::conn_control::ConnControl;
use lenos_pubsub::rate_limiter::RedisRateLimiter;
...
pub async fn run_registered_community_connection<Check, CheckFuture, Run, RunFuture>(
    ...
    CheckFuture: Future<Output = Result<bool, lenos_db::DbError>>,
...
) -> (usize, Vec<(CommunityId, lenos_db::DbError)>)
where
            ...
            match conn.tx.try_send(msg) {
                ...
                Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                    ...
                    if count >= conn.grace_limit {
                        ...
                        metrics::counter!("lenos_ws_backpressure_disconnects_total").increment(1);
                        conn.cancel.cancel();
                    } else {
...
pub struct AppState {
    ...
    pub audit_tx: Option<mpsc::Sender<lenos_audit::NewAuditEntry>>,
    ...
    /// See `crates/lenos-conformance/` and `crate::conformance` for the
    /// schema, emitter helpers, and the independent checker.
    pub tracer: Arc<dyn lenos_conformance::Tracer>,
...
impl AppState {
    ...
    pub fn new(
        ...
        let (audit_tx, mut audit_rx) = mpsc::channel::<lenos_audit::NewAuditEntry>(1000);
        ...
        let state = Self {
            ...
            git_store,
            git_pack_cache,
            audio_rooms: Arc::new(if let Ok(dir) = std::env::var("HUDDLE_RECORDING_DIR") {
    ...
    pub async fn is_member_cached(
        ...
        channel_id: Uuid,
        pubkey: &[u8],
    ) -> Result<bool, lenos_db::DbError> {
        let key = (community_id, channel_id, pubkey.to_vec());
        if let Some(cached) = self.membership_cache.get(&key) {
            metrics::counter!("lenos_membership_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("lenos_membership_cache_misses_total").increment(1);
    ...
    pub async fn disconnect_community_clusterwide(
        &self,
        tenant: &TenantContext,
    ) -> Result<usize, lenos_pubsub::PubSubError> {
        let closed = self
    ...
    pub async fn get_accessible_channel_ids_cached(
        ...
        community_id: CommunityId,
        pubkey: &[u8],
    ) -> Result<Vec<Uuid>, lenos_db::DbError> {
        let key = (community_id, pubkey.to_vec());
        if let Some(cached) = self.accessible_channels_cache.get(&key) {
            metrics::counter!("lenos_accessible_channels_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("lenos_accessible_channels_cache_misses_total").increment(1);
        let result = self
            .db
    ...
    pub async fn channel_visibility_cached(
        ...
        community_id: CommunityId,
        channel_id: Uuid,
        prefetched: Option<&lenos_db::channel::ChannelRecord>,
    ) -> Result<String, lenos_db::DbError> {
...
async fn log_audit_entry(audit: &lenos_audit::AuditService, entry: lenos_audit::NewAuditEntry) {
    let t = std::time::Instant::now();
    if let Err(e) = audit.log(entry).await {
        metrics::counter!("lenos_audit_log_errors_total").increment(1);
        tracing::error!("Audit log failed: {e}");
    } else {
        metrics::histogram!("lenos_audit_log_seconds").record(t.elapsed().as_secs_f64());
    }
}
...
mod tests {
    ...
    async fn test_state() -> Arc<AppState> {
        ...
        let media_storage = lenos_media::MediaStorage::new(&config.media).expect("media storage");
    ...
    fn shared_counter_between_direct_and_fanout() {
        ...
        let conn = ConnectionState {
            conn_id,
            tenant: lenos_core::tenant::TenantContext::resolved(
                lenos_core::tenant::CommunityId::from_uuid(Uuid::nil()),
    ...
    async fn revalidation_continues_after_one_community_lookup_failure() {
        ...
        let (closed, failures) =
            revalidate_registered_communities(&registry, |community| async move {
                if community == failed {
                    Err(lenos_db::DbError::InvalidData(
    ...
    async fn disconnect_pubkey_is_fenced_to_the_banning_community() {
        ...
        let community_a = lenos_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xa));
        let community_b = lenos_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xb));
    ...
    async fn drain_all_sends_restart_close_and_cancels_every_conn() {
        ...
        let (mut ctrl_a, cancel_a) = register(lenos_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xa),
        ));
        let (mut ctrl_b, cancel_b) = register(lenos_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xb),
        ));
