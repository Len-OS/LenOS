...
//! - `OTEL_SERVICE_NAME` (default: `lenos-relay`; read explicitly — not via SDK detector)
...
/// `LENOS_OTEL_FILTER` to override the default targets.
pub fn otel_env_filter(configured: Option<&str>) -> EnvFilter {
    EnvFilter::new(configured.unwrap_or("lenos_relay=info,lenos_datastore=info"))
}
...
/// 3. Hard-coded fallback `lenos-relay`.
///
...
/// `lenos-relay` default.  We therefore read `OTEL_SERVICE_NAME` explicitly
...
pub fn service_resource() -> Resource {
    // Honor OTEL_SERVICE_NAME when set+non-empty; otherwise use lenos-relay.
    ...
    let service_name = std::env::var("OTEL_SERVICE_NAME")
        .ok()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "lenos-relay".to_string());
...
mod tests {
    ...
    fn test_service_resource_default_when_env_unset() {
        ...
        assert_eq!(
            ...
            Some("lenos-relay"),
            "expected lenos-relay fallback when OTEL_SERVICE_NAME is unset"
        );
    }
    ...
    fn test_service_resource_empty_string_falls_back_to_default() {
        ...
        assert_eq!(
            service_name_from(&r).as_deref(),
            Some("lenos-relay"),
            "expected lenos-relay fallback when OTEL_SERVICE_NAME is empty"
        );
    }
