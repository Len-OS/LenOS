...
/// enforces membership (`LENOS_REQUIRE_RELAY_MEMBERSHIP=true`) AND has a
...
///
/// `max_limit` is [`lenos_db::DEFAULT_MAX_PAGE_LIMIT`], the same constant the
...
fn relay_limitation(max_message_length: usize) -> RelayLimitation {
    ...
    RelayLimitation {
        ...
        max_filters: Some(10),
        max_limit: Some(lenos_db::DEFAULT_MAX_PAGE_LIMIT as u32),
...
impl RelayInfo {
    ...
    /// kinds 39000/39001/39002, which LenOS signs with `state.relay_keypair`
    ...
    pub fn build(
        ...
        Self {
            name: "LenOS Relay".to_string(),
            description: "LenOS — private team communication relay".to_string(),
            ...
            push: None,
            software: "https://github.com/block/lenos".to_string(),
...
fn push_descriptor(
    ...
    Some(serde_json::json!({
        ...
        "app_profiles": [
            {"id": "lenos-ios-production", "transport": "apns"},
            {"id": "lenos-ios-sandbox", "transport": "apns"}
        ],
...
mod tests {
    ...
    #[test]
    fn build_advertises_lenos_repository_url() {
        let info = RelayInfo::build(None, None, false, DEFAULT_MAX_FRAME_BYTES, None);
        assert_eq!(info.software, "https://github.com/block/lenos");
    }
    ...
    fn auth_required_is_advertised_true() {
        ...
        // `AuthState::Authenticated` (see `crates/lenos-relay/src/handlers/`),
