...
///
/// The wire cap is 16 MiB (`lenos_relay_mesh::wire::MAX_STREAM_FRAME`), but using
...
mod tests {
    ...
    use lenos_relay_mesh::{InboundHandler, MeshDatagram, PeerInfo};
    ...
    async fn clear_keys(directory: &SessionDirectory, community_id: CommunityId, session_id: Uuid) {
        let base = format!("lenos:{}:tunnel:{}", community_id, session_id);
        let _ = directory
    ...
    struct DirectTransport {
        peer: lenos_relay_mesh::peer::MeshPeer,
    ...
    fn payload_chunking_covers_goose_sized_bodies() {
        ...
        assert_eq!(chunks, 50);
        assert!(MAX_RELIABLE_PAYLOAD_BYTES < lenos_relay_mesh::wire::MAX_STREAM_FRAME as usize);
    }
