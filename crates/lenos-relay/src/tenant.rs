...
//! implementation that queries the `communities` table lives in `lenos-db`
//! (`Db::resolve_host`); the relay depends on the trait, not the query, so the
//! binding is testable without a database.

use lenos_core::tenant::{normalize_host, CommunityId, TenantContext};
...
/// [`lenos_core::tenant::normalize_host`] — [`bind_community`] guarantees that,
...
///
/// This is a thin re-export of [`lenos_core::tenant::relay_url_authority`]: the
/// canonical implementation lives in `lenos-core` so the relay seam *and* the
/// `lenos-admin` CLI derive a byte-identical authority (same port/IPv6 handling).
pub use lenos_core::tenant::relay_url_authority;

/// Production [`HostResolver`]: the relay resolves hosts against the durable
/// `communities` host map in Postgres.
///
/// This is the *only* place the relay couples the row-zero seam to lenos-db. The
/// trait keeps `bind_community` and every call site database-free and testable;
/// this impl is the thin adapter from lenos-db's `lookup_community_by_host`
...
impl HostResolver for lenos_db::Db {
    type Error = lenos_db::DbError;
...
mod tests {
    ...
    mod redteam_attack2 {
        ...
        /// `cargo test -p lenos-relay --include-ignored
        ...
        /// trims to empty (`lenos-core::tenant::normalize_host_empty_stays_empty`),
        ...
        async fn whitespace_only_raw_host_fails_closed_even_if_db_has_empty_host_row() {
            ...
            let err = bind_community(&r, "   ").await.expect_err(
                "Inv_RowZero: whitespace-only raw_host normalizes to empty \
                 (see lenos-core::tenant::normalize_host) and carries no \
                 community evidence",
            );
            assert!(
                matches!(err, BindError::UnmappedHost),
