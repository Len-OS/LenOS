...
/// the JSON-array string that [`lenos_sdk::nip_oa::verify_auth_tag`] expects.
///
...
pub async fn handle_auth(event: nostr::Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    metrics::counter!("lenos_auth_attempts_total", "method" => "nip42").increment(1);
        ...
        Ok(mut auth_ctx) => {
            ...
            // extracted from the self-proving auth tag with no DB round-trip.
            {
                ...
                if let Some((metric_reason, deny_reason)) = denial {
                    ...
                    metrics::counter!("lenos_auth_failures_total", "reason" => metric_reason)
                        .increment(1);
                ...
                && auth_ctx.auth_method == lenos_auth::AuthMethod::Nip42
            {
                ...
                if !allowed {
                    ...
                    metrics::counter!("lenos_auth_failures_total", "reason" => "allowlist_denied")
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
                    conn.send(RelayMessage::ok(
                        &event_id_hex,
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    metrics::counter!("lenos_auth_failures_total", "reason" => "not_relay_member")
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
                    conn.send(RelayMessage::ok(
                        &event_id_hex,
