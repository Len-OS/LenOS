...
use lenos_db::channel::{MemberRecord, MemberRole};
...
/// The per-fire authority gate in `lenos-workflow` is the fail-closed backstop;
...
///
/// LenOS accepts standard deletions for self-authored events, plus the owning
...
async fn actor_owns_any_owner_agent(
    ...
    members: &[lenos_db::channel::MemberRecord],
    actor_bytes: &[u8],
) -> anyhow::Result<bool> {
    ...
    match kind {
        9000 => {
            ...
            let requested_role = match role_str {
                Some(ref s) => match s.parse::<lenos_db::channel::MemberRole>() {
                    Ok(r) => Some(r),
            ...
            let members = state.db.get_members(tenant.community(), channel_id).await?;
            let actor_role: Option<lenos_db::channel::MemberRole> = members
                .iter()
                ...
                .filter(|(m, role)| m.role != role.as_str())
            {
                ...
                if target.role == "owner"
                    && role != lenos_db::channel::MemberRole::Owner
        ...
        9002 => {
            ...
            for t in event.tags.iter() {
                if t.kind().to_string() == "name" {
                    match t.content() {
                        Some(v)
                            if !lenos_core::channel::canonical_channel_name(v)
                                .trim()
                                .is_empty() => {}
                        _ => {
...
pub async fn emit_group_discovery_events(
...

    {
        ...
        // LenOS channels always require explicit membership
                ...
                "name" => {
                    state
                        ...
                        .update_channel(
                            tenant.community(),
                            channel_id,
                            lenos_db::channel::ChannelUpdate {
...
async fn handle_create_group(
    ...
    let channel_type: lenos_db::channel::ChannelType = channel_type_str
        .parse()
    ...
    if visibility == lenos_db::channel::ChannelVisibility::Open {
        state.invalidate_all_accessible_channels(tenant);
    }
...
async fn handle_join_request(
    ...
    state
        ...
        .add_member(
            ...
            channel_id,
            &actor_bytes,
            lenos_db::channel::MemberRole::Member,
            None,
        )
        .await?;
    ...
    if let Err(e) = emit_membership_notification(
        ...
        &actor_bytes,
        &actor_bytes,
        lenos_core::kind::KIND_MEMBER_ADDED_NOTIFICATION,
    )
    .await
    {
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        ...
        lenos_core::kind::KIND_WORKFLOW_DEF => {
...
async fn handle_git_repo_announcement(
    ...
    use lenos_db::git_repo::ReserveOutcome;
...
/// carrying the announcer's pubkey in the `p` tag (lenos extension).
...
pub async fn reconcile_nip43_membership_snapshots(state: &Arc<AppState>) -> anyhow::Result<usize> {
    ...
    for community in communities {
        let community_id = lenos_core::CommunityId::from_uuid(community.id);
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("lenos_nip43_membership_reconciliation_failures_total")
                    .increment(1);
    ...
    metrics::counter!("lenos_nip43_membership_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub async fn publish_nip43_membership_list(
    ...
    metrics::counter!("lenos_nip43_membership_publications_total", "result" => "attempted")
        .increment(1);
    let result = publish_nip43_membership_list_inner(tenant, state).await;
    metrics::histogram!("lenos_nip43_membership_publication_seconds")
        .record(started_at.elapsed().as_secs_f64());
    metrics::counter!(
        "lenos_nip43_membership_publications_total",
