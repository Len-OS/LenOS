...
//! [`lenos_core::kind::is_moderation_command_kind`], list them in
...
//!
//! Lane ownership: L6 (Quinn) — plus `lenos-cli` `moderation` command group.
...
use lenos_db::moderation::NewAction;
...
async fn handle_resolve(
    ...
    let (target_pubkey, target_event_id) = match &report.target {
        lenos_db::moderation::ReportTarget::Pubkey(p) => (Some(p.as_slice()), None),
        lenos_db::moderation::ReportTarget::Event(e) => (None, Some(e.as_slice())),
        lenos_db::moderation::ReportTarget::Blob(_) => (None, None),
    };
...
mod tests {
    ...
    fn resolve_audit_actions_are_allowed_by_db_check_vocabulary() {
        for action in ["dismiss", "escalate", "delete", "kick", "ban", "timeout"] {
            let audit_action = resolution_audit_action(action);
            assert!(
                lenos_db::moderation::MODERATION_ACTION_CHECK_VOCAB.contains(&audit_action),
