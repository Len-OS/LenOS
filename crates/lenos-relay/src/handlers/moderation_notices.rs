...
//!    participant-hash-idempotent DM model (`lenos-db/src/dm.rs`).
...
use lenos_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            lenos_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
