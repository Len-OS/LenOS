...
use chrono::Utc;
use lenos_core::kind::KIND_STREAM_MESSAGE;
use lenos_core::tenant::CommunityId;
use lenos_workflow::action_sink::{ActionSink, ActionSinkError};
    ...
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    lenos_db::DbError::ChannelNotFound(_) | lenos_db::DbError::NotFound(_) => {
            ...
            //    - `lenos:workflow` tag prevents recursive workflow triggering
...
mod integration_tests {
    ...
    //!   `cargo test -p lenos-relay --lib workflow_sink -- --ignored`
    use super::*;
    use lenos_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use lenos_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
