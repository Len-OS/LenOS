//! Smart HTTP git transport for LenOS.
//!
        ...
        let auth_header = parts
            ...
            .ok_or_else(|| {
                Response::builder()
                    ...
                    .header(
                        "WWW-Authenticate",
                        format!("Nostr realm=\"lenos\", method=\"{method}\""),
                    )
...
fn acquire_git_permit(
    ...
    Arc::clone(&state.git_semaphore)
        ...
        .map_err(|_| {
            metrics::counter!(
                "lenos_git_semaphore_rejections_total",
                "operation" => operation
            )
            .increment(1);
            Response::builder()
...
///    resolve;
/// 2. its `lenos-channel` tag → channel UUID;
/// 3. [`lenos_db::Db::get_member_role`] for the caller — a read is allowed
///    only on `Ok(Some(role))` with a role the relay recognizes.
///
/// Fail-closed: missing/deleted announcement, invalid owner, missing or
/// malformed `lenos-channel` binding, non-member, unknown role, and every DB
...
/// `lenos-channel` tag, and the repo then 404s forever with no explanation
...
async fn authorize_git_read(
    db: &lenos_db::Db,
    community: lenos_core::CommunityId,
        ...
        RepoBinding::NotBound => {
            ...
            if repo_event.event.pubkey == *caller {
                ...
                return Err((
                    ...
                    format!(
                        "run: lenos repos bind --id {repo_name} --channel <channel-uuid> — repository {repo_name:?} has no channel binding, so the relay cannot authorize access"
                    ),
                )
                    .into_response());
            }
            warn!(repo = %repo_name, "git read gate: missing lenos-channel binding (deny)");
            return Err(denied());
        }
        RepoBinding::Broken => {
            warn!(repo = %repo_name, "git read gate: malformed lenos-channel binding (deny)");
            return Err(denied());
        }
    };
...
fn read_role_allows(role: Option<&str>) -> bool {
    match role {
        Some(r) => r.parse::<lenos_core::channel::MemberRole>().is_ok(),
        None => false,
    }
}
...
/// 0000
/// <pkt><head-oid> HEAD\0<caps> symref=HEAD:<head-ref> object-format=<fmt> agent=lenos-git\n
...
pub async fn receive_pack(
    ...
    let mut hook_env = vec![
        ("LENOS_HOOK_URL", hook_url),
        (
            "LENOS_HOOK_SECRET",
            state.config.git_hook_hmac_secret.clone(),
        ),
        ("LENOS_REPO_ID", repo_name.to_string()),
        ("LENOS_REPO_OWNER", params.owner.clone()),
        (
            "LENOS_COMMUNITY_ID",
            auth.tenant.community().as_uuid().to_string(),
        ),
        ("LENOS_PUSHER_PUBKEY", pusher_hex.clone()),
    ];
    ...
    S: futures_util::Stream<Item = Result<bytes::Bytes, std::io::Error>>,
{
    ...
    fn poll_next(
        ...
        if self.deadline.as_mut().poll(cx).is_ready() {
            self.finished = true;
            metrics::counter!("lenos_git_upload_pack_timeouts_total").increment(1);
...
mod track_c_tests {
    ...
    fn receive_pack_config_allows_deleting_current_branch() {
        ...
        assert_git_success(
            run_test_git(
                source.as_path(),
                &["config", "user.email", "lenos-test@example.com"],
                &[],
            ),
    ...
    fn git_nip98_rejects_token_signed_for_wrong_community_host() {
        ...
        let err = lenos_auth::nip98::verify_nip98_event(&event_json, &expected_for_b, "GET", None)
    ...
    fn git_nip98_accepts_token_signed_for_matching_community_host() {
        ...
        let pubkey =
            lenos_auth::nip98::verify_nip98_event(&event_json, &expected_for_a, "GET", None)
...
mod sec005_read_gate_tests {
    ...
    async fn setup_db() -> lenos_db::Db {
        let url = std::env::var("LENOS_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| TEST_DB_URL.to_string());
        let pool = sqlx::PgPool::connect(&url).await.expect("connect test DB");
        lenos_db::Db::from_pool(pool)
    }

    /// How the fixture's kind:30617 binds (or fails to bind) a channel.
    enum Binding {
        /// `lenos-channel` tag carrying the fixture channel's UUID.
        Channel,
        /// No `lenos-channel` tag at all.
        Missing,
        /// `lenos-channel` tag whose value is not a UUID.
        Malformed,
        /// `lenos-channel` tag carrying a well-formed UUID that names no
    ...
    async fn setup_repo(binding: Binding) -> RepoFixture {
        ...
        db.create_channel_with_id(
            ...
            lenos_db::channel::ChannelType::Stream,
            lenos_db::channel::ChannelVisibility::Open,
            None,
            &creator_pk,
            None,
        )
        ...
        db.add_member(
            ...
            channel,
            &member_pk,
            lenos_core::channel::MemberRole::Member,
            Some(&creator_pk),
        )
        .await
        .expect("member");
        ...
        match binding {
            Binding::Channel => {
                tags.push(Tag::parse(["lenos-channel", &channel.to_string()]).unwrap());
            }
            Binding::Missing => {}
            Binding::Malformed => {
    ...
    async fn read_gate_denies_missing_or_malformed_binding_and_absent_repo() {
        // Missing lenos-channel tag → deny even for a channel member, with
        ...
        // Malformed lenos-channel tag → deny with the generic body EVEN FOR
    ...
    async fn read_gate_gives_author_of_unbound_repo_remediation_body() {
        ...
        assert!(
            body.starts_with(&format!("run: lenos repos bind --id {}", f.repo)),
