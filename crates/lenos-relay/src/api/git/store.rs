...
//! ## The 412 sharp edge
//!
//! `rust-s3 = "0.37"` is shared across the workspace with `lenos-media`. The
...
impl GitStore {
    ...
    ///
    /// Credential selection mirrors [`lenos_media::MediaStorage::new`]:
    ...
    pub fn new(
        ...
        bucket_name: &str,
        region: &str,
        addressing_style: lenos_media::config::S3AddressingStyle,
        ...
        let bucket = match addressing_style {
            lenos_media::config::S3AddressingStyle::Path => bucket.with_path_style(),
            lenos_media::config::S3AddressingStyle::Virtual => bucket,
        };
        Ok(Self {
...
mod tests {
    ...
    fn constructor_applies_both_addressing_styles() {
        for (style, expected_url, path_style) in [
            (
                lenos_media::config::S3AddressingStyle::Path,
                "https://storage.example/lenos-git",
                true,
            ),
            (
                lenos_media::config::S3AddressingStyle::Virtual,
                "https://lenos-git.storage.example",
                false,
            ),
        ] {
            let store = GitStore::new(
    ...
    fn partial_static_keys_are_rejected() {
        for (access, secret) in [("lenos_dev", ""), ("", "lenos_dev_secret")] {
...
mod probe {
    ...
    //!
    //! Run manually:
    //!   LENOS_GIT_S3_PROBE=1 cargo test -p lenos-relay --lib \
    //!     api::git::store::probe -- --nocapture --test-threads=1
    //!
    //! Pre-req: `docker compose up minio` and the `lenos-git` bucket exists.

    use super::*;

    fn probe_enabled() -> bool {
        std::env::var("LENOS_GIT_S3_PROBE").as_deref() == Ok("1")
    }
    ...
    fn store() -> GitStore {
        ...
        let region = std::env::var("LENOS_S3_REGION").unwrap_or_else(|_| "us-east-1".into());
    ...
    async fn probe_412_surfacing() {
        if !probe_enabled() {
            eprintln!("skipping: set LENOS_GIT_S3_PROBE=1 to run against live MinIO");
            return;
        }
        let st = store();
