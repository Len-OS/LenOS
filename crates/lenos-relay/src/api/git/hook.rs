...
/// - `LENOS_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `LENOS_HOOK_SECRET` — per-push HMAC secret
/// - `LENOS_REPO_ID` — repo identifier (d-tag)
/// - `LENOS_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `LENOS_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# LenOS pre-receive hook — FAIL-CLOSED
...
: "${LENOS_REPO_ID:?error: LENOS_REPO_ID not set}"
: "${LENOS_REPO_OWNER:?error: LENOS_REPO_OWNER not set}"
: "${LENOS_COMMUNITY_ID:?error: LENOS_COMMUNITY_ID not set}"
: "${LENOS_PUSHER_PUBKEY:?error: LENOS_PUSHER_PUBKEY not set}"
: "${LENOS_HOOK_URL:?error: LENOS_HOOK_URL not set}"
: "${LENOS_HOOK_SECRET:?error: LENOS_HOOK_SECRET not set}"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$LENOS_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$LENOS_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${LENOS_REPO_OWNER}\",\"community_id\":\"${LENOS_COMMUNITY_ID}\",\"pusher_pubkey\":\"${LENOS_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
