...
use base64::Engine;
use lenos_audit::{AuditAction, NewAuditEntry};
use lenos_core::tenant::TenantContext;
use lenos_media::{BlobDescriptor, MediaError, UploadAttribution, UploadNetworkInfo};
...
fn should_stream_as_video(sniff: &[u8]) -> bool {
    infer::get(sniff).is_some_and(|kind| kind.mime_type() == "video/mp4")
        || lenos_media::looks_like_iso_bmff(sniff)
}
...
fn upload_rate_limited(
    state: &AppState,
    community_id: lenos_core::CommunityId,
...
impl FromRequestParts<Arc<AppState>> for AuthenticatedUpload {
    ...
    async fn from_request_parts(
        ...
        lenos_media::auth::verify_blossom_auth_event(&auth_event, Some(tenant.host()), 3600)?;
        ...
        if upload_rate_limited(state, tenant.community(), &auth_event.pubkey) {
            metrics::counter!("lenos_media_upload_rejections_total", "reason" => "rate_limit")
                .increment(1);
        ...
        let upload_permit = acquire_upload_permit(state, tenant.community(), &auth_event.pubkey)
            .inspect_err(|_| {
                metrics::counter!("lenos_media_upload_rejections_total", "reason" => "concurrency")
                    .increment(1);
            })?;
...
/// (`LENOS_MEDIA_UPLOAD_RECORDS`). Returns `None` when the feature is off —
...
///   (`LENOS_MEDIA_UPLOAD_IP_HEADER`) and validated as a public IP —
...
async fn upload_attribution(
    ...
    let ip = header_value(&cfg.upload_ip_header).and_then(lenos_media::parse_public_ip);
    let port = ip.and(header_value(&cfg.upload_port_header).and_then(lenos_media::parse_port));
...
pub async fn upload_blob(
    ...
    if auth.route_mode == UploadRouteMode::LegacyMedia {
        metrics::counter!("lenos_media_legacy_upload_route_total").increment(1);
    }
    ...
    let mut descriptor = if should_stream_as_video(&sniff) {
        ...
        lenos_media::process_video_upload(
            &state.media_storage,
            &state.config.media,
            &auth.tenant,
            &auth.auth_event,
        ...
        if is_image {
            ...
            lenos_media::process_file_upload(
                &state.media_storage,
                &state.config.media,
                &auth.tenant,
                &auth.auth_event,
...
async fn authenticate_media_read(
    ...
    lenos_media::auth::verify_blossom_get_auth(&auth_event, sha256, Some(tenant.host()), 3600)?;
...
pub(crate) async fn serve_blob_for_tenant(
    ...
    let disposition = if lenos_media::serve_inline(&content_type) {
        "inline"
    } else {
        "attachment"
    };
...
async fn resolve_s3_key(
    storage: &lenos_media::MediaStorage,
    tenant: &TenantContext,
    sha256_ext: &str,
