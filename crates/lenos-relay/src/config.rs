...
pub enum ConfigError {
    /// The `LENOS_BIND_ADDR` environment variable could not be parsed as a socket address.
    #[error("invalid LENOS_BIND_ADDR: {0}")]
...
pub struct Config {
    ...
    /// Replica read budget `B` in milliseconds (`LENOS_REPLICA_READ_MAX_AGE_MS`).
    /// `0` (the default) disables bounded-staleness replica routing; see
    /// [`lenos_db::DbConfig::replica_read_max_age_ms`].
    ...
    ///
    /// The `lenos-db` default of 20 was sized for a handful of pods against
    ...
    /// (`LENOS_DB_READ_POOL_SIZE`). Defaults to `db_pool_size`. Sized
    ...
    pub auth: lenos_auth::AuthConfig,
    ...
    /// `LENOS_HUDDLE_AUDIO_AVAILABLE=false` until the out-of-relay media/SFU
    /// service lands.
    pub huddle_audio_available: bool,

    /// Inter-relay mesh configuration (`LENOS_MESH`, `LENOS_MESH_BIND_ADDR`).
    /// Opt-in: mesh forms only when `LENOS_MESH=on` is explicit. The default
    ...
    pub mesh: lenos_relay_mesh::MeshConfig,

    /// Testbed-only reliable-stream echo consumer (`LENOS_MESH_DEMO_ECHO`).
    ...
    /// Same strict opt-in as `LENOS_MESH`; default off means inbound reliable
    ...
    ///
    /// Default: `false`. Set via `LENOS_ALLOW_NIP_OA_AUTH=true`.
    pub allow_nip_oa_auth: bool,

    /// Media storage configuration (S3/MinIO).
    pub media: lenos_media::MediaConfig,
    ...
    /// Set `LENOS_AUDIT_ENABLED=false` for deployments that do not require it.
    ...
    /// Example: `LENOS_EPHEMERAL_TTL_OVERRIDE=60` → all ephemeral channels expire
...
const DEFAULT_PUSH_GATEWAY_DELIVERY_URL: &str = "https://push.lenos.xyz/v1/deliveries/apns";

fn parse_push_gateway_delivery_url(raw: &str) -> Result<url::Url, ConfigError> {
    let url = url::Url::parse(raw.trim()).map_err(|e| {
        ConfigError::InvalidValue(format!(
            "LENOS_PUSH_GATEWAY_DELIVERY_URL is not a valid URL: {e}"
        ))
    })?;
    if url.scheme() != "https"
        || url.host().is_none()
    ...
    {
        return Err(ConfigError::InvalidValue(
            "LENOS_PUSH_GATEWAY_DELIVERY_URL must be an exact HTTPS /v1/deliveries/apns URL without credentials, query, or fragment"
                .to_string(),
        ));
    }
    Ok(url)
...
fn ensure_git_repo_path(
    ...
    ensure_git_path("LENOS_GIT_REPO_PATH", raw)
}
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        let bind_addr_raw =
            std::env::var("LENOS_BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:3000".to_string());
        let bind_addr = parse_bind_addr(&bind_addr_raw)?;

        let database_url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://lenos:lenos_dev@localhost:5432/lenos".to_string()); // sadscan:disable np.postgres.1
        ...
        if std::env::var("LENOS_REPLICA_HEAD_MAX_AGE_SECS").is_ok() {
            return Err(ConfigError::InvalidValue(
                "LENOS_REPLICA_HEAD_MAX_AGE_SECS was renamed to LENOS_REPLICA_READ_MAX_AGE_MS \
        ...
        let replica_read_max_age_ms = match std::env::var("LENOS_REPLICA_READ_MAX_AGE_MS") {
            Ok(raw) => raw.trim().parse::<u64>().map_err(|_| {
                ConfigError::InvalidValue(
                    "LENOS_REPLICA_READ_MAX_AGE_MS must be a non-negative integer".to_string(),
                )
            })?,
            Err(_) => 0,
        };
        ...
        let redis_pool_size = std::env::var("LENOS_REDIS_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(16);

        let db_pool_size = std::env::var("LENOS_DB_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<u32>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(50);

        let db_read_pool_size = std::env::var("LENOS_DB_READ_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<u32>().ok())
            .filter(|&v| v > 0);
        ...
        let pairing_relay_url = std::env::var("LENOS_PAIRING_RELAY_URL")
            .ok()
            ...
            .map(|value| {
                let parsed = url::Url::parse(&value).map_err(|e| {
                    ConfigError::InvalidValue(format!(
                        "LENOS_PAIRING_RELAY_URL must be a valid ws:// or wss:// URL: {e}"
                    ))
                })?;
                if !matches!(parsed.scheme(), "ws" | "wss") || parsed.host_str().is_none() {
                    return Err(ConfigError::InvalidValue(
                        "LENOS_PAIRING_RELAY_URL must be a valid ws:// or wss:// URL".to_string(),
                    ));
                }
                Ok(value)
            })
        ...
        let max_connections = std::env::var("LENOS_MAX_CONNECTIONS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(10_000);

        let max_concurrent_handlers = std::env::var("LENOS_MAX_CONCURRENT_HANDLERS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1024);

        let send_buffer_size = std::env::var("LENOS_SEND_BUFFER")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1_000);

        let max_frame_bytes = std::env::var("LENOS_MAX_FRAME_BYTES")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(DEFAULT_MAX_FRAME_BYTES);

        let slow_client_grace_limit = std::env::var("LENOS_SLOW_CLIENT_GRACE_LIMIT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(15);
        ...
        let require_relay_membership = std::env::var("LENOS_REQUIRE_RELAY_MEMBERSHIP")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
        ...
        let huddle_audio_available = std::env::var("LENOS_HUDDLE_AUDIO_AVAILABLE")
            .map(|v| !(v == "false" || v == "0"))
            .unwrap_or(true);
        ...
        // `LENOS_MESH=on`; anything else (absent, `off`, other values) keeps
        // exact single-instance behavior.
        let mesh_enabled = std::env::var("LENOS_MESH")
            .map(|v| v.eq_ignore_ascii_case("on") || v == "true" || v == "1")
            .unwrap_or(false);
        let mesh_bind_addr = std::env::var("LENOS_MESH_BIND_ADDR")
            .map(|raw| {
                raw.parse::<SocketAddr>().map_err(|e| {
                    ConfigError::InvalidValue(format!("invalid LENOS_MESH_BIND_ADDR: {e}"))
                })
            })
            .unwrap_or_else(|_| Ok("0.0.0.0:3478".parse().expect("static default parses")))?;
        let mesh = lenos_relay_mesh::MeshConfig {
            enabled: mesh_enabled,
            bind_addr: mesh_bind_addr,
            registry_refresh: std::time::Duration::from_secs(15),
        };

        // Demo echo opt-in: same strict pattern as LENOS_MESH — explicit
        // `on`/`true`/`1` only, anything else (absent, `off`, typos) is off.
        let mesh_demo_echo = std::env::var("LENOS_MESH_DEMO_ECHO")
            .map(|v| v.eq_ignore_ascii_case("on") || v == "true" || v == "1")
            .unwrap_or(false);

        let allow_nip_oa_auth = std::env::var("LENOS_ALLOW_NIP_OA_AUTH")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        // Note: intentionally not prefixed with LENOS_ — this is a relay-identity
        ...
        // Note: intentionally not prefixed with LENOS_ — same relay-identity
        ...
        let auth = lenos_auth::AuthConfig {
            rate_limits: rate_limit_config_from_env()?,
        };

        if !require_auth_token {
            warn!(
                "LENOS_REQUIRE_AUTH_TOKEN is false — REST API requests bypass token auth. \
                 WebSocket protocol auth is unaffected. Set to true for production."
            );
        }

        let cors_origins = std::env::var("LENOS_CORS_ORIGINS")
            .unwrap_or_default()
            .split(',')
        ...
        let relay_private_key = std::env::var("LENOS_RELAY_PRIVATE_KEY").ok();

        let uds_path = std::env::var("LENOS_UDS_PATH")
            .ok()
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty());

        let health_port = std::env::var("LENOS_HEALTH_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(8080);

        let metrics_port = std::env::var("LENOS_METRICS_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(9102);
        ...
        let media = lenos_media::MediaConfig {
            s3_endpoint: std::env::var("LENOS_S3_ENDPOINT")
                .unwrap_or_else(|_| "http://localhost:9000".to_string()),
            s3_access_key: std::env::var("LENOS_S3_ACCESS_KEY")
            ...
            s3_addressing_style,
            max_image_bytes: std::env::var("LENOS_MAX_IMAGE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(50 * 1024 * 1024),
            max_gif_bytes: std::env::var("LENOS_MAX_GIF_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(10 * 1024 * 1024),
            max_video_bytes: std::env::var("LENOS_MAX_VIDEO_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(500 * 1024 * 1024),
            max_file_bytes: std::env::var("LENOS_MAX_FILE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(100 * 1024 * 1024),
            public_base_url: std::env::var("LENOS_MEDIA_BASE_URL")
            ...
            upload_records_enabled: std::env::var("LENOS_MEDIA_UPLOAD_RECORDS")
                .map(|v| v == "true" || v == "1")
                .unwrap_or(false),
            upload_ip_header: std::env::var("LENOS_MEDIA_UPLOAD_IP_HEADER")
                .ok()
                .map(|s| s.trim().to_lowercase())
                .filter(|s| !s.is_empty()),
            upload_port_header: std::env::var("LENOS_MEDIA_UPLOAD_PORT_HEADER")
                .ok()
                .map(|s| s.trim().to_lowercase())
                .filter(|s| !s.is_empty()),
        };
        let media_max_concurrent_uploads: usize =
            std::env::var("LENOS_MEDIA_MAX_CONCURRENT_UPLOADS")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|&v| v > 0)
                .unwrap_or(8);
        let media_max_concurrent_uploads_per_pubkey: u32 =
            std::env::var("LENOS_MEDIA_MAX_CONCURRENT_UPLOADS_PER_PUBKEY")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|&v| v > 0)
                .unwrap_or(2)
                .min(u32::try_from(media_max_concurrent_uploads).unwrap_or(u32::MAX));
        let media_uploads_per_minute: u32 = std::env::var("LENOS_MEDIA_UPLOADS_PER_MINUTE")
            .ok()
            .and_then(|v| v.parse().ok())
            .filter(|&v| v > 0)
            .unwrap_or(30);

        let require_media_get_auth = std::env::var("LENOS_REQUIRE_MEDIA_GET_AUTH")
            .map(|v| {
                v == "true"
                    || v == "1"
        ...
        let ephemeral_ttl_override = std::env::var("LENOS_EPHEMERAL_TTL_OVERRIDE")
            .ok()
        ...
        if let Some(ttl) = ephemeral_ttl_override {
            warn!(
                "LENOS_EPHEMERAL_TTL_OVERRIDE={ttl}s — all ephemeral channels will use \
        ...
        let git_repo_path = ensure_git_repo_path(
            std::env::var("LENOS_GIT_REPO_PATH").unwrap_or_else(|_| "./repos".to_string()),
        )?;
        let git_pack_cache_path = ensure_git_path(
            "LENOS_GIT_PACK_CACHE_PATH",
            std::env::var("LENOS_GIT_PACK_CACHE_PATH")
                .map(std::path::PathBuf::from)
                .unwrap_or_else(|_| git_repo_path.join(".pack-cache")),
        )?;
        let git_max_pack_bytes: u64 = std::env::var("LENOS_GIT_MAX_PACK_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(500 * 1024 * 1024); // 500 MB
        let git_max_repo_bytes: u64 = std::env::var("LENOS_GIT_MAX_REPO_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or_else(|| git_max_pack_bytes.saturating_mul(2)); // 1 GB at defaults
        let git_pack_cache_max_bytes: u64 = std::env::var("LENOS_GIT_PACK_CACHE_MAX_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or_else(|| git_max_repo_bytes.saturating_mul(5)); // 5 GB at defaults
        let git_pack_cache_max_concurrent_populations: usize =
            std::env::var("LENOS_GIT_PACK_CACHE_MAX_CONCURRENT_POPULATIONS")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|value| *value > 0)
                .unwrap_or(2);
        let git_max_repos_per_pubkey: u32 = std::env::var("LENOS_GIT_MAX_REPOS_PER_PUBKEY")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(100);
        let git_max_concurrent_ops: usize = std::env::var("LENOS_GIT_MAX_CONCURRENT_OPS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(20);
        let git_hook_hmac_secret: String = std::env::var("LENOS_GIT_HOOK_HMAC_SECRET")
            .unwrap_or_else(|_| {
        ...
        let push_executor_key_id =
            std::env::var("LENOS_PUSH_EXECUTOR_KEY_ID").unwrap_or_else(|_| "relay-v1".to_string());
        if push_executor_key_id.is_empty() || push_executor_key_id.len() > 64 {
            return Err(ConfigError::InvalidValue(
                "LENOS_PUSH_EXECUTOR_KEY_ID must contain 1..=64 bytes".to_string(),
            ));
        }
        let push_gateway_delivery_url = match std::env::var("LENOS_PUSH_GATEWAY_DELIVERY_URL") {
        ...
        let push_gateway_timeout_millis = match std::env::var("LENOS_PUSH_GATEWAY_TIMEOUT_MS") {
            Ok(raw) => raw
                .parse::<u64>()
                .ok()
                ...
                .ok_or_else(|| {
                    ConfigError::InvalidValue(
                        "LENOS_PUSH_GATEWAY_TIMEOUT_MS must be an integer in 100..=10000"
                            .to_string(),
                    )
                })?,
            Err(_) => 2_000,
        ...
        let terms_markdown = read_policy_markdown("LENOS_TERMS_OF_SERVICE_MARKDOWN")?;
        let privacy_markdown = read_policy_markdown("LENOS_PRIVACY_POLICY_MARKDOWN")?;
        let age_attestation_required = parse_optional_bool("LENOS_AGE_ATTESTATION_REQUIRED")?;
        let audit_enabled = parse_bool("LENOS_AUDIT_ENABLED", true)?;
        ...
        let admin = match std::env::var("LENOS_ADMIN_HOST")
            .ok()
            ...
            .filter(|value| !value.is_empty())
        {
            ...
            Some(host) => {
                if host.contains(['/', '\\', '@']) {
                    return Err(ConfigError::InvalidValue(
                        "LENOS_ADMIN_HOST must be an exact authority".to_string(),
                    ));
                }
                let web_dir = std::env::var("LENOS_ADMIN_WEB_DIR")
                    .ok()
                ...
                if let Some(ref dir) = web_dir {
                    if !dir.join("index.html").is_file() {
                        return Err(ConfigError::InvalidValue(format!(
                            "LENOS_ADMIN_WEB_DIR={} does not contain index.html",
                            dir.display()
                        )));
                    }
                }
        ...
        let web_dir = std::env::var("LENOS_WEB_DIR")
            .ok()
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .map(std::path::PathBuf::from);
        let serve_git_web_gui = std::env::var("LENOS_SERVE_GIT_WEB_GUI")
        ...
        if let Some(ref dir) = web_dir {
            if !dir.join("index.html").is_file() {
                return Err(ConfigError::InvalidValue(format!(
                    "LENOS_WEB_DIR={} does not contain index.html",
                    dir.display()
                )));
            }
            tracing::info!(
        ...
        // only fires when someone sets LENOS_GIT_HOOK_HMAC_SECRET to a weak value.
        if std::env::var("LENOS_GIT_HOOK_HMAC_SECRET").is_ok() && git_hook_hmac_secret.len() < 32 {
            return Err(ConfigError::InvalidValue(
                "LENOS_GIT_HOOK_HMAC_SECRET must be at least 32 characters (16 bytes hex)"
                    .to_string(),
            ));
        }
...
mod tests {
    ...
    fn defaults_are_valid() {
        ...
        assert_eq!(
            ...
            lenos_media::config::S3AddressingStyle::Path,
    ...
    fn s3_addressing_style_env_accepts_virtual_and_rejects_invalid_values() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_S3_ADDRESSING_STYLE");

        std::env::set_var("LENOS_S3_ADDRESSING_STYLE", "virtual");
        let configured = Config::from_env()
            .expect("virtual style config")
            .media
            .s3_addressing_style;

        std::env::set_var("LENOS_S3_ADDRESSING_STYLE", "auto");
        let invalid = Config::from_env();

        if let Some(value) = previous {
            std::env::set_var("LENOS_S3_ADDRESSING_STYLE", value);
        } else {
            std::env::remove_var("LENOS_S3_ADDRESSING_STYLE");
        }

        assert_eq!(configured, lenos_media::config::S3AddressingStyle::Virtual);
        assert!(matches!(
            invalid,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("LENOS_S3_ADDRESSING_STYLE must be 'path' or 'virtual'")
        ));
    }
    ...
    fn redis_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_REDIS_POOL_SIZE");

        std::env::set_var("LENOS_REDIS_POOL_SIZE", "32");
        let overridden = Config::from_env().expect("config").redis_pool_size;

        std::env::set_var("LENOS_REDIS_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").redis_pool_size;

        std::env::set_var("LENOS_REDIS_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").redis_pool_size;

        if let Some(value) = previous {
            std::env::set_var("LENOS_REDIS_POOL_SIZE", value);
        } else {
            std::env::remove_var("LENOS_REDIS_POOL_SIZE");
        }
    ...
    fn db_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_DB_POOL_SIZE");

        std::env::set_var("LENOS_DB_POOL_SIZE", "80");
        let overridden = Config::from_env().expect("config").db_pool_size;

        std::env::set_var("LENOS_DB_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").db_pool_size;

        std::env::set_var("LENOS_DB_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").db_pool_size;

        if let Some(value) = previous {
            std::env::set_var("LENOS_DB_POOL_SIZE", value);
        } else {
            std::env::remove_var("LENOS_DB_POOL_SIZE");
        }
    ...
    fn db_read_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_DB_READ_POOL_SIZE");

        std::env::remove_var("LENOS_DB_READ_POOL_SIZE");
        let unset = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("LENOS_DB_READ_POOL_SIZE", "40");
        let overridden = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("LENOS_DB_READ_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("LENOS_DB_READ_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").db_read_pool_size;

        if let Some(value) = previous {
            std::env::set_var("LENOS_DB_READ_POOL_SIZE", value);
        } else {
            std::env::remove_var("LENOS_DB_READ_POOL_SIZE");
        }
    ...
    fn read_database_url_unset_or_blank_is_none() {
        ...
        assert_eq!(
            set.as_deref(),
            Some("postgres://lenos:pw@replica:5432/lenos") // sadscan:disable np.postgres.1
        );
    }
    ...
    fn replica_read_max_age_defaults_off_and_rejects_junk() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_REPLICA_READ_MAX_AGE_MS");
        let previous_old = std::env::var_os("LENOS_REPLICA_HEAD_MAX_AGE_SECS");
        std::env::remove_var("LENOS_REPLICA_HEAD_MAX_AGE_SECS");

        std::env::remove_var("LENOS_REPLICA_READ_MAX_AGE_MS");
        let unset = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("LENOS_REPLICA_READ_MAX_AGE_MS", "1000");
        let set = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("LENOS_REPLICA_READ_MAX_AGE_MS", "0");
        let zero = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("LENOS_REPLICA_READ_MAX_AGE_MS", "soon");
        ...
        std::env::set_var("LENOS_REPLICA_READ_MAX_AGE_MS", "1000");
        std::env::set_var("LENOS_REPLICA_HEAD_MAX_AGE_SECS", "5");
        let old_name = Config::from_env();

        std::env::remove_var("LENOS_REPLICA_HEAD_MAX_AGE_SECS");
        if let Some(value) = previous {
            std::env::set_var("LENOS_REPLICA_READ_MAX_AGE_MS", value);
        } else {
            std::env::remove_var("LENOS_REPLICA_READ_MAX_AGE_MS");
        }
        if let Some(value) = previous_old {
            std::env::set_var("LENOS_REPLICA_HEAD_MAX_AGE_SECS", value);
        }
        ...
        match old_name {
            Err(ConfigError::InvalidValue(message)) => assert!(
                message.contains("LENOS_REPLICA_READ_MAX_AGE_MS"),
    ...
    fn audit_logging_defaults_on_and_accepts_explicit_off() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_AUDIT_ENABLED");
        std::env::remove_var("LENOS_AUDIT_ENABLED");
        assert!(parse_bool("LENOS_AUDIT_ENABLED", true).unwrap());
        std::env::set_var("LENOS_AUDIT_ENABLED", "false");
        assert!(!parse_bool("LENOS_AUDIT_ENABLED", true).unwrap());
        if let Some(value) = previous {
            std::env::set_var("LENOS_AUDIT_ENABLED", value);
        } else {
            std::env::remove_var("LENOS_AUDIT_ENABLED");
        }
    }
    ...
    fn audit_logging_rejects_invalid_boolean() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_AUDIT_ENABLED");
        std::env::set_var("LENOS_AUDIT_ENABLED", "sometimes");
        let result = parse_bool("LENOS_AUDIT_ENABLED", true);
        if let Some(value) = previous {
            std::env::set_var("LENOS_AUDIT_ENABLED", value);
        } else {
            std::env::remove_var("LENOS_AUDIT_ENABLED");
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("LENOS_AUDIT_ENABLED")
        ));
    }
    ...
    fn join_policy_age_attestation_rejects_invalid_boolean() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_AGE_ATTESTATION_REQUIRED");
        std::env::set_var("LENOS_AGE_ATTESTATION_REQUIRED", "sometimes");
        let result = parse_optional_bool("LENOS_AGE_ATTESTATION_REQUIRED");
        if let Some(value) = previous {
            std::env::set_var("LENOS_AGE_ATTESTATION_REQUIRED", value);
        } else {
            std::env::remove_var("LENOS_AGE_ATTESTATION_REQUIRED");
        }
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("LENOS_AGE_ATTESTATION_REQUIRED")
        ));
    }
    ...
    fn rate_limits_can_be_overridden() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_RATE_LIMIT_HUMAN_MESSAGES_PER_MIN", "1001");
        std::env::set_var("LENOS_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN", "1002");
        std::env::set_var("LENOS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC", "1003");

        let config = Config::from_env().expect("config");

        std::env::remove_var("LENOS_RATE_LIMIT_HUMAN_MESSAGES_PER_MIN");
        std::env::remove_var("LENOS_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN");
        std::env::remove_var("LENOS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC");
    ...
    fn rate_limit_overrides_reject_zero() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC", "0");
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("LENOS_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC")
        ));
    }
    ...
    fn relay_operator_pubkeys_parse_dedupe_and_normalize() {
        ...
        std::env::set_var(
            "RELAY_OPERATOR_API_ORIGIN",
            "http://lenos.mesh.bb-production.com",
        );
        let config = Config::from_env().expect("config");
    ...
    #[test]
    fn push_gateway_defaults_to_lenos_and_can_be_disabled() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("LENOS_PUSH_GATEWAY_DELIVERY_URL");
        std::env::remove_var("LENOS_PUSH_GATEWAY_DELIVERY_URL");
        let config = Config::from_env().expect("default config");
        assert_eq!(
            config
                .push_gateway_delivery_url
    ...
    fn invalid_push_gateway_timeout_is_not_silently_defaulted() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_PUSH_GATEWAY_TIMEOUT_MS", "99");
        let result = Config::from_env();
        std::env::remove_var("LENOS_PUSH_GATEWAY_TIMEOUT_MS");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("LENOS_PUSH_GATEWAY_TIMEOUT_MS")
        ));
    }
    ...
    fn invalid_push_executor_key_id_is_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_PUSH_EXECUTOR_KEY_ID", "");
        let result = Config::from_env();
        std::env::remove_var("LENOS_PUSH_EXECUTOR_KEY_ID");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("LENOS_PUSH_EXECUTOR_KEY_ID")
        ));
    }
    ...
    fn huddle_audio_available_can_be_disabled_for_horizontal_scaling() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_HUDDLE_AUDIO_AVAILABLE", "false");
        let config = Config::from_env().expect("config");
        std::env::remove_var("LENOS_HUDDLE_AUDIO_AVAILABLE");
        assert!(
            !config.huddle_audio_available,
            "LENOS_HUDDLE_AUDIO_AVAILABLE=false must disable huddle audio (multi-pod deployments)"
        );
    }
    ...
    fn pairing_relay_url_accepts_websocket_urls_and_rejects_http() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_PAIRING_RELAY_URL", "wss://pairing.lenos.xyz");
        let config = Config::from_env().expect("config");
        assert_eq!(
            config.pairing_relay_url.as_deref(),
            Some("wss://pairing.lenos.xyz")
        );

        std::env::set_var("LENOS_PAIRING_RELAY_URL", "https://pairing.lenos.xyz");
        let result = Config::from_env();
        std::env::remove_var("LENOS_PAIRING_RELAY_URL");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref msg)) if msg.contains("LENOS_PAIRING_RELAY_URL")
        ));
    }
    ...
    fn max_frame_bytes_can_be_configured() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("LENOS_MAX_FRAME_BYTES", "262144");
        let config = Config::from_env().expect("config");
        std::env::remove_var("LENOS_MAX_FRAME_BYTES");
    ...
    fn git_repo_path_is_created_if_missing() {
        ...
        std::env::set_var("LENOS_GIT_REPO_PATH", &nested);
        let result = Config::from_env();
        std::env::remove_var("LENOS_GIT_REPO_PATH");
    ...
    fn git_repo_path_unwritable_returns_error() {
        ...
        assert!(
            matches!(result, Err(ConfigError::InvalidValue(ref msg)) if msg.contains("LENOS_GIT_REPO_PATH")),
            "expected InvalidValue mentioning LENOS_GIT_REPO_PATH, got {result:?}"
        );
    }
}
