//! git-credential-nostr — NIP-98 git credential helper for LenOS.
//!
...
/// Load the NIP-OA owner attestation injected by LenOS Desktop/ACP.
///
...
fn load_auth_tag() -> Result<Option<Tag>, String> {
    let raw = std::env::var("LENOS_AUTH_TAG")
        .ok()
...
fn parse_method(wwwauth: &str) -> Option<HttpMethod> {
    ...
    // Handles variations: `Nostr method="GET", realm="lenos"` and
    // `Nostr method="GET",realm="lenos"` (with or without space after comma).
...
pub fn run() -> i32 {
    ...
    // No Nostr challenge from the server — this isn't a LenOS remote.
    // Exit silently so git falls through to the next credential helper.
    // This check comes FIRST so non-LenOS remotes never hit validation errors.
