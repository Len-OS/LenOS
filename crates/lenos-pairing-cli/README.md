...
CLI tool for testing the [NIP-AB device pairing protocol](../lenos-core/src/pairing/NIP-AB.md) end-to-end. Exercises the full protocol over a live Nostr relay — designed for interop testing and NIP submission, not production use.

## Quick Start

```bash
cargo build --release -p lenos-pairing-cli

# Terminal 1 — source (holds the secret)
./target/release/lenos-pair source --relay wss://relay.damus.io

# Terminal 2 — target (receives the secret)
./target/release/lenos-pair target --show-secret
...
```
lenos-pair source --relay <RELAY_URL> [--nsec <BECH32_NSEC>]
```
...
```
lenos-pair target [--relay <OVERRIDE_URL>] [--show-secret]
```
...
```
lenos-pair test-vectors
```

## Testing Against a Local LenOS Relay

The CLI supports NIP-42 authentication, so it works with LenOS relays out of the box.

### Prerequisites

- Docker running (for Postgres, Redis, etc.)
- LenOS relay built: `cargo build --release -p lenos-relay`
...
screen -dmS relay bash -c "./target/release/lenos-relay 2>&1 | tee /tmp/lenos-relay.log"
...
```bash
# Terminal 1
./target/release/lenos-pair source --relay ws://localhost:3000
