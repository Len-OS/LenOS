//! `lenos-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! lenos-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! lenos-pair target [--relay wss://relay.example.com]
...
use lenos_core::kind::KIND_PAIRING;
use lenos_core::pairing::session::PairingSession;
use lenos_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(lenos_core::pairing::types::AbortReason::SasMismatch)?
        {
