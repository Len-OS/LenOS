//! Embedded SQLx migrations for LenOS.
//!
...
/// Run all pending LenOS database migrations.
...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        assert!(mesh_retention.contains("lenos-mesh-member-status:%"));
        assert!(mesh_retention.contains("lenos-mesh-status"));
        assert!(mesh_retention
            .contains("CREATE TRIGGER trg_events_purge_soft_deleted_lenos_mesh_status"));
        assert!(!migrations[0]
            .sql
            .as_str()
            .contains("purge_soft_deleted_lenos_mesh_status"));
        ...
        assert!(push_gate.contains("'lenos_push_gate:' || NEW.community_id::text"));
        ...
        assert!(ttl_shared.contains("'lenos_channel_ttl:' || NEW.community_id::text"));
