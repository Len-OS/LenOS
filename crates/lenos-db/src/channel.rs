...
// Re-export the canonical enum definitions from lenos-core.
// These live in core (zero I/O deps) so the SDK can share them
// without pulling in sqlx/tokio.
pub use lenos_core::channel::{ChannelType, ChannelVisibility, MemberRole};
...
/// `lenos_channel_ttl:`.
const CHANNEL_MEMBERSHIP_LOCK_NAMESPACE: &str = "lenos_channel_membership:";
...
pub async fn update_channel(
    ...
    if let Some(name) = updates.name.as_mut() {
        *name = lenos_core::channel::canonical_channel_name(name).to_owned();
        if name.is_empty() {
            return Err(DbError::InvalidData("channel name is required".into()));
        }
    }
