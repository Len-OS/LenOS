#![deny(unsafe_code)]
#![warn(missing_docs)]
//! lenos-db — Postgres event store for LenOS.
//!
//! ## Design invariants
...
pub mod partition;
/// LenOS product-feedback sidecar persistence.
...
use lenos_core::{CommunityId, StoredEvent};
            ...
            ReadSessionInner::Replica { tx, writer } => {
                match event::query_events_on(tx, q).await {
                    ...
                    Err(e) => {
                        ...
                        // Deliberately not a `lenos_db_route_decision` event:
                        // the page's route was already recorded, and the
                        // offload metric must stay one-event-per-request.
                        metrics::counter!("lenos_db_read_session_degraded").increment(1);
                        writer.clone()
                    }
                }
            }
...
enum RouteDecision {
    ...
    /// failure emits exactly one `lenos_db_route_decision` event per request
...
impl RoutePredicate {
    ...
    ///
    /// `routing_enabled` is whether `LENOS_REPLICA_READ_MAX_AGE_MS` is set
...
/// Map the configured read budget (`LENOS_REPLICA_READ_MAX_AGE_MS`) to the
...
pub struct DbConfig {
    ...
    /// `LENOS_DB_READ_POOL_SIZE`). `None` inherits [`Self::max_connections`].
    ...
    /// `LENOS_REPLICA_READ_MAX_AGE_MS`). `0` disables bounded-staleness
...
impl Default for DbConfig {
    ...
    fn default() -> Self {
        Self {
            database_url: "postgres://lenos:lenos_dev@localhost:5432/lenos".to_string(), // sadscan:disable np.postgres.1
...
impl Db {
    ...
    /// `lenos.created_at_floor` GUC — this is what makes the replica fence
    ...
    ///
    /// `arm_floor_guard` sets the `lenos.created_at_floor` session GUC on
        ...
        if arm_floor_guard {
            options = options.after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SELECT set_config('lenos.created_at_floor', $1, false)")
    ...
    /// the migration decision. On a relay with `LENOS_AUTO_MIGRATE` off, the
    ...
    > {
        ...
        .await
        {
            ...
            // `lenos_db_route_decision{decision="writer",reason="reader_acquire_timeout"}`
            ...
            // Runbook: correlate with `lenos_db_read_pool_active` / `_max`
            ...
            // sample (LENOS_POOL_METRICS_INTERVAL_SECS, default 10s) while
    ...
    /// the writer's: `lenos_db_read_pool_active / lenos_db_read_pool_max` is
    /// the operator's utilisation signal for tuning `LENOS_DB_READ_POOL_SIZE`,
    ...
    /// request/connection error. lenos-db only reads the durable host map.
    ...
    /// fully-formed [`lenos_core::tenant::TenantContext`] — host included — to
    ...
    ///
    /// Used by the runtime conformance read-seam emitters in `lenos-relay`:
    ...
    /// seam is gated on `LENOS_REPLICA_READ_MAX_AGE_MS` (default off): when
    ...
    ///   `LENOS_REPLICA_HEAD_MAX_AGE_SECS` until it has, proven by a
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![lenos_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = lenos_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
    ...
    /// ordering watermark. LenOS mesh status coordinates also hard-delete their
    ...
    pub async fn replace_parameterized_event(
        ...
        let read_state_t_tag_count = event
            ...
            .count();
        let is_nip_rs = kind_i32 == lenos_core::kind::KIND_READ_STATE as i32
            && d_tag_count == 1
            && has_exact_d_tag
        ...
        let is_lenos_mesh_status = kind_i32 == lenos_core::kind::KIND_BOOKMARK_SET as i32
            && d_tag.starts_with("lenos-mesh-member-status:")
            && event.tags.iter().any(|tag| {
                let parts = tag.as_slice();
                parts.len() == 2 && parts[0] == "k" && parts[1] == "lenos-mesh-status"
            });
        let hard_delete_superseded = is_nip_rs || is_lenos_mesh_status;
        ...
        if existing.is_some() {
            if is_nip_rs {
                ...
                sqlx::query("SELECT set_config('lenos.nip_rs_hard_delete', 'on', true)")
                    .execute(&mut *tx)
                    .await?;
            }
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://lenos:lenos_dev@localhost:5432/lenos";
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let d_tag = "lenos-mesh-member-status:owner-test";
        let tags = vec![
            Tag::parse(["d", d_tag]).expect("d tag"),
            Tag::parse(["k", "lenos-mesh-status"]).expect("k tag"),
        ];
        let base = Timestamp::now().as_secs();
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(lenos_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let kind = lenos_core::kind::KIND_PROJECT as i32;
        let d_tag = "stale-tombstone-project";
        let pubkey = keys.public_key().to_bytes().to_vec();
        let base = Timestamp::now().as_secs();

        let version = |content: &str, offset: u64| {
            EventBuilder::new(Kind::Custom(lenos_core::kind::KIND_PROJECT as u16), content)
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        for commit in [true, false] {
            ...
            let value: String =
                sqlx::query_scalar("SELECT set_config('lenos.nip_rs_hard_delete', 'on', true)")
                    .fetch_one(&mut *tx)
                    .await
            ...
            let leaked: Option<String> = sqlx::query_scalar(
                "SELECT NULLIF(current_setting('lenos.nip_rs_hard_delete', true), '')",
            )
            .fetch_one(&mut *conn)
            .await
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(lenos_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(lenos_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(lenos_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(lenos_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(lenos_core::kind::KIND_READ_STATE as i32)
    ...
    async fn test_usage_metrics_lock_has_single_owner_and_releases_on_drop() {
        ...
        // races any live lenos-relay on the same database (see #3619).
        ...
        // Same key as production (`lenos-relay` USAGE_METRICS_LOCK_KEY) — safe here
    ...
    /// `routing_enabled = false` (LENOS_REPLICA_READ_MAX_AGE_MS unset)
    ...
    /// ceiling, not the writer's — `lenos_db_read_pool_active / _max` is the
    ...
    /// directly, because `lenos_db_route_decision` is emitted by `route_read`
    ...
    async fn routed_fallback_spends_one_acquire_budget_when_aurora_cache_is_cold() {
        ...
        let reasons: std::collections::HashMap<(String, String), u64> = snapshotter
            ...
            .into_vec()
            .into_iter()
            .filter(|(key, ..)| key.key().name() == "lenos_db_route_decision")
            .map(|(key, _, _, value)| {
                let metrics_util::debugging::DebugValue::Counter(n) = value else {
                    panic!("lenos_db_route_decision must be a counter");
                };
    ...
    async fn lazy_reader_pool_still_spawns_fence_probe() {
        ...
        // `lenos.created_at_floor` GUC — `spawn_fence_probe` verifies the
    ...
    async fn created_at_floor_guard_aborts_old_channel_rows_at_commit() {
        ...
        let insert_raw = |ev: nostr::Event, channel_id: Option<Uuid>| {
            ...
            async move {
                ...
                sqlx::query("SELECT set_config('lenos.created_at_floor', $1, true)")
    ...
    async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
        ...
        let effective: String = sqlx::query_scalar("SHOW lenos.created_at_floor")
            .fetch_one(&db.pool)
            .await
    ...
    /// `LENOS_AUTO_MIGRATE` off: an armed GUC with no enforcing trigger must
    ...
    async fn fence_probe_refuses_to_start_without_verified_floor_guard() {
        ...
        // Sabotage B: trigger dropped entirely (the LENOS_AUTO_MIGRATE=off /
