#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `lenos-workflow` — Workflow engine for LenOS.
//!
...
pub struct WorkflowEngine {
    ...
    pub(crate) workflow_cache:
        moka::sync::Cache<(CommunityId, Uuid), Arc<Vec<lenos_db::workflow::WorkflowRecord>>>,
}
...
/// Build a [`executor::TriggerContext`] from a [`lenos_core::StoredEvent`].
///
...
pub fn build_trigger_context(event: &lenos_core::StoredEvent) -> executor::TriggerContext {
    let kind_u32 = event_kind_u32(&event.event);
...
fn trigger_matches_event(trigger: &TriggerDef, kind_u32: u32) -> bool {
    use lenos_core::kind::{KIND_REACTION, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF};
    match trigger {
...
"#;
    ...
    fn schedule_trigger_never_matches_events() {
        ...
        assert!(!trigger_matches_event(
            &trigger,
            lenos_core::kind::KIND_WORKFLOW_TRIGGERED
        ));
    }
    ...
    fn workflow_execution_kinds_do_not_match_any_trigger() {
        ...
        for kind in lenos_core::kind::KIND_WORKFLOW_TRIGGERED
            ..=lenos_core::kind::KIND_WORKFLOW_APPROVAL_DENIED
        {
            assert!(
    ...
    fn make_message_event() -> lenos_core::StoredEvent {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "hello world")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        lenos_core::StoredEvent::new(event, Some(Uuid::new_v4()))
    }

    /// Create a reaction event with an `e` tag pointing to a target message.
    fn make_reaction_event() -> (lenos_core::StoredEvent, String) {
    ...
    fn build_trigger_context_no_channel_id() {
        ...
        let stored = lenos_core::StoredEvent::new(event, None);
        let ctx = build_trigger_context(&stored);
    ...
    fn test_build_trigger_context_reaction_multiple_e_tags() {
        ...
        let stored = lenos_core::StoredEvent::new(event, Some(Uuid::new_v4()));
        let ctx = build_trigger_context(&stored);
    ...
    async fn setup_db() -> lenos_db::Db {
        let database_url = std::env::var("LENOS_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://lenos:lenos_dev@localhost:5432/lenos".to_owned());
        lenos_db::Db::new(&lenos_db::DbConfig {
            database_url,
            ..Default::default()
        })
        .await
    ...
    async fn setup_channel(
        ...
        {
            lenos_db::CreateCommunityWithOwnerResult::Created(rec) => rec.id,
    ...
    fn message_event(channel_id: Uuid) -> lenos_core::StoredEvent {
        let keys = nostr::Keys::generate();
        let event = nostr::EventBuilder::new(nostr::Kind::Custom(9), "hello")
            .sign_with_keys(&keys)
            .expect("sign");
        lenos_core::StoredEvent::new(event, Some(channel_id))
    }
