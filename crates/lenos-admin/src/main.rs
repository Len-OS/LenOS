...
//! LenOS instance administration CLI.
//!
...
#[derive(Parser)]
#[command(name = "lenos-admin", about = "LenOS instance administration")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    Migrate,
    /// Inspect deployment-wide LenOS product feedback.
    ProductFeedback {
        #[command(subcommand)]
    ...
    ReconcileChannels {
        ...
        /// LENOS_RELAY_PRIVATE_KEY env var. If neither is set, generates
...
async fn main() {
    ...
    // Redis TLS connection without this. Mirrors lenos-relay's main().
...
async fn run(cli: Cli) -> Result<i32> {
    match cli.command {
        Command::GenerateKey => {
            ...
            println!("\nSet LENOS_PRIVATE_KEY to the secret key to use this identity.");
            Ok(0)
        }
        Command::Migrate => {
...
async fn cmd_remove_member(pubkey_arg: String, role_filter: Option<String>) -> Result<i32> {
    ...
    let tenant = resolve_admin_tenant(&db).await?;
    use lenos_db::relay_members::RemoveResult;
...
async fn publish_membership_list_with_bump(
    ...
    tracing::info!(
        ...
        ts,
        "NIP-43 membership list published by lenos-admin"
    );
    Ok(())
}
...
///
/// `LENOS_RELAY_PRIVATE_KEY` is required — the CLI signs kind:13534 events.
async fn connect_member_services() -> Result<(Db, Arc<PubSubManager>, Keys)> {
    let db = connect_db().await?;

    let relay_keypair = {
        let hex = std::env::var("LENOS_RELAY_PRIVATE_KEY").map_err(|_| {
            anyhow::anyhow!(
                "LENOS_RELAY_PRIVATE_KEY is required for add-member/remove-member.\n\
                 The relay must have a stable signing key to publish kind:13534 events."
            )
        })?;
        Keys::parse(&hex).map_err(|e| anyhow::anyhow!("invalid LENOS_RELAY_PRIVATE_KEY: {e}"))?
    };
...
///
/// `lenos-admin` runs inside the relay container (`compose exec relay
/// lenos-admin …`), so it shares the relay's `RELAY_URL` and resolves the same
...
async fn resolve_admin_tenant(db: &Db) -> Result<TenantContext> {
    ...
    // resolution do (`lenos_core::tenant::relay_url_authority`): host plus an
    ...
    // the helper keeps lenos-admin byte-identical to the community startup seeds.
    ...
    let record = db.lookup_community_by_host(&host).await?.ok_or_else(|| {
        anyhow::anyhow!(
            "RELAY_URL host '{host}' is not mapped to a community.\n\
             lenos-admin operates on the configured relay's community; ensure the \
...
async fn reconcile_channels(relay_key_arg: Option<String>) -> Result<()> {
    use lenos_core::kind::KIND_NIP29_GROUP_ADMINS;
    use lenos_db::event::EventQuery;

    let db = connect_db().await?;

    // Resolve relay signing key: arg > env > ephemeral
    let relay_keys = match relay_key_arg.or_else(|| std::env::var("LENOS_RELAY_PRIVATE_KEY").ok()) {
        Some(key_hex) => {
        ...
        None => {
            ...
            eprintln!("Pass --relay-key or set LENOS_RELAY_PRIVATE_KEY for production use.");
            k
        }
    };
