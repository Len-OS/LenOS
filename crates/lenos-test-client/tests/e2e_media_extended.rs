...
fn relay_server_domain() -> String {
    let url = reqwest::Url::parse(&relay_http_url()).expect("relay URL");
    let host = url.host_str().expect("relay host");
    match url.port() {
...
fn tiny_png() -> Vec<u8> {
    include_bytes!("../../lenos-media/tests/fixtures/android/sanitized/bitmap-srgb-sanitized.png")
        .to_vec()
}
...
async fn test_auth_server_tag_correct() {
    ...
    let sha256 = hex::encode(Sha256::digest(&jpeg));
    let now = Timestamp::now().as_secs();
    let server_domain = relay_server_domain();
    let auth = sign_custom_auth(
        &keys,
        24242,
        "Upload test",
        vec![
            ...
            Tag::parse(["x", &sha256]).unwrap(),
            Tag::parse(["expiration", &(now + 300).to_string()]).unwrap(),
            Tag::parse(["server", &server_domain]).unwrap(),
        ],
    );
    let resp = upload_with_auth(&client, &auth, &sha256, &jpeg).await;
...
async fn test_ws_valid_imeta() {
    ...
    let create_event = EventBuilder::new(Kind::from(9007), "")
        ...
        .sign_with_keys(&keys)
        .unwrap();
    let create_resp = post_event(&http, &keys, &create_event).await;
    ...
    let mut client = LenOSTestClient::connect(&relay_ws_url(), &keys)
        .await
        .unwrap();
