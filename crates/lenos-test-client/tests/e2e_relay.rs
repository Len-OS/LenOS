//! End-to-end integration tests for the LenOS relay.
//!
...
fn test_owner_keys() -> Keys {
    std::env::var("LENOS_TEST_OWNER_PRIVATE_KEY")
        .ok()
...
async fn test_connect_and_authenticate() {
    ...
    let client = LenOSTestClient::connect(&url, &keys)
        .await
...
async fn test_invite_code_minted_for_one_host_fails_on_another() {
    let host_b = format!("unmapped-invites-{}.example", Uuid::new_v4().simple());
    let owner = test_owner_keys();
    let joiner = Keys::generate();
    ...
    assert_eq!(claim_response.status(), reqwest::StatusCode::NOT_FOUND);
}
...
async fn test_send_event_and_receive_via_subscription() {
    ...
    let mut client_a = LenOSTestClient::connect(&url, &keys_a)
        .await
    ...
    let mut client_b = LenOSTestClient::connect(&url, &keys_b)
        .await
...
async fn test_unauthenticated_rejected() {
    ...
    let mut client = LenOSTestClient::connect_unauthenticated(&url)
        .await
...
async fn test_multiple_concurrent_clients() {
    ...
    let mut clients: Vec<LenOSTestClient> =
        futures_util::future::try_join_all(keys.iter().map(|k| LenOSTestClient::connect(&url, k)))
            .await
...
async fn test_pubkey_mismatch_rejected() {
    ...
    let mut client = LenOSTestClient::connect(&url, &keys_a)
        .await
...
async fn test_nip29_put_user_default_policy_allows() {
    ...
    let mut ws = LenOSTestClient::connect(&url, &channel_owner_keys)
        .await
...
async fn test_nip29_put_user_self_add_bypasses_policy() {
    ...
    // Connect as agent.
    let mut ws = LenOSTestClient::connect(&url, &agent_keys)
        .await
...
async fn test_membership_notification_emitted_on_add() {
    ...
    let mut agent_client = LenOSTestClient::connect(&url, &agent_keys)
        .await
        .expect("connect as agent");
...
async fn create_private_channel_ws(client: &mut LenOSTestClient, keys: &Keys) -> String {
...
async fn test_private_channel_any_member_can_invite() {
    ...
    let mut owner_client = LenOSTestClient::connect(&url, &owner_keys)
        .await
        .expect("connect as owner");
    ...
    let mut member_client = LenOSTestClient::connect(&url, &member_keys)
        .await
...
async fn test_private_channel_non_member_cannot_invite() {
    ...
    let mut outsider_client = LenOSTestClient::connect(&url, &outsider_keys)
        .await
...
async fn test_reply_ingest_pushes_live_thread_summary() {
    ...
    async fn recv_summary(client: &mut LenOSTestClient) -> nostr::Event {
        loop {
            match client
                .recv_event(Duration::from_secs(5))
                .await
...
async fn test_nip29_put_user_cannot_demote_owner() {
    ...
    let mut ws = LenOSTestClient::connect(&url, &attacker_keys)
        .await
...
async fn test_nip29_owner_demotion_recovery_paths() {
    ...
    let put_user = |signer: Keys, target_hex: String, role: &'static str| {
        ...
        let url = url.clone();
        async move {
            let mut ws = LenOSTestClient::connect(&url, &signer)
                .await
                .expect("connect");
    ...
    let mut ws = LenOSTestClient::connect(&url, &victim_keys)
        .await
        .expect("connect");
...
async fn test_nip29_put_user_without_role_tag_preserves_role() {
    ...
    let mut ws = LenOSTestClient::connect(&url, &owner_a)
        .await
    ...
    let mut ws = LenOSTestClient::connect(&url, &owner_b)
        .await
...
async fn test_nip29_relay_rejects_role_change_by_unprivileged_actor() {
    ...
    let mut ws = LenOSTestClient::connect(&url, &attacker)
        .await
        .expect("connect as attacker");
...
async fn test_nip29_relay_rejects_last_owner_self_demotion() {
    ...
    let mut ws = LenOSTestClient::connect(&url, &owner)
        .await
        .expect("connect as owner");
