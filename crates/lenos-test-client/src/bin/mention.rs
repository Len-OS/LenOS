//! Send an @mention event to a LenOS channel targeting a specific pubkey.
...
async fn main() -> anyhow::Result<()> {
    ...
    let url = std::env::var("LENOS_RELAY_URL").unwrap_or_else(|_| "ws://localhost:3000".into());
    let keys = Keys::generate();
    println!("Sender pubkey: {}", keys.public_key().to_hex());

    let mut client = LenOSTestClient::connect(&url, &keys).await?;
