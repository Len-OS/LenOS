#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `lenos-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
...
//!         └── dynamic SUBSCRIBE lenos:{community}:channel:{id} / lenos:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: lenos_core::CommunityId,
...
/// Central pub/sub manager for a LenOS relay instance.
...
impl PubSubManager {
    ...
    /// `lenos:{community}:channel:{id}` / `lenos:{community}:global`, and
