//! LenOS V2 kind number registry.
//!
//! This module is the authoritative source for LenOS kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by LenOS today.
...
/// LenOS custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: LenOS command kinds (Pure Nostr plan)
...
/// `crates/lenos-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `lenos-db`.
///
...
/// LenOS product feedback submission. Accepted at ingest, sidecarred to the
...
// LenOS community moderation commands (mod-signed, processed like 9030-series:
...
/// LenOS: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — LenOS requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// A huddle session recording is available (relay-signed, carries S3 URL).
pub const KIND_HUDDLE_RECORDING: u32 = 48104;
...
/// or the LenOS workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a LenOS command kind that requires transactional execution.
...
/// Safe: all LenOS kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all LenOS kind constants fit in nostr's u16-backed Kind.
const _: () = assert!(KIND_AUTH <= u16::MAX as u32);
const _: () = assert!(KIND_CANVAS <= u16::MAX as u32);
const _: () = assert!(KIND_HUDDLE_RECORDING <= u16::MAX as u32);
