#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `lenos-core` — zero-I/O foundation types for the LenOS relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other LenOS crates depend on this one.
...
pub mod invite;
/// LenOS kind number registry — custom event type constants.
pub mod kind;
