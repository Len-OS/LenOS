...
//!
//! This module defines the core data types for the LenOS git permission system.
//! The permission model: channel role = repo role; `lenos-protect` tags on
...
/// announcement with no `lenos-channel` binding.
///
...
/// Maximum number of `lenos-protect` tags per repo.
...
/// A single protection rule parsed from a `lenos-protect` tag on kind:30617.
///
/// Format: `["lenos-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `lenos-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for RuleParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TooFewValues => write!(f, "lenos-protect tag needs pattern + at least one rule"),
...
/// Parse a single `lenos-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["lenos-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("lenos-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `lenos-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `lenos-protect` tag, returning unknown rules for logging.
...
/// Parse all `lenos-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `lenos-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("lenos-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `lenos-protect` tag matches.
