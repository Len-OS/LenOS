# Trace Schema (`lenos-conformance`)
...
- **`sanitized_error { reason }`** where `reason ∈ { restricted,
  ...
  `crates/lenos-relay/src/conformance/mod.rs`) goes non-exhaustive and
  CI catches it.
...
| File | What it emits |
|------|---------------|
| `crates/lenos-relay/src/conformance/mod.rs` | helpers + `EmitGuard` + `sanitized_reason_for` |
| `crates/lenos-relay/src/conformance/tracers.rs` | `NoopTracer` (prod default), `JsonlTracer` |
| `crates/lenos-relay/src/handlers/ingest.rs` | `AuthCheck`, `WriteInsert`, `WriteInsertGlobal`, `WriteDuplicate`, outer-wrapper `SanitizedError` |
| `crates/lenos-relay/src/handlers/req.rs` | **held back** — additive patch for integration onto Max's req.rs work |

## Where the checker lives

| File | What it does |
|------|--------------|
| `crates/lenos-conformance/src/lib.rs` | schema + `Tracer` trait |
| `crates/lenos-conformance/src/transitions.rs` | spec `Next` re-implementation |
| `crates/lenos-conformance/src/checker.rs` | replay engine: `IllegalTransition` / `StateMismatch` / `NonInterference` / `CoverageBreach` |
...
`crates/lenos-conformance/src/checker.rs::tests` proving the gate bites
