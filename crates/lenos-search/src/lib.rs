#![deny(unsafe_code)]
#![warn(missing_docs)]
//! LenOS search — community-scoped Postgres full-text search over LenOS events.
//!
...
//! by `lenos-db`. The relay refetches canonical events through `lenos-db`'s
...
pub use lenos_core::CommunityId;
