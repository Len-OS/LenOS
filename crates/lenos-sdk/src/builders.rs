...
///
/// Shortcodes are case-insensitive in LenOS's relay-global set; lowercase
...
/// Build a LenOS-native delete event (kind 9005).
...
/// Build a LenOS-native delete event (kind 9005) with optional moderation metadata.
...
pub const CUSTOM_EMOJI_SET_D_TAG: &str = "lenos:custom-emoji";
...
/// "lenos:custom-emoji")`. Replaces the caller's prior set. The workspace
...
pub fn build_update_channel(
    ...
    if name
        .map(lenos_core::channel::canonical_channel_name)
...
/// Maximum byte length of a project `lenos-channel` tag value.
pub const PROJECT_CHANNEL_MAX: usize = 256;
/// Maximum byte length of a project `lenos-visibility` tag value.
...
///
/// Rules enforced (matches relay `lenos-db` ingest logic):
...
/// 7. Singleton metadata: each of `name`, `description`, `lenos-channel`,
///    `lenos-visibility` appears at most once.
/// 8. Metadata byte lengths: `name` ≤256, `description` ≤2048,
///    `lenos-channel` ≤256, `lenos-visibility` ≤256.
...
pub fn build_project(
    ...
    if let Some(ch) = channel {
        uuid::Uuid::parse_str(ch).map_err(|_| {
            SdkError::InvalidInput(format!("lenos-channel must be a valid UUID (got {ch:?})"))
        })?;
    }
    ...
    if let Some(vis) = visibility {
        if vis != "listed" && vis != "unlisted" {
            return Err(SdkError::InvalidInput(format!(
                "lenos-visibility must be 'listed' or 'unlisted' (got {vis:?})"
            )));
        }
    }
    ...
    if let Some(ch) = channel {
        tags.push(tag(&["lenos-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(tag(&["lenos-visibility", vis])?);
    }
...
mod tests {
    ...
    fn agent_observer_frame_happy_path() {
        ...
        let recipient = keys();
        let agent = keys();
        let encrypted = lenos_core::observer::encrypt_observer_payload(
            &sender,
    ...
    fn repo_announcement_with_tags_preserves_metadata_and_canonicalizes_d() {
        let tags = vec![
            ...
            Tag::parse(["lenos-channel", "channel-id"]).unwrap(),
        ...
        assert!(has_tag(&ev, "lenos-channel", "channel-id"));
    ...
    fn git_patch_rejects_whitespace_only_content() {
        // Regression: a failed `git format-patch | lenos patches send
    ...
    fn build_project_emitted_envelope_has_correct_shape() {
        ...
        // name, description, lenos-channel, lenos-visibility present.
        ...
        assert_eq!(a_tags[0][1], format!("30617:{OWNER64}:lenos"));
    }
