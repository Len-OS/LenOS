...
//!
//! Run it against the docker-compose MinIO (creds `lenos_dev`/`lenos_dev_secret`,
//! bucket `lenos-media`, endpoint `http://localhost:9000`):
//!
//! ```bash
//! docker compose up -d minio minio-init
//! cargo test -p lenos-media --test static_creds_minio -- --ignored
//! ```
//!
//! Overridable via `LENOS_S3_ENDPOINT` / `LENOS_S3_ACCESS_KEY` /
//! `LENOS_S3_SECRET_KEY` / `LENOS_S3_BUCKET` / `LENOS_S3_REGION` /
//! `LENOS_S3_ADDRESSING_STYLE`. The default remains `path` for MinIO.

use lenos_media::config::MediaConfig;
use lenos_media::storage::MediaStorage;
...
fn minio_config() -> MediaConfig {
    MediaConfig {
        ...
        s3_region: std::env::var("LENOS_S3_REGION").unwrap_or_else(|_| "us-east-1".to_string()),
        s3_addressing_style: std::env::var("LENOS_S3_ADDRESSING_STYLE")
