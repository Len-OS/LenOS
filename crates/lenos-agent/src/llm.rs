...
/// consent for `databricks-cli` covers lenos-agent too.
...
async fn openrouter_post(
    ...
    for attempt in 0..MAX_RETRIES {
        let resp = match http
            .post(url)
            .header("content-type", "application/json")
            .header("HTTP-Referer", "https://github.com/block/lenos")
            .header("X-OpenRouter-Title", "LenOS")
            .bearer_auth(bearer)
            .body(body_bytes.clone())
            .send()
            .await
...
fn apply_openrouter_mutations(
    ...
    if let Some(obj) = body.as_object_mut() {
        ...
        // `prompt_caching` (`LENOS_AGENT_PROMPT_CACHING`) for the same reason as
...
mod tests {
    ...
    /// `LENOS_AGENT_PROMPT_CACHING=0` must reach the OpenRouter `anthropic/*`
    ...
    fn openrouter_body_caching_disabled_emits_no_cache_control() {
        ...
        assert!(
            ...
            "LENOS_AGENT_PROMPT_CACHING=0 must suppress every breakpoint: {body}"
        );
    ...
    async fn openrouter_post_sends_attribution_headers() {
        ...
        let header_str = headers
            ...
            .to_lowercase();
        assert!(
            header_str.contains("http-referer: https://github.com/block/lenos"),
            "got: {header_str}"
        );
        assert!(
            header_str.contains("x-openrouter-title: lenos"),
            "got: {header_str}"
        );
    }
