...
struct Session {
    ...
    /// `_goose/unstable/session/update` usage notification so lenos-acp's
...
/// `lenos-agent auth <provider>` — run the interactive auth flow for a
...
async fn auth_subcommand(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match provider {
        Some("databricks" | "databricks_v2" | "databricks-v2") => {
            ...
            eprintln!("Authenticated. Token cached under ~/.config/lenos-agent/oauth/databricks/.");
            Ok(())
        }
        Some(other) => Err(format!("auth: unknown provider {other:?}").into()),
        None => Err("auth: provider required (try: lenos-agent auth databricks)".into()),
    }
}
    ...
    wire::send(
        ...
        wire::ok(
            ...
            json!({
                ...
                "agentInfo": { "name": "lenos-agent", "version": env!("CARGO_PKG_VERSION") },
            }),
        ),
    )
    .await;
...
async fn run_prompt(app: Arc<App>, id: Value, params: Value, wire_tx: WireSender) {
    ...
    // BEFORE sending the session/prompt response. lenos-acp's UsageTracker
    ...
    if turn_input_tokens.is_some() || turn_output_tokens.is_some() {
            ...
            accumulated
        {
            ...
            let mut update = serde_json::json!({
                ...
                // contextLimit: 0 (lenos-agent has no context limit tracking).
