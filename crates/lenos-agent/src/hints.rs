...
mod tests {
    ...
    fn build_hints_section_combined() {
        ...
        let skill_dir = cwd.join(".agents/skills/lenos-cli");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: lenos-cli\ndescription: CLI reference for LenOS managed agents\n---\nUse `lenos` to manage agents.\n",
        )
        .unwrap();
        ...
        assert!(
            result.contains("lenos-cli: CLI reference for LenOS managed agents"),
            "missing skill bullet"
        );
        // Body must NOT be inlined — lazy loading only.
        assert!(
            !result.contains("Use `lenos` to manage agents."),
