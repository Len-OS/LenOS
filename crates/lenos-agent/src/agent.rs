...
const REPLY_GUARD_SERVER: &str = "lenos-agent";
...
const REPLY_GUARD_NAG: &str = "You are about to end this turn without calling `lenos messages send`. \
...
/// Whether `call` is a recognized attempt to publish a reply to LenOS.
///
...
fn is_lenos_reply_call(call: &ToolCall, mcp: &McpRegistry) -> bool {
    mcp.has(&call.name) && !mcp.is_hook(&call.name) && is_reply_shaped(&call.name, &call.arguments)
}

/// Whether a tool name and arguments have the shape of a LenOS publish command.
///
/// Split from [`is_lenos_reply_call`] only so the matcher is testable without a
...
/// (`echo "lenos messages send"`) matches. Missing a real post is the expensive
...
impl RunCtx<'_> {
    pub async fn run(&mut self, prompt: Vec<ContentBlock>) -> Result<StopReason, AgentError> {
        ...
        *self.turn_total_state = TurnTotalState::Unseen;
        ...
        // successful publish. See `is_lenos_reply_call`.
        let mut lenos_reply_call_seen = false;
        ...
        loop {
            ...
            if self.cfg.require_reply && !lenos_reply_call_seen {
                lenos_reply_call_seen = calls.iter().any(|c| is_lenos_reply_call(c, self.mcp));
            }
            self.history.push(HistoryItem::Assistant {
                text: response.text,
...
fn synthetic_hook_id(hook: &str, server: &str, ordinal: u64) -> String {
    format!("lenos_hook_{hook}_{server}_{ordinal}")
}
...
mod tests {
    ...
    fn reply_shape_matches_documented_send_forms() {
        for cmd in [
            "lenos messages send --channel X --content Y",
            "lenos --relay wss://r messages send --channel X --content Y",
            "/abs/path/lenos messages send",
            "printf 'hi' | lenos messages send --content -",
            "lenos messages send-diff --diff -",
            "lenos reactions add --event E --emoji +",
            // Assembled through another shell: rev 3's tokenizer missed this.
            r#"sh -c "lenos messages send --channel X""#,
        ] {
            assert!(
    ...
    fn reply_shape_rejects_non_reply_commands() {
        for cmd in [
            "lenos messages get --channel X",
            "lenos channels list",
            "lenos reactions remove --event E",
            "lenos pr open --title T",
            "lenos social publish --content hi",
            "lenos notes set --name n",
    ...
    fn reply_shape_requires_the_qname_separator() {
        let args = json!({ "command": "lenos messages send --channel X" });
        for name in [
            "dev__powershell",
            "dev__noshell",
            "shell",
        ...
        assert!(is_reply_shaped("lenos-dev-mcp__shell", &args));
    }
    ...
    fn reply_shape_reads_only_the_command_field() {
        assert!(!is_reply_shaped(
            "dev__shell",
            &json!({ "description": "lenos messages send --channel X" })
        ));
        assert!(!is_reply_shaped(
            "dev__shell",
            &json!({ "workdir": "lenos messages send" })
        ));
