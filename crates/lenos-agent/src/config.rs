...
///
/// Set via `LENOS_AGENT_THINKING_EFFORT` (`none|minimal|low|medium|high|xhigh|max`).
    ...
    if is_manual_budget_model(model) {
        ...
        if budget < MIN_ANSWER_TOKENS {
            tracing::warn!(
                ...
                level_budget,
                headroom,
                "LENOS_AGENT_THINKING_EFFORT: max_output_tokens too small to fit thinking budget + answer headroom; omitting thinking fields"
            );
            return (None, None);
        }
        (
...
pub fn clamp_adaptive_effort(model: &str, effort: ThinkingEffort) -> ThinkingEffort {
    ...
    if clamped != effort {
        tracing::warn!(
            ...
            "LENOS_AGENT_THINKING_EFFORT is not available for this model; clamping to highest supported level"
        );
    }
    clamped
}
...
fn resolve_openai_effort(
    ...
    tracing::warn!(
        ...
        "LENOS_AGENT_THINKING_EFFORT={} is not supported by this OpenAI model; using nearest supported level",
    ...
    match openai_efforts_for_model(model) {
        ...
        None if effort == ThinkingEffort::Max => {
            tracing::warn!(
                requested = "max",
                resolved = "xhigh",
                "LENOS_AGENT_THINKING_EFFORT=max not confirmed for unknown OpenAI model; clamping to xhigh"
            );
            ThinkingEffort::XHigh
        }
        None => effort,
...
pub fn normalize_effort_for_anthropic_route(effort: ThinkingEffort) -> Option<ThinkingEffort> {
    match effort {
        ThinkingEffort::None | ThinkingEffort::Minimal => {
            tracing::warn!(
                requested = effort.openai_effort_str(),
                "LENOS_AGENT_THINKING_EFFORT={} is not expressible as an Anthropic thinking level; \
...
/// Parse `LENOS_AGENT_THINKING_EFFORT`. Pure (env-free) for testability.
pub fn parse_thinking_effort(raw: Option<&str>) -> Result<Option<ThinkingEffort>, String> {
    match raw.map(|s| s.trim().to_ascii_lowercase()).as_deref() {
        ...
        Some(other) => Err(format!(
            "config: LENOS_AGENT_THINKING_EFFORT={other} not supported (use none|minimal|low|medium|high|xhigh|max)"
        )),
    }
}
...
/// Text is governed by the much smaller `LENOS_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
/// 10 KB. Tunable via `LENOS_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
const DEFAULT_SYSTEM_PROMPT: &str =
    "You are lenos-agent. Use the provided tools to act. Tool calls are your only output.";
...
pub struct Config {
    ...
    /// Set via `LENOS_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
    ...
    /// `LENOS_AGENT_MAX_CONTEXT_TOKENS`.
    ...
    /// recognized attempt to post to LenOS. Default off; opt in per agent with
    /// `LENOS_AGENT_REQUIRE_REPLY=1`.
    ///
    ...
    /// Set by LenOS's relay-mesh provider via
    /// `LENOS_AGENT_PREFER_MESH_FOR_AUTO=1`; other providers keep their
    /// existing `auto` semantics.
    pub prefer_mesh_for_auto: bool,
    pub hints_enabled: bool,
    /// Thinking/reasoning effort level. `None` = use provider default (no
    /// thinking config sent). Set via `LENOS_AGENT_THINKING_EFFORT`.
    ...
    /// disable with `LENOS_AGENT_PROMPT_CACHING=0`. Consulted on every route that
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let provider = resolve_provider(
            env("LENOS_AGENT_PROVIDER").as_deref(),
        ...
        let lenos_agent_model = env("LENOS_AGENT_MODEL");
        ...
        let (api_key, model, base_url, openai_api) = match provider {
            ...
            Provider::Databricks | Provider::DatabricksV2 => (
                ...
                resolve_model(lenos_agent_model.as_deref(), databricks_model.as_deref())
        ...
        let system_prompt = match (env("LENOS_AGENT_SYSTEM_PROMPT"), env("LENOS_AGENT_SYSTEM_PROMPT_FILE")) {
            (Some(_), Some(_)) => return Err(
                "config: LENOS_AGENT_SYSTEM_PROMPT and LENOS_AGENT_SYSTEM_PROMPT_FILE are mutually exclusive".into()),
            (Some(s), _) => s,
        ...
        let cfg = Config {
            ...
            openai_api,
            prefer_mesh_for_auto: parse_env("LENOS_AGENT_PREFER_MESH_FOR_AUTO", 0u8)? != 0,
            max_rounds: parse_env("LENOS_AGENT_MAX_ROUNDS", 0)?,
            max_output_tokens: parse_env("LENOS_AGENT_MAX_OUTPUT_TOKENS", 32_768)?,
            llm_timeout: Duration::from_secs(parse_env("LENOS_AGENT_LLM_TIMEOUT_SECS", 240)?),
            tool_timeout: Duration::from_secs(parse_env("LENOS_AGENT_TOOL_TIMEOUT_SECS", 660)?),
            mcp_init_timeout: Duration::from_secs(parse_env(
                "LENOS_AGENT_MCP_INIT_TIMEOUT_SECS",
                30,
            )?),
            mcp_max_restart_attempts: parse_env("LENOS_AGENT_MCP_RESTART_MAX_ATTEMPTS", 3u32)?,
            mcp_restart_base_ms: parse_env("LENOS_AGENT_MCP_RESTART_BASE_MS", 500u64)?,
            mcp_restart_max_ms: parse_env("LENOS_AGENT_MCP_RESTART_MAX_MS", 30_000u64)?,
            max_sessions: parse_env("LENOS_AGENT_MAX_SESSIONS", usize::MAX)?,
            max_line_bytes: parse_env("LENOS_AGENT_MAX_LINE_BYTES", 4 * 1024 * 1024)?,
            max_history_bytes: parse_env("LENOS_AGENT_MAX_HISTORY_BYTES", 16 * 1024 * 1024)?,
            max_tool_result_text_bytes: parse_env(
                "LENOS_AGENT_MAX_TOOL_RESULT_TEXT_BYTES",
                DEFAULT_TOOL_RESULT_TEXT_BYTES,
            )?,
            max_context_tokens: parse_env("LENOS_AGENT_MAX_CONTEXT_TOKENS", 200_000u64)?,
            max_handoffs: parse_env("LENOS_AGENT_MAX_HANDOFFS", 10)?,
            max_parallel_tools: parse_env("LENOS_AGENT_MAX_PARALLEL_TOOLS", 8usize)?,
            hook_timeout: Duration::from_millis(parse_env("LENOS_AGENT_HOOK_TIMEOUT_MS", 2500u64)?),
            stop_max_rejections: parse_env("LENOS_AGENT_STOP_MAX_REJECTIONS", 3u32)?,
            require_reply: parse_env("LENOS_AGENT_REQUIRE_REPLY", 0u8)? != 0,
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if self.max_output_tokens < 1 {
            return Err("config: LENOS_AGENT_MAX_OUTPUT_TOKENS must be >= 1".into());
        }
        if self.max_context_tokens <= u64::from(self.max_output_tokens) {
            return Err(format!(
                "config: LENOS_AGENT_MAX_CONTEXT_TOKENS ({}) must be > LENOS_AGENT_MAX_OUTPUT_TOKENS ({}) — the context window must leave room for the response",
                self.max_context_tokens, self.max_output_tokens
            ));
        }
        if self.max_history_bytes < MIN_HISTORY_BYTES {
            return Err(format!(
                "config: LENOS_AGENT_MAX_HISTORY_BYTES must be >= {MIN_HISTORY_BYTES}"
            ));
        }
        if self.max_history_bytes < MAX_PROMPT_BYTES {
            return Err(format!(
                "config: LENOS_AGENT_MAX_HISTORY_BYTES ({}) must be >= MAX_PROMPT_BYTES ({MAX_PROMPT_BYTES})",
                self.max_history_bytes
            ));
        }
        if self.max_line_bytes < MIN_LINE_BYTES {
            return Err(format!(
                "config: LENOS_AGENT_MAX_LINE_BYTES must be >= {MIN_LINE_BYTES}"
            ));
        }
        ...
        {
            return Err(format!(
                "config: LENOS_AGENT_MAX_TOOL_RESULT_TEXT_BYTES must be in {MIN_TOOL_RESULT_TEXT_BYTES}..={MAX_TOOL_RESULT_BYTES}"
            ));
        }
        if self.llm_timeout < MIN_TIMEOUT {
            return Err("config: LENOS_AGENT_LLM_TIMEOUT_SECS must be >= 1".into());
        }
        if self.tool_timeout < MIN_TIMEOUT {
            return Err("config: LENOS_AGENT_TOOL_TIMEOUT_SECS must be >= 1".into());
        }
        if self.mcp_init_timeout < MIN_TIMEOUT {
            return Err("config: LENOS_AGENT_MCP_INIT_TIMEOUT_SECS must be >= 1".into());
        }
        if self.max_parallel_tools < 1 {
            return Err("config: LENOS_AGENT_MAX_PARALLEL_TOOLS must be >= 1".into());
        }
        if self.mcp_max_restart_attempts < 1 {
            return Err("config: LENOS_AGENT_MCP_RESTART_MAX_ATTEMPTS must be >= 1".into());
        }
        if self.mcp_restart_base_ms < 1 {
            return Err("config: LENOS_AGENT_MCP_RESTART_BASE_MS must be >= 1".into());
        }
        if self.mcp_restart_max_ms < self.mcp_restart_base_ms {
            return Err(
                "config: LENOS_AGENT_MCP_RESTART_MAX_MS must be >= LENOS_AGENT_MCP_RESTART_BASE_MS"
                    .into(),
            );
        }
        ...
        if let Some(effort) = self.thinking_effort {
            ...
            {
                return Err(format!(
                    "config: LENOS_AGENT_THINKING_EFFORT={} is not valid for Anthropic providers \
...
/// Returns the first present value. `explicit_override` (LENOS_AGENT_MODEL,
...
mod tests {
    ...
    fn resolve_provider_errors_when_provider_env_absent() {
        // No implicit inference — absent LENOS_AGENT_PROVIDER is an error.
        let err = resolve_provider(None, None, None, None).unwrap_err();
        assert!(err.contains("LENOS_AGENT_PROVIDER is required"), "{err}");
    }
    ...
    fn resolve_provider_requires_databricks_host_and_model_for_fallback() {
        ...
        // When LENOS_AGENT_PROVIDER=databricks, resolve_provider succeeds regardless
    ...
    fn resolve_provider_unsupported_error_preserves_user_casing() {
        ...
        assert!(err.contains("LENOS_AGENT_PROVIDER=OpenAIish"));
    }
    ...
    fn parse_thinking_effort_rejects_unknown_value() {
        ...
        assert!(err.contains("LENOS_AGENT_THINKING_EFFORT=extreme"), "{err}");
        assert!(
    ...
    fn validate_rejects_none_effort_for_anthropic() {
        ...
        assert!(
            err.contains("LENOS_AGENT_THINKING_EFFORT=none"),
    ...
    fn validate_rejects_minimal_effort_for_anthropic() {
        ...
        let err = cfg.validate().unwrap_err();
        assert!(err.contains("LENOS_AGENT_THINKING_EFFORT=minimal"), "{err}");
    ...
    // the TS test in `lenosAgentConfig.test.mjs`) and verifies that this Rust
    ...
    /// `getProviderEffortConfig` in `lenosAgentConfig.ts`.
    ///
