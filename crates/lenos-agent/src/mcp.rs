...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "SSL_CERT_FILE",
    "SSL_CERT_DIR",
    // LenOS identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). LENOS_PRIVATE_KEY
    // and LENOS_RELAY_URL are kept for the lenos CLI. LENOS_AUTH_TAG is a
    ...
    // spawn lenos-agent without declaring it.
    "LENOS_ACP_DISPLAY_NAME",
];
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_lenos_owner_attestation() {
        assert!(PASSTHROUGH_ENV.contains(&"LENOS_AUTH_TAG"));
    }
