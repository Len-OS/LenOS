...
/// one. `expected_run_id` must match the run id lenos-agent advertised via
...
/// session use `model_id` instead of the configured `LENOS_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) are unexpected — lenos-agent
...
/// method goose uses for custom usage and status events.  Used by lenos-agent
/// to emit the `usage_update` payload so lenos-acp's `UsageTracker` can treat
/// lenos-agent and goose symmetrically.
