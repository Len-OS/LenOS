...
/// `cache_namespace` is the directory under `~/.config/lenos-agent/oauth/`
...
pub struct PkceOAuthConfig {
    ...
    /// `~/.config/lenos-agent/oauth/<cache_namespace>/`. Production code
...
impl PkceOAuthTokenSource {
    ...
    pub(crate) async fn try_bearer_no_browser(&self) -> Result<String, AgentError> {
        ...
        Err(AgentError::LlmAuth(
            "no cached Databricks token; run `lenos-agent auth databricks` first".into(),
        ))
    }
}
    ...
    let app = Router::new().route(
        ...
        get(move |Query(params): Query<HashMap<String, String>>| {
            ...
            async move {
                ...
                match result {
                    Ok(_) => Html(
                        "<h2>LenOS: signed in</h2><p>You can close this window.</p>".to_string(),
                    ),
                    Err(e) => Html(format!("<h2>LenOS auth failed</h2><pre>{e}</pre>")),
                }
            }
        }),
    );
...
mod tests {
    ...
    fn cache_path_includes_namespace_and_hash() {
        ...
        assert!(p.to_string_lossy().contains("/lenos-agent/oauth/demo/"));
