...
//! exercised manually via the `lenos-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `lenos-agent` binary with
...
use lenos_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real lenos-agent binary with `DATABRICKS_TOKEN` set (so the
...
impl AgentHarness {
    async fn spawn_provider(provider: &str, base_url: &str, model: &str) -> Self {
        ...
        cmd.env("LENOS_AGENT_PROVIDER", provider)
