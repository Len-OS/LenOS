...
impl Harness {
    async fn spawn_with_env(base_url: &str, extra: &[(&str, &str)]) -> Self {
        let bin = env!("CARGO_BIN_EXE_lenos-agent");
        let mut cmd = tokio::process::Command::new(bin);
        cmd.env("LENOS_AGENT_PROVIDER", "openai")
            .env("OPENAI_COMPAT_API_KEY", "test")
            .env("OPENAI_COMPAT_MODEL", "fake-model")
            .env("OPENAI_COMPAT_BASE_URL", base_url)
            .env("LENOS_AGENT_LLM_TIMEOUT_SECS", "5")
            .env("LENOS_AGENT_TOOL_TIMEOUT_SECS", "5")
            .env("LENOS_AGENT_MAX_ROUNDS", "8")
            .env("LENOS_AGENT_MCP_INIT_TIMEOUT_SECS", "2");
        for (k, v) in extra {
            cmd.env(k, v);
        }
        cmd.stdin(Stdio::piped())
            ...
            .stderr(Stdio::piped())
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn lenos-agent");
...
async fn history_budget_evicts_old_turns() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("LENOS_AGENT_MAX_HISTORY_BYTES", &BUDGET.to_string()),
            ("LENOS_AGENT_MAX_HANDOFFS", "0"), // exercise truncation, not handoff
        ],
    )
    .await;
    let sid = init_session(&mut h, json!([])).await;
...
async fn hook_stop_blocks_premature_end() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("LENOS_AGENT_STOP_MAX_REJECTIONS", "10"),
        ],
    )
    .await;
...
async fn hook_stop_budget_exhausted() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("LENOS_AGENT_STOP_MAX_REJECTIONS", "1"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn hook_stop_consecutive_end_turn_uses_rejection_budget() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("LENOS_AGENT_STOP_MAX_REJECTIONS", "2"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn hook_post_compact_injects_after_handoff() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("LENOS_AGENT_MAX_HISTORY_BYTES", &(1024 * 1024).to_string()),
            // Allow at least one handoff.
            ("LENOS_AGENT_MAX_HANDOFFS", "3"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn handoff_summary_prompt_includes_full_history_within_context_budget() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("LENOS_AGENT_MAX_CONTEXT_TOKENS", "10000"),
            ("LENOS_AGENT_MAX_OUTPUT_TOKENS", "1000"),
            ("LENOS_AGENT_MAX_HANDOFFS", "3"),
            (
                "LENOS_AGENT_MAX_HISTORY_BYTES",
...
async fn token_usage_over_budget_triggers_handoff() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("LENOS_AGENT_MAX_CONTEXT_TOKENS", "1000"),
            ("LENOS_AGENT_MAX_OUTPUT_TOKENS", "100"),
            ("LENOS_AGENT_MAX_HANDOFFS", "3"),
...
/// `_Stop` hook that takes longer than `LENOS_AGENT_HOOK_TIMEOUT_MS`
...
async fn hook_stop_timeout_failopen() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("LENOS_AGENT_HOOK_TIMEOUT_MS", "200"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
/// sends `notifications/cancelled` to the MCP server. With lenos-dev-mcp,
...
async fn cancel_kills_inflight_tool_via_mcp_notification() {
    // lenos-dev-mcp is a separate crate; locate its binary relative to
    // the lenos-agent test binary (they share the same target dir).
    let self_bin = std::path::PathBuf::from(env!("CARGO_BIN_EXE_lenos-agent"));
    let dev_mcp_bin = self_bin.parent().unwrap().join("lenos-dev-mcp");
    ...
    if !dev_mcp_is_executable {
        eprintln!(
            "SKIP: lenos-dev-mcp not built at {}; run `cargo build -p lenos-dev-mcp` first",
            dev_mcp_bin.display()
        );
        return;
    }
...
/// any MCP server (not just lenos-dev-mcp) when a session is cancelled
...
// ---------------------------------------------------------------------------
// Reply guard (`LENOS_AGENT_REQUIRE_REPLY`)
//
// The guard reminds the model to publish when a turn is about to end without
// any recognized attempt to post to LenOS. It rides the existing `_Stop` gate
...
/// in-process guard (`server: "lenos-agent"`) at the `_Stop` hook point — the
                ...
                .filter(|m| {
                    m["role"] == "tool"
                        && serde_json::from_str::<Value>(m["content"].as_str().unwrap_or(""))
                            .map(|p| p["hook"] == "_Stop" && p["server"] == "lenos-agent")
                            .unwrap_or(false)
                })
                .count()
        })
...
fn openai_shell_send(id: &str) -> Value {
    openai_tool_call(
        id,
        "fake__shell",
        json!({ "command": "lenos messages send --channel c --content hi" }),
    )
}
...
/// `LENOS_AGENT_REQUIRE_REPLY=0` is off too — the toggle is numeric, so a
/// literal `0` must not read as "set, therefore on".
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reply_guard_explicit_zero_is_off() {
    let llm = spawn_capturing_llm(vec![openai_text("done"), openai_text("unexpected")]).await;
    let mut h = Harness::spawn_with_env(&llm.url, &[("LENOS_AGENT_REQUIRE_REPLY", "0")]).await;
    let sid = init_session(&mut h, json!([])).await;
...
async fn reply_guard_nags_twice_then_lets_the_turn_end() {
    ...
    .await;
    let mut h = Harness::spawn_with_env(&llm.url, &[("LENOS_AGENT_REQUIRE_REPLY", "1")]).await;
    let sid = init_session(&mut h, json!([])).await;
    ...
    let nag = msgs
        ...
        .find(|p| p["server"] == "lenos-agent")
        .expect("reminder body");
    let text = nag["text"].as_str().unwrap_or("");
    assert!(
        text.contains("lenos messages send"),
...
async fn reply_guard_ignores_calls_lost_to_the_turn_cap() {
    ...
    calls.push(json!({
        ...
        "function": {
            "name": "fake__shell",
            "arguments": json!({ "command": "lenos messages send --channel c --content hi" })
                .to_string(),
        },
    }));
...
async fn reply_guard_off_when_stop_budget_is_zero() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("LENOS_AGENT_REQUIRE_REPLY", "1"),
            ("LENOS_AGENT_STOP_MAX_REJECTIONS", "0"),
        ],
    )
    .await;
    let sid = init_session(&mut h, json!([])).await;
...
fn reply_guard_rejects_unparseable_toggle() {
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_lenos-agent"))
        .env("LENOS_AGENT_PROVIDER", "openai")
        .env("OPENAI_COMPAT_API_KEY", "test")
        .env("OPENAI_COMPAT_MODEL", "fake-model")
        .env("LENOS_AGENT_REQUIRE_REPLY", "true")
        .stdin(Stdio::null())
        .output()
        .expect("run lenos-agent");
    assert!(
        !out.status.success(),
    ...
    assert!(
        stderr.contains("LENOS_AGENT_REQUIRE_REPLY"),
