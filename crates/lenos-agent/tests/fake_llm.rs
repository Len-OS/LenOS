//! Integration test: fake LLM HTTP server + lenos-agent subprocess.
//!
...
async fn init_session(h: &mut Harness) -> String {
    ...
    assert_eq!(r["result"]["agentInfo"]["name"], "lenos-agent");
...
async fn rejects_oversized_line() {
    ...
    cmd.env("LENOS_AGENT_PROVIDER", "openai")
        ...
        .env("LENOS_AGENT_MAX_LINE_BYTES", "256")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true);
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are lenos-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are lenos-agent"),
...
/// Wait for the `activeRunId` advert lenos-agent emits at prompt start and
...
/// completion_tokens). lenos-agent maps these to `accumulatedInputTokens` /
...
/// lenos-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// lenos-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, lenos-agent must
...
/// (so token counts are observed), lenos-agent must still emit the usage
