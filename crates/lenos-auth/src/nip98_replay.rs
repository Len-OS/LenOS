...
//!
//! ```ignore
//! let pubkey = lenos_auth::verify_nip98_event(json, url, method, body)?;
//! if !replay.try_mark(&ctx, &event_id, lenos_auth::DEFAULT_REPLAY_TTL_SECS).await? {
...
///
/// The production implementation lives in `lenos-pubsub` (Redis `SET NX EX`).
...
/// `lenos:{community}:nip98:{event_id_hex}`.
///
...
pub fn nip98_replay_key_for_scope(scope: &str, event_id: &EventId) -> String {
    format!("lenos:{scope}:nip98:{}", event_id.to_hex())
}
...
mod tests {
    ...
    fn key_includes_community_prefix() {
        ...
        let expected_prefix = format!("lenos:{}:nip98:", ctx.community());
        assert!(
