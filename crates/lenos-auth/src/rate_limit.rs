...
//! `lenos-relay` / `lenos-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `lenos-relay` / `lenos-pubsub`.
...
/// key is community-prefixed (`lenos:{community}:ratelimit:{pubkey}:{suffix}`). The
...
/// `lenos:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Redis key for IP-based rate limit: `lenos:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("lenos:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("lenos:{}:ratelimit:", ctx.community());
        assert!(
