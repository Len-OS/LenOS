#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `lenos-auth` — Authentication and authorization for the LenOS relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("lenos-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("lenos-test-key:{username}");
