...
//!
//! Defines [`ChannelAccessChecker`] so `lenos-auth` can enforce access
//! without depending on `lenos-db` directly.
...
///
/// Implemented by the database layer (`lenos-db`) in production. The `lenos-auth`
