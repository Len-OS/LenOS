...
/// name (e.g., `lenos-acp --verbose models`) are not supported.
...
/// Timeout for `lenos-acp authenticate`. Browser-based vendor auth can require
...
async fn publish_presence(
    ...
    use lenos_core::kind::KIND_PRESENCE_UPDATE;
...
///
/// Priority:
/// 1. `LENOS_AUTH_TAG` env var — NIP-OA attestation signed by the owner.
///    Verified against the agent's own pubkey to extract the owner pubkey.
/// 2. `--agent-owner` CLI flag / `LENOS_ACP_AGENT_OWNER` env var.
fn resolve_agent_owner(config: &Config) -> Option<String> {
    // Try LENOS_AUTH_TAG first (NIP-OA attestation).
    if let Ok(auth_tag) = std::env::var("LENOS_AUTH_TAG") {
        if !auth_tag.is_empty() {
            let agent_pk = config.keys.public_key();
            match lenos_sdk::nip_oa::verify_auth_tag(&auth_tag, &agent_pk) {
                Ok(owner_pk) => {
                    let owner_hex = owner_pk.to_hex().to_ascii_lowercase();
                    tracing::info!("owner resolved from LENOS_AUTH_TAG: {owner_hex}");
                    return Some(owner_hex);
                }
                Err(e) => {
                    tracing::warn!("LENOS_AUTH_TAG verification failed: {e} — falling back");
                }
            }
        }
    }
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match lenos_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in lenos-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing lenos-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (lenos-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match lenos_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = lenos_core::verify_event(&event) {
...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("lenos_acp=info")),
        )
        .compact()
        .init();
    ...
    // model, or provider), it spawns lenos-acp with LENOS_ACP_SETUP_PAYLOAD set.
    ...
    {
        tracing::info!("lenos-acp: setup payload present, entering setup-listener mode");
        return setup_mode::run_setup_listener(config, payload).await;
    }

    tracing::info!("lenos-acp starting: {}", config.summary());
    ...
    // Parse LENOS_AUTH_TAG into a nostr::Tag for NIP-OA relay membership delegation.
    let relay_auth_tag: Option<nostr::Tag> = std::env::var("LENOS_AUTH_TAG")
        .ok()
        .filter(|s| !s.is_empty())
        .and_then(|s| lenos_sdk::nip_oa::parse_auth_tag(&s).ok());
    ...
    // Priority: LENOS_AUTH_TAG (NIP-OA attestation) → --agent-owner flag.
            ...
            RespondTo::OwnerOnly => {
                tracing::warn!(
                    "respond-to=owner-only but no owner is set — all events will be \
                     dropped. Set LENOS_AUTH_TAG or --agent-owner, or use --respond-to=anyone."
                );
            }
            RespondTo::Allowlist => {
                tracing::warn!(
    ...
    let runtime_start_nonce = std::env::var("LENOS_MANAGED_AGENT_START_NONCE").unwrap_or_default();
    ...
    if !config.memory_enabled {
        tracing::info!(
            target: "engram::core",
            "NIP-AE core memory injection disabled (re-enable by removing --no-memory / LENOS_ACP_NO_MEMORY)"
        );
    }
    ...
    loop {
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                lenos_event = relay.next_event() => {
                    let _ = result_rx; // end split borrow before relay handling
                    match lenos_event {
                        Some(lenos_event) => {
                            let kind_u32 = lenos_event.event.kind.as_u16() as u32;

                            if kind_u32 == KIND_MEMBER_ADDED_NOTIFICATION
                                || kind_u32 == KIND_MEMBER_REMOVED_NOTIFICATION
                            {
                                let ch = lenos_event.channel_id;
                                let ts = lenos_event.event.created_at.as_secs();
                                let eid = lenos_event.event.id.to_hex();
                            ...
                            if config.ignore_self && lenos_event.event.pubkey.to_hex() == pubkey_hex {
                                tracing::debug!(channel_id = %lenos_event.channel_id, "dropping self-authored event");
                                continue;
                            }
                            ...
                            if is_shutdown {
                                ...
                                if let Some(owner) = owner {
                                    if lenos_event.event.pubkey.to_hex() == *owner {
                                        tracing::info!(
                                            channel_id = %lenos_event.channel_id,
                                            sender = %lenos_event.event.pubkey.to_hex(),
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if lenos_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(lenos_event.channel_id);
                                            tracing::info!(
                                                channel_id = %lenos_event.channel_id,
                                                invalidated,
                            ...
                            {
                                let author = lenos_event.event.pubkey.to_hex();
                                // DM hardening: resolve channel type (fail-closed
                                // to DM) so allowlist/anyone modes cannot be
                                // exercised by non-owner authors inside DMs.
                                let is_dm =
                                    is_dm_channel(lenos_event.channel_id, &ctx.channel_info).await;
                                ...
                                if !allowed {
                                    tracing::debug!(
                                        channel_id = %lenos_event.channel_id,
                                        author = %lenos_event.event.pubkey.to_hex(),
                            ...
                            let matched = filter::match_event(&lenos_event.event, lenos_event.channel_id, &rules, &pubkey_hex).await;
                            let prompt_tag = match matched {
                                Some(m) => m.prompt_tag,
                                None => {
                                    tracing::debug!(channel_id = %lenos_event.channel_id, kind = lenos_event.event.kind.as_u16(), "event matched no rule — dropping");
                                    continue;
                                }
                            };
                            // Capture author pubkey before queue.push() moves
                            // lenos_event.event (needed for mode gate below).
                            let author_hex = lenos_event.event.pubkey.to_hex();
                            let event_id_hex = lenos_event.event.id.to_hex();
                            ...
                            let event_for_steer = lenos_event.event.clone();
                            let prompt_tag_for_steer = prompt_tag.clone();
                            let accepted = queue.push(QueuedEvent {
                                channel_id: lenos_event.channel_id,
                                event: lenos_event.event,
                            ...
                            if accepted && queue.is_channel_in_flight(lenos_event.channel_id) {
    ...
    tracing::info!("lenos-acp stopped");
    Ok(())
}
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[LenOS event: …]`
...
fn handle_prompt_result(
    ...
    // which live in lenos-agent's session state and are what `llm: (model) …`
...
mod agent_draft_prompt_tests {
    ...
    fn shared_base_prompt_teaches_portable_agent_drafts() {
        ...
        assert!(prompt.contains("lenos agents draft-create"));
    ...
    fn shared_base_prompt_teaches_real_newlines_for_multiline_messages() {
        ...
        assert!(prompt.contains("lenos messages send ... --content -"));
    }
    ...
    fn shared_base_prompt_teaches_single_command_mentions_and_preflight() {
        ...
        assert!(prompt
            .contains("add them explicitly with `lenos channels add-member` only when authorized"));
...
fn default_heartbeat_prompt() -> String {
    ...
    format!(
        "[System: Heartbeat]\nTime: {now}\n\n\
         ...
         Your tasks:\n\
         1. Run `lenos feed get --types needs_action` to check for pending workflow approvals or\n\
            high-priority requests addressed to you.\n\
         2. Run `lenos feed get --types mentions` to check for unanswered @mentions.\n\
         3. If you find actionable items, address them using the appropriate CLI commands\n\
            (e.g., `lenos workflows approve --token <UUID>`, `lenos messages send`,\n\
            `lenos messages send --reply-to <event-id>`).\n\
         4. If there are no pending actions or mentions, end your turn immediately.\n\n\
         Do not run `lenos channels list` or `lenos messages search` unless you have a specific reason.\n\
...
/// `lenos-acp auth-methods` — spawn an adapter, initialize it, print authMethods.
...
/// `lenos-acp authenticate` — invoke one adapter-owned auth method.
...
fn build_mcp_servers(config: &Config) -> Vec<McpServer> {
    ...
    vec![McpServer {
        ...
        env: {
            let mut env = vec![
                EnvVar {
                    name: "LENOS_RELAY_URL".into(),
                    value: config.relay_url.clone(),
                },
                EnvVar {
                    name: "LENOS_PRIVATE_KEY".into(),
            ...
            // Forward LENOS_AUTH_TAG (NIP-OA owner attestation credential)
            // so the MCP server can attach it to every signed event.
            if let Ok(auth_tag) = std::env::var("LENOS_AUTH_TAG") {
                if !auth_tag.is_empty() {
                    env.push(EnvVar {
                        name: "LENOS_AUTH_TAG".into(),
                        value: auth_tag,
                    });
                }
            }
            ...
            if let Ok(display_name) = std::env::var("LENOS_ACP_DISPLAY_NAME") {
                if !display_name.is_empty() {
                    env.push(EnvVar {
                        name: "LENOS_ACP_DISPLAY_NAME".into(),
                        value: display_name,
                    });
                }
            }
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            config_path: std::path::PathBuf::from("./lenos-acp.toml"),
    ...
    fn session_new_mcp_server_has_required_fields() {
        ...
        assert!(
            names.contains(&"LENOS_RELAY_URL"),
            "missing LENOS_RELAY_URL; got {names:?}"
        );
        assert!(
            names.contains(&"LENOS_PRIVATE_KEY"),
            "missing LENOS_PRIVATE_KEY; got {names:?}"
        );
    }

    #[test]
    fn session_new_mcp_server_forwards_lenos_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("LENOS_AUTH_TAG", "test-attestation-tag");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("LENOS_AUTH_TAG");

        let server = &servers[0];
        let auth_tag_env = server.env.iter().find(|e| e.name == "LENOS_AUTH_TAG");
        assert!(
            auth_tag_env.is_some(),
            "LENOS_AUTH_TAG should be forwarded when set"
        );
        assert_eq!(auth_tag_env.unwrap().value, "test-attestation-tag");
    }

    #[test]
    fn session_new_mcp_server_skips_empty_lenos_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("LENOS_AUTH_TAG", "");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("LENOS_AUTH_TAG");
    ...
    fn test_display_name_set_is_forwarded_to_mcp_server() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("LENOS_ACP_DISPLAY_NAME", "Duncan");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("LENOS_ACP_DISPLAY_NAME");

        let entry = servers[0]
            .env
            .iter()
            .find(|e| e.name == "LENOS_ACP_DISPLAY_NAME");
        assert_eq!(
    ...
    fn test_display_name_unset_omits_the_key_entirely() {
        ...
        assert!(
            !servers[0]
                .env
                .iter()
                .any(|e| e.name == "LENOS_ACP_DISPLAY_NAME"),
    ...
    fn test_display_name_empty_omits_the_key_entirely() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("LENOS_ACP_DISPLAY_NAME", "");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("LENOS_ACP_DISPLAY_NAME");
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[LenOS event: …]…<huge>"}].
        ...
        // [LenOS event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[LenOS event: @mention]"))
            .unwrap();
        assert!(
