...
//! model, or credentials), it spawns `lenos-acp` with a setup-mode payload
...
///
/// lenos-acp is a separate crate and must NOT depend on desktop types —
...
pub(crate) const SETUP_PAYLOAD_ENV_VAR: &str = "LENOS_ACP_SETUP_PAYLOAD";
...
/// (`managed_agents/readiness.rs`). Desktop serializes it; `lenos-acp`
...
impl SetupPayload {
    ...
    /// 2. A fenced `lenos:config-nudge` sentinel block containing the structured
    ...
    fn nudge_body(&self) -> String {
        let prose = if self.requirements.is_empty() {
            ...
            let footer = if has_doctor_requirement {
                ...
                // Mixed: some LenOS-managed fields, some external config.
                "Open Edit Agent in the LenOS app for the LenOS-managed fields; fix the external CLI config files manually and restart the agent.".to_string()
            } else {
                // All LenOS-managed — original footer unchanged.
                "Open Edit Agent in the LenOS app to set these.".to_string()
            };
...
pub(crate) async fn run_setup_listener(config: Config, payload: SetupPayload) -> Result<()> {
    tracing::info!(
        ...
        "lenos-acp entering setup mode"
    );

    let pubkey_hex = config.keys.public_key().to_hex();

    // Parse LENOS_AUTH_TAG for relay membership / NIP-OA.
    ...
    loop {
        let Some(lenos_event) = relay.next_event().await else {
        ...
        {
            handle_setup_membership(&mut relay, &lenos_event, &config, &rules, &channel_ids).await;
            continue;
        }
        ...
        let is_dm = crate::is_dm_channel(lenos_event.channel_id, &channel_info).await;
...
async fn handle_setup_membership(
    ...
    lenos_event: &crate::relay::LenOSEvent,
    config: &Config,
    rules: &[filter::SubscriptionRule],
    _initial_channel_ids: &[Uuid],
) {
    let kind_u32 = lenos_event.event.kind.as_u16() as u32;
    let channel_id = lenos_event.channel_id;
...
async fn publish_setup_nudge(
    ...
    let event_builder = lenos_sdk::build_message(
        channel_id,
        &body,
        thread_ref.as_ref(),
...
mod tests {
    ...
    fn setup_payload_deserializes_git_bash_requirement() {
        let payload: SetupPayload = serde_json::from_str(
            r#"{"agent_name":"LenOS Agent","agent_pubkey":"test","requirements":[{"surface":"git_bash"}]}"#,
        )
        .unwrap();
        assert!(matches!(
    ...
    fn nudge_body_git_bash_copy_points_to_agent_runtimes() {
        let payload = SetupPayload {
            agent_name: "LenOS Agent".to_string(),
            agent_pubkey: "test".to_string(),
    ...
    fn nudge_body_mixed_requirements_uses_split_footer() {
        // Mixed list: one LenOS-managed env key + one external config invalid.
    ...
    #[test]
    fn nudge_body_all_lenos_managed_retains_original_footer() {
        // Pure LenOS-managed requirements → original "Open Edit Agent" footer unchanged.
        let payload = SetupPayload {
            agent_name: "Fizz".to_string(),
            agent_pubkey: "test".to_string(),
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("Open Edit Agent in the LenOS app to set these."),
    ...
    fn nudge_body_contains_sentinel_block() {
        // The body must end with a ```lenos:config-nudge fence so the desktop
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("```lenos:config-nudge\n"),
    ...
    fn nudge_body_sentinel_round_trips_payload() {
        ...
        let fence_open = "```lenos:config-nudge\n";
        let fence_close = "\n```";
        let start = body
            .rfind(fence_open)
    ...
    //
    // These tests prove the desktop→lenos-acp→sentinel path preserves the
    // `availability` field. They simulate what actually happens at runtime:
    // desktop serializes a `cli_login` JSON blob → lenos-acp parses it via
