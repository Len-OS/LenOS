...
use lenos_core::engram::{conversation_key, d_tag, select_head, validate_and_decrypt, Body};
...
///
/// Wording is from Tyler's brief: "No core memory found. Use `lenos mem`
/// to create a core memory. Ask your user about yourself."
pub const ONBOARDING_NUDGE: &str = "No core memory found. \
Use `lenos mem set core \"…\"` to create one (it will hold your identity, \
...
async fn fetch_core_body(
    ...
    let d = d_tag(&k_c, lenos_core::engram::CORE_SLUG);
...
mod tests {
    use super::*;
    use lenos_core::engram::{build_event, Body};
    use serde_json::json;
