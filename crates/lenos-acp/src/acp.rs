...
pub struct AcpClient {
    ...
    ///
    /// Both goose and lenos-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/lenos-agent emit `activeRunId: null` at end of turn).
    ...
    /// deltas. Both goose and lenos-agent emit this notification; goose gates
    /// on client capability advertisement, lenos-agent emits unconditionally.
...
///
/// Returns `Some(json_string)` when `has_generated_codex_config` is true (LenOS injected a
...
pub(crate) fn build_codex_config_env(
    ...
    // Without an explicit LenOS-generated overlay signal, skip the merge entirely.
...
/// `_meta.goose.activeRunId`. Emitted by goose and lenos-agent only.
...
/// `outcome` value meaning the steer was applied to the turn LenOS is waiting
/// on, which therefore keeps running.
const STEER_OUTCOME_INJECTED: &str = "injected";

/// `outcome` value meaning the turn LenOS was steering had already finished, so
...
fn build_client_capabilities() -> serde_json::Value {
    serde_json::json!({
        // Signal to ACP adapters that LenOS can hand users to terminal-native
        // auth flows. Adapters decide which auth methods to expose; LenOS does
...
impl AcpClient {
    ...
    pub async fn spawn(
        ...
        // Per-persona env vars (e.g., GOOSE_PROVIDER, LENOS_AGENT_PROVIDER).
    ...
    ///   (ACP protocol v2, lenos-agent, goose unused).
    ...
    /// `["/cmd args", "<lenos context>"]` instead of one wrapped block.
    ...
    ///
    /// Both goose and lenos-agent emit `session_info_update`; other agents
                            ...
                            if let Some((steer_id, _, _)) = pending_steer.as_ref() {
                                if *id == serde_json::json!(*steer_id) {
                                    ...
                                    let ack = if let Some(error) = msg.get("error") {
                                        ...
                                        // turn LenOS awaits is still running —
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and lenos-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and lenos-agent both
...
///
/// - **`Field`** — bare `systemPrompt` field (ACP protocol v2, lenos-agent).
...
mod tests {
    ...
    fn session_new_mcp_server_has_required_fields() {
        ...
        assert_eq!(
            ...
            Some("LENOS_RELAY_URL")
        );
    }

    #[test]
    fn session_prompt_request_format() {
        let prompt_text = "[LenOS @mention]\nChannel: test\nFrom: npub1...\nMessage: hello";
        let msg = serde_json::json!({
            "jsonrpc": "2.0",
            "id": 2u64,
            "method": "session/prompt",
    ...
    /// LenOS-owned Hermes processes get the configured-MCP isolation default,
    ...
    async fn goose_system_prompt_request_uses_append_contract() {
        ...
        assert_eq!(received["params"]["key"], "lenos");
    ...
    async fn session_new_full_sends_session_title_in_meta_when_some() {
        ...
        let resp = client
            .session_new_full("/tmp", vec![], None, Some("Fizz · #lenos-dev"))
            .await
            .expect("session_new_full should succeed");
    ...
    /// `SessionInfoUpdate` schema), matching what goose and lenos-agent
    /// emit on the wire.
    ...
    fn capture_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("lenos-acp-steer-capture");
    ...
    /// Test 1b: no `_meta` at all (goose, lenos-agent, any older adapter) must
    ...
    /// LenOS maps `SteerAck::Success` to `queue.remove_event`, so decoding
    ...
    /// the turn LenOS was steering had already ended and the adapter began a
    ...
    fn build_codex_config_env_generated_only_single_entry_with_signal_true_merges_with_parent() {
        // No persona: LenOS injects one CODEX_CONFIG; signal=true.
    ...
    fn build_codex_config_env_persona_only_signal_false_returns_none() {
        // Persona set CODEX_CONFIG; LenOS did not inject a generated overlay (signal=false).
