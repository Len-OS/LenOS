...
pub struct PromptContext {
    ...
    /// `--no-memory` / `LENOS_ACP_NO_MEMORY`.
    pub memory_enabled: bool,
    /// Harness identity string for NIP-AM `harness` field. Derived from the
    /// configured `agent_command` at startup (e.g. `"goose"`, `"lenos-agent"`).
...
pub async fn run_prompt_task(
    ...
    //
    // Operator opt-out: `--no-memory` / `LENOS_ACP_NO_MEMORY` skips the fetch.
    ...
    // (`prompt[0].text.startsWith("/")`) fires; the wrapped LenOS context
                ...
                mode = rx => {
                    ...
                    if agent.acp.has_in_flight_prompt() {
                            ...
                            .await
                        {
                            Ok(stop_reason) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(lenos_core::agent_turn_metric::StopReason::Cancelled),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                            ...
                            Err(error) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(lenos_core::agent_turn_metric::StopReason::Error),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                        ...
                        publish_agent_turn_metric(
                            ...
                            &session_id,
                            &turn_id,
                            Some(lenos_core::agent_turn_metric::StopReason::EndTurn),
                        )
                        .await;
                        send_prompt_result(
                            &result_tx,
...
pub(crate) async fn fetch_channel_info(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            lenos_core::kind::KIND_NIP29_GROUP_METADATA as u16,
        ))
...
async fn fetch_canvas_section(channel_id: Uuid, rest: &RestClient) -> Option<String> {
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(lenos_core::kind::KIND_CANVAS as u16))
...
pub(crate) fn canvas_section_from_query_response(
    ...
    if event.kind != nostr::Kind::Custom(lenos_core::kind::KIND_CANVAS as u16) {
        tracing::warn!(
            target: "canvas::fetch",
            channel = %channel_uuid,
...
pub(crate) fn render_canvas_section(event_id: &str, timestamp: &str, channel_uuid: &str) -> String {
    format!(
        "[Channel Canvas]\n\
         ...
         Fetch current content with: lenos canvas get --channel {channel_uuid}"
    )
}
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let replies_filter = nostr::Filter::new()
        .kinds([
            nostr::Kind::Custom(lenos_core::kind::KIND_STREAM_MESSAGE as u16),
            nostr::Kind::Custom(lenos_core::kind::KIND_STREAM_MESSAGE_V2 as u16),
        ])
...
fn acp_stop_to_core(r: &StopReason) -> lenos_core::agent_turn_metric::StopReason {
    use lenos_core::agent_turn_metric::StopReason as CoreStop;
    match r {
...
pub(crate) fn build_turn_metric_counts(
    ...
    Option<lenos_core::agent_turn_metric::TokenCounts>,
    Option<lenos_core::agent_turn_metric::TokenCounts>,
) {
    use lenos_core::agent_turn_metric::TokenCounts;
    ...
    let turn_counts = if usage.delta_reliable {
        Some(TokenCounts {
            ...
            // lenos-agent does not emit a cache-write count on the wire today;
    ...
    let cumulative_counts = Some(TokenCounts {
        ...
        // reported this field (e.g. goose or older lenos-agent sessions).
...
async fn publish_agent_turn_metric(
    ...
    session_id: &str,
    turn_id: &str,
    stop_reason: Option<lenos_core::agent_turn_metric::StopReason>,
) {
    use lenos_core::agent_turn_metric::AgentTurnMetricPayload;
    ...
    let ciphertext = match lenos_core::agent_turn_metric::encrypt_agent_turn_metric(
        &ctx.agent_keys,
        owner_pk,
        &payload,
    ) {
    ...
    let event = match EventBuilder::new(
        Kind::Custom(lenos_core::kind::KIND_AGENT_TURN_METRIC as u16),
        ciphertext,
    )
    .tags([
...
///
/// Builds a reaction event with `lenos_sdk::build_reaction`, signs it with
...
pub(crate) async fn reaction_add(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match lenos_sdk::build_reaction(target_id, emoji) {
        Ok(b) => b,
        Err(e) => {
...
pub(crate) async fn post_failure_notice(
    ...
    let thread_ref = thread_tags.root_event_id.as_deref().and_then(|root| {
        ...
        let parent_id = thread_tags
            ...
            .unwrap_or(root_id);
        Some(lenos_sdk::ThreadRef {
            root_event_id: root_id,
            parent_event_id: parent_id,
        })
    });
    let builder =
        match lenos_sdk::build_message(channel_id, content, thread_ref.as_ref(), &[], false, &[]) {
            Ok(b) => b,
            Err(e) => {
...
pub(crate) async fn reaction_remove(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match lenos_sdk::build_remove_reaction(target_id) {
        Ok(b) => b,
        Err(e) => {
...
mod tests {
    ...
    fn goose_uses_system_prompt_only_after_custom_method_succeeds() {
        ...
        assert!(has_system_prompt_support(2, "lenos-agent", None));
        ...
        assert_eq!(
            session_new_system_prompt(false, 2, "lenos-agent", Some("instructions")),
    ...
    fn test_framed_system_prompt_absolute_cwd_prepends_workspace_before_base() {
        let framed = framed_system_prompt("/Users/me/.lenos", Some("base text"), None)
            .expect("base yields Some");
        assert!(
            framed.starts_with("[Workspace]\n"),
            "workspace section must lead: {framed}"
        );
        assert!(framed.contains("`/Users/me/.lenos`"));
        assert!(
            framed.contains("\n\n[Base]\nbase text"),
    ...
    fn test_framed_system_prompt_persona_only_omits_workspace() {
        ...
        let framed = framed_system_prompt("/Users/me/.lenos", None, Some("persona text"))
    ...
    /// A lenos-agent-commanded context must not panic — verifies the harness
    /// field flows through encrypt/sign without error.
    #[tokio::test]
    async fn test_publish_agent_turn_metric_lenos_agent_harness_name() {
        let agent_keys = nostr::Keys::generate();
        let owner_keys = nostr::Keys::generate();
        let mut ctx = make_prompt_context_with_owner(&agent_keys, owner_keys.public_key());
        ctx.harness_name = "lenos-agent".to_string();
        let usage = crate::usage::TurnUsage {
            session_id: "sess-ba".to_string(),
            turn_seq: 1,
            delta_reliable: false, // first turn from lenos-agent
    ...
    fn test_build_turn_metric_counts_cache_read_tokens_thread_through() {
        // Wire-parse a lenos-agent payload with cache, run it through the tracker,
        ...
        // cache_write_tokens is always None — lenos-agent doesn't emit it.
        assert!(
            turn2.cache_write_tokens.is_none(),
            "cache_write_tokens must be None — not emitted by lenos-agent"
        );
    ...
    fn make_canvas_event_value(content: &str) -> serde_json::Value {
        ...
        let event = EventBuilder::new(Kind::Custom(lenos_core::kind::KIND_CANVAS as u16), content)
            .tags([h_tag])
            .sign_with_keys(&keys)
            .expect("sign");
    ...
    fn test_canvas_section_from_query_response_happy_path() {
        ...
        assert!(section.contains("lenos canvas get --channel"));
    ...
    async fn test_new_session_channel_context_qualifies_a_normal_channel() {
        ...
        let id = Uuid::new_v4();
        let response = channel_metadata_response(id, &[["name", "lenos-dev"], ["t", "stream"]]);
        ...
        assert_eq!(title_channel.as_deref(), Some("lenos-dev"));
        assert_eq!(requests.load(Ordering::SeqCst), 1);

        let (_, again) = resolve_new_session_channel_context(&resolver, id).await;
        assert_eq!(again.as_deref(), Some("lenos-dev"));
        assert_eq!(
