...
//! turn.  Both goose and lenos-agent use this same wire format.  The payload
...
/// ```
///
/// `used` and `contextLimit` are optional because lenos-agent does not track a
...
pub(crate) struct UsageUpdatePayload {
    /// Total tokens used (context-usage proxy). Optional — lenos-agent omits
    /// this field or sends 0 because it does not track a context window limit.
    #[serde(default)]
    #[allow(dead_code)]
    pub used: u64,
    /// Context window size. Optional — lenos-agent omits this field.
    #[serde(default)]
    #[allow(dead_code)]
    ...
    /// emitted by lenos-agent when every turn in the session so far supplied a
...
pub struct TurnUsage {
    ...
    /// Session-cumulative genuine provider total tokens as reported by lenos-agent;
    ...
    /// Session-cumulative cache-read input tokens as reported by lenos-agent.
...
mod tests {
    ...
    /// The camelCase key lenos-agent actually puts on the wire must land on the
    ...
    fn notification_deserializes_without_used_and_context_limit() {
        // lenos-agent emits usage_update without used/contextLimit.
        let raw = serde_json::json!({
            "sessionId": "lenos-sess",
            "update": {
                "sessionUpdate": "usage_update",
    ...
    #[test]
    fn lenos_agent_notification_flows_through_tracker() {
        // End-to-end: a lenos-agent-shaped usage_update (no used/contextLimit)
    ...
    #[test]
    fn lenos_agent_payload_no_context_fields_processes_correctly() {
    ...
    fn pool_omitted_cache_field_publishes_no_cache_read_tokens_in_kind44200() {
        // End-to-end: a lenos-agent or goose payload that omits the cache field
    ...
    fn pool_reported_cache_field_publishes_nonzero_cache_read_tokens_in_kind44200() {
        // End-to-end: a lenos-agent payload with a nonzero cache count must
