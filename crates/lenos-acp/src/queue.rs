//! Event queue state machine for lenos-acp.
//!
...
/// LenOS always generates marker-based tags (see relay messages.rs:762-783).
...
/// checking whether the **first** prompt content block starts with `/`. LenOS
...
pub fn extract_slash_command(content: &str, known_names: &[&str]) -> Option<String> {
    ...
    loop {
        if rest.starts_with("nostr:npub1") || rest.starts_with("nostr:nprofile1") {
            ...
            // token of the characters LenOS allows in plain @mentions.
            let name_len = names
                .iter()
                .find_map(|name| {
...
fn append_reply_instruction(s: &mut String, event_id: &str) {
    s.push_str(&format!(
        "\nIMPORTANT: For ordinary replies in this turn, use `--reply-to {event_id}` \
         on `lenos messages send` so the conversation stays threaded. \
...
fn append_new_thread_reply_instruction(s: &mut String, event_id: &str) {
    s.push_str(&format!(
        "\nIMPORTANT: This is a new top-level message. For ordinary replies in \
         this turn, use `--reply-to {event_id}` on `lenos messages send` — the \
...
fn format_context_hints(
    ...
    if is_dm {
        ...
        let ctx_hint = if has_conversation_context && is_reply {
            "Thread context included below. Use `lenos messages thread --channel <UUID> --event <ID>` for full history if truncated."
        } else if has_conversation_context {
            "Conversation context included below. Use `lenos messages get --channel <UUID>` for full history if truncated."
        } else if is_reply {
            "Use `lenos messages thread --channel <UUID> --event <ID>` to fetch the reply chain."
        } else {
            "Use `lenos messages get --channel <UUID>` for conversation context."
        };
        let mut s = format!(
            "[Context]\n\
             Scope: dm\n\
        ...
        let ctx_hint = if has_conversation_context {
            ...
            "Use `lenos messages thread --channel <UUID> --event <ID>` to fetch thread context."
        };
        let mut s = format!(
            "[Context]\n\
             Scope: thread\n\
        ...
        let mut s = format!(
            "[Context]\n\
             Scope: channel\n\
             Channel: {channel_display}\n\
             Hint: Use `lenos messages get --channel <UUID>` for recent messages if needed."
        );
        if let Some(event_id) = reply_anchor {
...
/// 5. `[Event]` / `[LenOS events]` — the triggering event(s)
///
...
pub fn format_prompt(batch: &FlushBatch, args: &FormatPromptArgs<'_>) -> Vec<String> {
    ...
    let event_section = if batch.events.len() == 1 {
        ...
        let header = if has_cancelled {
            ...
            format!("[LenOS events — {} events]", batch.events.len())
        };
        let mut s = header;
...
mod tests {
    ...
    fn test_format_prompt_single() {
        ...
        assert!(prompt.contains("[LenOS event: @mention]\n"));
    ...
    fn test_format_prompt_batch() {
        ...
        assert!(prompt.contains("[Context]"));
        assert!(prompt.contains("[LenOS events — 3 events]"));
    ...
    fn test_format_prompt_dm_reply_hints_get_thread() {
        ...
        assert!(
            prompt.contains("lenos messages thread"),
            "DM reply hint should mention `lenos messages thread`, got:\n{prompt}"
        );
    ...
    fn test_format_prompt_dm_non_reply_hints_get_messages() {
        ...
        assert!(prompt.contains("Scope: dm"));
        assert!(
            prompt.contains("lenos messages get"),
            "DM non-reply hint should mention `lenos messages get`"
        );
        assert!(
            !prompt.contains("lenos messages thread"),
            "DM non-reply should NOT mention `lenos messages thread`"
        );
    }
    ...
    fn test_reply_instruction_allows_explicit_root_post_requests() {
        ...
        assert!(
            !prompt.contains("on EVERY `lenos messages send` call"),
