//! Harness-side LenOS relay client.
//!
//! Connects to the LenOS relay via NIP-01 WebSocket, authenticates via NIP-42,
...
//! - Forwards `LenOSEvent`s through an `mpsc` channel
...
/// Override with `LENOS_ACP_EVENT_BUFFER` env var at startup.
...
fn event_channel_capacity() -> usize {
    std::env::var("LENOS_ACP_EVENT_BUFFER")
        .ok()
...
///
/// Connects to the LenOS relay, authenticates via NIP-42, and streams
...
pub struct HarnessRelay {
    ...
    event_rx: mpsc::Receiver<Option<LenOSEvent>>,
...
impl HarnessRelay {
    ...
    pub async fn connect(
        ...
        let (event_tx, event_rx) = mpsc::channel::<Option<LenOSEvent>>(event_channel_capacity());
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                lenos_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
    ...
    pub async fn next_event(&mut self) -> Option<LenOSEvent> {
...
async fn run_background_task(
    ...
    event_tx: mpsc::Sender<Option<LenOSEvent>>,
            ...
            match relay_msg {
                RelayMessage::Event {
                    ...
                    if subscription_id == OBSERVER_CONTROL_SUB_ID {
                        ...
                        match event_tx.try_send(Some(lenos_event)) {
                            Ok(()) => {
...
/// sends (`crates/lenos-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        lenos_sdk::build_agent_observer_frame(
