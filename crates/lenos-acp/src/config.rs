//! Configuration for the lenos-acp harness.
//!
...
/// while running long sub-tools (e.g. a lenos-agent running another agent, or
...
/// Override via `--idle-timeout` / `LENOS_ACP_IDLE_TIMEOUT`.
pub(crate) const DEFAULT_IDLE_TIMEOUT_SECS: u64 = 900;

/// Default absolute wall-clock cap per agent turn (2 hours).
/// Override via `--max-turn-duration` / `LENOS_ACP_MAX_TURN_DURATION`.
...
/// CLI args for `lenos-acp models` — query available models from an agent.
///
...
pub struct AuthAgentArgs {
    /// Agent binary to spawn (e.g. "goose", "claude-agent-acp", "codex-acp").
    #[arg(long, env = "LENOS_ACP_AGENT_COMMAND", default_value = "goose")]
    pub agent_command: String,

    /// Arguments passed to the agent binary.
    #[arg(
        long,
        env = "LENOS_ACP_AGENT_ARGS",
        default_value = "acp",
        value_delimiter = ','
    )]
...
/// CLI args for `lenos-acp auth-methods` — query adapter-advertised login methods.
#[derive(Debug, Parser)]
#[command(
    name = "lenos-acp auth-methods",
...
/// CLI args for `lenos-acp authenticate` — start an adapter-owned login flow.
#[derive(Debug, Parser)]
#[command(
    name = "lenos-acp authenticate",
...
#[command(
    name = "lenos-acp",
    about = "ACP harness that bridges LenOS events to AI agents"
)]
pub struct CliArgs {
    #[arg(long, env = "LENOS_RELAY_URL", default_value = "ws://localhost:3000")]
    pub relay_url: String,

    #[arg(long, env = "LENOS_PRIVATE_KEY", hide_env_values = true)]
    pub private_key: String,

    /// Agent owner pubkey (64-char hex). Used for --respond-to=owner-only gate.
    #[arg(long, env = "LENOS_ACP_AGENT_OWNER")]
    ...
    #[arg(long, env = "LENOS_ACP_MCP_COMMAND", default_value = "")]
    pub mcp_command: String,

    /// Idle timeout: max seconds of silence before killing a turn.
    /// Resets on any agent stdout activity.
    #[arg(long, env = "LENOS_ACP_IDLE_TIMEOUT")]
    pub idle_timeout: Option<u64>,

    /// Absolute wall-clock cap per turn (safety valve).
    #[arg(long, env = "LENOS_ACP_MAX_TURN_DURATION", default_value_t = DEFAULT_MAX_TURN_DURATION_SECS)]
    pub max_turn_duration: u64,

    /// Deprecated: alias for --idle-timeout. If both set, --idle-timeout wins.
    #[arg(long, env = "LENOS_ACP_TURN_TIMEOUT", hide = true)]
    pub turn_timeout: Option<u64>,

    #[arg(
        long,
        env = "LENOS_ACP_SYSTEM_PROMPT",
    ...
    #[arg(
        long,
        env = "LENOS_ACP_HEARTBEAT_PROMPT_FILE",
        conflicts_with = "heartbeat_prompt"
    )]
    pub heartbeat_prompt_file: Option<PathBuf>,

    #[arg(long, env = "LENOS_ACP_INITIAL_MESSAGE")]
    pub initial_message: Option<String>,

    #[arg(
        long,
        env = "LENOS_ACP_SUBSCRIBE",
        default_value = "mentions",
        value_enum
    )]
    pub subscribe: SubscribeMode,

    #[arg(long, env = "LENOS_ACP_KINDS", value_delimiter = ',')]
    pub kinds: Option<Vec<u32>>,

    #[arg(long, env = "LENOS_ACP_CHANNELS", value_delimiter = ',')]
    pub channels: Option<Vec<String>>,

    #[arg(long, env = "LENOS_ACP_NO_MENTION_FILTER")]
    pub no_mention_filter: bool,

    #[arg(long, env = "LENOS_ACP_CONFIG", default_value = "./lenos-acp.toml")]
    pub config: PathBuf,

    #[arg(long, env = "LENOS_ACP_DEDUP", default_value = "queue", value_enum)]
    pub dedup: DedupMode,
    ...
    #[arg(
        long,
        env = "LENOS_ACP_MULTIPLE_EVENT_HANDLING",
        default_value = "steer",
        value_enum
    )]
    pub multiple_event_handling: MultipleEventHandling,

    #[arg(long, env = "LENOS_ACP_NO_IGNORE_SELF")]
    ...
    /// when the relay confirms no core engram exists). The `lenos mem` CLI
    ...
    /// Pass `--no-memory` / `LENOS_ACP_NO_MEMORY=true` to disable.
    #[arg(
        long,
        env = "LENOS_ACP_MEMORY",
    ...
    #[arg(long, env = "LENOS_ACP_NO_MEMORY", conflicts_with = "memory")]
    pub no_memory: bool,

    /// Disable the [Base] platform-context section prepended to every prompt.
    /// When set, agents receive only the persona [System] prompt with no LenOS orientation.
    #[arg(long, env = "LENOS_ACP_NO_BASE_PROMPT")]
    ...
    #[arg(
        long,
        env = "LENOS_ACP_BASE_PROMPT_FILE",
    ...
    /// Use `lenos-acp models` to discover available model IDs.
    #[arg(long, env = "LENOS_ACP_MODEL")]
    ...
    #[arg(long, env = "LENOS_ACP_SESSION_TITLE")]
    ...
    #[arg(long, env = "LENOS_ACP_RESPOND_TO_ALLOWLIST", value_delimiter = ',')]
...
pub struct Config {
    ...
    /// `--no-memory` / `LENOS_ACP_NO_MEMORY` opt-out.
    ...
    /// Per-persona env vars to inject at agent spawn time (e.g., GOOSE_PROVIDER, GOOSE_MODEL, LENOS_AGENT_MODEL).
...
///
/// LenOS is the only guard here: Codex's own `normalize_thread_name` merely
...
fn default_agent_args(command: &str) -> Option<Vec<String>> {
    match normalize_agent_command_identity(command).as_str() {
        ...
        | "claudecode" | "lenos-agent" => Some(Vec::new()),
        _ => None,
    }
}

/// Per-runtime environment defaults applied when LenOS owns the agent process.
///
...
/// startup budget (see block/lenos#3355). Skip that unrelated global startup
...
///
/// Codex sandboxes MCP subprocesses (including `lenos-cli`) behind a Seatbelt sandbox
/// that blocks all outbound network by default. Without this env var, `lenos-cli`
...
pub fn propagate_legacy_env_vars() {
    for (legacy, canonical) in [
        ("LENOS_ACP_PRIVATE_KEY", "LENOS_PRIVATE_KEY"),
        ("LENOS_ACP_API_TOKEN", "LENOS_API_TOKEN"),
    ] {
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        let idle_timeout_secs = {
            let raw = match (args.idle_timeout, args.turn_timeout) {
                (Some(idle), Some(_turn)) => {
                    tracing::warn!(
                        "--turn-timeout / LENOS_ACP_TURN_TIMEOUT is deprecated and ignored \
                         when --idle-timeout / LENOS_ACP_IDLE_TIMEOUT is also set"
                    );
                    idle
                }
                (Some(idle), None) => idle,
                (None, Some(turn)) => {
                    tracing::warn!(
                        "--turn-timeout / LENOS_ACP_TURN_TIMEOUT is deprecated; \
                         use --idle-timeout / LENOS_ACP_IDLE_TIMEOUT instead"
                    );
                    turn
                }
            ...
            for s in &raw {
                RespondTo::from_str(s.trim(), true).map_err(|_| {
                    ConfigError::ConfigFile(format!(
                        "invalid value in LENOS_ACP_ALLOWED_RESPOND_TO: '{s}' \
        ...
        // opens the Seatbelt network sandbox for lenos-cli (an MCP subprocess). No-op
...
mod tests {
    ...
    fn test_config(mode: SubscribeMode) -> Config {
        Config {
            ...
            config_path: PathBuf::from("./lenos-acp.toml"),
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&lenos_core::kind::KIND_STREAM_MESSAGE));
            assert!(kinds.contains(&lenos_core::kind::KIND_WORKFLOW_APPROVAL_REQUESTED));
            assert!(kinds.contains(&lenos_core::kind::KIND_STREAM_REMINDER));
        }
    }
    ...
    #[test]
    fn normalizes_lenos_agent_args_to_empty() {
        assert_eq!(
            normalize_agent_args("lenos-agent", Vec::new()),
            Vec::<String>::new()
        );
        assert_eq!(
            normalize_agent_args("lenos-agent", vec!["acp".into()]),
            Vec::<String>::new()
        );
    }
    ...
    fn default_agent_env_recognizes_hermes_identities() {
        ...
        for command in ["goose", "codex-acp", "claude-agent-acp", "lenos-agent", ""] {
            assert!(
    ...
    fn codex_network_env_non_codex_agent_returns_none() {
        ...
        assert!(codex_network_env("lenos-agent", "wss://relay.example.com").is_none());
    }
    ...
    fn test_load_rules_valid_toml() {
        let dir = std::env::temp_dir().join("lenos-acp-test-valid");
...
"#,
    ...
    fn test_load_rules_empty_name_rejected() {
        let dir = std::env::temp_dir().join("lenos-acp-test-empty-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_duplicate_name_rejected() {
        let dir = std::env::temp_dir().join("lenos-acp-test-dup-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_invalid_filter_rejected() {
        let dir = std::env::temp_dir().join("lenos-acp-test-bad-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    fn test_load_rules_channel_scope_typo_rejected() {
        let dir = std::env::temp_dir().join("lenos-acp-test-scope-typo");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_too_many_rules_rejected() {
        let dir = std::env::temp_dir().join("lenos-acp-test-too-many");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_load_rules_filter_too_long_rejected() {
        let dir = std::env::temp_dir().join("lenos-acp-test-long-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn inactivity_exit_defaults_disabled_and_accepts_cli_value() {
        let key = "0".repeat(64);
        let default = CliArgs::parse_from(["lenos-acp", "--private-key", &key]);
    ...
    fn lazy_pool_defaults_off() {
        let key = "0".repeat(64);
        assert!(!CliArgs::parse_from(["lenos-acp", "--private-key", &key]).lazy_pool);
    }
    ...
    fn test_permission_mode_value_enum_camel_case_aliases() {
        ...
        // (e.g. LENOS_ACP_PERMISSION_MODE=bypassPermissions). The #[value(alias)]
    ...
    fn test_multiple_event_handling_default_is_steer() {
        ...
        let args = CliArgs::parse_from(["lenos-acp", "--private-key", &"0".repeat(64)]);
    ...
    // --- LENOS_ACP_ALLOWED_RESPOND_TO gate ---
    ...
    fn allowed_respond_to_rejects_invalid_mode_string() {
        ...
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("invalid value in LENOS_ACP_ALLOWED_RESPOND_TO"),
    ...
    //
    // These tests exercise the actual wiring: LENOS_ACP_ALLOWED_RESPOND_TO in the
    ...
    fn compose_session_title_qualifies_the_agent_name_with_the_channel() {
        assert_eq!(
            compose_session_title("Fizz", Some("lenos-dev")),
            "Fizz · #lenos-dev"
        );
    }
    ...
    fn compose_session_title_drops_the_channel_when_the_agent_name_fills_the_cap() {
        ...
        assert_eq!(compose_session_title(&agent, Some("lenos-dev")), agent);
    }
