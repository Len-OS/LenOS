...
//! (OPS metadata) and tells LenOS where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the LenOS runtime must satisfy (e.g. `">=0.9.0"`).
    #[serde(skip_serializing_if = "Option::is_none", alias = "lenos")]
    pub lenos: Option<String>,
}
...
/// validator issues advisory warnings for LenOS-unknown keys.
...
mod tests {
    ...
    fn parse_full_manifest() {
        ...
        assert_eq!(m.engines.unwrap().lenos.as_deref(), Some(">=0.9.0"));
