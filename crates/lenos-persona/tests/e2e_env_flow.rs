...
//! - LenOS-agent personas emit LENOS_AGENT_MODEL, LENOS_AGENT_PROVIDER
...
use lenos_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_lenos_agent_persona_emits_lenos_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert_eq!(
        env.get("LENOS_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit LENOS_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("LENOS_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit LENOS_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "lenos-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "lenos-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // LenOS-agent persona
    fs::write(
        root.join("agents/lenos-bot.persona.md"),
        r#"---
name: "lenos-bot"
display_name: "LenOS Bot"
description: "A lenos-agent runtime bot"
runtime: "lenos-agent"
model: "openai:gpt-4o"
---
You are a lenos bot.
"#,
    ...
    let lenos = pack
        .personas
        .iter()
        .find(|p| p.name == "lenos-bot")
        .expect("lenos-bot should exist");
    ...
    assert!(
        !goose_env.contains_key("LENOS_AGENT_MODEL"),
        "goose persona must not emit LENOS_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("LENOS_AGENT_PROVIDER"),
        "goose persona must not emit LENOS_AGENT_PROVIDER"
    );

    // LenOS-agent persona gets LENOS_AGENT_* env vars
    let lenos_env: std::collections::HashMap<_, _> = lenos
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(lenos_env.get("LENOS_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(lenos_env.get("LENOS_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !lenos_env.contains_key("GOOSE_MODEL"),
        "lenos-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !lenos_env.contains_key("GOOSE_PROVIDER"),
        "lenos-agent persona must not emit GOOSE_PROVIDER"
    );
}
