# LenOS Push Gateway deployment

`lenos-push-gateway` is the standalone public APNs last hop intended for `push.lenos.xyz`. Build it with `Dockerfile.push-gateway`; do not run it in the relay image or give relays APNs credentials.

## Network and health

- Public listener: `LENOS_PUSH_BIND_ADDR` (default `0.0.0.0:8080`). Route `https://push.lenos.xyz` to this port.
- Private health listener: `LENOS_PUSH_HEALTH_ADDR` (default `0.0.0.0:8081`). Probe `/_liveness` and `/_readiness`; do not expose this port publicly. The chart has no pod-ingress allowance for 8081; Kubernetes node/kubelet-origin probe traffic is exempt from NetworkPolicy. Add a narrowly selected monitoring source only if the target CNI requires pod-origin health scraping.
...
| `LENOS_PUSH_PUBLIC_DELIVERY_URL` | Exact externally signed URL, normally `https://push.lenos.xyz/v1/deliveries/apns`. |
| `LENOS_PUSH_MAX_GRANT_LIFETIME_SECONDS` | Maximum delegation capability lifetime (`1..=31536000`). |
| `LENOS_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS` | Maximum encrypted-token installation lifetime (default 90 days, max one year). Clients must renew before expiry. |
| `LENOS_PUSH_ENABLED_PROFILES` | Comma-separated `lenos-ios-production` and/or `lenos-ios-sandbox`. |
| `LENOS_PUSH_APP_ATTEST_APP_ID` | Exact Apple App Attest application identifier (`TEAMID.bundle-id`). |
| `LENOS_PUSH_APP_ATTEST_ROOT_CERT_PATH` | Read-only mounted Apple App Attest root certificate PEM. |
| `LENOS_PUSH_APNS_KEY_PATH` | Read-only mounted Apple APNs `.p8` provider key. |
| `LENOS_PUSH_APNS_KEY_ID` | APNs provider key id. |
| `LENOS_PUSH_APNS_TEAM_ID` | Apple developer team id. |
| `LENOS_PUSH_APNS_TOPIC` | LenOS iOS bundle id. |
| `LENOS_PUSH_GRANT_KEYS` | Capability AEAD keyring, `id:base64-32-bytes[,predecessor...]`; current key first. |
| `LENOS_PUSH_TOKEN_KEYS` | Independent token-custody AEAD keyring in the same format. Never reuse grant keys. |

Optional endpoint quota policy variables are `LENOS_PUSH_ENDPOINT_QUOTA_WINDOW_SECONDS` (default `10`, max `86400`) and `LENOS_PUSH_ENDPOINT_QUOTA_MAX_DELIVERIES` (default `10`, max `10000`). These are LenOS policy hypotheses, not Apple-published limits; tune under load while retaining a hard ceiling.
...
|---|---|---|---|
| `PushGatewayConfigurationFault` | any `configuration_fault` outcomes for 10m | critical | APNs provider token/topic is unhealthy. Check the `.p8` key, `LENOS_PUSH_APNS_KEY_ID`, `..._TEAM_ID`, and `..._TOPIC`. No endpoints are being invalidated, but nothing is delivering. |
...
Relays default `LENOS_PUSH_GATEWAY_DELIVERY_URL` to the exact public delivery URL
`https://push.lenos.xyz/v1/deliveries/apns`. Operators can override it with
...
gh attestation verify \
  oci://ghcr.io/buildgrowthnow/lenos-push-gateway@sha256:<64-lowercase-hex> \
  --owner block
```

Only after that command succeeds, set the exact digest as `image.digest`; the chart then renders `ghcr.io/buildgrowthnow/lenos-push-gateway@sha256:...` and ignores the mutable tag. `values-production.yaml` is an intentionally invalid production-input contract: deployment CI must inject this verified `image.digest`, the provisioned Apple application identifier, an environment-owned Gateway parent reference, and the actual PostgreSQL network. Schema validation rejects the artifact when any remains empty; the render guard proves both rejection and a fully injected render.
...
Kubernetes does not restart pods when referenced Secret bytes change. AEAD or APNs credential rotation therefore requires an explicit rolling restart after the secret manager update (for example, `kubectl rollout restart deployment/<release>-lenos-push-gateway`) and readiness verification before removing predecessor keys. Service-account token automount is disabled.

## Gateway chart release

The gateway chart has a collision-free release lane separate from the main
`lenos` chart. To publish version `X.Y.Z`, update both `version` and `appVersion`
in `deploy/charts/lenos-push-gateway/Chart.yaml`, validate the chart, and open a
same-repository PR whose branch is exactly `push-chart-release/X.Y.Z`:

```bash
deploy/charts/lenos-push-gateway/tests/render.sh
git switch -c push-chart-release/X.Y.Z
git add deploy/charts/lenos-push-gateway/Chart.yaml
...
`oci://ghcr.io/buildgrowthnow/lenos/charts/lenos-push-gateway`. A manually pushed
