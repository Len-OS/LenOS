...
**The manifest pointer is the sole source of truth.** In LenOS, a successful
...
- **kind:30618 derived after CAS.** Emission is conditional on
  ...
  the pusher's pubkey rides in a `p` tag (lenos extension; NIP-34 does not
...
exists in code at PR #726's tip (`crates/lenos-relay/src/api/git/`), with the
