...
LenOS can expose a private, deployment-wide read-only dashboard from the existing
relay process. It shows open moderation reports and recent product feedback.

Configure `LENOS_ADMIN_HOST` to activate the dashboard. A private ingress limits
access to the operator VPN or approved source IPs.

Required configuration:

```text
LENOS_ADMIN_HOST=admin.example.com
...
unchanged, including `LENOS_REQUIRE_MEDIA_GET_AUTH`; the browser receives no
