# LenOS shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`LenOS Desktop → lenos-acp → lenos-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `BuildGrowthNow/LenOS` repository root on the mesh-enabled branch.
...
`~/.lenos-dev`, and local Docker volumes; it preserves the installed LenOS app's
data, production keyring, and `~/.lenos`. The first dev page load also clears
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to LenOS**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local LenOS**.
...
Stop only stale LenOS/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the LenOS window to open and for the terminal to
...
LenOS may download the model on first use. The model picker ranks models for the
...
resolves to the bundled `lenos-agent`. No API key is required.
...
**lenos-agent** as the runtime, **LenOS shared compute** as the LLM provider,
...
- no current LenOS membership snapshot is available (admission fails closed).

## Security boundary

LenOS publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current LenOS membership controls which node identities are
...
`LENOS_MESH_IROH_RELAYS` defaults to Iroh's production relay set. Set it to `0`
...
MeshLLM—not the LenOS relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the LenOS-side checks above.
