...
| Blank window on startup, no crash output | dmabuf renderer incompatibility (NVIDIA or AppImage) | `WEBKIT_DISABLE_DMABUF_RENDERER=1 ./LenOS.AppImage` or `--safe-rendering` |
...
**Affected distributions:** Fedora 40+ and any distro shipping Google's Noto Color Emoji in COLRv1 format (`Noto-COLRv1.ttf`). Issues [#2548](https://github.com/BuildGrowthNow/LenOS/issues/2548), [#2982](https://github.com/BuildGrowthNow/LenOS/issues/2982).
...
**Fix:** Upgrade to the latest AppImage (v0.5.2+). The build container was bumped to `ubuntu:24.04` ([#3602](https://github.com/BuildGrowthNow/LenOS/pull/3602)), which ships FreeType 2.13.2. The compiled layout now matches every crash-affected host (FreeType ≥ 2.13), eliminating the ABI mismatch.
...
**Workaround (before upgrading):** Add a fontconfig override that removes color-format fonts from LenOS's view:

```bash
mkdir -p ~/.config/lenos-fontconfig
cat > ~/.config/lenos-fontconfig/fonts.conf <<'XML'
<?xml version="1.0"?>
...
**Native packages (`deb`/`rpm`):** The COLRv1 crash ([#2548](https://github.com/BuildGrowthNow/LenOS/issues/2548), [#2982](https://github.com/BuildGrowthNow/LenOS/issues/2982)) is AppImage-only — native packages use the system WebKit, which has a consistent FreeType ABI, and are not affected.

---

## Blank window on startup (no crash): dmabuf renderer

**Affected hardware:** NVIDIA GPUs (proprietary and nouveau drivers) and AppImage installs on any GPU. Issue [#2338](https://github.com/BuildGrowthNow/LenOS/issues/2338).
...
```bash
./LenOS_*.AppImage --safe-rendering
# or for a native install:
lenos-desktop --safe-rendering
```
...
**Conflict detection:** If you set a WebKit variable in your environment and also pass `--safe-rendering`, LenOS will refuse to start and print exactly which variable conflicts. Unset the conflicting variable or drop the flag.

---

## AMD RDNA4 / transparent window

**Affected hardware:** AMD RDNA4 GPUs (RX 9000 series) with the `radv` driver. Issue [#2643](https://github.com/BuildGrowthNow/LenOS/issues/2643).

**Symptom:** The LenOS window is transparent or renders with graphical corruption on AMD RDNA4 hardware.

**Workaround (verified by reporter):** Set these three variables before launching LenOS:
...
A dedicated fix for RDNA4 detection is being tracked in [#2643](https://github.com/BuildGrowthNow/LenOS/issues/2643).
...
1. Run LenOS from a terminal and capture the output:
   ```bash
   ./LenOS_*.AppImage 2>&1 | tee lenos-crash.log
   ```
...
4. File a [new issue](https://github.com/BuildGrowthNow/LenOS/issues/new) with your distro, GPU, driver version, and the terminal output.
