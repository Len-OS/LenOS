...
This document specifies the protocol by which LenOS Desktop delegates the
...
1. **The provider protocol** — a zero-registration plugin contract between the
   desktop and any executable named `lenos-backend-<id>`: discovery, the `info`
...
3. **The Kubernetes binding** — the first conforming provider,
   `lenos-backend-kubernetes`, which realizes the contract as a bare Pod
...
launcher among many**. What makes a process a live LenOS agent is a keypair,
a NIP-OA auth tag, and a relay URL, handed as environment to the `lenos-acp`
...
- **Agent conversational behavior.** What the agent does with events is
  governed by the ACP harness (`lenos-acp`) and the NIPs it implements
...
- **Desktop** `D` — the LenOS Desktop app. Holds the agent's identity (nsec in
  the OS keyring), its configuration record, and the only UI. Trusted.
- **Provider** `P` — an executable `lenos-backend-<id>` on `D`'s machine.
...
- **Substrate** `S` — the remote compute environment `P` deploys into (a
  ...
  `D` never talks to `S`.
- **Agent** `A` — a `lenos-acp` harness process (plus the ACP agent under it)
  running on `S`, holding the nsec it was given, connected to the relay.
- **Relay** `R` — the LenOS relay. The *only* channel that connects `D` to a
...
harness spawns — `goose`, `claude-agent-acp`, `codex-acp`, `lenos-agent`, or
...
1. **The agent/harness contract — binds every launcher.** A live LenOS agent
   is a `lenos-acp` process holding a keypair, a NIP-OA auth tag (or resolved
   ...
   restart** (I5). A bash script that exports `LENOS_PRIVATE_KEY`,
   `LENOS_RELAY_URL`, `LENOS_AUTH_TAG` and execs the harness is a conforming
...
- **(I3) Presence is the status.** `D` derives a remote agent's live state
  ...
  (`PRESENCE_TTL_SECS`, `lenos-pubsub/src/presence.rs:16`; the vision's
  ...
  presence-suppression knob, `LENOS_ACP_NO_PRESENCE`, MUST join
...
`lenos-backend-<id>`. The suffix after the prefix is the provider id and MUST
...
`lenos-backend-kubernetes` will ask. (At `28ae6cd21` discovery records only
...
`LENOS_PRIVATE_KEY`, `NOSTR_PRIVATE_KEY`, `LENOS_AUTH_TAG`, `LENOS_RELAY_URL`,
...
payload fields (`private_key_nsec` → `LENOS_PRIVATE_KEY`/`NOSTR_PRIVATE_KEY`,
`auth_tag` → `LENOS_AUTH_TAG`, `relay_url` → `LENOS_RELAY_URL`); reading
...
POSIX-shaped names before merge, because a key like `LENOS_AUTH_TAG=x`
...
"launch": {
  ...
  "policy_env":   {str: str},   // overridable behavior defaults (tier 1, below):
                                ...
                                // LENOS_ACP_RELAY_OBSERVER, LENOS_ACP_LAZY_POOL=true,
                                // LENOS_ACP_SESSION_TITLE (resolved),
                                // LENOS_ACP_TEAM_INSTRUCTIONS, LENOS_ACP_MODEL,
                                // MCP_HOOK_SERVERS=* (mcp_hooks runtimes only)
  "owner_pubkey": str | null    // resolved workspace owner (hex) — legacy
                                // LENOS_ACP_AGENT_OWNER fallback, non-secret
}
```
...
Claude; `LENOS_AGENT_MODEL`/`LENOS_AGENT_PROVIDER` for lenos-agent). A fixed
`provider → LENOS_AGENT_PROVIDER` mapping is wrong for three of the four
...
- `LENOS_ACP_TEAM_INSTRUCTIONS` — the only truly non-reconstructible policy
  value: `effective_team_instructions` (`spawn_hash.rs:41-52`) needs the
  desktop's `TeamRecord` store, which no pod can reach.
- `LENOS_ACP_SESSION_TITLE` — sent **resolved** (`resolve_session_title`,
...
- `LENOS_ACP_LAZY_POOL=true` — a **deliberate pick, not a transcription**:
...
- `MCP_HOOK_SERVERS=*` when the resolved runtime has `mcp_hooks`
  (`runtime.rs:594-598`; lenos-agent only at `28ae6cd21`) — gates the
  `_Stop`/`_PostCompact` hook tools.
- `LENOS_ACP_SYSTEM_PROMPT`, `LENOS_ACP_IDLE_TIMEOUT`,
  `LENOS_ACP_MAX_TURN_DURATION`, `LENOS_ACP_AGENTS` — resolved by the desktop
...
`LENOS_ACP_DEDUP` and `LENOS_ACP_MULTIPLE_EVENT_HANDLING` are **deliberately
...
imply a divergence that does not exist. `LENOS_MANAGED_AGENT` is likewise
...
3. **Authoritative** — unoverridable at every layer, written last and
   ...
   `LENOS_ACP_AGENT_OWNER`, the inactivity bound, `LENOS_ACP_MCP_COMMAND`,
   and `LENOS_MANAGED_AGENT_START_NONCE`. For the nonce, the provider MUST
   set it to the attempt's **generation token** (§K8s Secrets): the harness
   stamps it into every observer lifecycle frame (`lenos-acp/lib.rs:1501`),
...
- the harness and agent binaries: `launch.command` is a *name*, resolved
  against the image's own `PATH` (`LENOS_ACP_AGENT_COMMAND`), and
  `LENOS_ACP_MCP_COMMAND=lenos-dev-mcp` likewise;
...
- `LENOS_ACP_SETUP_PAYLOAD` is desktop-computed readiness state and MUST NOT
  appear in a remote pod.

**Owner resolution (normative):** the provider MUST have either a non-null
`auth_tag` (→ `LENOS_AUTH_TAG`) or a non-null `launch.owner_pubkey`
(→ `LENOS_ACP_AGENT_OWNER`) before any mutation; if both are null it MUST
refuse the deploy. Without an owner the harness cannot match `!shutdown`
(`lenos-acp/src/lib.rs: resolve_agent_owner`, main-loop owner check) and the
agent answers its own stop command conversationally — §Stop would be
describing a mechanism that does not work. `LENOS_ACP_AGENT_OWNER` is a
reserved key, so this value can only arrive as authoritative launch data,
never through user env.

**LenOS shared compute (relay-mesh) is non-deployable, and this is forced,
...
**Auto-repair is fenced to LenOS-authored, positively identified residue
...
management marker — `app.kubernetes.io/managed-by: lenos-backend-kubernetes`
...
is substrate-operator residue survives with one qualifier: *LenOS-authored*
...
`lenos.block.xyz/create-intent`, written at pod create — the same shape as
...
```
--exit-after-inactivity <secs>   /   LENOS_ACP_EXIT_AFTER_INACTIVITY
```
...
- **Mechanism**: on expiry of the bound, the harness fires the same shutdown
  ...
  **The expiry check MUST NOT depend on pool readiness.** This is a design
  ...
  `LENOS_ACP_LAZY_POOL=true` (§Launch data) into a deadlock in I5's single
...
- **Reserved keys**: `LENOS_ACP_EXIT_AFTER_INACTIVITY` MUST join
  ...
  door. `LENOS_ACP_NO_PRESENCE` (`config.rs:378`) MUST join in the same
...
The first conforming provider: a Rust crate in `BuildGrowthNow/LenOS`, distributed as a
...
generated `lenos-agents-<rand6>` as the `namespace` field's schema *default*
...
`ghcr.io/buildgrowthnow/lenos-sprig`: Alpine base + `bash` (required by the dev-MCP
shell tool) + `git` + CA certificates + the static musl `sprig` multicall
binary with its personality links (`lenos-acp`, `lenos-agent`, `lenos-dev-mcp`,
`rg`, `tree`, `lenos`, `git-credential-nostr`, `git-sign-nostr`) + a baked
...
`ghcr.io/buildgrowthnow/lenos-sprig@sha256:<that-digest>` — a `sha-<git-sha>` *tag*
...
**An image override MUST contain the runtime ABI** — the `lenos-acp`
entrypoint and everything §Entrypoint and launch ABI requires — not merely
alternate-harness dependencies. A conforming custom image is "lenos-sprig
...
exec lenos-acp   # exec, not a call — lenos-acp must be PID 1
```

`bash -c "setup && lenos-acp"` (no `exec`) is non-conforming: bash becomes
...
| source | env var |
|---|---|
| `relay_url` | `LENOS_RELAY_URL` |
| `private_key_nsec` | `LENOS_PRIVATE_KEY` and `NOSTR_PRIVATE_KEY` (the git helpers read the latter) |
| `auth_tag` | `LENOS_AUTH_TAG` (omitted when null; then `launch.owner_pubkey` → `LENOS_ACP_AGENT_OWNER` is REQUIRED — §Launch data owner rule) |
| `launch.command` | `LENOS_ACP_AGENT_COMMAND` — the *name*, resolved against the image's own PATH; never a forwarded host path |
| `launch.args` | `LENOS_ACP_AGENT_ARGS`, comma-joined |
| `launch.env`, `launch.policy_env` | verbatim, at their precedence tiers |
| generation token (§K8s Secrets) | `LENOS_MANAGED_AGENT_START_NONCE` — the lifecycle-frame correlator and the Secret generation are one identity (§Launch data tier 3) |
| `system_prompt`, `idle_timeout_seconds`, `max_turn_duration_seconds`, `parallelism` | **not mapped by the provider** — the desktop resolves these into `launch.policy_env` (`LENOS_ACP_SYSTEM_PROMPT`, `LENOS_ACP_IDLE_TIMEOUT`, `LENOS_ACP_MAX_TURN_DURATION`, `LENOS_ACP_AGENTS`), because they are tier-1 behavior knobs: locally they are written *before* the user env layer and none is reserved (`runtime.rs:716-729,763` vs `:860`; `env_vars.rs:54-57`), so user env beats them. A provider that mapped the top-level copies after `launch.env` would silently defeat an override that works locally. The top-level fields remain as display/bookkeeping inputs only |
| `turn_timeout_seconds` | not mapped — deprecated upstream and ignored; the local spawn also does not emit it |
| `respond_to` | `LENOS_ACP_RESPOND_TO` |
| `respond_to_allowlist` | `LENOS_ACP_RESPOND_TO_ALLOWLIST`, comma-joined |
| — | `LENOS_ACP_MCP_COMMAND=lenos-dev-mcp` (image-local; the dev-MCP requirement) |
| `provider_config.inactivity_seconds` | `LENOS_ACP_EXIT_AFTER_INACTIVITY` (schema default 7200; the I5 opt-in, §Auto-Stop — the config field and this env var are one knob, not two) |
...
(per-runtime vars, `provider_locked` suppression, `LENOS_ACP_MODEL`) arrives
...
**Encoding honesty note.** `LENOS_ACP_AGENT_ARGS` is comma-delimited by the
...
- **Naming/labeling — the exact contract** (63-char label-value limit; a hex
  ...
  - pod name: `lenos-agent-<first-12-hex-of-pubkey>` — also the returned
    `agent_id`
  - label `lenos.block.xyz/agent-pubkey: <first-32-hex>` — the selector key
    ...
    not decorative
  - label `app.kubernetes.io/managed-by: lenos-backend-kubernetes` and label
    `lenos.block.xyz/binding-version: <schema-version>` — the **management
  ...
  - annotation `lenos.block.xyz/agent-pubkey-full: <full-64-hex>` —
  ...
  - annotation `lenos.block.xyz/create-intent: <sha256-of-intent-template>` — the
    ...
    never-started pods
  - Secret name: `lenos-agent-<first-12-hex>-<gen>`, where `<gen>` is a random
...
(`lenos-agent-<first-12-hex>-<gen>`, §Pod shape). The rationale is a
...
`LENOS_MANAGED_AGENT_START_NONCE`, so lifecycle frames and pod logs share a
...
2. It does not suppress the harness's promises on a remote agent:
   presence stays enabled (`LENOS_ACP_NO_PRESENCE` never set — remotely,
   presence is the only signal, I3), and the inactivity knob
   (`LENOS_ACP_EXIT_AFTER_INACTIVITY`) carries the owner's *deliberate*
...
3. **Deploy payload bypasses the launch resolver** (the prerequisite this
   ...
   policy (`LENOS_ACP_RELAY_OBSERVER`, runtime `default_env` such as
   ...
   (`LENOS_ACP_DEDUP`/`LENOS_ACP_MULTIPLE_EVENT_HANDLING` are *not* on this
...
4. **The I5 reaper does not exist, and its natural home is a trap**
   (harness code prerequisite). `LENOS_ACP_EXIT_AFTER_INACTIVITY` appears
...
| Presence publish / offline-on-exit | `crates/lenos-acp/src/lib.rs` (`publish_presence`, shutdown path) |
| `!shutdown` owner check | `crates/lenos-acp/src/lib.rs` (main loop) |
| Graceful shutdown path (budget enforcement *to be added* — Known Defect 7) | `crates/lenos-acp/src/lib.rs` (pool shutdown, then drain / reap / presence / relay close) |
| Clean-exit exit-code contract | *to be added*: `crates/lenos-acp` distinguished exit codes + pinning test (Known Defect 6; gates `OnFailure`) |
| Auto-stop flag | *to be added*: `crates/lenos-acp/src/config.rs` + a pool-independent timer (NOT the `pool_ready`-gated maintenance tick — Known Defect 4) + `RESERVED_ENV_KEYS` entry |
| Kubernetes binding | *to be added*: `crates/lenos-backend-kubernetes` |
...
- **G. Remote override semantics** — the spec keeps local semantics: user
  env continues to beat LenOS behavior defaults remotely (three-tier
...
- **I. Never-started escape hatch** — the create-intent fingerprint
  ...
  to LenOS-authored, positively identified residue** (§Deploy State Machine
...
Remote agents extend LenOS's managed-agent model across a deliberately thin
