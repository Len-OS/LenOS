...
export type LenOSDownloadPlatform = {
...
): LenOSDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): LenOSDownloadPlatform["architecture"] {
...
export async function detectLenOSDownloadPlatform(
  navigatorValue: Navigator,
): Promise<LenOSDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: LenOSDownloadPlatform): RegExp | undefined {
...
export function selectLenOSDownloadUrl(
  releases: GitHubRelease[],
  platform: LenOSDownloadPlatform,
): string | undefined {
...
export async function resolveLenOSDownloadUrlForPlatform(
  platform: LenOSDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(LENOS_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return LENOS_RELEASES_URL;
    const url = selectLenOSDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return LENOS_RELEASES_URL;
    try {
    ...
    return LENOS_RELEASES_URL;
  }
}

export async function resolveLenOSDownloadUrl(): Promise<string> {
  return resolveLenOSDownloadUrlForPlatform(
    await detectLenOSDownloadPlatform(navigator),
  );
}
