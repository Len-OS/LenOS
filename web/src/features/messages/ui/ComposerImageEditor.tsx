...
/**
 * Fetch image bytes via the browser fetch API and wrap in a same-origin
 * blob: URL so the canvas stays un-tainted and toBlob works.
 */
async function fetchMediaBytesWeb(
  url: string,
 ...
 *
 * Bytes are fetched via the browser fetch API and wrapped in a `blob:` URL.
 * Blob URLs are same-origin, so the canvas stays un-tainted and `toBlob`
 * works without CORS involvement.
 */
async function renderAnnotatedPng(
  ...
  const bytes = await fetchMediaBytesWeb(sourceUrl);
...
type ComposerImageEditorProps = {
  ...
  src: string;
  /** Original relay media URL — export fetches its bytes via fetch. */
  sourceUrl: string;
