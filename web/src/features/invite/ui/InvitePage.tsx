...
import {
  LENOS_RELEASES_URL,
  type LenOSDownloadPlatform,
  detectLenOSDownloadPlatform,
  resolveLenOSDownloadUrlForPlatform,
} from "@/shared/lib/lenos-download";
import { hasDurableIdentity } from "@/shared/lib/nostr-signer";
...
export function InvitePage({ code }: { code: string }) {
  ...
  const [downloadUrl, setDownloadUrl] = React.useState(LENOS_RELEASES_URL);
  ...
  React.useEffect(() => {
    let active = true;
    detectLenOSDownloadPlatform(navigator).then(async (platform) => {
      if (!active) return;
      if (
      ...
      const url = await resolveLenOSDownloadUrlForPlatform(platform);
  ...
  const openInvite = async () => {
    ...
    try {
      ...
      window.location.href = `lenos://join?${query.toString()}`;
    } finally {
      setOpening(false);
    }
  };
  ...
  const browserSigningAvailable = hasDurableIdentity();
  const disabled =
    policy === undefined ||
    opening ||
    joiningBrowser ||
        ...
        <div className="flex w-full flex-col items-center rounded-3xl bg-white px-6 py-10 sm:px-12 sm:py-12">
          ...
          <div className="mt-9 w-full max-w-md space-y-2">
            ...
            {policy === null ? (
              ...
              >
                <a
                  href={`lenos://join?relay=${encodeURIComponent(relay)}&code=${encodeURIComponent(code)}`}
                >
                  Accept invite in LenOS
                </a>
              </Button>
            ) : (
              <Button
